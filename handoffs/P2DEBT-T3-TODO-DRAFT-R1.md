# P2 債票 3 — frontend feature-factory 測試檔 TypeScript 錯誤清零 — TODO 草案 R1
...
# 預期：Test Files 5 passed (5)；Tests 31 passed (31)
```
...
```
task-id: p2debt-t3
讀 handoffs/P2DEBT-T3-TODO-DRAFT-R1.md §0 + Phase 1 Task 1.1–1.5。
...
| # | 位置 | 精確變更 | 驗證命令 + 預期輸出 |
|---|------|----------|---------------------|
| **1.5.1** | L43–45（有日期 `it`） | `vi.fn(async () => response(200, …))` → `vi.fn(async (_url: string, init?: RequestInit) => response(200, …))` | `cd frontend && npx tsc --noEmit 2>&1 \| rg "batchDate.test.ts\(55"` → 0 行（基線 2 行 TS2493/TS2339） |
...
```bash
# §1 主驗收 — TypeScript 零錯誤
cd frontend && npx tsc --noEmit
tsc_rc=$?
cd frontend && npx tsc --noEmit 2>&1 | grep -c "error TS"
# Composer 實跑壞基線：tsc_rc≠0；error count=11
# 完工預期：tsc_rc=0；grep -c → 0
...
cd frontend && npm test -- --run \
  ...
  src/lib/runExplorer.test.ts \
  src/store/featureFactoryStore.test.ts
# Composer 實跑 2026-07-11：Test Files 5 passed (5)；Tests 31 passed (31)
# 完工預期：同上

# §3 Gate A/B/C rc 契約（見 Task 2.1–2.3）
# Gate A：rg_rc=1 → test exit 0
# Gate B：合計 87
# Gate C Leg1：diff pre/post per-file counts → rc=0
# Gate C Leg2：diff pre/post expect bodies → rc=0（或 adversarial 簽核）

# §4 scope gate（見 Task 3.1）
# delta-dirty 與 whitelist diff → rc=0
...
SPEC=handoffs/P2DEBT-T3-SPEC-DRAFT-R4.md TODO=handoffs/P2DEBT-T3-TODO-DRAFT-R1.md FOCUS=11-error test-only tsc fix + Gate A/B/C + pre-dirty scope gate；用 templates/SPEC_TODO_ADVERSARIAL_REVIEW_PROMPT.md 獨立審查；Blocking 修補後才 Frozen。
