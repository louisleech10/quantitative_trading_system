# P2 債票 3 — frontend feature-factory 測試檔 TypeScript 錯誤清零 — SPEC 初稿 R2

> 來源：HANDOFF 票 3 + `templates/SPEC_TEMPLATE.md`  
> 日期：2026-07-11　|　起草：Composer　|　task-id：`p2debt-t3`  
> 狀態：**初稿 R2**（Codex BLOCK 兩項修正；待 Grok + Codex 雙家族 adversarial 審後正式化；起草人不得自審）
...
- **防假綠（可執行 gate；區分測試數 vs expect 站點）**：
  - **基線計數（派工前一次擷取）**：`rg -c '\bexpect\('` 五檔合計 **87** 站點（非 31）；per-file：`run_lifecycle.test.tsx:20`、`RunManagerPanel.batchDeleteWhole.test.tsx:10`、`useFeatureFactory.batchDate.test.ts:4`、`runExplorer.test.ts:9`、`featureFactoryStore.test.ts:44`（Composer 實跑 2026-07-11）。
  ...
  - **Gate B — expect 站點數不變（預期 87）**：
    ...
    ```
  - **Gate C — expect 行內容不變（派工前擷取 pre，完工 diff）**：
    ```bash
    ...
    rg -n '\bexpect\(' \
      ...
      frontend/src/hooks/useFeatureFactory.batchDate.test.ts \
      frontend/src/lib/runExplorer.test.ts \
      frontend/src/store/featureFactoryStore.test.ts | sort > /tmp/p2debt-t3-pre-expect.txt
    # Composer 實跑 2026-07-11：wc -l /tmp/p2debt-t3-pre-expect.txt → 87
    #
    # 完工後：
    rg -n '\bexpect\(' \
      frontend/src/components/feature-factory/__tests__/run_lifecycle.test.tsx \
      ...
      frontend/src/hooks/useFeatureFactory.batchDate.test.ts \
      frontend/src/lib/runExplorer.test.ts \
      frontend/src/store/featureFactoryStore.test.ts | sort > /tmp/p2debt-t3-post-expect.txt
    diff -u /tmp/p2debt-t3-pre-expect.txt /tmp/p2debt-t3-post-expect.txt
    # Composer 實跑 2026-07-11（pre=post 壞基線）：rc=0、無 diff 輸出
    # 完工驗收：rc=0（任一 expect 行或期望值變更 → FAIL）
    ```
  - 允許同檔**非** `expect` 行之型別補欄（fixture/helper/mock 簽名）；**禁止**改動 `expect(...)` 行本身。
- **邊界目錄**（本任務適用）：  
