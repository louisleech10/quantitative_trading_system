# 完整地圖 階段五「多因子與系統觀」— Claude 互審綜合（待委員檢查）
...
| 5 因子暴露/歸因 | **🔌+雷達誤導** | FactorExposureAnalyzer有,但attribution多np.nan空、positions等權非真持倉、用label當market_proxy;前端雷達讓人誤以為做了投組歸因 |
| 6 多因子組合 | **❌最大缺口** | IC加權因子合成缺;「這因子有獨特新資訊嗎」的產品答案之一缺 |

## 逐型定案（9 欄）

### 1. 相關 / Clustering / VIF（冗餘）
- 🔍 這批因子是不是互相抄答案?保留哪個代表? | 📐 corr/hierarchical clustering/VIF;先IC/ICIR排序再去重 | 🗂 factor×factor矩陣
...
- 🔍 對已知因子回歸後殘差還有預測力嗎? | 📐 OLS residual IC、Gram-Schmidt、PCA residual、sector/market/beta/vol neutralized IC;報residual IC/ICIR | 🗂 factor矩陣+label
...
- 🧩 **後端🔌(deep預設關) 前端❌/🔌(deep tab無正交化專屬圖,有centrality/exposure/net IC無orthogonalization chart) 連結⛓️‍💥** → **🔌+誤稱**:可跑、前端無視覺化、預設不跑、**名實不符(claims neutralized IC實為transform summary)**
...
- 🧩 **後端🔌 前端🎨(radar) 連結⛓️‍💥(未接策略/回測真positions)** → **🔌+雷達誤導**:前端雷達讓使用者誤以為已做投組歸因,實際attribution多空、positions等權非真持倉
- 🛡️ beta neutral用全期covariance;vol neutral用rolling std較好;market_proxy用label語義可疑 | ⚡ O(n·p)exposure可接受;真regression p大O(p²/p³) | 🔧 ①attribution多空 ②positions等權非真持倉 ③market_proxy=label可疑 ④雷達誤導已做歸因 | 🏷️ P1(接回測/模型真positions+真portfolio_returns,否則UI標「診斷proxy」)

### 6. 多因子組合 (IC加權/組合IC)
- 🔍 多弱因子合成更穩?合成後經濟意義還在? | 📐 IC/ICIR weighted composite、risk parity、walk-forward learned weights、組合IC/turnover/capacity | 🗂 多因子+權重→composite signal
...
- **最大產品缺口**:IC→ML橋(型4)+多因子組合IC(型6)——ML-first平台的核心敘事缺。
