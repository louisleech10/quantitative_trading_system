...
TESTS_RUN: `venv/bin/python handoffs/20260911-splitunify-b9-probe-multitf.py` rc=0（C 靜默丟列）；`venv/bin/python scripts/freeze_splitunify_golden.py` rc=0/GOLDEN OK；兩組 pytest 分別 115 passed、85 passed；doc_format、obligation、synth-xref rc=0；交件 completeness 命令待最後重跑。
FAILURES_SEEN: 曾用過時 `spec_xref_check.sh --synth` 形式 rc=2，改用正確 `--synth <synth> <target>` 後 rc=0；未改碼、未改測試斷言。
