# Reconcile — 20260912-verdictgate-x-review-r11

**來源** 20260912-verdictgate-x-review-r11-codex.md, 20260912-verdictgate-x-review-r11-composer.md, 20260912-verdictgate-x-review-r11-grok.md　|　**roster** codex,composer,grok

## 群集 / 處置

**修訂標的**：scripts/verdictgate_check.sh

**Verdict**：需修補後合併——三家皆 `blocked`，同一 P1（closure 首進即算「已進入」）＋兩條 P2（登記表行號語意偏、codex 交件語料）。全採納已修；派三家閉合 R12（各自重驗本家 P1）。

| 群集 | 嚴重度 | 來源 ID | 處置 |
|---|---|---|---|
| **S1 closure-first 繞過**——「C-4 的「本批已有任一 round_open」；「`verdictgate_check.sh` 之「本批已有審查輪」判定對任意輪」；「`verdictgate_check`「本批已有任一 `committee_round_open` 則跳過前批驗證」加上 closure 開輪免除 verdictgate」⇒ 先開 closure 輪即可越過前批 blocked | P1 | CODEX-R11-P1-01、COMPOSER-R11-P1-01、GROK-R11-P1-01 | 採納（「已進入」只認本批 brief_kind=review 或 legacy review-like 之 round_open，且未 debt_abandon；closure／consult／stamp 不算；p2 新測試 sole-closure／consult／abandoned 皆擋、legacy review-like 算；mutate-b2 M4b／M4c／M4d） |
| **S2 登記表行號語意偏**——「E-022、E-023、E-024 的 registry reason 含可執行但語義不指向所稱 enforcement 的行號」；「E-024 理由欄寫 `ticket_batch_check.sh:49` 為 `--push-range`」 | P2 | CODEX-R11-P2-02、GROK-R11-P2-01 | 採納（E-022 第二層改 gate.sh:959 呼叫點；E-023 :951→:959；E-024 :49→:88；fact-key 區塊重生成後 check rc=0。codex 建議「registry check 覆蓋語意對位」屬新機制，不在本票——登記為 §E E-8 needs-research：研究問題＝行號語意對位能否降為「引用行必含所稱函式名／旗標字串」封閉判準；完成判準＝對 E-001～E-025 回放誤擋 0、抓到本輪三處） |

### 本輪程序記錄
- codex 交件 CLOSED R10 三條被 `register-output` 拒收：主委 brief task-id 用 20260912 日期前綴 ⇒ root 變 `20260912-verdictgate`，語料不含 R10。移除該三條（R10 已於 TODO v3 凍結時處置；X 層不進 verdictgate）後重註冊。🔴 教訓：task-id 日期前綴屬 root，同票跨日沿用首日前綴。
- 三家對 E-5 probe 皆重跑 `ORDER-ALWAYS-HOLDS`、立場一致「序判無增量、mtime 保留」；E-1／E-2／E-7 user-ruling 有效；E-3／E-6 needs-research 維持；E-4 由 SPLITUNIFY B7-REVIEW-R2 處置。
- 三家皆要求：C-4 範圍修正須有 sole-closure 負向測試（已補）；codex 另要求走 FROZEN 修訂程序改 SPEC 字面——主委處置：本輪 synth＋HANDOFF 記錄，SPEC 修訂於下張治理票（§E E-7 同批）併入，不在收票內重開 SPEC 審（使用者 2026-09-11「無法收斂…適時停止」）。此立場交 R12 三家核。
