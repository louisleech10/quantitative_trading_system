# P2 債票 3 — frontend feature-factory 測試檔 TypeScript 錯誤清零 — TODO 草案 R2

> 狀態：**DRAFT**（待 Grok + Codex adversarial 複驗；起草人不得自審）  
> 基於：`handoffs/P2DEBT-T3-SPEC-DRAFT-R4.md`（雙戳記：grok + codex；**不得改 SPEC 內容**）  
> task-id：`p2debt-t3`　|　日期：2026-07-11　|　起草：Composer  
...
> **R2**：`set +e` + `run_step` 累積 `fail`；禁鏈式二次 `cd frontend`（會變 `frontend/frontend`）；tsc 先斷言 compiler rc 再計數；vitest 斷言 `package.json` `"test": "vitest run"` 契約。
...
tsc_count=$(grep -c "error TS" /tmp/p2debt-t3-batch-tsc.log 2>/dev/null || echo 0)
echo "TSC_ERROR_COUNT=$tsc_count"
run_step tsc_zero bash -c 'test "$tsc_count" -eq 0'
# 壞基線：STEP_RC[tsc]=1；TSC_ERROR_COUNT=11
...
```
task-id: p2debt-t3
讀 handoffs/P2DEBT-T3-TODO-DRAFT-R2.md §0 + Phase 1 Task 1.1–1.5。
...
## Phase 1 — 測試型別對齊（目標：11 顆 tsc 錯誤 → 0；vitest 31 仍全綠） VERIFY-EXEMPT:draft-superseded:p2debt-t3
...
> **R2**：六個可執行 leg + `run_step` 累積；每 leg 自有 exit 契約；禁 `producer | consumer` 掩蓋 producer rc；§3–§5 不得僅註解指向他處。
...
# §1 — TypeScript 零錯誤（先斷言 compiler rc，再計數；禁 `tsc | grep -c` 當唯一 gate）
run_step tsc bash -c 'cd frontend && npx tsc --noEmit 2>&1 | tee /tmp/p2debt-t3-final-tsc.log; exit ${PIPESTATUS[0]}'
tsc_count=$(grep -c "error TS" /tmp/p2debt-t3-final-tsc.log 2>/dev/null || echo 0)
echo "TSC_ERROR_COUNT=$tsc_count"
run_step tsc_count bash -c 'test "$tsc_count" -eq 0'
# Composer 實跑壞基線：STEP_RC[tsc]=1；TSC_ERROR_COUNT=11
...
# Composer 實跑 2026-07-11：STEP_RC[vitest]=0；Test Files 5 passed；Tests 31 passed

# §3 — Gate A（rg 三極性：1=PASS）
...
# Composer 實跑：rg_rc=1 → STEP_RC[gate_a_rg]=0

# §4 — Gate B（先寫檔再 awk；禁 `rg | awk` 掩蓋 rg rc=2）
rg -c '\bexpect\(' \
  frontend/src/components/feature-factory/__tests__/run_lifecycle.test.tsx \
  frontend/src/components/feature-factory/__tests__/RunManagerPanel.batchDeleteWhole.test.tsx \
  frontend/src/hooks/useFeatureFactory.batchDate.test.ts \
  frontend/src/lib/runExplorer.test.ts \
  frontend/src/store/featureFactoryStore.test.ts > /tmp/p2debt-t3-final-gate-b.txt
gate_b_rg_rc=$?
echo "GATE_B_RG_RC=$gate_b_rg_rc"
run_step gate_b_rg bash -c 'test "$gate_b_rg_rc" -eq 0'
gate_b_sum=$(awk -F: '{s+=$2} END {print s+0}' /tmp/p2debt-t3-final-gate-b.txt)
echo "GATE_B_SUM=$gate_b_sum"
run_step gate_b_sum bash -c 'test "$gate_b_sum" -eq 87'

# §5 — Gate C Leg1（pre/post per-file counts；派工前須已存 /tmp/p2debt-t3-pre-expect-counts.txt）
rg -c '\bexpect\(' \
  frontend/src/components/feature-factory/__tests__/run_lifecycle.test.tsx \
  frontend/src/components/feature-factory/__tests__/RunManagerPanel.batchDeleteWhole.test.tsx \
  frontend/src/hooks/useFeatureFactory.batchDate.test.ts \
  ...
  frontend/src/store/featureFactoryStore.test.ts | sort > /tmp/p2debt-t3-post-expect-counts.txt
gate_c_rg_rc=$?
run_step gate_c_rg bash -c 'test "$gate_c_rg_rc" -eq 0'
diff -u /tmp/p2debt-t3-pre-expect-counts.txt /tmp/p2debt-t3-post-expect-counts.txt
gate_c_diff_rc=$?
echo "GATE_C_DIFF_RC=$gate_c_diff_rc"
run_step gate_c_diff bash -c 'test "$gate_c_diff_rc" -eq 0'

# §6 — scope gate（pre-dirty `comm -13`；禁 `git status | awk` 管線當唯一 rc）
git status --porcelain | awk '{print $NF}' | sort -u > /tmp/p2debt-t3-post-dirty.txt
scope_git_rc=$?
run_step scope_git bash -c 'test "$scope_git_rc" -eq 0'
comm -13 /tmp/p2debt-t3-pre-dirty.txt /tmp/p2debt-t3-post-dirty.txt | sort -u > /tmp/p2debt-t3-delta-dirty.txt
printf '%s\n' \
  frontend/src/components/feature-factory/__tests__/run_lifecycle.test.tsx \
  frontend/src/components/feature-factory/__tests__/RunManagerPanel.batchDeleteWhole.test.tsx \
...
run_step scope_diff bash -c 'test "$scope_diff_rc" -eq 0'
# 壞基線（無實作）：SCOPE_DIFF_RC=1

# §7 — 解耦回歸（先計數再 test -eq；禁 `grep | wc -l` 管線 rc 當唯一 gate）
decouple_n=$(grep -r "from api\." momentum/ 2>/dev/null | wc -l | tr -d ' ')
echo "DECOUPLE_N=$decouple_n"
run_step decouple bash -c 'test "$decouple_n" -eq 0'
...
SPEC=handoffs/P2DEBT-T3-SPEC-DRAFT-R4.md TODO=handoffs/P2DEBT-T3-TODO-DRAFT-R2.md FOCUS=11-error test-only tsc fix + Gate A/B/C + pre-dirty scope gate；用 templates/SPEC_TODO_ADVERSARIAL_REVIEW_PROMPT.md 獨立審查；Blocking 修補後才 Frozen。
