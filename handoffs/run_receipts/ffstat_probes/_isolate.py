...
def isolate(prefix: str = "ffstat_probe_") -> Path:
...
def isolate_dstar_cache(root: Path) -> Path:
    """把 d* 快取目錄導向 `root/dstar_cache`（現行 `FeaturePreprocessor._d_star_cache_dir` 固定指向專案
    `data_cache/feature_preprocessing`，不受上方環境變數影響；2026-09-24 FF-STAT 探針實跑時因此寫入共用快取）。
    須於匯入 momentum 之後呼叫。"""
