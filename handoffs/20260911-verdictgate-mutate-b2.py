...
MUTATIONS = [
    ...
    ("M4b 任何輪（含 closure）都算進入（收票審 R11 三家同題）", "scripts/verdictgate_check.sh",
     "    if is_review:\n        sys.exit(0)", "    if True:\n        sys.exit(0)",
     f"{TEST}::test_check_sole_closure_round_does_not_count_as_entered"),
    ("M4c 已進入批次判定恆假（閉合輪也重驗前批 ⇒ 補裁決輪連鎖回溯）", "scripts/verdictgate_check.sh",
     "    if is_review:\n        sys.exit(0)", "    if False:\n        sys.exit(0)",
     f"{TEST}::test_check_batch_already_entered_skips_prev_verdicts"),
    ("M4d abandon 之 review 輪也算進入", "scripts/verdictgate_check.sh",
     'if not t.lower().startswith(pfx) or r.get("round_id") in abandoned:', 'if not t.lower().startswith(pfx):',
     f"{TEST}::test_check_sole_closure_round_does_not_count_as_entered"),
