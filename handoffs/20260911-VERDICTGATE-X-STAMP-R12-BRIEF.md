...
brief-kind: stamp
task-id: 20260911-VERDICTGATE-X-STAMP-R12
findings-round: R12
...
🔴 **composer**：你 R10 已 `proceed`；本輪只核 synth 忠實性。codex 已於 R11 APPROVED 並對 R10 三條寫 `CLOSED:`（檔內既有戳記行保留，勿抄其雜湊）。

## 🔴 交件形態（缺這段整輪會銷不了帳）

無論結論為何，**交件檔內必須至少有一個 canonical heading**：
- 有問題 → `## <FAMILY>-R12-P<0-3>-<NN>`，含 `**斷言**`／`**碼證**`／`**來源摘要**`（P0/P1 須 12 位雜湊）。
