# GAP-2a／2b TODO adversarial 審查 R1（TODO DRAFT R1 vs SPEC R7 FROZEN）

brief-kind: review

## 範本
照 `templates/SPEC_TODO_ADVERSARIAL_REVIEW_PROMPT.md` 全文執行（§0 挑戰前提／§1 必查 11 類／§2 錨點＋獵空殼＋殘留／canonical 四欄／Verdict）。
findings 用 canonical ID `## <FAMILY>-R7-P<0-3>-<NN>`（本票 SPEC 已走 R1–R6，TODO 首輪＝**R7**）；四欄含 `**來源摘要**: <路徑>#<sha256 前 12 碼>`。0 finding 寫 sentinel `## <FAMILY>-R7-P3-00`。

## 審查標的
- **TODO（本輪主標的）**：`docs/GAP2_MARGINAL_IC_TODO.md`（DRAFT R1；`template_check todo` PASS；`todo_spec_crosscheck` SMOKE PASS）
- **SPEC（義務來源，R7 FROZEN，不改；SPEC 缺陷走延伸檔 `docs/GAP2_MARGINAL_IC_AMENDMENTS.md`）**：`docs/GAP2_MARGINAL_IC_SPEC.md`
