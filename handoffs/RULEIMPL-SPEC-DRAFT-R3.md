# RULEIMPL — 編排端自產驗收尺機械兜底 — SPEC+TODO 修訂稿 R3
(VERIFY-EXEMPT:doc-example:ruleimpl-draft-pre-formalization——本檔=初稿,驗收句於正式化+實作時取真收據)

> **狀態**：Composer 修訂（吸收 Grok R2 BLOCK：`要害 B` / `N1` / `N2`）；正式化由編排端 `bash scripts/gate.sh artifact` + `templates/SPEC_TEMPLATE.md` 走 artifact 流程。  
> **來源條文**：`handoffs/RULE-PROPOSAL-RECONCILE.md` 四條 v2 + R2 節。  
> **審查對照**：`handoffs/RULEIMPL-REVIEW-R2-grok.md`（R2 複驗 VERDICT: BLOCK；本稿逐條關閉 STILL-OPEN）。  
> **前稿**：`handoffs/RULEIMPL-SPEC-DRAFT-R2.md`（已 supersede）。  
> **task-id**：`RULEIMPL`  
> **日期**：2026-07-11
...
- **FACT-RECEIPT: 歷史 a/d SPEC 現況（2026-07-11 實跑）**
  - **11** 份 `docs/*SPEC*.md` 含 `RISK-HIT` 之 `a` 或 `d`、**零** `VALIDATION-ARTIFACT` 欄、今日 `template_check` 仍 PASS（例：`IC_PHASE1_1E1B_SIGNIF_SPEC.md`、`IC_PHASE1_1a_CUT1_SPEC.md`、`TEMPLATE_GATE_FIX_SPEC.md`）。
...
- **向後相容（歷史 SPEC grandfather，鎖死選項 A）**：
  ...
  - **enforce 日期後**：**新建** SPEC，或 **commit 觸碰 §G 段**之既有 SPEC → 觸發條件成立則三欄 **FAIL** 若缺（未觸碰 §G 之歷史 a/d 檔可繼續 WARN+PASS，不因模板新增欄位而失敗）。
  ...
  - **回歸門檻（吸收 Grok R2 N2）**：
    ...
    - `tests/governance/test_verify_gate_b4.py`、`test_verify_gate_b5.py`、`test_verify_gate_redteam.py` 中列於 §V「已知 P2 債」之 9 條 **不在本票通過條件**；修復屬 **P2 另票**（`HANDOFF.md` P2 債登記）。
    - **禁止**為 RULEIMPL 通過而弱化 b4/b5/redteam 斷言或從 CI 移除上述測試。
    - 全量 `pytest tests/governance/`：**fail 數不得超過 9**；任何**新增** fail → STOP（含本票新測試與非 P2 既有測試）。
...
- 改法要點：
  1. **觸發**（Grok R2 聯集，任一成立）：
     ...
     - 或 **§G 段**（`## §G` 至下一 `## §` 標題，**不含 §N**）匹配機讀 regex：`(baseline_manifest|generate_baseline|capture_.*baseline|new-or-changed 驗收|golden.*對照|oracle|canonical.*快照)`（大小寫不敏感；**禁止**用全文掃描誤觸 §V 之「Golden 對照」敘述，吸收 Grok N4）。
     - 或同 task `--todo` 檔含 capture/baseline 語言（見 Task 2.2）。
  ...
  3. **觸發後**：
     ...
     - `new-or-changed`：
       ...
       - 對 manifest 檔 `## 戳記` 前 body 跑戳記檢查：`VALIDATION-STAMP: <family> APPROVED <date> sha256:<body-hash> task:<id>`（格式對齊 `RECONCILE-STAMP`；**D1 已鎖 A**）；
  ...
  4. **`VALIDATION_ENFORCE_AFTER` grandfather**（§C 選項 A）：日期前舊 SPEC 缺欄 → WARN+PASS；日期後新建或觸碰 §G → FAIL（見 §C 精確措辭，吸收 Grok N5）。
...
- **Validation run receipt 必填欄**（`schema_version: "1.0"`, `receipt_type: "validation_run"`）：
  ...
  - append 至 `verify_audit.log` + `event:validation_run`（**D2 已鎖 B**）；consumer **必須**濾 `event`+`emitter`，禁與 VERIFY receipt 混判。
...
- 檢查項（fail-closed，依序）：
  ...
  2. 若 `enforce=False` 且 caller **不在** `tests/governance/`（含 `scripts/` harness）→ **忽略 enforce 參數，繼續 step 3+**（fail-closed；吸收 Grok N6）。
  3. 若 manifest 含合法 `validation_waive`（§C 窄類別+記名+未過期）→ 記錄 WARN 至 stderr **一次**，PASS（**不得**用於新 baseline；僅 `pre-ruleimpl-baseline`）。
  4. 否則 manifest **必**含 `validation_run_receipt` 路徑；
  5. receipt 檔存在；`receipt_type == validation_run`；`emitter == validation_run_receipt.py`；
  6. 審計 log 有匹配 `receipt_id` 且 `event:validation_run`；
  7. 重算 `generator/inputs/config/outputs/envelope` hash 與 receipt 欄位一致；
  8. `manifest_path` 與 receipt.`manifest_path` 一致。
...
- **必做**：
  ...
  - **至少一處** `scripts/` 生產 harness 強制消費端：**D3 已鎖 B** — `scripts/ic1eb_b5_replay.py` 在 `load_manifest()` 後呼叫 `require_validation_receipt`（manifest 無 receipt 且無合法 waive → FAIL）。
...
### 已知 P2 債（排除於 RULEIMPL 通過條件；出處 `HANDOFF.md` P2 債登記，2026-07-11）

先於本 session 之既有紅燈（`template_check` commit `3edfa6c` 之後 fixture 過時）；**P2 另票修**；本票 **禁止** 弱化斷言換綠。
...
**FACT-RECEIPT: 2026-07-11 實跑全量** — `pytest tests/governance/ -q --tb=no` → **9 failed, 140 passed**（149 collected）；上表 9 條與 fail 清單一一對應。
...
**層級 B — 全量 governance（Phase 4.4；排除已知 9 紅後其餘須全綠；fail 總數不得 >9）**

```bash
pytest tests/governance/ -q --tb=short
```

**FACT-RECEIPT: 2026-07-11 實跑** → **9 failed, 140 passed**（僅上表 9 條可紅；**禁止**新增 fail）

**驗收規則（吸收 Grok R2 N1/N2/要害 B）**：
- 不得把層級 A 的 `35 passed` 與層級 B 混寫。
- 不得把 `test_verify_gate_b5.py` 納入層級 A 卻標「全綠」。
- RULEIMPL 合入後：層級 A 仍須 **35 passed**；層級 B **fail 數 ≤ 9** 且新增 fail 集合 **⊆ ∅**（相對上表不得擴大）。
...
**V-T5 觸發面邊界（吸收 Grok N4）**：Task 2.1 產生尺 regex **僅掃 §G 段**；`INSTREV_PHASEB` 全文 §V 含「Golden 對照」敘述**不得**單獨觸發 validation 三欄（實作須有段界測試或 fixture 鎖定）。
...
- **F6（與 §V 回歸同步，吸收 Grok 要害 B）**：
  - 基線命令：`pytest tests/governance/ -q --tb=line`
  - **2026-07-11 實跑**：**9 failed, 140 passed**（149 collected）；fail **僅**允許為 §V「已知 P2 債」上表 9 條。
  - RULEIMPL 驗收：**fail 數不得增加**；層級 A 兩檔須維持 **35 passed**。
  - 與 validation-run 子命令不存在時之「既有全綠」**不**構成假綠證據。
  - **禁止**為通過 F6 而弱化 b4/b5/redteam 斷言或從集合剔除非 P2 測試。
...
- **完工分級（凍結，吸收 Grok CHALLENGE #5、#17）**：
  - **`MECH-HELPER-DONE`**：Phase 1–3 + governance 測試（V-T/V-G/V-C fixture）全綠；**尚無**真實 harness 強制消費端；層級 A 回歸 **35 passed**；層級 B **fail ≤ 9**（僅 P2 債）。
  - **`MECH-FAILCLOSED-DONE`**：在上一級基礎上，≥1 真實 `scripts/` harness（Task 4.2 D3=B）預設呼叫 `require_validation_receipt` 且 V-C4–C7 全綠；層級 B 仍 **fail ≤ 9**。
...
- **P2 governance fixture 債（b4×3 + b5×5 + redteam r7）**：N/A — 本票不修；見 §V 已知 P2 債表與 `HANDOFF.md` P2 節。

---

# TODO

> 執行順序：Phase 1 → 2 → 3 → 4；每 Phase 收尾跑「層級 A 回歸 + 本 Phase §V」再進下一 Phase。  
> **允許改動檔案（白名單）**：  
...
> **禁止**：`momentum/**`、`api/**`、`data_cache/**`、弱化既有測試斷言、Task 4.2 整項 waive、為通過回歸而削弱 §V 已知 P2 債所列測試。
...
- [ ] **1.3 回歸**：層級 A → **35 passed**（命令見 §V）。

## Phase 2 — template_check（Task 2.1–2.2）

- [ ] **2.1** 實作 `_validation_triggered` + 三態邏輯 + `VALIDATION_ENFORCE_AFTER` + 未知 enum FAIL + §G 段界 regex（N4）。
- [ ] **2.2** 實作 todo 聯檢（V-T7）。
...
- [ ] **3.4 驗收**：`pytest tests/governance/test_ruleimpl_validation_gate.py -k validation_run -q` 全綠；`pytest tests/governance/test_dispatch_wrapper.py -q` 全綠；層級 A → **35 passed**。

## Phase 4 — 消費端拒收（Task 4.1–4.2）— **不可 waive**

- [ ] **4.1** 實作 `validation_consumer_check.py`（預設 enforce=True；窄類別 waive；`enforce=False` 非 governance caller 仍 fail-closed，N6）。
...
- [ ] **4.4 全量回歸**：`pytest tests/governance/ -q --tb=line` — **fail 數 ≤ 9** 且僅 §V P2 表內測試可紅；否則 STOP。
...
## 已鎖決策（吸收 Grok R2 N3；D4 仍待委員會）

| # | 決策 | 鎖定值 | 備註 |
|---|------|--------|------|
| D1 | 戳記附著檔 | **A** — manifest 本體 `## 戳記` | 對齊 `reconcile_stamps_check.sh` |
| D2 | validation audit log | **B** — 併入 `verify_audit.log` + `event` 欄 | consumer 濾 event |
| D3 | IC1EB manifest 接入 | **B** — `ic1eb_b5_replay` 強制消費端 | Task 4.2 不可 waive |
| D4 | `VALIDATION_ENFORCE_AFTER` 日期 | **待鎖** | 開票日+14d **或** 固定 `2026-08-01` |

---

## Grok R2 STILL-OPEN / 新縫修訂對照

| ID | R2 狀態 | R3 落點 |
|----|---------|---------|
| 要害 B | STILL-OPEN（§V/F6 基線句假） | §V 層級 A/B 分離 + FACT 實跑 35/9+140；F6 同步；§C 回歸門檻 |
| N1 | BLOCKING（三檔綁 35 passed 假） | 刪錯誤三檔命令；層級 A 兩檔 35 passed；層級 B 全量含 P2 排除規則 |
| N2 | BLOCKING（`test_verify_gate*.py` 全過不可達） | §C 拆核心兩檔 vs P2 債；禁止弱化 b5 |
| N3 | 非阻塞（開放決策表張力） | D1/D2/D3 改「已鎖」表；僅 D4 待鎖 |
| N4 | 非阻塞（§G vs 全文 golden） | Task 2.1 §G 段界 + §V V-T5 邊界句 |
| N5 | 非阻塞（enforce 日期措辭） | §C「新建或 commit 觸碰 §G」精確化 |
| N6 | 非阻塞（enforce=False 非測試 caller） | Task 4.1 step 2 釘死 fail-closed |

---

## Grok CHALLENGE 修訂對照（R1 22 條，R2 已 CLOSED，R3 維持）

| # | Grok 審查點 | R3 落點（繼承 R2） |
|---|-------------|-------------------|
...
| 11 | 消費端未接真實 harness BLOCKING | D3 鎖 B；Task 4.2 不可 waive |
| 12 | disposable cp 無機械禁 | §C 誠實 PARTIAL |
| 13 | validation-run 非 gate_check kind | Task 3.1 run lease 語意 |
| 14 | run_with_receipt audit event 混判 | D2 鎖 B + consumer 濾 event |
...
- **產出檔**：`handoffs/RULEIMPL-SPEC-DRAFT-R3.md`（本檔）
- **ASSUMPTIONS_VERIFIED**：§A FACT-RECEIPT 2026-07-11 實跑；層級 A 35 passed；層級 B 9 failed + 140 passed；V-T5 三錨 exit 1/0/0；歷史 a/d 無 VA 仍 PASS 掃描 11 份
- **TESTS_RUN**：
...
- **FAILURES_SEEN**：上列 9 fail 為已知 P2 基線（非本稿引入）；VERIFY_GATE_SPEC template_check 預期 exit 1（反例）
- **SCOPE_CHANGES**：none（僅本修訂檔）
- **NUMERIC_OR_SCHEMA_IMPACT**：新增 validation_run receipt schema（治理）；不變 momentum 數值輸出；回歸基線改為「排除 9 已知紅 + 不得新增 fail」
