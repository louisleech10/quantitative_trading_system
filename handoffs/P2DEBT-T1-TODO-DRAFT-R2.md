# P2 債票 1 — governance 9 紅 fixture 遷移 — TODO 草案 R2
...
- **防假綠（§C）**：
  ...
  3. scope 驗收 = 派工前快照（`git rev-parse HEAD > /tmp/p2debt-t1-pre-head.txt`）vs 完工後四檔 whitelist `diff`（見 Final Acceptance §2）；**不得**用未扣 baseline 的全域 `git diff --name-only` 當驗收。
...
```
task-id: p2debt-t1
讀 handoffs/P2DEBT-T1-TODO-DRAFT-R1.md §0 + Phase 1 Task 1.1–1.3（含 1.2b）。
...
| scope | diff 白名單 | 派工前 `git rev-parse HEAD > /tmp/p2debt-t1-pre-head.txt`；完工 `PRE_HEAD=$(cat /tmp/p2debt-t1-pre-head.txt)` + `diff -u /tmp/p2debt-t1-whitelist.txt <(git diff --name-only "$PRE_HEAD" \| sort -u)` → exit 0（whitelist 四檔見 Final Acceptance §2；Composer 實跑壞基線：6 外檔，**rc=1**） |
...
# 2) scope 驗收 — 僅允許路徑（B 案；可執行 gate）
# 派工前（一次）：
git rev-parse HEAD > /tmp/p2debt-t1-pre-head.txt
git diff --name-only HEAD > /tmp/p2debt-t1-pre-diff.txt
# Composer 實跑 2026-07-11：HEAD=f0f89c1c0bb751f6fb2b75ab68e973c677f5b6e9；pre-diff 6 行（.claude/gate/audit.log、.claude/settings.json、tests/golden/ic_phase1_1a_cut1/*×4）
# 完工後比對（四檔 whitelist oracle）：
PRE_HEAD=$(cat /tmp/p2debt-t1-pre-head.txt)
git diff --name-only "$PRE_HEAD" | sort -u > /tmp/p2debt-t1-post-diff.txt
...
diff -u /tmp/p2debt-t1-whitelist.txt /tmp/p2debt-t1-post-diff.txt
# 預期：無 diff（exit 0）；不得含 scripts/
# Composer 實跑壞基線：diff 顯示 whitelist vs 6 外檔，exit 1
...
SPEC=handoffs/P2DEBT-T1-SPEC-DRAFT-R3.md TODO=handoffs/P2DEBT-T1-TODO-DRAFT-R1.md FOCUS=governance fixture 遷移 + §V 三負例 + B 案 VERIFY_GATE_SPEC 錨點；用 templates/SPEC_TODO_ADVERSARIAL_REVIEW_PROMPT.md 獨立審查；Blocking 修補後才 Frozen。
