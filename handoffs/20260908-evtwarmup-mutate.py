...
MUTATIONS: Tuple[Mutation, ...] = (
    ...
    Mutation("T3-semantic-invalid-timeframe-accepted", 3, "momentum/Analysis/ic_engine.py",
             "        return hours is not None and np.isfinite(hours) and hours > 0\n",
             "        return hours is not None\n",
             [*PYTEST, "tests/api/test_tfwindow.py", "-k", "semantic_invalid"], "0h／inf／nan 當 applied ⇒ 引擎邊界測試紅（R5 CODEX-R5-P1-02）"),
    Mutation("T4-reference-tf-not-synced", 3, ORCH,
             "            reference_tf=config.ic_calculation.icir.reference_tf,\n",
             "            reference_tf=None,\n",
             [*PYTEST, "tests/api/test_tfwindow.py", "-k", "config_override_reference_tf"], "effective reference 不進引擎 ⇒ 揭露≠實際鍵 紅（R5 CODEX-R5-P2-03）"),
