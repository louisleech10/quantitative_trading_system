# P2 債票 3 — frontend feature-factory 測試檔 TypeScript 錯誤清零 — TODO 草案 R3

> 狀態：**DRAFT**（待 Codex adversarial 複驗；起草人 Grok 不得自審）  
> 基於：`handoffs/P2DEBT-T3-SPEC-DRAFT-R4.md`（雙戳記：grok + codex；**不得改 SPEC 內容**）  
> 修訂依據：`handoffs/P2DEBT-T3-TODO-REVERIFY-codex.md`（R2 BLOCK：unexported parent vars + zero-match `grep -c||echo 0` multi-line）；**僅**改 acceptance shell；Task 1.1–1.5 實作清單零改  
> task-id：`p2debt-t3`　|　日期：2026-07-11　|　起草：Grok（斷路器換手；R2=Composer）  
