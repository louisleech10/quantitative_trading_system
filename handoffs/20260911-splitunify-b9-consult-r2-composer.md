...
1. 本輪三家裁定 **REVERT** 未 commit 四檔（或等同：checkout 至 HEAD 且 pytest 兩檔全綠）。 VERIFY-EXEMPT:doc-example:committee-proposed-criterion
   〔主委註記，非委員原文〕本條是委員**提出的**閉合條件，不是主委的完成宣稱，故標 doc-example。實際執行與 receipt 記於 `HANDOFF.md` 與本批 commit 訊息（receipt id `20260913T125919Z-splitunify-b9-revert-verify`）。
