...
MUTATIONS: Tuple[Mutation, ...] = (
    ...
    Mutation(
        ...
        "purge_gap=max(effective_horizon, label_window_rows)",
        "purge_gap=effective_horizon",
        [*PYTEST, "tests/momentum/event_samples/test_isolation_terms.py", "tests/momentum/Analysis/test_evtlabel_isolation_channel.py", "-k", "purge"],
        "purge 改回只吃主線 horizon ⇒ G-2(ii)／G-3 期望 12 變 5 ⇒ 紅",
    ),
    Mutation(
        "M-P2-2-embargo-back-to-max-depth-window", 2, SERVICE,
        "max(int(cell_override.get(\"embargo\") or 0), int(staged[\"lookahead_depth_rows\"]))",
        "max(int(cell_override.get(\"embargo\") or 0), int(staged[\"purge_rows\"]))",
        [*PYTEST, "tests/api/test_isolation_disclosure.py", "-k", "embargo_source"],
        "embargo 改回 max(config, max(depth,window)) ⇒ depth<window 情境 embargo_source 斷言紅",
    ),
    Mutation(
        "M-P2-3-isolation-via-config-override", 2, ORCH,
        "raise ValueError(\"event isolation must be passed as kwarg, not config_override\")",
        "pass  # mutated: config_override channel silently accepted",
        [*PYTEST, "tests/momentum/Analysis/test_evtlabel_isolation_channel.py"],
        "入口 fail-closed 拿掉 ⇒ config_override 帶隔離鍵應 raise 之斷言紅",
    ),
    # ── Phase 3 ─────────────────────────────────────────────────────────
    Mutation(
