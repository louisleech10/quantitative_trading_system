...
> **票的狀態、問題描述、歸類，一律看 `docs/GOV_TICKET_SOT.md`。**
>
> 使用者裁定：治理票收集成唯一一份文檔，其他文件都是舊的、**不再引用或更改**。
>
> - 🔴 **本檔內的 `TICKET-STATUS:` 欄位全部作廢**——不論寫什麼，一律不採信。
>   （刻意不移除：那是歷史紀錄，且「修正只考慮以後、不溯及既往」是使用者定死的原則。）
> - 🔴 **本檔不再更新**。票有任何變動（新增／狀態／描述／歸類）只改
>   `scripts/fact_keys.json` 之 `governance-ticket-sot`，再跑 `--write`。
> - 本檔的長文（病／代價／修法方向／討論）**仍可查閱**，那是它唯一保留的用途。
>
> 之所以會走到「一份文檔」，根因是**文件隨手生、內容隨手寫**——
> 同一件事在三個地方各記一份，彼此對不齊，連「做完幾張」都會得出三個答案。
...
6. **「判準資料化」（2026-08-07 併入項）本體未做**——🔴 **2026-08-13 更新**：其 schema 前置
   （`x-consult-r12 J-1` 指出的「只有平面列」限制）**已由待辦清單 `WL-01` 解除**：
   `scripts/fact_keys.json` 現支援 `columns`（具名欄＋列長不變式）與 `render: table`
   （markdown 表格投影），三家 review＋閉合複驗＋三家戳記核可。
   🔴 **2026-08-13 再更新**：兩條機械檢查之**第一條已由 `WL-02` 上線**——
   判準登記表（`docs/GOV_CRITERIA_REGISTRY.md`）＋三道檢查（狀態列舉封閉／
   同適用範圍同條件不得相異期望／判準宿主區塊外不得陳述期望結束狀態）。
   三輪委員（consult → review → closure），三家戳記核可。
   🔴 **但本項字面設計已被實測推翻並改寫**：原文「掃散文抽條件→期望配對」在現況語料上
   **零訊號**（封閉抽取 146 配對、三種分組衝突皆 0）；且**出生事故那型仍偵測不到**
   ——兩段話用不同條件字串描述同一物理事件時鍵不相等，三家一致判為**能力邊界非實作缺陷**。
   ⇒ 本項**仍不得宣稱閉合**；剩餘工作＝待辦清單之 `WL-03`（`FACT-RECEIPT`）。
...
**當日觀測到的代價（同一句話的多份手寫副本）**：

| # | 事件 |
|---|---|
| 1 | 主委在 `白話說明/README.md` 寫「`B3R✅` … 第 1 批全數收案」——`B3R` 屬第 0 批、漏列 `B3`、且「全數收案」與機械判準不符 |
| 2 | 更正 README 後**未同步** `第1批-施工清單.md` 的同一句 |
| 3 | 再更正後，`治理待辦總覽.md:7`、`治理進度日誌.md:15`、`HANDOFF.md:12` **三處仍寫著該句**；最終以 `grep -rn` 機械掃才找齊 |
| 4 | 主委讀 `docs/GOVERNANCE_EXECUTION_ORDER.md` 的**敘述段**（非 generated block），向使用者誤報「`B3R` 待審」——實際已於 08-09 落地 |

⇒ 第 4 項正是本檔 schema `residual` 第 3 條自己預言的失效
（「宿主檔內 generated block 以外的敘述段仍是第二份副本」）。

**閉合條件（新增）**：批次／票的完成狀態納入 fact-key，
文件（`HANDOFF.md`／`白話說明/*`／`docs/GOVERNANCE_EXECUTION_ORDER.md` 敘述段）
**只得放指標**；手寫狀態即 `--check` 非零。

**排期**：使用者 2026-08-10 解除排序凍結並指示以淨摩擦排序 ⇒ 本 scope 擴充列
**`032 站 2.5`**（見 `docs/GOVERNANCE_EXECUTION_ORDER.md`）。
...
#### 🔴 2026-08-12 併入：兩條可機械化之封閉檢查（使用者追問「有票在做這個嗎」）

**不另開票**——與本票同一機制（散文判準 → 結構化），照 2026-08-07 之處理方式併入。

**觸發事故（同日，主委）**：
- **殘留①當日再度發作**：`docs/GOV_GATECHAIN_SPEC.md` 之 Task A.1 正文寫
  「依賴腳本缺檔 ⇒ FAIL 且 rc=1」，同 Task 之「邊界②」仍寫「具名略過、rc 不變」
  ⇒ 三家獨立命中（`CODEX-R2-P0-01`／`COMPOSER-R3-P1-01`／`GROK-R3-P1-02`）。
- **先斷言後驗證**：同一份 SPEC 之「改法」段寫入 `setsid` 與 `ulimit -H -u`
  兩個機制，**兩者皆未實跑**。主委事後實測：本機 macOS **無 `setsid` 指令**；
  `ulimit -H -u` 回 `Invalid argument`（hard 不能降），只降 soft **必然被子程序抬回**
  ⇒ 該段設計整段作廢，耗掉三輪審查。

**兩條修法（皆封閉可導出，非黑名單）**：
1. **同 Task 內互斥判準偵測**：抽出每個 Task 之所有 `(條件 → 期望 rc)` 配對，
   同一條件出現不同期望 ⇒ FAIL。不需理解散文，**前提是判準須結構化**
   ——與本票之「判準資料化」為同一前置。
2. **改法段之機制須有 receipt**：「改法」段內出現之指令／旗標
   （如 `setsid`、`ulimit -H`），須於同檔 `FACT-RECEIPT` 出現，
   或明確標 `assumed`；兩者皆無 ⇒ FAIL。
   ⇒ 這條會在**撰寫當下**攔下「先斷言後驗證」，而非等到第三輪審查。

🔴 **同一前置**：兩條皆卡在本票已記載之 schema 限制（`.rows[]|@tsv` 只支援平面列）。
⇒ **不得宣稱本項已閉合**；解除該限制前，殘留①仍為活缺口。
...
⇒ 本票由 `批 0.9` 提前至 **`034 站 2.6`（唯讀最小版）**，完整版留 `140 批 0.9`
（使用者 2026-08-10 解除排序凍結並指示以淨摩擦排序；見 `docs/GOVERNANCE_EXECUTION_ORDER.md`）。

### 🔴 動工前必先處理的地雷：`gate_deny` 的 JSON 間距與其他事件不同

```
其他事件： "event": "committee_dispatch"    ← 冒號後有空格
...
⇒ 閉合條件新增：`friction_tally.sh` **不得**以 `"event": "` 字面掃描；
須以能吃兩種間距的解析（或先正規化 audit 行），並附「兩種間距皆計入」的正反測試。

### 2026-08-10 首次導出（唯讀，供設計參考）

| 命中規則 | 次數 | 性質 |
|---|---|---|
| `token_expired` | 944 | 多為**誤擋**與 token TTL 過期，純開銷 |
| `open_debt` | 399 | **保護**（一扇門規則生效） |

被擋指令 Top：`codex exec` 381（真派工）；其餘約 295 筆為唯讀指令
（`printf`／`find`／`ls`／`test`／`mkdir`／**`bash scripts/gate.sh` 自身**）⇒ 屬 `票 B-15`。
`票 B-15` 修補（第 1 批 `B7`）落地前後誤擋率：**39–41% → 20%**。
...
### 🔴 2026-08-10 第三次發作 — **形態④：執行端自行 commit**（本票原本沒有這一類）

`20260810-govb1-x-review-r2` 期間，執行端對**真實 repo** 做了**三個 commit**：
`d63773a4 old`／`b8f5c4c2 q`／`7be8a11c a`，把探針 fixture
`a`／`qfile.txt`／`tracked.txt`／`old.txt` 提交進版控，並留下 staged rename。
brief 硬規矩第 2 條逐字：「**不得 commit 任何檔**」。

**後果**：`g7` 轉紅（`未宣告即修改: a / old.txt / qfile.txt / tracked.txt`），
`test_govb1_contract_matrix.py` **4 條紅**。清理後全套 **1281 passed**
⇒ 4 條紅**全部由污染造成**，與本輪修補無關。

**處置**：三個 commit 只含其自身垃圾且全部未推 ⇒ `git reset --mixed 6663e191`
（不動工作區，主委修補完整保留）；污染檔 `mv` 至
`.claude/tmp/executor-pollution-20260810/`（**未刪，留供稽核**）。

🔴 **形態④ 比前三種嚴重**：前三種留在工作區，這一種**進版控歷史**。
若當時已 push，清理成本會從一次 reset 變成改寫遠端歷史。

### 🔴 同一次事故的第二個教訓：**偵測到 ≠ 被讀到**

本層**確實作動了**——派工日誌尾巴印出 `RM newer.txt`／`old.txt`／`?? a-new`，
正是 `_report_workspace_drift` 的輸出。

**但主委沒看見**：它印在 `committee_run` 日誌尾端，主委當時只 `tail` 了幾行就轉去讀委員報告，
**是全套測試 4 條紅才逼他回頭查**。

⇒ 這是 r1 `CODEX-R1-P2-03`（收噪）裁決的**反面代價**：訊號變安靜了，也更容易被略過。
兩者不是矛盾——收噪對的是「每輪都紅」，而本次是**真的紅了卻沒人讀**。

**閉合條件（新增）**：
1. 形態④＝比對派工前後 `git rev-parse HEAD`；不同即**明確回報**（仍不必擋）
2. 形態②③④ 命中時，訊息須出現在 `committee_run` 的**最終摘要行**（主控端必讀之處），
   不能只在日誌中段
3. 🔴 本層的價值**取決於主控端會不會讀**——這一點無法由本層自證，須併入 `票 B-51`
   的允許清單機制才有硬性效果

### ✅ 2026-08-10 **實作面收案**（`20260810-govb1-x-stamp-r2`，兩家 APPROVED）

`bash scripts/reconcile_stamps_check.sh handoffs/reconcile/20260810-govb1-x-review-r2/synth.md` rc=**0**
（sha256 `8b285aee…9eec`）。測試 **22 條**，全套 **1283 passed**。

**四輪收斂軌跡**：r1 九條 → r2 三條 → stamp-r1 一條 → stamp-r2 零。

| 面向 | 狀態 |
|---|---|
| 形態② ambient 被還原 | ✅ 閉合（含含空白／中文／**含換行**路徑、**rename 裸路徑**不誤報） |
| 形態③ mutation 未還原 | ✅ 閉合 |
| checker 不可用 | ✅ 留 receipt，不靜默停用 |
| 訊噪比 | ✅ ②③ 未命中壓成一行 |
| **流程面** | 🔴 **永久標記為跳步**（`CODEX-R2-P1-01`）——**任何文件不得宣稱流程閉合** |
| 形態① 逸出允許清單 | 🔴 未做（需 `票 B-51`） |
| 形態④ 執行端 commit | 🔴 已入票、閉合條件已寫、**未實作** |
| porcelain v2 | 🔴 非本實作輸入；改用 v2 須重驗裸路徑判定 |

⇒ **`TICKET-STATUS` 維持 OPEN**。

🔴 **本票換來的一條可援引判準**（stamp-r2 兩家核可，codex 收緊版）：

> **只有在固定 producer contract 與輸入字母表下機械不可達，才可稱「上界」；
> 任何可跑 harness 反例均屬缺陷。**

主委在本票內**兩次**把可重現的真缺陷寫成「宣告上界」（r1 C4 已判過一次），
第二次被 stamp-r1 以 hex 收據打回。此判準即為該事故的產物。
...
## B-54 票 `GOV-STAMP-SHA-HANDCOPY`

TICKET-STATUS: OPEN

🔴 **開票依據**：`20260810-GOVB1-B4-STAMP-R2` 實際發作 —— codex 之
`RECONCILE-STAMP` 行 sha256 為 **59 個 hex 字元**（正確為 64，掉 `01e8`）。
🔴 **同一份交件檔之 `TESTS_RUN` 行印出的值完全正確** ⇒ 不是算錯，是**手抄掉字**。

### 病

`RECONCILE-STAMP: <family> APPROVED <date> sha256:<64 hex> task:<id>`
這一行由**委員以自然語言輸出**，其中 64 位十六進位需人工轉錄。
`reconcile_stamps_check.sh` 只能判「不符」，無法區分「拒絕」與「抄錯」。

### 代價（本次實測）

- 一輪額外 codex 單獨派工（重取戳記）＋ 一次 `debt_clear --abandon`
- 主委必須**明確拒絕自行補字**——戳記是委員簽名，主委補寫等於偽造。
  這條紀律沒有機械保護：主委手上有寫入權，能一秒補完且無人察覺。

### codex 自己的判定（`20260810-GOVB1-B4-STAMP-R3`）

> `DEFECT_NOTE: 是；人工手抄 64 位 hex 是缺陷來源，建議由 harness 以實際 body hash
> 自動生成並驗證整行戳記。`

### 修法方向（未裁決）

由 `cx_run.sh`／`committee_run.sh` 在委員判 APPROVED 後**自動生成整行**，
委員只輸出 `APPROVED`／`REJECTED` 與理由。
🔴 **反面風險須先答**：自動生成會不會使「戳記」退化為橡皮圖章
（委員沒真的比對 body hash 也能蓋章）？現行手抄至少強迫它跑一次 hash 指令。
⇒ 這是**取捨題**，不是純實作題，須走 consult。

### 淨摩擦

發生 1 次（本次），每次成本＝一輪派工（分鐘級）＋一次銷帳。
🔴 **樣本數 1，不足以定優先序**；先記錄，累積到 ≥3 次再排。

---

## B-55 票 `GOV-FLAG-EMPTY-VALUE-INCONSISTENT`

TICKET-STATUS: OPEN

🔴 **開票依據**：`handoffs/reconcile/20260810-govb1-b4-review-r2/synth.md` 群集 4
（`R-16`，codex＋composer 共識列為可接受之具名殘留）。

### 病

`gate.sh dispatch` 之旗標對「顯式傳空值」處置不一致：

| 旗標 | 顯式傳空值 |
|---|---|
| `--spec` | **拒絕**（`CODEX-R1-P1-01` 修法，2026-08-11） |
| `--todo`／`--manifest`／非 impl 之 `--brief` | 靜默視同未給（optional no-op） |

### 兩家為何判可接受

`--spec` 是**選擇 impl 模式**的旗標，其空值語義有安全後果（會使整組 impl 閘不啟動）；
其餘旗標之空值只影響該旗標自身的檢查是否執行，不改變派工模式。

### 為何仍開票

「同型旗標行為不一致」本身是**未來誤判的來源**：下一個讀這段碼的人（含主委）
會合理推論「空值一律拒」或「空值一律放行」，兩個推論都錯。
⇒ 修法方向＝**把判準寫成資料**（每個旗標宣告 required/optional/mode-selecting），
而非在三處各寫一次 if。
