...
🔴 **本行於 2026-09-11 作廢**（使用者當面定死：「95% 就收」是治理票原則，**量化主線絕對禁止**）。本票之停輪判準以 `CLAUDE.md` 三方簽核鐵律為準：**任一方有疑就不通過**。
...
2. **D1 的裁定是否可接受**：事件掃描端恆走 event-study-only、`R-5` 具名殘留。
   這是正當的範圍裁定還是「把主目標偷偷降級」？請正面打。（🔴 2026-09-11 更正：原問句以「95% 解法就收」設框，那條原則量化主線禁用，不得作為接受降級的理由。）
