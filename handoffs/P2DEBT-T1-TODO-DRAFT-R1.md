# P2 債票 1 — governance 9 紅 fixture 遷移 — TODO 草案 R1
...
- **防假綠（§C）**：
  ...
  3. scope 驗收 = 派工前快照 vs 完工後 `git diff` 比對允許路徑；**不得**用污染 working tree 的全域 `git diff --name-only` 當驗收。
...
|---|----------|-----------------------------------|---------------------|
| **1.2b.1** | `## §RISK` | `**high**` 行之後加 `- RISK-HIT: b`（與原則 (b) 一致） | `bash scripts/template_check.sh spec docs/VERIFY_GATE_SPEC.md 2>&1 \| grep -q 'RISK-HIT'` 或全檢：`…` → 不再報「§RISK 缺 RISK-HIT」 |
...
| **1.2b.4** | 整檔機檢 | 三錨點齊全後 | `bash scripts/template_check.sh spec docs/VERIFY_GATE_SPEC.md; echo $?` → stdout 含 `TEMPLATE PASS`；**exit 0** |
...
| 層級 | 內容 | 命令 + 預期 |
|------|------|-------------|
| 單元 | B4 全檔 | `pytest tests/governance/test_verify_gate_b4.py -v` → 0 failed |
| 單元 | B5 spec_* | `pytest tests/governance/test_verify_gate_b5.py -k spec_ -v` → 0 failed |
| 單元 | R7 | `pytest tests/governance/test_verify_gate_redteam.py::test_r7_gate_task_id_appends_committee_dispatch -v` → PASSED |
...
| scope | diff 白名單 | 完工 `git diff --name-only`（相對派工前快照）**僅**含 §0 四檔；**不得**含 `scripts/` |
...
# 2) scope 驗收 — 僅允許路徑（B 案）
# 派工前快照 vs 完工後 git diff 僅含：
#   tests/governance/test_verify_gate_b4.py
#   tests/governance/test_verify_gate_b5.py
#   tests/governance/test_verify_gate_redteam.py
...
# 4) B 案生產路徑
bash scripts/template_check.sh spec docs/VERIFY_GATE_SPEC.md; echo $?
# 預期：TEMPLATE PASS；exit 0
```
