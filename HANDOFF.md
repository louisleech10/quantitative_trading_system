...
> **PART1 核心(現在做)**:P1-1 brief閘硬化(前提逐條fact-verified/assumed+反空殼,非只grep關鍵字)/P1-2「驗守衛測試必附常駐mutation」機械強制(治本)/P1-3 本session手動mutation補常駐(全蒸發)/P1-4 brief閘自身mutation(我漏了)/**✅P1-5 完成(commit d9a6c65:governance.yml+conftest容錯;乾淨venv模擬CI 263 passed;誠實邊界=擋merge需branch protection使用者設)**/P1-6 B opt-in便宜版(特定派工強制帶--reconcile;完整版未結案債狀態機=專輪)/P1-7 C3 waiver不得跳stamp檢查/P1-8 C4後半target-synth綁定(先定慣例)/P1-9 template_check補mutation(天天用沒證偽)/P1-10 戳記檢查補mutation/P1-11 artifact路徑無completeness/P1-12 C5測試覆蓋+計數。
