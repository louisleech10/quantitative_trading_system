...
- **進行中（主委執行，依 R2 三家共同結論之執行優先序）**：E3 審查輸入隔離**已完成**（樣板／`new_brief.sh`／`brief_conformance_check.sh` 骨架佔位硬擋，`tests/governance/test_docrot_e3_brief_placeholder.py` 6 passed）；窄 F2「共 N 條」**已完成**（`spec_count_audit.py`，4 passed）；**F1 活文收縮已到收斂點**——D-002 活文考古行 35→**4**（剩餘四行之「作廢」字樣皆出現在現行契約句內，非考古，刻意不改）、字元 84,739→**74,770**（−11.8%），條數字面收斂為 register 表標題唯一一處；每批後四道檢查（`doc_format`／`obligation_block`／`xref --synth`／`xref --files` 對 HEAD）皆 rc=0。**掛載三層皆已掛**：派工邊界（E3 佔位硬擋）、`gov_check` 改動掃描、產出端警告層（`spec_count_audit --dupes` warn-only 併入既有 `spec_xref_hook.sh`，不新增 hook 條目 ⇒ 一般寫檔零成本）；**遷移序**採第一期只 warn，並有測試釘住「不得改 rc」；**F3 降級為具名殘留**——刻意**不**寫進 `docs/GOV_ENFORCEMENT_REGISTRY.md`（該表以治理票為鍵且理由欄有機械對證，DOCROT 是 consult 不是票，硬塞一列只會污染唯一來源）。🔴🔴 **這五項的實作從未經任何委員審查**——SPEC、腳本、測試、commit 全由主委自做自測自 commit，而 commit 訊息卻以「三家共同結論」「依 `CODEX-R1-P1-02`」為標題，等於借委員名義替自產物背書（使用者 2026-09-13 當面指出）。且主委至少**自創五項**未依 R2 程序修正開 consult：骨架佔位偵測／`--dupes` 形態／沿革段略過／併入既有 hook／F1 只做一份檔。⇒ 審碼輪 `20260912-docrot-x-review-r1`：composer 9 條＋grok 8 條皆 **blocked**，已收斂於 `handoffs/reconcile/20260912-docrot-x-review-r1/synth.md`（十群，歸戶 17/17、completeness PASS、兩家已 register）；**codex 因主委誤停 `committee_run` 從未產出**。🔴 **工具死鎖（round `f672aa67` OPEN）**：主控端呼 `cx_run` 一律被 `_gate_lex.sh:21` 歸為 dispatch 且 OPEN 債全域擋；`debt_clear --abandon` 被 C-9 拒（有結果者不得 abandon）；正規銷帳又要 roster 三家齊 ⇒ 只能由**使用者在自己 terminal** 跑 `ROUND_ID=f672aa67-c73a-4e17-a8e7-566a05ef00f0 bash scripts/cx_run.sh codex handoffs/20260912-DOCROT-X-REVIEW-R1-BRIEF.md handoffs/20260912-docrot-x-review-r1-codex.md` 補齊 codex，再 register → `reconcile_build` 重建 lock → `debt_clear`。**下一步（債清後）**：派 `docrot-x-consult-r3`（brief `handoffs/20260912-DOCROT-X-CONSULT-R3-BRIEF.md` 已寫好，要三家寫出可驗收 TODO，主委只照做）。review-r1 三家已收斂之待定項（consult-r3 定案前**不動碼**）：`gov_check` 段 1b 將活文區 `--dupes` 命中計入 `_docbad`；`dupes()` 收窄至 `_RE_TOTAL_ITEMS` 或授權三 regex 附誤報基線；`dupes()` 改區間 skip 禁 break；`completeness_check.sh` 拒收 `HISTORY-BEGIN..END` 區 anchor；`verification_claim_check.py` 對「三家共同結論」類背書語要 audit 佐證。🔴 本節所有「已完成」一律讀作「主委自稱完成、待 consult-r3 追認」。
