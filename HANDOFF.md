...
**更新：2026-09-12｜現行票：`SPLITUNIFY 收尾`（大；RISK a,b,c）——用已結案之四閘治理票全程跑並記錄摩擦（使用者 2026-09-12 指示）｜規格階段：consult 18 條 → D-001 → R5 11 條 → R6 6 條 → R7 2 → R8 1 → R9 2 → R10 3 → R11 已派兩家閉合（session `20260911-splitunify-x-review-r11`）**
...
- **mutation**：`M-SU-D1-01`～`17`（10＝成員判定用全框值、12＝缺欄回退、15＝attest 判準退回 frame 序、16＝入口略過指紋重算、17＝唯讀旗標取代不可變底）。
...
## 待辦（R8 回來後）
①收斂 R11 → ②若 proceed 則派三家戳記輪（歷輪戳記因規格續改皆已失效，須重簽；R8 有一份戳記寫 `PLACEHOLDER` 屬無效）→ ③b8 實作。
