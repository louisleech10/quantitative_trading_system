...
- 🔴 **改 SPEC 一律用「插入 v<N> 補充」，禁止整段重寫**（2026-09-12 實測定；這是動作紀律，不是閘）。本 session 數據：**整段重寫 4／4 被 xref 擋、插入式補充 1／15**——差別不在閘而在改檔方式。要作廢舊句就標 `v<N> 作廢` 並**保留原字面**，這同時滿足閘的判準與 SPEC 自己的追溯慣例。
