...
| 0020 | S0.2 | 進行中 | 定義**唯一一套**狀態語意。🔴 原描述之前提已被實測推翻並更正：backlog 之 TICKET-STATUS 共 25 張（非 14），且有四態 OPEN／PARTIAL／DONE／PROVISIONAL——**並非只有兩態、亦非表達不出「做了一半」**。真正的問題是兩套值**語意不對應且無映射**（closure 用 未開工／部分完成／收案），加上兩邊涵蓋的票不同。我起草→三家審 | 四個狀態值全在 status_enum；逐值反例——在受管檔寫「票號＋該值」皆須 rc≠0（含 已完成／狀態未確認） |
...
| 0040 | S0.4 | 未開工 | 逐張盤點 61 張票之真實狀態＋證據。🔴 我自產一版 ＋ 三家平行，逐張對照；分歧以證據裁定不數人頭（三家上輪各得 11／13／不同集合，因三來源本就對不齊）。每張須有可查證據（commit／測試名／收斂檔路徑），無證據者標「狀態未確認」不得猜 | SoT 中狀態未確認為 0；或未確認者逐張有具名理由且理由非「尚未查」 |
| 0050 | S0.5 | 收案 | 落成機器可讀之 SoT（新 fact-key）。驗收：61 張全在、欄位齊備、--check 綠 | 61 張全在、--check rc=0、已納入 status_keys |
| 0060 | S0.6 | 進行中 | 🔴 消滅第二套：backlog 之 TICKET-STATUS 與 governance-ticket-closure 降為投影或作廢，不再是可獨立編輯之狀態源。**不可逆體例變更，動手前改法先給使用者確認** | 舊來源皆標作廢；反例——在舊來源改狀態不影響 SoT，且新寫入被擋 |
| 0070 | S0.7 | 未開工 | 機械閘：SoT 之外手寫票狀態即 fail-closed（沿用既有手寫狀態偵測器）。驗收＝反例可構造。我→三家審 | 反例可構造：於 SoT 之外手寫任一狀態值即 rc≠0（逐值驗） |
| 0110 | S1.1 | 未開工 | 封死幽靈票：enforcement 之 對應票 欄必須在票全集內（B-38 即實例：在登記表、不在票表）。🔴 依賴 S0.5 | enforcement 之對應票填不存在之票號 ⇒ rc≠0 |
| 0120 | S1.2 | 未開工 | rc=2 略過 改為可判別之 fail-closed。🔴 此洞為主委為修測試而引入（codex：『大聲印出不是阻擋』） | settings.json 缺席時 rc≠0（非略過印訊息） |
| 0210 | S2.1 | 未開工 | 收案綁定不只認「收案」字面，改綁 SoT 之所有完成語意。🔴 現行 部分完成／backlog DONE／worklist 收案 三條路徑皆繞得過（grok 實構） | 以任一「完成」語意標記票而未登記覆蓋 ⇒ rc≠0（不限收案字面） |
...
| 0410 | S4.1 | 未開工 | 依 S0.5 之 SoT 機械導出所有已交付之票，不再靠判定式 | 已交付票集合由 SoT 機械導出（指令可重跑），非人工判定 |
