...
含：§0 狀態與前五批交付物＋R3 出口清單、§1 開工前稽核（含期望值）、
§2 **B6 是什麼＋已做完的偵察**、§2B **B7 是什麼＋偵察**（兩者皆**不必重查**）、
§3 派工管線（逐字指令＋**B4／B5 學到的六件事**，含三件套 RECHECK 常設條款）、
§4 完成判準與 mutation 寫法（**九個假綠實例**）、§5 未辦、§6 地雷與治理現況、
§7 具名殘留全文、§8 檔案地圖。**讀完即可開工。本檔只放指標。**
...
Task 1.5／1.6／1.7（匯入前端）之六輪 code review findings **7 → 4 → 4 → 1 → 1 → 0**，三家一致、未派 R7。
commit＝`8e0f8023`＋`c0ac22e0`＋`2ebab25d`＋`09884811`＋`cb9617ba`＋`07968005`。
過程與裁定理由見六份收斂檔 `handoffs/reconcile/2026082{5,6}-gap3ux-b4-review-r{1..6}/synth.md`（兩道機檢皆 rc=0）。
...
**下兩批＝B6（Phase 3 刪除，無前置依賴）＋ B7（Phase 4 匯出端報酬欄）**。
B7 之前置＝第一批之契約與深度根基，以及 Task 2.1b 之唯一深度函式，兩者皆已交付。
兩批之偵察皆已做完並寫進交接 §2／§2B，**不必重查**。

🔴 **兩件必須在 brief 具名請三家裁的事**（不裁就做會做錯）：
① **B6 之 Task 3.3「已被引用」目前無資料來源**——`ic_survivors_*.json` 以 case_id 為鍵，
repo 內查不到任何把 `import_id` 寫進分析產物之處；判準未定就做，警語會變成永遠顯示或永遠不顯示。
② **B7 之 Task 4.1 要移除匯出面板之「主答案窗」單選**，而前一批的下界守衛整套綁在它上面
⇒ 守衛之存廢須重新裁定（不得默默刪，也不得留死碼）。
🔴 **B7 之 Task 4.2 會讓 G-2 golden 合法改變**（受管變更，須重凍並在 commit message 說明）。
