...
| 04-004 | HP-PROCOPT | 待審 | docs/PROCOPT_DECISION.md | 診斷完成（r1→r2→r3 三輪、零駁回），**未實施任何流程改動**。使用者四問已答；供逐條審閱之白話版＝`白話說明/SPEC與TODO流程優化.md`（§七 D1–D8 為決策點）。🔴 2026-09-22 使用者提問 1-1 後主委實查更正兩處：①「治理票 0 次相位躍升」為誤，兩組皆有、差在幅度 ②輪次計數器綁票不綁文件，拆成每份文件後量化中位數約 6、治理約 4.5（非 8 vs 4）⇒ **輪數主體在 SPEC 階段內部**，改法排序改為第 0 步優先、第 2 步降級。下一步＝使用者續逐條審閱並裁決 D1–D8 |
| 04-005 | HP-FFDSTAR | 進行中 | docs/FFDSTAR_SPEC.md | R2 十三條閉合複查（R3）→ 凍結 SPEC → 寫 TODO；中票（`RISK-HIT: none`），範圍限「共用 `d*` 快取之可追溯性」。🔴 定性輪已擋下一次對**非缺陷**之修改：四型中兩型為設計意圖、一型不成立，僅 provenance 為真缺陷 |
