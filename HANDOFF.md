...
此句**覆蓋**本檔以下所有治理待辦的優先序，也覆蓋兩條舊裁決
（`docs/ROADMAP.md` 之 P0「完成後才回 IC」2026-07-05、「治理優先於產品線」2026-08-04）。

**→ 打開 `docs/ROADMAP.md` 的狀態表（檔案第 12 行起），第一列就是下一步。**

治理＝**留現狀、不再擴建**。已掛上的機制繼續運作，不需要動它們。
本檔以下的治理內容**降為參考**，不是待辦。

🔴 **為什麼這段要放在最前面**：2026-08-05 的 `aae04295` 把量化主線的下一步
連同 ROADMAP 敘事一起砍掉，十天無人發現；而本檔是每個 session 自動注入的那一份——
**ROADMAP 改了、本檔沒改，下一手照樣會走回治理**。使用者 2026-08-14 追問
「其他相關文檔和 HANDOFF.md 有更新嗎」時，本檔對回主線一事的提及數為 0。

🔴 **回主線後的測試紀律**（使用者定「邊走邊建立」，完整版見 `docs/ROADMAP.md` 測試策略節）：
新測試優先選**性質檢驗／真實 kline／第三方對照**這三類——它們的紅綠使用者可在
**不讀程式碼**的前提下採信。凍結 golden 比對能不用就不用（基準與測試兩側皆由本 agent 產出，
互相量測是循環論證）。既有失效基準與疑似孤兒**一律不大清**，碰到才處理。
