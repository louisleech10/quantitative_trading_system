...
- 🔴🔴 **文檔病之三家裁定**（2026-09-12 DOCROT consult R1；債已清 c0bd1479）
  ...
  - 🔴 **路線裁定：見 `handoffs/reconcile/20260912-docrot-x-consult-r2/synth.md` 之「三家共同結論之執行優先序」**（唯一權威，本處不複述）。經過：主委第一版裁定（採 grok、不建強制機制）被使用者當面指正「靠紀律你絕對失敗」；第二版折衷（判定採 grok、強制掛既有 `PostToolUse`）為主委自創、三家皆未提過，經 R2 三家審查**判定不可行**（三家全 P0：無 token 清單則 F3 做不出來；`Edit|Write` 不含 Bash 而主委寫 synth 正是走 bash）。**R2 起裁定不再由主委單方生效**——凡主委產出非任一家原文之折衷，自動開一輪 consult。
