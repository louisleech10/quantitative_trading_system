...
- **`P1-01` 比 composer 更進一步**：我的 `(G-4e)` 第三份仍是「**同一三段式的另一次編碼**」⇒ 擋不住「同一次對 SPEC 的**錯誤解讀**寫進三份」。**正解方向**：第三份改為 fixture 內**人手填寫的字面 `expected_side`**（與公式編碼獨立），而非再寫一份純函式；並明令**不得**與投影／oracle 共用函式或 import——`(G-4e)` 正文目前**沒有**像 `(G-4c)` 對 oracle 那樣的「不 import 投影」紀律，實作者可抽共用 `_side()` 餵三處，**結構上保證三份同錯**。主委自跑探針（`handoffs/20260912-splitunify-b9-probe-g4e-triple.py`）已確認此結論。
- **`P1-02` 多抓一處 composer 沒提的**：除 `(6.2)` L92 外，`Task 9.4` **L208** 仍寫「依 (6.2) 將 `n_test` 明確定為事件數」，與 L211 之雙量定案互相拉扯 ⇒ 第十一次修訂須**三處同改**（L92／L208／L211），並寫死舊鍵 `n_test` 的命運（刪除／或僅作 `n_test_samples` 別名並限期移除）。
- **`P1-03` 與 composer `P1-02` 同**：§V L220 對 metadata 只 `ASSERT 帶該鍵`，而 summary 句已有「值與 producer 回傳相同」⇒ 兩句**不對稱**，孤立單測可手塞空 dict 通過。
- **L9 經其 dense scan 探針確認界內互斥且窮盡**（本輪不開 finding），但指出 `Task 9.2b` 之 Rule 4 與 Step 0④ **語意重複**，建議刪一或標「已由 Step 0 涵蓋」。

🔴 **R10 composer 已交（另兩家未交）；三條主委已複驗、全部成立——但它把 R9 八條全判 CLOSED，是九輪來第一次**：
