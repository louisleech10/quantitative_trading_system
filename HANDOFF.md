...
## 🔴🔴 接手第一件事（使用者 2026-08-14 交辦，**做完這兩件才可以 push**）

使用者原話：「**等做好那兩件事再 Push 上 GitHub 給我看**」。
⇒ 本地已累積十餘筆 commit，**那兩步未完成前不得 push**（狀態一律看上方投影表）。
兩件事已寫入上方機械投影，**不在此重述內容**，只記背景：

- **`S6.1` 的由來**：`S4.3` 我把 16 則登記標成豁免，理由是「輸入為完整文件，非單次編輯內容」。
  🔴 那個理由只證明**不能掛 `PreToolUse`**，不能證明不能掛 `PostToolUse`——
  `factkey_write_guard` 就是反例（它也要看整棵樹，照樣掛 `PostToolUse`）。
  codex 在 `S4.4` 的 `E-004` 修法建議已點到此理，我照做了那一列，卻沒把同一推理套到其他列。
- **`S6.2` 的由來**：61 張票只有 13 張的依據欄寫了「還缺什麼」（全來自原 closure 表），
  48 張沒有。完整改法**只存在於已標作廢的 backlog 長文**
  ⇒ 狀態已單一化，但「待辦內容」還沒有。這是 `S0.6` 未處理到的一半。

🔴 **做 `S6.2` 前先讀 `S4.4` 的教訓**：我在 `S4.3` 憑印象一口氣填 13 則，
三家複驗打回 5 則（**四成錯誤率**）——填「為什麼做不到」比填「做到了」更需要證據。
補登記前必須先跑碼證掃描（`.claude/tmp/s42_scan.sh` 可參考），不得憑印象。
