...
Task 1.5／1.6／1.7（匯入前端）之程式碼已寫入 repo，三家 code review **尚未派**。
新檔＝`frontend/src/components/case/EventCsvMappingForm.tsx`＋`lib/{csvPreview,suspiciousBinaryColumns,eventIdNormalization}.ts`
＋`tests/api/test_gap3_csv_provenance.py`；契約新增 `receipt_schema.mapping_provenance` 五欄，
R3 出口新增 `EventSamplePipeline.validate_receipt_values()`。殘留 `R-B2-1` 於本批解除
（單位偵測後於送出前顯示契約毫秒版 `event_id` 期望值）。
...
| 項 | 值 |
|---|---|
| mutation | 32／14／13／**3** 條（前四批），皆 `closure: CLOSED` |
| 第四批驗收 | `gap3_csv_provenance` 7（下限 2）／vitest `gap3_csv` 8（下限 5）／vitest `suspiciousBinaryColumns` 5（下限 2） |
| 全套 | vitest 40 檔 221 條；`npm --prefix frontend run build` rc=0；`gap3_freeze_golden --check` rc=0；`pytest tests/api tests/momentum/event_samples` 之既有債名單見交接 §6.5 |
...
代號：`R-GOV7-1`／`R-GOV7-2`／`R-B1-1`／`R-A005-1`／~~`R-B2-1`~~（第四批已解除）／`R-B2-2`／
