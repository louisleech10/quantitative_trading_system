...
- 🔴 **全套 `pytest tests/governance` 於 2026-09-23 實跑 78 紅／2,590 綠（7,625 秒，比預期慢一倍）**，逐條歸因皆早於 TODOFMT 或屬偶發／環境，清單與方法見 `handoffs/run_receipts/20260923-todofmt-govsuite-attribution.json`。下次動共用控制流前後拿它當基線比對，不必再逐條歸因。歸因方法之坑：**`test_govb1_contract_matrix.py` 部分測試內部會再跑整套檢查，單條 20 分鐘級**，逐條重跑前先看它屬不屬這類；舊版本對照用 `git worktree add --detach <路徑> <commit>`，該 worktree 沒有 `handoffs/`（被 exclude），依賴它的測試要改看失敗訊息。**修復進度（使用者裁定由主委自處理、不派委員）**：於 scratchpad worktree `wt_reds` 修測試，已處理 48／78，**尚未搬回 main**；多數根因＝測試 harness 之依賴清單少了後來新增之腳本、錨點字串跟不上改寫；受使用者裁定約束者（R-GOVTEST-1、R-G7-OFF-2）改 `xfail(strict=True)` 並引用登記 ID。
