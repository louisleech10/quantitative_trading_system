...
## 🏁 GAP-2 已收案（2026-08-19）
- 五批全部：各三家 code review＋三家蓋章；規格延伸決策 A1-1..A1-11；24 條 mutation 全部改壞就紅（最終實跑 receipts）；改前==改後 golden PASS；倖存者檔每次分析都會落檔（`ic_survivors_{case_id}.json`）；前端表格＋開關預設開。
- 殘留 7 條（G2-R1／R2／R3／R5 原有＋R6 前端 tsc 既有 8 紅／R7 bench 內嵌 gate 太慢／R8 狀態文案未中文化）已登記 registry「GAP-2 待補完」，每條附三值理由與觸發條件。
...
| B5 | 前端：型別鏡像＋開關（預設開）＋一張唯讀表格 | ✅ **收案**（三家 code review 零意見＋三家蓋章 PASS；vitest 9、build、wiring 25 toggles）|
...
- 第 5 批寫完（8/19）：型別鏡像（只在 ICHC 契約段外加）、三個 preset 預設把「邊際 IC／多因子組合」開關打開且 custom／具名 preset 兩條路徑都會把開關值送到後端、面板多一列（計數改用列表長度不寫死）、唯讀表格 `MarginalICTable`（節不是 ok 就只顯示狀態文字；oos 不保證就出紅色警語；恆顯示「倖存者選於同一測試段；本節數字為描述統計，非獨立驗證」且測試斷言不含「獨立 OOS 驗證」）、掛在 IC 結果頁**基礎分析**分頁末段（A1-5 補正）。tsc 有 8 條既有紅在兩個舊測試檔，非本批。
