...
**第二批＝CSV 匯入主線**，四個 Task：讓你**用自己的 CSV 欄名**匯入（1.2）、
事件編號與指紋改由後端統一算（1.3）、時間戳是秒還是毫秒自動判（1.4）、
一批裡混了不同方向／情境就直接拒收不亂猜（1.8）。

🔎 開工前我已經先查過碼，有一個發現值得你知道：**CSV 其實現在就能匯入了**——
所以第二批的 1.2 不是「加 CSV 功能」，而是「加**欄位對映**」，讓你不必先把 CSV 標頭
改成系統要的欄名。這個差別決定了要怎麼做，已寫進交接檔免得下次重查。
