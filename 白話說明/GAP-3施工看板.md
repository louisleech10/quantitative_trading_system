...
| **實作** | **第四批已收斂**（六輪審查 7 → 4 → 4 → 1 → 1 → **0**，三家一致可進第五批）；下一批＝第五批（匯出前篩選） |
| Task 總數 | **42** |
| 已完成 | **13** |
| 進行中 | **1** |
| 未開工 | **28** |
...
**第五批＝匯出前篩選**（2.1、2.2、2.3）：`/search` 的篩選面板、把條件寫進匯出檔、
以及「會匯出幾筆」的即時顯示。前置（第一批）已完成。

這批還要順手解掉三條掛著的殘留：下界值來源沒接線（接上前，第一批做的鎖定在生產上等於死碼）、
系統內篩選路徑的端到端對證、以及條件物件的形狀定案。
