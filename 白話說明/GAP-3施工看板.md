...
> 由 `docs/GAP3_EVENT_UX_TODO.md`（**FROZEN v1.0**）＋ 其延伸檔
> `docs/GAP3_EVENT_UX_TODO.D-001.md` **機械產生，禁手抄**。
...
🔴 **下一個動作是你的**：要不要開第一批（B1）。我不會自己開始。

## 批次順序（做的順序）

| 批 | 含哪些 Task | 依賴 | 狀態 |
|---|---|---|---|
| **B1 契約與深度根基** | 1.1、1.10、2.1b、4.2（僅 §G S-9 參考實作部分）　🔴 **依 D-001 A-001 更正** | 無 | ⬜ 未開工 |
| **B2 CSV 匯入主線** | 1.2、1.3、1.4、1.8 | B1 | ⬜ 未開工 |
| **B3 深度三層防線** | 1.11、1.12、1.9 | B1、B2、**Task 2.1b** | ⬜ 未開工 |
| **B4 匯入前端** | 1.5、1.6、1.7 | B2 | ⬜ 未開工 |
| **B5 匯出前篩選** | Phase 2 全部（扣除已於 B1 完成之 2.1b） | B1 | ⬜ 未開工 |
| **B6 刪除** | Phase 3 全部 | 無 | ⬜ 未開工 |
| **B7 匯出端報酬欄** | Phase 4 全部（4.2 之 S-9 部分已於 B1 完成） | B1、**Task 2.1b** | ⬜ 未開工 |
| **B8 訊息與表頭** | Phase 5 全部 | Task 5.0 | ⬜ 未開工 |
| **B9 IC 止血閘** | Phase 6 全部 | Task 6.0 | ⬜ 未開工 |
| **B10 全棧接線** | Phase 7 全部 | B1–B9 | ⬜ 未開工 |

🔴 **B1 是唯一沒有前置的一批**，其餘全部等它。
B1 裡有兩個「被很多批共用」的工具（深度計算函式、序列化參考實作），
**原本被排在需要它們的批次之後**——第二輪審查抓出來才提前的。

⚠️ **B1 那一列是我在做這份看板時才發現要更正的**：施工清單的批次表寫「1.1、1.10」，
但同一節下方的依賴表寫的是四個 Task——**兩處互斥**。
三輪委員審查加一輪蓋章都沒抓到，是「把文件做成看板」這個動作抓到的。
更正記在 `docs/GAP3_EVENT_UX_TODO.D-001.md`（凍結後不就地改，走延伸檔）。
