...
| **實作** | **第二批已收斂**（兩輪審查，三家一致可進第三批）；下一批＝第三批（深度三層防線） |
| Task 總數 | **42** |
| 已完成 | **7** |
| 進行中 | **1** |
| 未開工 | **34** |
...
**第三批＝深度三層防線**，三個 Task。這批在處理一件事：**「這個欄位看到多遠的未來」如果系統
算不出來，該怎麼辦**。

- **1.11**：系統算不出深度時，**強制你自己宣告**，並明白告訴你「這個數字我驗證不了，填錯會洩漏未來資料」。
  🔴 **不准**因為「其他欄都算得出來」就拿它們的最大值頂替。
- **1.12**：宣告缺失或與系統認知衝突時，這批資料**禁止進入訓練/測試切分與條件 IC**，
  但**仍然可以產出事件研究表**（不做訓練就不會洩漏）。這需要新增一條「只做事件研究」的執行路徑——
  因為現在的主流程**無條件**會做切分，照現況只能在「違規」和「產不出表」之間二選一。
- **1.9**：上傳 CSV 時收集答案窗宣告的介面。預設取檔案裡**最大**的可用值（有 1~12 就預設 12），
  要調低必須勾選聲明；批次裡有多種時間週期時**逐個各填一格**，不准用一格套用全部。

前置（第一批的 registry 與深度函式、第二批的匯入主線）**全部已完成**，可直接開工。
