...
| 033 | HP-TODOFMT | 已完成 | 白話說明/TODO優化結論.md | 2026-09-23 收案。新票之 TODO＝五類落點 manifest（`docs/manifests/<EPIC>.json`，規格 docs/TODOFMT_SPEC.md）；寫散文 TODO 即擋、manifest 寫入當下機檢、派工閘門認新格式（`--impl-self` 須帶 `--spec`）自 W（`26048178`）生效。三批皆經三家審碼閉合，末輪 r3 零 finding；收案聚合器 12 項 rc=0（HEAD `d8107deb`）；全套治理測試 78 紅逐條歸因，皆早於本票或屬偶發／環境。其餘票使用者將另開 session 處理。**收案後補遺（第 4 批）**：使用者追問 TODO 相關文件是否都更新，主委盤點查出審查提示詞、TODO 範本之全域約束落點、SPEC 範本、分工手冊與 `debt_clear` 之延後目標查找五處仍預設散文 TODO，已改（docs/TODOFMT_SPEC.md Phase 4）。審碼 r1：codex 擋 1 條（新格式之延後目標對整份 manifest 搜字，目標只在描述欄亦被接受；composer 同判），另 doc-sync 三條；6 條全數已修（延後目標改為只認 not_executable 之 item 完全相等，commit `14ccb24b`）；審碼 r2 三家 proceed，r1 六條全數由原提出方確認閉合、零新 finding（`handoffs/reconcile/20260922-todofmt-b4-review-r2/synth.md` 已銷帳）；第 4 批收，收案聚合器 12 項 rc=0（HEAD `14ccb24b`）。**全套治理測試之 78 條舊紅燈**依使用者裁定由主委自處理：67 條修綠、11 條 strict xfail 引登記 ID（R-GOVTEST-1／3／4／5、R-G7-OFF-2，見 docs/IC_QUANT_GAP_REGISTRY.md）；fact-key 生成器變慢（R-GOVTEST-5）經使用者 2026-09-23 裁定開票，見 HP-FKPERF |
