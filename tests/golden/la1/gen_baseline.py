...
def check_all() -> int:
    ...
    print("[gen_baseline] --check OK (sha + early-flip sides + five paths)")
    return 0
...
def main() -> int:
    ...
    parser.add_argument(
        "--check",
        action="store_true",
        help="驗證 kline sha + early-flip 兩側 + 五路徑鍵（不符 exit 1）",
    )
    args = parser.parse_args()
    if args.check:
        return check_all()
    generate_all()
    # generate 後順跑結構 assert（不含強制要求 pre-existing）
    for run in RUNS:
        path = OUTPUT_DIR / run["baseline_name"]
        data = json.loads(path.read_text(encoding="utf-8"))
        _assert_five_path_keys(data, path.name)
