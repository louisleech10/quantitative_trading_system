...
def test_happy_path_rules_and_test_score():
    X, y, plan = synth()
    rep = extract_event_patterns(X, y, plan, None, cfg())
...
def test_shuffled_labels_auc_in_permutation_band():
    ...
    rep = extract_event_patterns(X, y_sh, plan, None, cfg())
...
def test_test_one_class_unavailable():
    ...
    y2 = y.copy()
    y2.loc[test_ids] = 1
    rep = extract_event_patterns(X, y2, plan, None, cfg())
    assert rep["test_discrimination"]["overall"]["capability_status"] == "unavailable"


def test_j8_ic_prescreen_train_only():
    X, y, plan = synth(n=200, n_feat=60)                                   # train 140 // 10 ⇒ cap 14 < 60
    rep = extract_event_patterns(X, y, plan, None, cfg(min_support=3))
    ps = rep["ic_prescreen"]
    ...
    rep2 = extract_event_patterns(X2, y, plan, None, cfg(min_support=3))
...
def test_survivor_v2_restricts_features_and_v1_rejected():
    ...
    rep = extract_event_patterns(X, y, plan, sv, cfg())
    assert rep["feature_names_used"] == ["f0", "f1"] and rep["survivor_restricted"] is True
    with pytest.raises(ValueError):
        extract_event_patterns(X, y, plan, {"schema_version": 1, "survivors": [{"feature_name": "f0"}]}, cfg())
    with pytest.raises(ValueError):
        extract_event_patterns(X, y, plan, {"schema_version": 2, "survivors": []}, cfg())


def test_labels_and_inputs_guards():
    X, y, plan = synth()
    with pytest.raises(ValueError):
        extract_event_patterns(X, y.replace({1: 2}), plan, None, cfg())
    with pytest.raises(ValueError):
        extract_event_patterns(X.iloc[0:0], y, plan, None, cfg())
    # train 單類別 ⇒ loud
    y3 = y.copy()
    tr = plan.assignments.loc[plan.assignments["split_label"] == "train", "event_id"]
    y3.loc[tr] = 0
    with pytest.raises(ValueError):
        extract_event_patterns(X, y3, plan, None, cfg())


def test_receipt_deterministic():
    X, y, plan = synth()
    a = extract_event_patterns(X, y, plan, None, cfg())
    b = extract_event_patterns(X, y, plan, None, cfg())
