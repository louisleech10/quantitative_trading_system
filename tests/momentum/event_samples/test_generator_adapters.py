...
def test_G2_multi_label_manifest_and_hand_exact_labels(bars):
    ...
    ev, prov = generate_events(spec, {"12h": bars}, RULES, cfg(scenario="A"))   # A：全留（不因去重掉 label_id）
...
def test_G4_dedupe_reports_raw_and_after(bars):
    ...
    assert prov["dedupe"]["n_events_raw"] == prov["n_events_raw"]
    assert prov["dedupe"]["policy"]["primary"] == "cluster_first"
    ...
    assert prov_a["dedupe"]["policy"]["requires_cluster_robust"] is True
...
def test_G6_calls_evaluate_all_bars_not_parallel(bars, monkeypatch):
    ...
    # prevalence_learn＝該 label_id 產出事件（去重前）之正例率
    ids = ev["label_definition"].apply(lambda d: d["rule_id"])
    assert calls[0][1] == pytest.approx(prov["manifest"].table.merge(ev[["event_id"]], on="event_id")["label"].mean(), abs=1.0)
