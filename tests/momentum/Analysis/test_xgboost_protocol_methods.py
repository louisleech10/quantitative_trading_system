...
def test_xgboost_load_and_predict_roundtrip():
    X, y, feature_names = _make_binary_data()
    analyzer = XGBoostAnalyzer(params={"n_estimators": 10, "max_depth": 2})
    analyzer.train_model(X, y, feature_names=feature_names, cv_folds=3)

    path = Path("data_cache/models/xgb_roundtrip_protocol.pkl")
    analyzer.save_model(str(path))

    loaded = XGBoostAnalyzer()
    loaded.load_model(str(path))
    proba = loaded.predict_proba(X)

    assert proba.shape == (len(X), 2)
    assert loaded.get_model_type() == "xgboost"
