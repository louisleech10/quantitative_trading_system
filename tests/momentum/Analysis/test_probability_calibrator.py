...
def test_C1_single_class(probability_calibrator):
    ...
    result = probability_calibrator.fit_from_predictions(y_true=y_true, y_pred_proba=y_pred, require_receipt=False)
...
def test_C3_small_sample_auto_fallback(probability_calibrator):
    ...
    result = probability_calibrator.fit_from_predictions(y_true=y_true, y_pred_proba=y_pred, method="auto", require_receipt=False)
...
def test_C4_isotonic_small_sample_fallback(probability_calibrator):
    ...
    result = probability_calibrator.fit_from_predictions(y_true=y_true, y_pred_proba=y_pred, method="isotonic", require_receipt=False)
    assert result["probability_calibration"]["method"] in {"platt", "original"}
...
def test_C5_venn_abers_downsampling(probability_calibrator):
    ...
    result = probability_calibrator.fit_from_predictions(y_true=y_true, y_pred_proba=y_pred, method="venn_abers", require_receipt=False)
...
def test_C6_cv_exceeds_half_samples(probability_calibrator):
    ...
    result = probability_calibrator.fit_from_predictions(y_true=y_true, y_pred_proba=y_pred, cv=80, require_receipt=False)
...
def test_C7_calibration_degradation_fallback(probability_calibrator, monkeypatch):
    ...
    result = probability_calibrator.fit_from_predictions(y_true=y_true, y_pred_proba=y_pred, method="platt", require_receipt=False)
...
def test_C10_beta_package_missing_fallback(monkeypatch):
    ...
    result = calibrator.fit_from_predictions(y_true=y_true, y_pred_proba=y_pred, method="beta", require_receipt=False)
    assert result["probability_calibration"]["method"] in {"platt", "original"}
...
def test_C11_feature_mismatch(synthetic_binary_data):
    ...
    calibrator = ProbabilityCalibrator()
    calibrator.fit(model=model, X_cal=X, y_cal=y, require_receipt=False)
...
def test_fit_from_predictions_improves_or_keeps_ece(synthetic_predictions):
    ...
    calibrator = ProbabilityCalibrator()
    result = calibrator.fit_from_predictions(y_true=y_true, y_pred_proba=y_pred, require_receipt=False)
...
def test_platt_scaling_schema(synthetic_predictions):
    y_pred, y_true = synthetic_predictions
    calibrator = ProbabilityCalibrator()
    result = calibrator.fit_from_predictions(y_true=y_true, y_pred_proba=y_pred, method="platt", require_receipt=False)
...
def test_auto_selects_best_method_exists(synthetic_predictions):
    y_pred, y_true = synthetic_predictions
    calibrator = ProbabilityCalibrator()
    result = calibrator.fit_from_predictions(y_true=y_true, y_pred_proba=y_pred, method="auto", require_receipt=False)
    payload = result["probability_calibration"]
...
def test_transform_proba_after_fit_from_predictions(synthetic_predictions):
    y_pred, y_true = synthetic_predictions
    calibrator = ProbabilityCalibrator()
    calibrator.fit_from_predictions(y_true=y_true, y_pred_proba=y_pred, method="platt", require_receipt=False)
...
def test_venn_abers_intervals_valid_bounds(synthetic_binary_data):
    ...
    model = _train_model(X, y)
    calibrator = ProbabilityCalibrator()
    calibrator.fit(model=model, X_cal=X, y_cal=y, method="venn_abers", require_receipt=False)
