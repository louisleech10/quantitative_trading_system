...
def export_task(tmp_path_factory: pytest.TempPathFactory) -> dict:
    ...
    if symbol and timeframe:
        filtered_name = f"{symbol}_{timeframe}_filtered.h5"
    else:
        filtered_name = "filtered_features.h5"

    filtered_path = features_dir / filtered_name
    with h5py.File(filtered_path, "w") as file:
        group = file.create_group("filtered")
        group.create_dataset("features", data=np.array([[1.0, 2.0]], dtype=np.float32))
