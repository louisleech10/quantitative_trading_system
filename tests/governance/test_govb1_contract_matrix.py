...
# 🔴 **定義已於 2026-08-14 移出至 `scripts/b49_closure_static_check.py`**，本處只 import。
#    原因（`E-007`／三家 r1）：這一段是閉合證據中**靜態可判定**的子集，
#    無需 commit、無需隔離重放，寫檔當下就判得出來 ⇒ 已前移至產出端（PostToolUse）。
#    留在測試檔裡的話，產出端腳本就得抄一份，兩邊必然分岔。
#    🔴 名稱一律沿用（`_B49_CLOSURE_*`／`_b49_selector_is_substantive`），
#    下游 mutation 測試以 `monkeypatch.setattr(_CM, …)` 綁在本模組屬性上，不受影響。
_B49_SCRIPTS_DIR = str(REPO / "scripts")
if _B49_SCRIPTS_DIR not in sys.path:
    sys.path.insert(0, _B49_SCRIPTS_DIR)
from b49_closure_static_check import (  # noqa: E402
    _B49_CLOSURE_EXPECTED,
    _B49_CLOSURE_FILE,
    _B49_CLOSURE_SELECTORS,
    _b49_selector_is_substantive,
)
