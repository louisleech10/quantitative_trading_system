"""FKPERF 核心開檔計數（docs/FKPERF_SPEC.md Task 0.2）。

以子程序 python3 啟動：先 `sys.addaudithook` 計 `open` 事件，再以 `runpy.run_path` 執行核心；
只計 ROOT 之下的路徑（註冊表所在 repo＝ROOT），不計核心腳本檔自身。
"""
...
with open(out, "w", encoding="utf-8") as fh:
    json.dump({"code": code, "hits": hits}, fh)
"""
...
def count_opens(root: Path, args: Sequence[str], core: Path) -> Tuple[int, List[str]]:
    ...
    with tempfile.TemporaryDirectory() as t:
        out = Path(t) / "opens.json"
        argv = [sys.executable, "-c", _DRIVER, str(root), str(core), str(out), *args]
