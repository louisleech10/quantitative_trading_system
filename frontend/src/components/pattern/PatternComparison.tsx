  ...
  return (
    <div className="space-y-6">
      ...
      <div className="bg-white rounded-lg border p-6">
        <h2 className="text-lg font-bold mb-4">選擇要比較的樣式 (最多 {MAX_PATTERNS} 個)</h2>
        <select
        ...
        {selectedIds.length > 0 && (
          <div className="mt-4 space-y-2">
            {selectedPatterns.map((pattern, index) => (
              <div key={pattern.pattern_id} className="flex items-center justify-between p-3 bg-gray-50 rounded">
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    ...
                    <p className="text-sm text-gray-500">{pattern.case_id}</p>
                  </div>
                </div>
                <button
      ...
      {selectedPatterns.length >= 2 && (
        <div className="space-y-6">
          {/* 雷達圖 */}
          <div className="bg-white rounded-lg border p-6">
            <h3 className="text-lg font-bold mb-4">效能雷達圖</h3>
            ...
            <table className="w-full text-sm">
              ...
              <tbody>
                <tr className="border-b">
                  <td className="px-4 py-3 font-semibold">準確度</td>
                ...
                <tr className="border-b">
                  <td className="px-4 py-3 font-semibold">精確度</td>
                  {selectedPatterns.map(p => (
                    <td key={p.pattern_id} className="px-4 py-3 text-center">
                ...
                <tr className="border-b">
                  <td className="px-4 py-3 font-semibold">召回率</td>
                  {selectedPatterns.map(p => (
                    <td key={p.pattern_id} className="px-4 py-3 text-center">
                ...
                <tr className="border-b">
                  <td className="px-4 py-3 font-semibold">F1 分數</td>
                  {selectedPatterns.map(p => (
                    <td key={p.pattern_id} className="px-4 py-3 text-center">
                ...
                <tr className="border-b">
                  <td className="px-4 py-3 font-semibold">規則數量</td>
                  {selectedPatterns.map(p => (
                    <td key={p.pattern_id} className="px-4 py-3 text-center">
                      {p.rules.length}
                    </td>
                ...
                <tr className="border-b">
                  <td className="px-4 py-3 font-semibold">樣本數</td>
                  {selectedPatterns.map(p => (
                    <td key={p.pattern_id} className="px-4 py-3 text-center">
                ...
                <tr className="border-b">
                  <td className="px-4 py-3 font-semibold">狀態</td>
                  {selectedPatterns.map(p => (
                    <td key={p.pattern_id} className="px-4 py-3 text-center">
                      <span className={`px-2 py-1 rounded-full text-xs ${
                        p.status === 'active' ? 'bg-green-100 text-green-700' :
                        p.status === 'testing' ? 'bg-yellow-100 text-yellow-700' :
          ...
          {/* 規則對比 */}
          <div className="bg-white rounded-lg border p-6">
            <h3 className="text-lg font-bold mb-4">規則對比</h3>
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              {selectedPatterns.map((pattern, index) => (
                <div key={pattern.pattern_id} className="border rounded p-4">
                  ...
                  <div className="space-y-2">
                    {pattern.rules.map((rule, rIndex) => (
                      <div key={rIndex} className="text-sm bg-gray-50 p-2 rounded font-mono">
      ...
      {selectedPatterns.length < 2 && (
        <div className="bg-gray-50 rounded-lg border-2 border-dashed p-12 text-center">
          <p className="text-gray-500 text-lg">請選擇至少 2 個樣式進行比較</p>
        </div>
      )}
    </div>
  );
