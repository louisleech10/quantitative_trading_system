...
export default function FeatureImportanceChart({ featureImportance, topN = 10 }: Props) {
  ...
  const getColor = (rank: number) => {
    ...
    if (rank === 2) return '#f97316'; // orange-500
    if (rank === 3) return '#f59e0b'; // amber-500
  ...
  return (
    <div className="bg-white rounded-lg border p-4">
      ...
      <div className="flex justify-between items-center mb-4">
        <h3 className="text-lg font-bold">特徵重要性 (Top {topN})</h3>
...
function CustomTooltip({ active, payload }: any) {
  ...
  return (
    <div className="bg-white p-3 border rounded shadow-lg">
      <p className="font-semibold mb-1">排名 #{data.rank}</p>
      <p className="text-sm text-gray-600 mb-1">{data.feature}</p>
      <p className="text-sm">
        重要性: <span className="font-semibold">{(data.importance * 100).toFixed(2)}%</span>
      </p>
    </div>
  );
}
