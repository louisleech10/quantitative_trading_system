  ...
  useEffect(() => {
    ...
    try {
      ...
      if (lineSeriesRef.current) {
        chartInstance.removeSeries(lineSeriesRef.current);
      }
      if (buyAreaSeriesRef.current) {
        chartInstance.removeSeries(buyAreaSeriesRef.current);
      }
      if (sellAreaSeriesRef.current) {
        chartInstance.removeSeries(sellAreaSeriesRef.current);
      }

      // 創建背景色區域 - 買盤強勢區域（0.5-1，綠色）
      const buyAreaSeries = chartInstance.addAreaSeries({
        topColor: 'rgba(38, 166, 154, 0.1)',  // 綠色，低透明度
      ...
      const sellAreaSeries = chartInstance.addAreaSeries({
        topColor: 'rgba(239, 83, 80, 0.1)',  // 紅色，低透明度
      ...
      // 背景區域數據（買盤強勢區：0.5到實際ratio）
      const buyAreaData = takerRatioData.map(d => ({
        time: d.time,
        value: d.value >= 0.5 ? d.value : 0.5,  // 只在>0.5時顯示
      }));
