...
class MomentumIndicatorEngine:
    ...
    def compute_all(self, data: pd.DataFrame) -> pd.DataFrame:
        ...
        if not indicators:
            indicators = [spec.name for spec in TALibWrapper.list_indicators("momentum")]
        ...
        for indicator in indicators:
            ...
            try:
                ...
                logger.warning("Momentum indicator %s failed: %s", name, exc)
    ...
    def _resolve_params(self, name: str, indicator_def: Dict) -> List[Dict]:
        ...
        if isinstance(periods, list):
            ...
            if name == "MACDFIX":
                return [{"signalperiod": period} for period in values]
        ...
        if isinstance(periods, str):
            ...
            if name == "MACDFIX":
                return [{"signalperiod": int(period)} for period in values]
