...
class CryptoSentimentCollector:
    ...
    def fetch_fear_greed_index(self, limit=2000):
        """
        抓取加密貨幣恐懼與貪婪指數
    ...
    def aggregate_to_weekly(self):
        ...
        # 按週聚合（週一到週日）
        weekly_data = self.sentiment_data.groupby('week_start').agg({
            'value': 'mean',  # 週平均值
        })
        ...
        logger.info(f"將{len(self.sentiment_data)}筆數據聚合為{len(weekly_data)}筆週級別數據")
        return weekly_data
    ...
    def save_market_phases(self, filename=None):
        ...
        self.sentiment_data.to_csv(csv_path)
    ...
    def visualize_market_phases(self):
        ...
        try:
            ...
            plt.plot(self.sentiment_data.index, self.sentiment_data['value'], 'b-', alpha=0.7)
            plt.fill_between(self.sentiment_data.index, self.sentiment_data['value'], alpha=0.2)
            ...
            plt.text(self.sentiment_data.index[0], 12.5, 'Extreme Fear', verticalalignment='center')
            plt.text(self.sentiment_data.index[0], 35, 'Fear', verticalalignment='center')
            plt.text(self.sentiment_data.index[0], 50, 'Neutral', verticalalignment='center')
            plt.text(self.sentiment_data.index[0], 65, 'Greed', verticalalignment='center')
            plt.text(self.sentiment_data.index[0], 87.5, 'Extreme Greed', verticalalignment='center')
...
def main():
    ...
    collector = CryptoSentimentCollector(output_dir=args.output)
