"""Phase 2.9 效能抽樣(F5.1 解除 quarantine)。

原 stopgap 以 SystemExit 封鎖——因 ls_returns 時間錯位。
1c-FR-FULL F0 序列重建後可再跑;NetIC 簽名已對齊 F4 ``factor_return_series``。
"""
...
# F4 簽名:factor_return_series= dict[str, FactorTimingReturnSeries]
ni = NetICAnalyzer({"cost_enabled": True, "cost_bps": 10.0})
ics = {
...
fr_series = {
    f"n{i}": FactorTimingReturnSeries(
        feature=f"n{i}",
        ls_return=pd.Series(rng.normal(size=2000)),
        position=pd.Series(rng.choice([-1.0, 0.0, 1.0], size=2000)),
        index_policy="frame_dropna_intersection",
    )
    for i in range(30)
}
start = time.perf_counter()
ni.batch_analyze(ics, to, factor_return_series=fr_series)
record("Net IC Analysis (30x2K)", time.perf_counter() - start, 2.0)

cfg_raw = ICConfig().model_dump(by_alias=True)
cfg_raw["factor_return"]["enabled"] = True
cfg = ICConfig.model_validate(cfg_raw)
...
rolling_ic_full = {
    name: {"30": pd.Series(rng.normal(size=1500)).tolist()} for name in full_features.columns
}
orch._ic_cache = {
