...
#
# 🔴🔴 這裡沒有機械保護（票 B-24，使用者 2026-08-14T18:05+08:00 要求寫死）
#   B-24 要求「驗收欄凡要求跑某腳本者，**必須同時要求狀態斷言**」——只驗 rc 不算。
#   本檔驗的是 brief 的**結構**（區塊在不在、格式對不對），**不驗驗收欄的語意**
#   ⇒ 寫「跑 X 應通過」而不寫「且狀態須為 Y」的 brief，**照樣過關**。
#   缺的不是掛載點，是**該判準尚未被寫成可機械判定的形式**（純散文要求）。
#   在那之前，驗收欄的品質只靠人眼與委員 review。
#
