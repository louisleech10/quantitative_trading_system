...
#
# 🔴🔴 這裡沒有機械保護（票 B-37，使用者 2026-08-14T18:05+08:00 要求寫死）
#   本腳本是**彙整報表，本質沒有「不通過」這個語意**——它掛得上任何閘，
#   但掛上去也不會拒絕任何東西。⇒ 不是掛載問題，是本票的產物就不是阻擋型檢查。
#   該票原修法③「文件裡不准手寫統計數字」之自動檢查，已於 r1 被兩家實測判死
#   （誤擋率近 100%）並砍掉 ⇒ **文件裡的裸統計數字目前無任何機械防護**，只靠人眼。
#
# 用法：
