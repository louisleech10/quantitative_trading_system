...
_gc_summary() {   # $1=**成功時**之收尾語；印摘要並回傳 rc_all
  # 🔴 失敗分支**不得回印 $1**：呼叫端傳的是成功語（如「全數通過」），
  #   原碼兩個分支共用 $1 ⇒ 失敗時印出「❌ 全數通過」這種自相矛盾的最後一行。
  #   2026-08-14 實際誤導過主委一次（先讀到最後一行以為通過，再往上才看到 FAILED 摘要）。
  #   最後一行是讀者最先看到的東西，它必須與 rc 一致。
  if [ "${rc_all}" -eq 0 ]; then
    # 🔴 原句尾附「push 後仍看 CI」已作廢：CI 已於 2026-08-13 由使用者裁定整個刪除
    #   （連續五次全紅、無人查看＝零保護純噪音）。叫人去看不存在的東西是誤導。
    echo "[gov_check] ✅ ${1}"
  else
    {
      echo ""
