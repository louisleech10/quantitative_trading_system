...
# 3b) 驗證欄之**佔位形態**：有 token 但命令本體是省略號
#
# 病根（2026-08-22，COMPOSER-R4-P2-01）：GAP-3 SPEC 之 Task 6.0 驗證欄字面為
#   `- 驗證：`python3 -c "..."` 斷言該 reason 存在於登記檔；grep -c '...' … `== 0`。`
#   ——命令本體是 `"..."`，Agent 無任何可跑之物；但該行含 `==` 與 `.py`
#   ⇒ 順利通過 3) 之 token 檢查。**有 token ≠ 可執行**，這是 3) 的結構性盲區。
# 判準為**封閉可列舉集合**（feedback_mechanize_dont_police_prose：不列舉散文，只列佔位字面）：
#   引號內只有省略號（`"..."` / `'...'` / `“…”`）、或中文佔位（（略）／（同上）／（待填））。
# 🔴 **刻意不含 `TODO`／`XXX`**：首版含之，對 166 份 docs/*.md 實掃出 **17 個誤報**——
#   本 repo 以「TODO」為文件型別名（`GAP3_EVENT_TODO.md`、「SPEC/TODO 統一此一命令」），
#   `XXX` 亦見於用法字串（`docs/XXX_SPEC.md`）。收窄後同一掃描誤報 0。
#   （`TBD`／`FIXME` 一併移除：非本 finding 之觀察形態，加了只是投機擴張。）
# 誠實邊界：只擋「明顯佔位字面」；把命令寫得具體但其實跑不起來（路徑打錯、測試不存在）
#   本層抓不到 → 交實跑與 adversarial。
if awk -v re="${_hollow3_re}" '
    $0 ~ re {
      if ($0 ~ /"\.\.\."|'"'"'\.\.\.'"'"'|“…”|（略）|（同上）|（待填）|\(待填\)/) { found=1 }
    }
    END { exit(found?0:1) }' "${file}"; then
  hollow="${hollow}  · 驗證欄為佔位形態：命令本體是省略號/待填/TODO（有 token 但不可執行）——Agent 無機械入口\n"
fi
