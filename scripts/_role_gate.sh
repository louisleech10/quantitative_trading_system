...
#
# 🔴🔴 這裡沒有機械保護（票 B-34，使用者 2026-08-14T18:05+08:00 要求寫死）
#   本閘把 implementer 排除在 review 之外，而戳記檢查卻要求「全部 review_families 皆戳記」
#   ⇒ 兩者對「誰該出現」的定義**不一致**，該不一致目前**沒有任何檢查會發現**。
#   缺的不是掛載點，是**該票的嚴重度與修法皆未裁定**（已列入第 0 批 SPEC R2 必答題，未實作）。
#   在裁定之前，兩份定義的一致性只靠人眼；不得宣稱有機械防護。
#
