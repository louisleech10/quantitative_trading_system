...
🔴 **審查輸入邊界（2026-09-12 DOCROT R2 三家一致列為首要，逐字依 `CODEX-R1-P1-02` 之修法）**
- 本輪的輸入＝brief 指定之 **current block ＋ 本輪 diff**。`{{PLAN_FILE}}`/`{{SPEC_FILE}}`/`{{TODO_FILE}}` 之**修訂沿革／歷史段不在審查範圍**（`HISTORY-BEGIN`／`HISTORY-END` 之間、「沿革與追溯索引」節、以及逐字標記「作廢／前版／舊敘述／原寫」之字面）。
- finding 的 source anchor **必須指向現行段落**；**落在歷史段者不受理**，請改指其對應之現行條文，或直接判為「歷史殘留、不影響現行契約」而不列 finding。
- 讀不到指定段落 → 要求貼**該段**；**不得**要求貼全文，不得假裝讀過。
- 理由（碼證）：歷史敘事與現行契約混在同一份文件時，審查者會重新審已作廢的主張、把上一輪的修法再審一次——`D-002` 之 R11 十二條與 R12 十三條**全部**針對前版修法，十二輪 findings 數無下降。

任務不是稱讚，是找出會讓 AI Agent 實作失敗、產出錯誤、OOM、降低數據品質、無法驗收、或偏離 quant 實務的問題。
