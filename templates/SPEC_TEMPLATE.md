...
per-Task 必填欄（awk 機檢）：驗證 / 邊界 / **存活至** / **覆蓋風險** / 不可做。
※「存活至/覆蓋風險」為 2026-07-20 制度改進案（三家裁決 GOV-NECESSITY-REVIEW-*）新增：機檢**只驗欄位存在**，語義正確性交 adversarial（gate metadata 無法證明答案內容）。
  出生事故：1d SPEC 走六輪 adversarial 未發現「Phase 1 產出被 Phase 3 刪除」的白工，由**使用者**一句提問揭露。
舊 V12 的重型機制（per-Task 偽碼/函式名、Golden、邊界測試、人工確認）全保留，只是重構成緊湊+分層+機檢。

★ 2026-07-29 使用者定死 —— **文件分層：工作文件 ≠ 對人類的說明，兩者不得混寫**
  · **工作文件**（SPEC／TODO／brief／reconcile）＝Claude 與委員之間的**契約**。
    要求：**嚴謹、精確、無歧義、可用技術專業描述**。**不得為了讓人類好讀而散文化。**
  · **白話說明**＝文件**定案後**，Claude **另外**寫給使用者的解釋（白話閘、完工回報等）。
    要求：門外漢看得懂、專有名詞附「中文(English/縮寫)」對照。
  · **兩者是兩份產物，不是同一份的兩種語氣。** 把白話要求套進 SPEC，會逼出修飾性散文。
  **出生事故**：P1-6 的驗收條文寫成散文，導致「條文與真實行為相反」無法機械檢出；
    起草者寫字串比對想補救，**被委員連六輪用不同散文形式打穿**
    （全形/半形標點各 6 種、斷言跨行、前綴/後綴否定 8 種），耗掉整個 session。
  **根治**：見下「條件式驗收斷言：固定文法」。

★ **條件式驗收斷言一律用固定文法**（凡「行為隨模式/狀態而異」者）：
  ```
  ASSERT <命令> WHEN <key>=<value> [<key>=<value> ...] THEN rc=<n>|rc!=<n>
  ```
  · 關鍵字僅 `ASSERT` `WHEN` `THEN`；條件一律 `key=value`；結果一律 `rc=`／`rc!=`。
  · **一行一斷言**；不得跨行；不得用標點串接多條。
  · **條件不得用自然語言否定**（無「非/不帶/除了/未指定」）——要表達「非 review」就寫 `mode=discovery`。
  · **非條件式驗收**（`pytest … 全綠`、`grep -c … == 0`、help 輸出含某字串）維持散文，不受此限。
  效果：不需切段、不需判否定、不需處理標點 ⇒ **散文形式繞法全部不適用**。
...
> **⚠️ 定義新資料結構時（2026-07-26 制度改進，出生事故＝P1-6）**
> 若本 SPEC 要定義**新的事件/schema/枚舉/常數**，一律建 **JSON/schema 檔當單一真相源**，SPEC 只 pointer，
> **不得在散文中列舉欄位表/枚舉值/常數值**。
> **理由（實證非偏好）**：markdown **沒有型別系統**，改一處不會有任何機制告訴你漏同步其他處。
> P1-6 SPEC 內含 11 個 JSON 塊、10 種事件、61 處交叉引用 → **連續三輪（R9/R10/R11）都是「改一處漏同步 5-6 處」**，
> 直到把定義搬進 `scripts/audit_events.json` 才變成機器可檢。全 repo 其餘 39 份 SPEC 皆為 0 個資料結構定義，故只有這份炸。
> **檢查點**：本 SPEC 是否出現「同一欄位名/枚舉值在兩處以上被列舉」？有 → 抽成檔案。
>
> **⚠️ SPEC 階段禁止寫實作（同一事故的另一面）**
> 委員以「某腳本尚不存在」作 BLOCKING 碼證時，**那不是 SPEC 缺陷**，是該 Task 未實作的正常狀態。
> 正確回應＝在 §V 寫成**驗收條件**，**不是動手把它寫出來**。
> 本 SPEC 若新增檔案數超出預期 → 命中「膨脹升級 5 訊號」，**當場重新分類任務**，退回 TODO 流程。
...
> **殘留（本票不做、留待以後）之登記規則（2026-08-17 使用者定死；出生事故＝GAP-1 九項殘留中三項為主委偷懶/誤分類，
> 由使用者一句「現在不能做嗎」抓出）**：每條殘留**必須**帶 `為何現在不做:` 欄，值**只允許三種**：
> `blocked-by:<具體依賴（檔/層/前置票）>`／`user-ruling:<日期＋裁決內容>`／`needs-research:<研究題目>`。
> **沒有「之後再說」這種值**——寫不出三種之一＝該現在做，收回為 Task。
> 每條並附**觸發條件**（何時轉為新票），且同步登記於該 epic 之權威登記處（如 `docs/IC_QUANT_GAP_REGISTRY.md`），
> ROADMAP 只放 pointer。白話閘須**逐條**向使用者說明「為何現在不做」，不得只列清單。
- {{殘留例：<項目> — `為何現在不做: blocked-by:<…>`；觸發：<…>；登記處：<…>}}
