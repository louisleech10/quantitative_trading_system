...
> ## 🔴 本檔自 2026-09-02 起為 **R 前狀態**，實作授權以 SPEC／TODO **R35-R** 為準
> `docs/GAP3_EVENT_UX_SPEC.md` 已 R 重開（§D **D-8**：移除匯出前篩選、深度改使用者宣告）；
> 本檔中所有關於 **Task 2.1b／`filters`／B5 匯出前篩選／D-003／D-005 延伸檔／「B1＋2.1b 前置」** 之敘述**作廢**
> （D-001…D-005 已 `SUPERSEDED-BY-R`，內容依 SPEC 檔頭落點表併回本體）。
> 下一批實作＝R-R1 收斂＋三家戳記後之 **Task 1.9′ ＋ Phase 2 退役清單**；本檔待該批開工前重寫。
> （出處：`CODEX-R35-P1-09`／`GROK-R35-P2-01`）
