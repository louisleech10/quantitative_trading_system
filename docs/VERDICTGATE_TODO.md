...
- 實作要點：
  1. `scripts/verdictgate_baseline.sh --report`（bash 包 Python；v2 依 `CODEX-R9-P1-01` 補介面）：`load_events(path: str) -> list[dict]`（只取 `^\{` JSONL 行；只留 `committee_round_open`／`committee_output`／`debt_clear`）；`group_rounds(events, is_review:Callable[[dict], bool]) -> dict[RoundKey, RoundState]`，`RoundKey=(root:str, batch:int, task_prefix:str)`：`root/batch` 由 `task_id` 以 `re.I` 匹配 `^(?P<root>.+?)-b(?P<batch>\d+)-`，`task_prefix`＝`task_id` 去尾碼 `-r\d+`；`is_review`＝**與 `prev_review_resolve.sh` 同一分類**（`brief_kind=review`，或缺欄且 task_id 不命中 C-4 排除 regex）——v3 依 `CODEX-R10-P1-02`：v2 之 `-review-r<M>` 字面對實際 audit 787 筆 `round_open` 命中 0，漏 `P16-B5-TASK31-REV` 型；同 `task_prefix` 之多個 `round_open` 以 append 序編輪次；`RoundState={quorum_eligible:set[str], outputs:dict[family, verdict|None], abandoned:bool}`；`classify(state) -> dict[family, "has_verdict"|"unknown"|"stamp"|"no_output"]`；`build_report(rounds, cleared_roots:set[str]) -> Report{lines:list[str], unknown_total:int, live_roots_unwatched:list[str], legacy_open_by_root:list[str]}`；`main()` 印 `Report.lines` 後固定三行 `unknown=`／`live_roots_unwatched=`／`legacy_open_by_root=`。偽碼：`for k,st in rounds: for fam in st.quorum_eligible: cls[fam] = "has_verdict" if st.outputs.get(fam) in VERDICT_VALUES else "stamp" if fam in st.outputs and st.outputs[fam] is None else "no_output" if fam not in st.outputs else "unknown"`。
...
- 風險緩解：⊘。
- 驗證：`bash scripts/verdictgate_baseline.sh --report` rc=0；`unknown=` 之值須**等於**測試內以 `jq` 對同一 audit 獨立實算之值（v3 依 `CODEX-R10-P1-02`：SPEC 之「≥621」為 review 產出**檔**數，非 round×family 數，不作為斷言）；本票 R1–R10 已登記產出印 `has_verdict`；SPEC Task 2.1 之 3 條 ASSERT 各一 test。
- **存活至**：報表工具保留。　**覆蓋風險**：無。
...
- 實作要點：
  ...
  5. `synth_attribution_hook.sh` 改為呼叫同一模組之「寫入時子集」：`check_ids`＋`check_target`＋`check_disposition(strict_defer=False)`＋**只對已完成列**（第 4 欄含處置 token）跑 `check_quote20`（v3 依 `CODEX-R10-P1-01` 與 codex Q4 立場：「列已完成」判準＝含處置 token；無 token 之列視為草稿不擋，避免中途存檔誤擋；v2 依 `CODEX-R9-P1-04`／`GROK-R9-P1-02` 之原則不變——填錯引用或漏 token 要在寫入當下紅）；debt_clear 跑全量：`check_disposition(strict_defer=True, todo_text=<--todo 檔>)`＋對全部非佔位列 `check_quote20`。hook 與閘對同一 fixture 之 `check_ids`／`check_target`／`check_quote20` 三類結果逐字相同（測試）；`check_disposition` 兩模式各測。
...
| E-3 | 「決議是否真的處理了 finding」語意驗證 | needs-research:**研究問題**＝「finding 斷言 → 處置文字」之蘊涵能否降為封閉判準（例：處置須引用斷言中至少一個反引號 token 且含動詞集合之一）；**完成判準**＝對本票十份 synth（consult R1、review R1–R9；85 條 finding，以 `rg -c '^## (CODEX|COMPOSER|GROK)-R[0-9]+-P[0-3]-[0-9]{2}'` 實算為準——v3 依 `CODEX-R10-P2-03` 更正 v2 之「R2–R9／86」）回放，人工標記之「未處理」樣本召回率 ≥90%、誤擋 ≤1 條／份（v2 依 `CODEX-R9-P2-05` 補） | 上述研究產出判準並過三家 consult |
...
| E-6 | synth 與 SPEC 之語意等價（`spec_xref_check --synth` 只驗存在） | needs-research:**研究問題**＝「synth 處置欄與修訂標的段落之等價能否降為『反引號 token 集合相等』或『處置欄每個 token 在標的同一段落（同 `**Task N.N`／`C-n` 標題下）出現』」；**完成判準**＝對同 E-3 之十份 synth（85 條）回放，誤擋 ≤1 條／份且抓到 R7 U1 那類「synth 寫 A、SPEC 寫 A′」之已知案例 ≥1 | 研究表完成 |
