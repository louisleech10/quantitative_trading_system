...
| B-D4 | 實作 `37fa0910`（訊息具名：benchmark 先於凍結 cap、`scan_grid_max_runs` 121→110 之算術錯誤、golden 合法改變、既有斷言之對象變更四項、對 `D-001` D4.2 之具名偏離） | `pytest tests/momentum/event_samples/ -q` **485 passed rc=0**（B-D3 收工時 415，本批 +70）；`pytest tests/api/test_gap3_scan_grid.py -q` **13 passed**；`pytest tests/api -q -k "gap3 or event"` **342 passed／1 failed**（`test_service_passes_event_timestamps_kwarg`＝既有紅 `G3-R11`／`B1-WEAKTEST-1`，本批 diff 為純新增、未觸及該斷言之目標行）；`npx vitest run` **515 passed**（B-D3 收工時 502，本批 +13）；`npx tsc --noEmit` **8 行（既有債，無新增）**；`check_decoupling_imports.py --baseline scripts/decouple_baseline.txt` **BASELINE OK（R2=1 R3=17 R4=2，無新增違反）** | `--check "tests/golden/gap3_label/*.json"` **rc=0（46 cases，23→46）**；既有 4 檔僅移除 `matrix_pending`，值與 hash 逐位元組不變 | 待派（三家全員） | **實作完成，待 review**。mutation `handoffs/20260905-gap3d2-b4-mutate.py` **14/14 符合預期**（13 條改壞→紅、1 條對照組→綠、還原後工作區乾淨）。benchmark receipt `handoffs/run_receipts/gap3_scan_benchmark.json`。新殘留 `B4-COVERAGE-1`／`B4-SPECGAP-1`（已登記 registry） |
| B-D5 | — | — | — | — | — |
