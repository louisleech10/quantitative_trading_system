...
| B-D4 | 實作 `37fa0910`（訊息具名：benchmark 先於凍結 cap、`scan_grid_max_runs` 121→110 之算術錯誤、golden 合法改變、既有斷言之對象變更四項、對 `D-001` D4.2 之具名偏離）＋收據／殘留補登 `34137230`＋**前端接線補做 `7904c0dd`** | `pytest tests/momentum/event_samples/ -q` **485 passed rc=0**（B-D3 收工時 415，本批 +70）；`pytest tests/api/test_gap3_scan_grid.py -q` **13 passed**；`pytest tests/api -q -k "gap3 or event"` **342 passed／1 failed**（`test_service_passes_event_timestamps_kwarg`＝既有紅 `G3-R11`／`B1-WEAKTEST-1`，本批對該檔 diff 為純新增 270 行、0 刪除，未觸及該斷言之目標行）；`cd frontend && npx vitest run` **518 passed**（B-D3 收工時 502，本批 +16）🔴 **必須在 `frontend` 目錄跑**（`noTicketIdInUi.test.ts` 以相對路徑掃 `src/`，在 repo 根跑會假紅——我踩過一次）；`npx tsc --noEmit` **8 行（既有債，無新增）**；`check_decoupling_imports.py --baseline scripts/decouple_baseline.txt` **BASELINE OK（R2=1 R3=17 R4=2，無新增違反）** | `--check "tests/golden/gap3_label/*.json"` **rc=0（46 cases，23→46）**；既有 4 檔僅移除 `matrix_pending`，值與 hash 逐位元組不變 | `20260905-gap3d2-b4-review-r1`（codex＋composer＋grok 三家全員，debt round `48ddf30a`） | **實作完成，review 派工中**。mutation `handoffs/20260905-gap3d2-b4-mutate.py` **14/14 符合預期**（13 條改壞→紅、1 條對照組→綠、還原後工作區乾淨）。benchmark receipt `handoffs/run_receipts/gap3_scan_benchmark.json`。新殘留 `B4-COVERAGE-1`／`B4-SPECGAP-1`（已登記 registry）。🔴 **派工前自查抓到並補做**：面板之 `labelScan`／`onChangeLabelScan`／`disclosure` 三個 props 原本無任何呼叫端傳值（元件測試就位卻按不到）⇒ `7904c0dd` 接上 store／hook／page 並加三條攔 HTTP body 之全棧測試 |
| B-D5 | — | — | — | — | — |
