...
| **B9C** | 9.2b | ✅ **完成**（impl `a1e9680e`；審碼 r27，閉合 r28–r35 共九輪＋stamp-r6＋consult-r3/r4/r5；🔴 **consult-r4/r5 三家一致判定方向反轉**（見 `Task 9.3`）；~~現待 review 覆核 v32＋重簽~~ v32 之覆核與重簽已於 r36–r47 走完，方向反轉之退回由 `Task 9.3`（`4dcae642`）落地） | B9B | 側別改 `decision_at_ms` 錨定 ＋ `(3.2)` 跨表互斥；鍵不唯一時「同側」無定義，故須在複合鍵已存在後 | 大 |
...
### Task 9.4 ✅ **已完成（B9E）** —— 記帳與報告鏈（`票 SPLITUNIFY`）
