...
| `SU-RESID-PAUSED-NO-RESULT` | 委員該輪**完全無結果列**時，銷帳之暫停缺席出口不適用 | blocked-by | 🔴 v37 新增（`CODEX-R40-P1-02`）。**為何現在不做**：`blocked-by:缺該次 attempt 已終止且無產出之可信證據`——無結果列時無從區分尚在跑／崩於寫結果前／已產出未登記，推論成缺席即放寬；程式維持拒銷（codex 實跑 rc=1）。**觸發條件（可執行）**：派工器對同 round、同 family 寫出可信終止失敗回條（attempt 已終止、無產出或 0 byte、該輪無同家 `committee_output`），下一輪 review 以之建反例測試後關閉。**owner**：治理主委。詳見 `docs/SPLITUNIFY_SPEC.D-002.md` §N 同名條目 |
| `SU-RESID-COMMITTEE-MODEL-EVIDENCE` | 委員實際模型與 reasoning effort **不入審計** | needs-research | 🔴 v37 新增（`CODEX-R40-P2-01`；effort 面兩家一致併入）。**為何現在不做**：`needs-research:兩 CLI 非互動輸出之實際型號／effort 欄位與穩定解析來源尚未實測，不得以設定檔冒充實跑值`。不採 effort 下限與寫死型號（皆覆寫使用者設定）。**觸發條件（可執行）**：下一次正常授權派工能從原始 CLI 回條或結果審計取得 model、effort 三值（值／unknown／unavailable）並可重放核對。**owner**：治理主委。詳見 `docs/SPLITUNIFY_SPEC.D-002.md` §N 同名條目 |
