...
| ~~`SU-RESID-2`~~ **已關閉（2026-09-14，批次 B9B）** | 多 TF 之 `(event_id, feature_timeframe)` 複合鍵 | — | 🔴 **這條殘留正是 `Task 9.2`＋`9.2a` 的存在理由，已由該批解決**：producer 停止單選、輸出全量複合鍵列；`assignments`／`purged` 加 `feature_timeframe` 欄；兩道 guard 判準改複合鍵唯一。原文「本票以『每事件恰一個 selected per_tf row，否則 raise』fail-closed；複合鍵要連 `EventSplitPlan` 之下游一起改」保留供追溯——下游確實一起改了（`Task 9.3` 逐處消費面處置為其延續）。🔴 主委自查補記（R21 `CODEX-R21-P1-02` 之同型掃描）：codex 只點名 `Task 2.2`，本列是同一次掃描下抓到的第二處 |
