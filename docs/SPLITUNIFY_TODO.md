...
| `SU-RESID-PAUSED-NO-RESULT` | 委員該輪**完全無結果列**時，銷帳之暫停缺席出口不適用 | blocked-by | 🔴 v37 新增（`CODEX-R40-P1-02`）。**為何現在不做**：`blocked-by:缺該次 attempt 已終止且無產出之可信證據`——無結果列時無從區分尚在跑／崩於寫結果前／已產出未登記，推論成缺席即放寬；程式維持拒銷（codex 實跑 rc=1）。**觸發條件（可執行；🔴 v38 釘死，`CODEX-R41-P1-03`）**：audit log 出現同 round、同 family 之 `committee_family_result`，`result_state=failed`、`cli_rc` 非 0、`output_sha256` 為空、`output_path` 實體缺失或 0 byte、該輪無同家 `committee_output`（全用既有 schema）⇒ 既有暫停缺席出口即適用，其餘 participant ≥2 時 `debt_clear` rc=0；無該列之負例維持 rc=1。取得途徑＝`ROUND_ID=<rid> bash scripts/cx_run.sh <family> <brief> <out>` 重跑該家。**owner**：治理主委。詳見 `docs/SPLITUNIFY_SPEC.D-002.md` §N 同名條目 |
| `SU-RESID-COMMITTEE-MODEL-EVIDENCE` | 委員實際模型與 reasoning effort **不入審計** | needs-research | 🔴 v37 新增（`CODEX-R40-P2-01`；effort 面兩家一致併入）。**為何現在不做**：`needs-research:兩 CLI 非互動輸出之實際型號／effort 欄位與穩定解析來源尚未實測，不得以設定檔冒充實跑值`。不採 effort 下限與寫死型號（皆覆寫使用者設定）。**觸發條件（可執行；🔴 v38 釘死，`CODEX-R41-P2-05`）**：派工後每家 `committee_family_result` 皆帶 `model`、`reasoning_effort`（實際值／`unknown`／`unavailable`）與 `cli_receipt_path`；以固定 `jq -e` 檢查該輪結果列三欄皆存在，任一缺欄 rc=1；不得以設定檔推定值。**owner**：治理主委。詳見 `docs/SPLITUNIFY_SPEC.D-002.md` §N 同名條目 |
