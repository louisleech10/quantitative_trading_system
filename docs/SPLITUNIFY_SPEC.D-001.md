...
> 🔴 **C-4 覆寫範圍之限定**：本延伸**只覆寫 C-4 的函式簽名段**（單一 plan／單一 index → per-symbol Mapping）。**BASE C-4 其餘段落全部原文仍有效**——包含 `event_keys` 之 keyed 輸入契約、禁 positional zip、兩段式判定（答案窗 purge 先、集合成員判定後）、`build_event_keys` 具名為 producer 等。**未在本延伸重述 ≠ 已廢止**；實作與後續 refactor 不得以「延伸檔沒寫」為由刪除該節任一既有義務。
> 🔴 **唯一例外：座標語彙**。BASE 檔（`docs/SPLITUNIFY_SPEC.md`）仍以 `feature_index[row_index]` 之形式描述成員判定，那是 D-001-C2 第 4 點定案**之前**的寫法。🔴 **本條刻意不寫行號**：行號會漂（R13 已實測，原先所列三處有一處不存在、且實際落點多達十餘處），寫死等於製造下一個過期引用。判準改為**可 grep 之形狀**——凡 BASE 中形如 `feature_index[…row_index…]` 之索引句，一律適用本例外。凡 BASE 中以全框 `row_index` 索引該 symbol `feature_index` 之句子，**一律由本延伸之 `row_index_local` 取代**；其餘義務（keyed 輸入契約、禁 positional zip、兩段式判定、`build_event_keys` 具名）不受影響。b8 實作以本延伸之座標語彙為準。
...
2. 🔴 **hash 不變式**：
   ...
   - 🔴 **身分保證之分工**：D-001-C2 之指紋**已含 `symbol` 欄**，故可區分列所屬標的；但本節第 3 點之三角相等仍是**獨立必查**，不得只靠指紋——指紋證的是「這些列屬於這個 universe 且未被動過」，三角相等證的是「這批事件確實屬於這個 plan」。兩者目的不同，缺一不可。
...
4. **跨 symbol 禁共用 row_index 數字空間**：每個 symbol 各自以自己的 `feature_index` 解釋**該 symbol 之 `row_index_local`**；合併僅發生在 `assignments`／`purged`／`clusters` 的**縱向串接**，不得跨 symbol 比較 row 位置。

### D-001-C2 逐列時刻同源對證（落實 TODO §E 之 SU-RESID-3）

現行同源對證只比每段**首尾**兩列，殘留＝「首尾相同、中間間距不同」仍會被放行。本延伸改為 producer-attested 的**完整有序**指紋：

1. 🔴 **指紋 payload 之精確形狀（形狀未釘死會使 plan 指紋、§G G-5 與獨立 oracle 各凍一份）**：`rows` 為 **`list[list]`**，每列元素**順序固定**為
...
4. 🔴 **`position` 之語意與 producer attest**
   ...
   本點只列**現行態義務**，逐條編號以便機械比對；沿革與追溯索引見本檔末之「沿革與追溯索引」專區。
...
<!-- OBLIGATIONS-BEGIN id=D-001-C2-4 -->
   ...
   **(4.4) 三個 producer 之取值**：一律**直接取用其已有之 local ordinal**（`split_per_symbol` 與 `_build_plan_pair` 之 `train_local`／`test_local`；orchestrator holdout 路徑因單標的而等同 `row_index`），**不得**在 producer 端重新反推。
   **(4.5) attest 為必做，判準＝時間序往返**：取該 symbol 之列、**依時刻排序**後得 `sorted_positions`，驗 `sorted_positions[row_index_local]` 與 `row_index` 逐值相等；不等 ⇒ fail-closed。
   **(4.6) 禁用之判準**：**禁**以 `symbol_positions` 或 `_local_ordinals_for_symbol` 之結果逐值相等作 attest 判準——該二者為 **frame 序**，而 `row_index_local` 之語意是**時間序**內之序號（`split_per_symbol` 先依時刻排序才呼叫 splitter）。兩者僅在「該標的之 frame 序恰等於時間序」時相等，亂序輸入下會誤擋正確資料。
   **(4.7) 前置合法性閘（先於往返比對）**：須驗 `row_index_local` 為整數、值落在 `sorted_positions` 長度範圍內、無重複、且**嚴格遞增**；任一不合 ⇒ fail-closed。**不得**自寫該閘，須複用 `momentum/core/split_preview.py` 之 `assert_positional_rows`，且**不得**關閉其 `require_sorted`（負索引會回捲，使往返誤判相等）。
   **(4.8) 該 helper 未覆蓋、b8 須自行補之兩面**：①**dtype**——`row_index_local` 須為 `numpy` 整數型；整數值之浮點、布林、物件型數字與數字字串會被該 helper 靜默轉型放行，故須獨立擋下，**不得靠轉型救**。②**等長**——須先驗 `len(row_index_local) == len(row_index)`；**禁**以 `zip` 實作逐值比對（會截斷），一律用 `np.array_equal`。空序號僅在 `row_index` 亦為空時合法。
   **(4.9) 映不到即 fail-closed**：`row_index_local` 以該 symbol 之 post-trim universe 為序；映不到者**不得丟棄或近似**。
   **(4.10) 投影端只消費 `row_index_local`**：`derive_event_split_from_plans` 內部**一律不得**索引 `row_index`；新增任何消費點亦同，**禁**再引入對 `row_index` 的轉換或索引。
   **(4.11) 受 (4.10) 覆蓋之消費點為封閉清單**（`momentum/Analysis/event_samples/split_projection.py`，行號為抽驗時現況）：①長度閘（`:453-458`）②首尾同源對證（`:473-484`）③成員判定之 train／test 時刻集合（`:486-487`）④測試段起點（`:488`）⑤本節之指紋計算與比對。**連帶**：該函式 docstring（`:351-353`）現述舊座標語意，b8 須同步改寫為「以 `row_index_local` 索引該 symbol 之 `feature_index`」。
   **(4.12) 缺欄即 fail-closed**：`derive` 入口收到之 plan 缺 `row_index_local` ⇒ 明確報錯並指名欄位，**不得**以 `row_index` 回退。非 `derive` 之呼叫點給相容 default。
   **(4.13) 權威守衛＝入口重驗，不是不可變性**：`row_time_fingerprint` 為**字串欄**（Python 字串不可變，凍結擋得住改綁），本節第 6 點之比對點在投影入口以傳入之 `feature_index_by_symbol[symbol]` 重算並逐值比對。建構後竄改 `row_index_local` 若**改變成員集合**，必被該比對擋下。b8 **不得**移除或弱化該比對點。
   **(4.14) 指紋與遞增閘為合取，缺一不可**：指紋**先依序號遞增排序再雜湊**，故同集合之**重排**指紋完全相同 ⇒ 指紋單獨擋不住重排，擋它的是 (4.7) 之嚴格遞增。改集合由指紋擋、改順序由遞增閘擋；同集合且遞增之排列**只有恆等**。規格與實作**不得**只做其一；投影入口之 `assert_positional_rows` **不得**改為 `require_sorted=False`。🔴 **面板**順序與 `row_index_local` 順序是兩件事，不得混淆。
   **(4.15) 不可變性為縱深防禦，且有誠實邊界**：`SplitPlan.__post_init__` 應對兩個 row 欄各自複製一份（消除呼叫端別名）並以不可變 buffer 為底設為唯讀（擋直接寫入，亦擋 `setflags(write=True)` 之旗標翻回）。🔴 **但序列化往返（`pickle`／`deepcopy`）仍會還原成可寫**，故此層只降低意外竄改、**不構成保證**；規格**不得**宣稱兩欄「不可變」。
...
7. **誠實邊界（明示接受）**：本改動會使既有 IC golden digest 位移。依原檔 §G 之規矩，golden 變動須經 review；b8 收案前須把受影響的 golden 以「改前／改後逐值對照」重凍，**不得**只更新 hash。另加**獨立 oracle**：由同一 fixture 的 `row_index_local` 與該 symbol 之 universe 依本節規則重算 `sha256`（oracle 與 producer 必須同座標，否則兩邊永遠不等），與 **producer 實際寫入 plan 的指紋欄**逐值相等（oracle 不得只算 fixture 自己的值而不對證 producer 寫入值）。
...
> 🔴 **本節是本檔唯一允許出現歷史敘述與裁決編號的地方**（2026-09-12 使用者定：「如果要寫歷史或日誌，那就要有一個專區專放，不要穿插在項目中間，項目中都只能有最新版本」）。正文與義務項只留現行態；機械檢查為 `scripts/obligation_block_check.sh`。

<!-- HISTORY-BEGIN -->
**D-001-C2 第 4 點之重寫**：本點於 R12 後整節重寫為單一現行態；R6～R12 之決策沿革（某輪曾如何、為何不採某方案）一律不在本檔重述，見 `handoffs/reconcile/20260911-splitunify-x-review-r6/synth.md` 至 `…-r12/synth.md`。
