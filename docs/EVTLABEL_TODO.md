...
- 實作要點：
  ...
  2. `:1102-1160`：`w = event_isolation.label_window_rows if event_isolation else 0`；`purge_gap = max(effective_horizon, w)`；`purge_gap_source = "event_label_window" if w > effective_horizon else "mainline_horizon"`；`event_label_window_rows = w`；`lookahead_depth_rows = event_isolation.lookahead_depth_rows if event_isolation else None`；非事件（`event_isolation=None`）時三鍵**不寫**（G-1 逐位元組）。
...
- 實作要點：
  ...
  4. `digest = binary_label_digest((eid, ts, int(v)) for eid,(ts,v) in consumed_event_rows.items())`；`self._ic_cache["event_binary_label"] = ValidatedBinaryLabel(series=pd.Series(bvals, index=filtered.index).copy(), digest=digest, n_pos=..., n_neg=...)`；`series.flags.writeable=False`（底層 ndarray）；`event_info["binary_label_digest"]=digest`。
...
- SPEC ref：Task 3.6　目標：主統計 `rank_biserial`（效應量閘取絕對值、獨立門檻，R1 C1）、p 走 MW→BH；stage5 消費前 digest 對證（R1 C4）；stage6／6b 消費規則定死（R1 C9）；報酬版保留第二欄。
...
- 實作要點：
  1. stage5 事件分支（`:3766-3900`）：`vb = self._ic_cache["event_binary_label"]`；`X = features.loc[sel_idx, feature_cols]`；`y = vb.series.loc[sel_idx].to_numpy(int)`；**消費前** `binary_label_digest((owner[ts], ts, y_i) for ts,y_i in zip(sel_idx_ms, y)) == vb.digest` 否則 `raise AlignmentViolationError("binary label consumed != validated")`（注意：digest 定義域＝驗證時全部事件列；selection 子集之對證改為「子集三元組 ⊆ 驗證時集合」——以 `vb.rows_frozenset` 存放驗證時 `(eid,ts,label)` frozenset，子集逐筆 `in`；兩者皆算 M-P3-5 之守衛）。
  ...
  5. stage6 redundancy（`:3266-3281`）：`is_event_label_consumed` 且 binary ⇒ 分數＝`abs(rank_biserial)`、tiebreak `feature_name`；stage6b（`:4609-4649`）：binary 模式其結果標 `role="diagnostic"`，**不得**從倖存集移除任何特徵（測試：雙特徵 fixture）。
...
- 輸入 / 輸出：倖存者名單、`X`、`y`、`sel_idx`（時間序） → `removed["permutation_oracle_disagree"]`、`removed["permutation_unavailable"]`；`metadata.event_label_rule.permutation_receipt={seed, n_perm, block_len, n_blocks, first_permutation_digest, budget_floor_hit}`、`.negative_control={n_survivors_shuffled, seed, block_len}`；`n_survivors_shuffled>0` ⇒ `self._survivor_suppressed_reason="negative_control_failed"`（Task 3.8 消費）＋report `degraded`。
- 實作要點：
  1. `momentum/Analysis/binary_discrimination.py::block_ids_for_events(sel_idx_ms, label_window_feature_bars, feature_bar_ms) -> np.ndarray`：`gap_rows = median(diff(sel_idx_ms))/feature_bar_ms`；`L = max(1, ceil(label_window_feature_bars / gap_rows))`；事件依時間序每 L 個一 block。
  ...
  4. 負對照：`y_sh = _permute_blocks(rng(cfg.oracle_seed), y, block_ids)`；`tbl_sh = mann_whitney_table(X, y_sh, ...)`；`q_sh = apply_fdr(...)`；`n_survivors_shuffled = sum((q_sh<=alpha) & (abs(rb_sh)>=cfg.rank_biserial_min))`；`>0` ⇒ suppressed（Task 3.8）。
...
- 風險緩解：M-P3-4、M-P3-6。
- **驗證**：`pytest tests/momentum/Analysis/test_evtlabel_oracle.py -q` rc=0：植入特徵留下；固定 seed 置亂 fixture（200 合成特徵×真實 kline 165 列）⇒ `n_survivors_shuffled==0` 且 survivor 可寫；強制 `n_survivors_shuffled=1` ⇒ `survivor_output.status=="suppressed"`、`reason=="negative_control_failed"`、無 consumable 檔；monkeypatch `_permute_blocks` 恆等 ⇒ raise；`L=3` fixture ⇒ 置換後同 block 標籤仍相鄰；benchmark（`handoffs/20260910-probe-oracle-bench.py`，K=2000、budget 200000、39,373×165）`< 120s` **否則測試 FAIL**。
- **存活至**：全票完工後保留。
- **覆蓋風險**：無。
...
- 實作要點：
  ...
  5. 新 `LabelModeBanner`（`data-testid="label-mode-banner"`，R1 C14b）：`effective==='imported_binary'` ⇒ 非 degrade 之摘要「本次 IC 對象＝你匯入的 0/1 標籤（selection 段正 n_pos_selection／反 n_neg_selection）；報酬版 IC 在第二欄」；掛 `page.tsx` `DegradedBanner` 之後。
