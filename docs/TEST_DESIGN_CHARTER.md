...
- **B1 可證偽硬門檻(mutation)**:聲稱 P0 的測試**必須有 mutation probe**——注入已知 bug 必 FAIL。**最低 probe 集(改相關模組抽 1)**:① 移除 purge → 必紅;② train/test 顛倒 fit(fit_mask=test_mask)→ 必紅;③ cache key 少 symbol → 隔離測試必紅。**須留證據**:patch 摘要 + 哪個測試紅 + 錯誤摘要。做不到→降 P3,禁寫「已驗證」。不全庫 mutmut(成本),用人工 probe。
  - **B1.1 可執行、自證的 in-file 探針(2026-06-28,FF C1-2 事故後定;機制 review 兩家硬化)**:每個正確性測試**必附同檔 `test_mutation_*`**,該探針自證「基線綠 → 注入壞改 → 原斷言真紅(`pytest.raises` 或 `assert not allclose` 等)→ 還原」。**合格探針硬性結構**(`scripts/mutation_probe_static.py` AST 驗):① 非空心(不得只 `pass`/`assert True`);② 含 falsification(raises/monkeypatch/setattr/真 assert);③ **真碰到待測系統**(monkeypatch/setattr 待測碼 或 引用 momentum/api 被測符號)——防偽 raises(`pytest.raises(ZeroDivisionError): 1/0`)。**缺探針/空心/偽自證 = BLOCKING**。**N/A 豁免**:純邊界/契約/smoke 檔用**行首** `# MUTATION-PROBE: n/a — <非空理由>`(docstring 內嵌或空理由無效)。`mutation_probe_check.sh`:規則1 每檔有探針或行首N/A;規則2 AST 靜態;規則3 `pytest -k test_mutation_` 全綠且 passed>0。
  - **B1.2 oracle 獨立性(防自指假綠)**:oracle **不得從待測自身衍生**(不得用 `list_indicators()`/被測同一張 map/import 被測函式當期望值);須獨立來源(TA-Lib/scipy 直呼、硬編表、文獻手算、byte golden)。adversarial **標準必問**:「oracle 是否從待測自身衍生?」是→假綠 BLOCK。(C1-2:semantics 表從 `_INPUT_TYPE_MAP` 衍生→改 source 同時改 oracle→永遠相等。)**機器僅 `ORACLE-SELF-REF WARN` 啟發告警(偵測 list_indicators/_*_MAP/INDICATOR_REGISTRY 出現),不宣稱證獨立**;最終靠 adversarial 判。
