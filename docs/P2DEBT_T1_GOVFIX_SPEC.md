# P2 債票 1 — governance 9 紅 fixture 遷移 — SPEC(正式版)

> 定稿來源:handoffs/P2DEBT-T1-SPEC-DRAFT-R3.md(三輪 adversarial:R1 雙 BLOCK→R2→R3)
> RECONCILE-STAMP:grok(R2)=handoffs/P2DEBT-T1-SPEC-REVERIFY-grok.md;codex(R3)=handoffs/P2DEBT-T1-SPEC-REVERIFY-R3-codex.md
> task-id:p2debt-t1 | 正式化:2026-07-11 | 內容=R3 凍結稿;正式化僅補 2 行機檢錨點(L14 RISK-HIT canonical 行、L73 FACT-RECEIPT),語意零改動
...
- **已確認結果**：2026-07-11 HANDOFF 票 1 — 修法方向=遷移 fixture 至 canonical fact-scope 形狀 + B 案補生產 SPEC 錨點，禁放鬆檢查器換綠。
  FACT-RECEIPT: 使用者裁定紀錄=HANDOFF P2 債票 1(commit 5cb5006)+雙 STAMP 鏈 handoffs/P2DEBT-T1-SPEC-REVERIFY-{grok,R3-codex}.md（Claude 正式化核對 2026-07-11）
...
2. 兩條 §A fact-scope 行各補**真實非 stub** `FACT-RECEIPT:`（須指向實跑命令與 stdout 摘要）：
   ...
   - `scripts/mutation_probe_check.sh` 規則 3 行 →  
     `FACT-RECEIPT: grep -n '^echo "→ 跑 mutation 探針: pytest -k test_mutation_' scripts/mutation_probe_check.sh → 印出 74:echo "→ 跑 mutation 探針: pytest -k test_mutation_ $*"（Composer 實跑 2026-07-11）`
...
R3-CLOSURE: NEW-CODEX-R2-1 → Task 1.2b mutation FACT-RECEIPT 命令改為 `grep -n '^echo "→ 跑 mutation 探針: pytest -k test_mutation_'`（排除 L15 誤匹配）；stdout 改為 `74:echo "→ 跑 mutation 探針: pytest -k test_mutation_ $*"`（補全 echo 與 `$*`）
