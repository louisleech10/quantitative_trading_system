...
- **run 位置正規化（封閉集）**：manifest 任一層之 `source_registry_manifest`（CGSA 工作目錄絕對路徑）換成定值；task record 內 features 根目錄之絕對路徑字面（`manifest_path`、`raw_path` 等）換成定值。
