...
> 版本：v6（r5 收斂 `handoffs/reconcile/20260924-fftfmeta-x-review-r5/synth.md`：Task 1.2／1.3 釘死 resume 層證據經 `layer_status_by_tf` 傳入 resolver；§G 位置正規化收窄為只換工作目錄前綴）；v5（TODO 產出時實跑修正，併入 TODO 審查輪：§G 改輕量真實設定、健康 reference 設 `max_nan_ratio=1.0`、task record 牆鐘分段秒數列允許路徑、run 位置字面正規化、群組指紋以逐欄四 hash 摘要存放；Task 2.2／2.3／3.1 驗收改以真實 kline 輕量 run 取代打樁）；v4（r3 收斂 `handoffs/reconcile/20260924-fftfmeta-x-review-r3/synth.md`：Task 1.3 驗收改為跳過分支與閘拒絕分支兩路、各以 spy 證路徑；§G 降級 reference 改為先量測異常 NaN 比率）；v3（r2 收斂 `handoffs/reconcile/20260924-fftfmeta-x-review-r2/synth.md`：Task 1.3 記憶體欄隨 flush 往返、parallel worker 回傳六層狀態、resume 閘不改而以可 resume 組態驗收、IC-first 保留根之全部 completeness 與品質欄、§G 加降級單週期 reference）；v2（r1 收斂 `handoffs/reconcile/20260924-fftfmeta-x-review-r1/synth.md`：層失敗集合對齊 `FAILOPEN_LAYER_FAILURE_STATUSES`、resume 保存逐週期層狀態、legacy 路徑契約改為 meta.json、IC-first 二次寫入保留週期欄、品質降級同步定範圍、registry 迭代介面更正）
...
## §G Golden / Baseline
- **feature/kline 條件**：適用。以真實 `data_cache/feature_klines/kline_cache.h5`、沿用 `tests/feature_engineering/ff_truncation_mr_helpers.py` 之 `_run_generation`（`:320-365`）建多週期（primary `1h`、training `["1h","12h"]`）CGSA 小窗 run；禁合成 fixture。設定為輕量真實設定（`tests/feature_engineering/fftfmeta_golden_helpers.py` 之 `fast_payload`：資料源只 close、原子指標只 trend、operators 關、rolling 窗 5／13、L6.5 只 winsorization、dead-drop 關）；BTCUSDT、窗 2026-01-01～2026-01-31。
  - FACT-RECEIPT: `venv/bin/python handoffs/run_receipts/fftfmeta_probes/time_one_run.py 2026-01-01 2026-01-31 multi` → 印出 `seconds=9.9 groups=84 rows=721`，task record `quality_status` 為 `partial`、`failure_reasons` 為 `nan_ratio=0.0958914945688>max_nan_ratio=0.0157991655707`，manifest 根 `present_timeframes` 為 `["1h"]`（主委 實跑 2026-09-24）——全開設定單次約 150 秒（820 群組）不宜多跑，故用輕量設定；預設 NaN 基線上界使此設定之健康 run 被降級，故健康 reference 設既有設定鍵 `max_nan_ratio=1.0`。
- **凍結時機 / reference**：動工前以當下 HEAD 跑一次（`handoffs/run_receipts/fftfmeta_probes/freeze_baseline.py`），存 `tests/_golden/fftfmeta/baseline.json`：run 參數；群組檔名集合 sha256；每群組一個摘要＝該群組以 pyarrow 讀出之逐欄（欄名序）`dtype`、`shape`、NaN mask sha256、以 little-endian 原生 dtype 序列化之值 sha256 之 canonical JSON 的 sha256（10,792 欄逐欄存放約 3.2MB，摘要存放約 20KB；任一欄四 hash 之一改變即該群組摘要改變）；manifest 與 task record 之 canonical JSON（`sort_keys=True`、`ensure_ascii=False`、`separators=(",", ":")`）去除下列**允許變動路徑**並做 run 位置正規化後之 sha256。
- **run 位置正規化（封閉集）**：manifest 任一層之 `source_registry_manifest` 只把 CGSA 工作目錄**前綴**換成定值，其後之相對位置與檔名保留進 digest（v6，r5 codex P2-01）；task record 內 features 根目錄之絕對路徑字面（`manifest_path`、`raw_path` 等）換成定值。
  - FACT-RECEIPT: `venv/bin/python handoffs/run_receipts/fftfmeta_probes/golden_determinism.py 2026-01-01 2026-01-31 multi` → 印出兩跑群組檔名、每群組四 hash、去除允許路徑之 manifest 皆等，task record 僅 `manifest_path`、`raw_path`（各跑之暫存根）與 `multi_tf_stage_seconds.*`（牆鐘）不等（主委 實跑 2026-09-24）；pytest 下 `tests/feature_engineering/conftest.py:13` 另把 CGSA 工作目錄設於 tmp，manifest `source_registry_manifest` 隨之不同（主委 實跑 2026-09-24）。
- **允許變動路徑（封閉集）**：manifest 根與 `artifacts.raw`（及 `artifacts.processed`，若存在）之 `expected_timeframes`、`present_timeframes`、`failed_timeframes`、`expected_layers`、`present_layers`、`failed_layers`、`failure_reasons`、`quality_status`、`run_status`，以及 `created_at`、`updated_at`、`generation_metadata.generation_time`；task record 之 `metadata` 下同名鍵、`generation_time`、`persisted_at`、`multi_tf_stage_seconds`（牆鐘分段秒數）。
- **通過條件（可證偽）**：改後同參數重跑 ⇒ 群組檔名集合、每群組每欄四個 hash、去除允許路徑後之兩份 canonical JSON sha256 **全等**；允許路徑之改後值**逐鍵恰為**預期（v6，r5 codex P1-03：digest 去除之欄一律另有斷言）——健康 run：manifest 根與 raw、task record 之週期三欄＝`["1h","12h"]`／`["1h","12h"]`／`[]`，`quality_status`＝`complete`，`run_status`＝`complete` 限根與 task record（raw artifact 無 `run_status` 鍵，依 §C 不新增；v6，r6 codex P1-02），`expected_layers`＝`present_layers`＝改前值、`failed_layers`＝`failure_reasons`＝`[]`；降級 reference 之層欄與週期欄等於改前 manifest 值。任一不等即列出路徑與 diff＝FAIL。
- **降級單週期 reference（r2 codex P2-03）**：同一真實資料、單週期 `1h`（不經 MultiTF），以既有設定鍵 `max_nan_ratio=0.0` 觸發 NaN 門檻降級。品質閘計的是排除首尾 warmup 後之異常 NaN 比率且以嚴格大於比較（r3 codex P2-02／grok P1-02）⇒ 凍結前先對候選小窗唯讀量測 `validation.nan_ratio`，只用實測 `> 0` 之窗（觀測值寫入收據）；找不到則此 reference 列 §N 殘留，不得把 warmup NaN 改算入門檻。改前改後各跑一次：特徵資料四 hash 與去除允許路徑後之 canonical JSON 全等；允許路徑逐鍵預期＝manifest 根之 `quality_status`／`run_status` 與 `artifacts.raw` 之 `quality_status` 由 `complete` 變 `partial`（raw 無 `run_status` 鍵，v6 r6 codex P1-02）、`failure_reasons` 等於 task record 之 `failure_reasons`（含 `nan_ratio=…>max_nan_ratio=…`），task record 之 `quality_status`、`failure_reasons`、`quality_thresholds` 改前改後相等；manifest、task record 之 JSON 位元組大小差寫入收據。
...
- 目標：把週期三欄與跨週期層失敗併入 layer completeness，得單一 canonical 物件。　檔案：`feature_storage.py` 之 `resolve_completeness_meta`（加 keyword-only 參數 `timeframe_completeness: Optional[Dict[str, List[str]]] = None`、`cross_tf_layer_failures: Sequence[str] = ()`、`layer_status_by_tf: Optional[Dict[str, Dict[str, Tuple[str, str]]]] = None`〔v6〕）　既有 caller：`feature_storage.py:1171`、`:1298`。
...
- 改法：①registry 新增記憶體欄 `layer_status_by_tf: Dict[str, Dict[str, Tuple[str, str]]]`（週期 → L1–L6 → (status, reason)），`record_layer_status(tf, statuses)` 以整組取代該週期舊條目；`write_manifest()` 之 payload 帶此欄，`resume_from_manifest()` 讀回同一記憶體欄（r2 grok P1-02：只讀進區域變數會於下一次 flush 被整檔重寫抹除）。②循序路徑：每週期 L1–L6 執行後、`write_manifest()` 前記錄六層（含非失敗狀態）。③parallel 路徑：`_tf_worker_entry` 回傳增加六層 `(status, reason)`；parent 於該週期群組註冊成功後、`:586` 之 `write_manifest()` 前記錄；註冊失敗 rollback 時清掉該週期條目；primary 於 `:466` flush 前記錄（r2 codex P1-02、composer P2-01）。④stale alignment 拆除群組後重跑之週期，以新六層記錄取代舊條目。⑤resume 跳過之週期自此欄讀回，其失敗層併入 `cross_tf_layer_failures`；**全部 present 週期之層狀態經 `resolve_completeness_meta(..., layer_status_by_tf=…)` 傳入**（v6，r5 codex P2-02：證據之傳遞介面釘死於 resolver，不得另由 factory 讀 registry 自算）——有此參數時層證據以之為準、`layer_results` 不參與：`expected_layers`＝各 present 週期狀態非 `empty_disabled` 之 `L<n>` 聯集（L1–L6 序）、`present_layers`＝其減去任一週期失敗之 `L<n>`；任一 present 週期無條目、或條目未含 L1–L6 全部六鍵 ⇒ 缺證據 ⇒ `quality_status`＝`unknown`（不得解為無失敗）；層失敗之唯一權威仍為 `cross_tf_layer_failures`。⑥**resume 閘不改**：`_prepare_cgsa_registry`（`feature_factory.py:1034`）於有 config hash 時，L7 manifest 缺席或不可快取即不 resume、層全數重算，無層狀態流失；本 Task 只保證「確實 resume 時」層狀態不流失。依現行閘，跳過分支之可達輸入＝工作 manifest 含該週期 L1–L6 群組，且 L7 `feature_manifest.json` 之 `resolve_run_status` 為 `complete`（r3 grok 探針 `resume_calls=1`）；`FFACT_CGSA_WORK_DIR` 只固定工作目錄、不放寬閘（r3 三家更正 v3 之錯述）。
- **驗證**：`pytest tests/test_cgsa_resume.py -k layer_status` 全綠，兩分支皆以 spy 證明實際路徑：①**跳過分支**——工作 manifest 含 `1h`／`12h` 之 L1–L6 群組與 `layer_status_by_tf`（12h 之 L2 為 `dependency_failed`），另寫 `resolve_run_status == "complete"` 之 L7 stub（比照既有 `test_cgsa_resume_from_existing_manifest`），直接呼叫 `_prepare_cgsa_registry` 與 MultiTF 產生流程（使 `_try_load_cache` 因 metadata 不可快取而落空）⇒ spy 證 `resume_from_manifest` 被呼叫且兩週期皆走跳過分支；canonical `failed_layers` 含 `L2:12h:` 前綴、`quality_status == "partial"`；循序與 parallel 各一；再觸發一次 `write_manifest()` 後工作 manifest 之條目仍在；以無 `layer_status_by_tf` 之同形 checkpoint ⇒ `quality_status == "unknown"`；parallel 註冊失敗 rollback ⇒ 該週期無條目。②**閘拒絕分支**（對照）——無 L7 檔（或 L7 為 `partial`）⇒ spy 證 `resume_from_manifest` 未被呼叫、層重算，最終 completeness 為重算結果（不得把重算說成 resume）。
...
- **驗證**：`pytest tests/feature_engineering/test_failopen_producer.py -k persist_completeness_same_source` 全綠：以真實 kline 輕量 run（v5：取代打樁，禁合成 fixture）讀回 manifest 根與 `artifacts.raw`（frame 路徑為 `meta.json`），對 `COMPLETENESS_FIELD_NAMES` 每鍵、`quality_status` 與 `failure_reasons` 斷言 `artifact[k] == result.metadata[k]`；多週期 CGSA、降級單週期 CGSA、frame 單週期各一。
...
- **驗證**：`pytest tests/feature_engineering/test_failopen_manifest.py -k degradation_in_manifest` 全綠：真實 kline 輕量單週期 run、`max_nan_ratio=0.0`（v5：實測 `nan_ratio` 0.0096 > 0，取代打樁）⇒ `manifest["quality_status"] == result.metadata["quality_status"] == "partial"` 且 `failure_reasons` 相等；frame 路徑 L6.5 失敗 ⇒ `meta.json` 與 `result.metadata` 皆含 `L6.5:preprocessing_failed`。
