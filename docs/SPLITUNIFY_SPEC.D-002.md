...
### D-002-C5 單鍵消費面（register 共 25 條；分四層敘述見 (5.2)–(5.5)）
...
- **大小**：大（命中 (a)(b)(d)）。
- **命中高風險原則**：(a) 多 TF 記帳影響 IC 樣本數與統計量；(b) 觸及 `D-002-C5` (5.6) register 之 25 條共用消費面，含前端與 golden（🔴 v11：原寫「16 處」，依 register 修正）；(d) 錯誤折疊或異側放行會直接改變 ML／回測輸入之樣本構成。
- **RISK-HIT: a,b,d**
...
- `Task 9.1`（🔴 **三層標的已隨 v8 之殘留決策更新**；v7 之「API 回應／前端型別」兩層**作廢**，不得再據以寫測試）：`ASSERT build_event_keys WHEN per_tf 含 1h 與 4h 而 selected=1h THEN discarded == {"4h": 2}`；`ASSERT WHEN 單一 feature TF THEN discarded == {}`；`ASSERT EventSplitPlan.summary 帶 discarded_rows_by_feature_tf 且值與 producer 回傳相同`；`ASSERT metadata.split_unify["discarded_rows_by_feature_tf"] == EventSplitPlan.summary["discarded_rows_by_feature_tf"] == producer 回傳之 discarded`（🔴 v11：原寫 `ASSERT … 帶該鍵`，與同句 summary 斷言**不對稱**，孤立單測手塞空 dict 即可通過——三家 R10 撞題）；`ASSERT metadata.split_unify 之 reason 封閉值集未被改動`；`ASSERT 移除上述三層後行為與 9A 前逐值相同`（獨立回退之可證偽斷言）。終端可見性之斷言待接線後補（見 §N 殘留）。
...
**mutation 目錄**（🔴 逐處對應，不得以單一 generic mutant 冒充；每列皆須有**完整 ID** 與**應紅之測試**；共 **32** 條，🔴 本數字須與表列實數相符——前一版寫 23 而實列 22，由 R5 兩家以計數抓出；🔴 **一 mutation 一 defect**：R6 指出 `M-SU-D2-23` 曾以 OR 合併兩個獨立缺陷，已拆為 `23`／`26` 各配自己的應紅測試）
...
| `M-SU-D2-29` | `freeze_splitunify_golden.py` 之 `--write` 直接覆寫 v8 baseline 鍵 | (G-4d)① 之「v8 鍵保留、只增不覆寫」斷言 |
...
單 TF 舊值保留為回歸錨；Phase 9A 可獨立回退（移除三層揭露即可，由 §V 之可證偽斷言保證）。Phase 9B 回退需連同 `D-002-C5` (5.6) register 之 **25 條**消費面一併還原（🔴 v11：原寫「16 處」與 register 不符，依 R10 codex 修正），故 9B 須在單一批次內完成，**不得部分上線**。
