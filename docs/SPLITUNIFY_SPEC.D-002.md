...
- 目標：在複合鍵落地**之前**，先消除「靜默丟棄」之誠實性缺陷，**本延伸交付至 producer 層**（producer 回傳 → `EventSplitPlan.summary` → `metadata.split_unify` 三層完整記帳）；**終端可見性**（API 回應欄位與前端顯示）見 §N 之 `SU-RESID-9A-UI`，**不在本延伸完成條件內**。
