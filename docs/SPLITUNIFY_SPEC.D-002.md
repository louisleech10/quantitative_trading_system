...
**(5.3) 第二層｜表格鏈（D-001 原列六處）**：🔴 **本項為現況描述；每處之處置一律以 `(5.1)` 三分類為準**。`feature_materialization` 之 `merge validate="many_to_one"`（複合鍵後仍成立、**不需改**）與 `groupby("event_id")+row_vals.update`（**設計上的橫向合併**，(5.1) 甲類維持）與輸出 `set_index("event_id")`（同屬甲類，只需斷言多列輸入不靜默覆蓋）；`baseline`（繼承上游事件級索引，甲類）；`pattern_bridge` 之 `set_index("event_id")["split_label"]`（複合鍵後取到 Series ⇒ **(5.1) 丙類**，去重取唯一側）；`tables` 之 `set_index`（`tables.py` 共 **4 處**——`:214` `receipts.event_level`、`:229` 與 `:373` `clusters`、`:372` `assignments`；前三處為甲類，`:372` 屬**丙類真缺陷**，已另立 `C5-29`）；`ic_feed` 兩處 `set_index` ＋ `.loc[keep["event_id"]]`（先過濾單一 TF，索引本就唯一，甲類）；`dedupe` 之 `merge validate="one_to_one"`（對事件級 `events`，仍成立）與 `cluster_first` 保留集（甲類，保留之 `event_id` 廣播到 per-TF 列）。
...
| `C5-13` | `tables` 之 `set_index` ＋ `.loc[eid]` **三處**：`tables.py:214`（`receipts.event_level`）／`:229`／`:373`（`clusters`）〔四處中的甲類三處；`:372` 之 `assignments` 屬丙類、見 `C5-29`〕 | 甲 | 維持事件級＋加防誤改回歸 | `Task 9.3` | `M-SU-D2-06` |
| `C5-14` | `baseline` 之事件級索引（`baseline.py:105-121`） | 甲 | 隨上游同步；另依 `(6.2)` 拆 `n_test_events`／`n_test_samples` 並刪舊鍵 | `Task 9.4` | `M-SU-D2-31`／`M-SU-D2-32` |
...
| `C5-19` | `split_projection.build_event_keys` 輸出（`split_projection.py:291-303`） | 乙 | 以 `per_tf` 為行粒度接合、新建 `feature_timeframe` 欄 | `Task 9.2` | `M-SU-D2-23`／`M-SU-D2-26` |
| `C5-20` | `split_projection` 之 `assignments` 組裝 | 乙 | 加 `feature_timeframe` 欄，`(event_id, feature_timeframe)` 唯一 | `Task 9.2a` | `M-SU-D2-35` |
...
- 改法：依 (6.2) 將 `n_train`／`n_test`／`n_purged` 明確定為事件數（🔴 **`baseline` 除外**：其舊鍵 `n_test` 依 (6.2) v11 **刪除**，改輸出 `n_test_events` 與 `n_test_samples` 兩量，見本 Task 末條）；新增列數欄；`dict(zip(...))` 改為複合鍵映射。
...
- 🔴 **`Task 9.2b`／§G (G-4d)(G-4e) 之五條**（原只寫在 §G 散文、未進 §V；當時 §V 範圍為 `:216-257`）：
  1. `ASSERT WHEN decision_at_ms == feature_cutoff_ms THEN 該事件相對 v8 baseline 零位移`（(G-4d)②；🔴 現行 fixture **全部事件**皆 `decision == cutoff`（`freeze_splitunify_golden.py:117` 直接複製），故此斷言在補入下一條之 fixture 前為**空心通過**）。
...
| `M-SU-D2-03` | 下列任一即缺陷：summary 帶了但**不傳入** `metadata.split_unify`；鍵在而**值不等於** summary／producer 回傳；或改動了 `reason` 封閉值集 | `Task 9.1` 之 `metadata.split_unify` **值相等**斷言 ＋ `tests/api/test_splitunify_disclosure.py` 之 exact-key 斷言 |
