...
| `C5-13` | `tables` 兩處 `set_index` ＋ `.loc[eid]` | 甲 | 維持事件級＋加防誤改回歸 | `Task 9.3` | `M-SU-D2-06` |
...
- `D-002-C5` (5.6) register 之 25 條消費面中（🔴 v11：原寫「16 處」，依 register 修正），標記**靜默**者驗收不得只看「有沒有報錯」，須斷言取到的**值**正確。
...
**mutation 目錄**（🔴 逐處對應，不得以單一 generic mutant 冒充；每列皆須有**完整 ID** 與**應紅之測試**；共 **33** 條，🔴 本數字須與表列實數相符——前一版寫 23 而實列 22，由 R5 兩家以計數抓出；🔴 **一 mutation 一 defect**：R6 指出 `M-SU-D2-23` 曾以 OR 合併兩個獨立缺陷，已拆為 `23`／`26` 各配自己的應紅測試）
...
| `M-SU-D2-30` | derive 路徑略過 `validate_split_pair_integrity`，或 train 段為空時 `continue` 而不 raise | `Task 9.2b` 前置之 pair 完整性斷言 |
