...
- 🔴 **(G-4e) v10：(G-4c) 擋不住「兩邊同錯」，須加第三份判準（R9 三家獨立撞題）**：G-3b 僅驗「投影 vs 第二份實作」是否相等，若投影與 oracle 被**同一次改動寫入同一錯誤語意**，兩者仍相等而放行。三家各給反例（`decision=950/cutoff=900`（`train_last=900`）正解 `purged` 而兩邊同判 train；`decision=cutoff=train_last=200`；`decision=250/cutoff=200`），皆為 G-3b 綠而成員集錯。⇒ **保留 (G-4c) 作為抓「不對稱錯誤」之 regression**，**另加第三份判準**：以本 SPEC `Task 9.2b` 之三段式（**含步驟 0**）從 fixture 欄位（`decision_at_ms`／`feature_cutoff_ms`／`train_last_ms`／`test_start_ms`）**純函式**算出每事件之期望側，並與投影、oracle **雙邊**比對；三者不全等即 FAIL。可內嵌於 `freeze_splitunify_golden.py` 之 `main()`（現有 G-3b 比對骨架旁），**不必新增 allowlist 檔**。
...
- 🔴 **`Task 9.2b`／§G (G-4d)(G-4e) 之五條（v10 補；前版只寫在 §G 散文，三家 R9 指出未進 §V——當時 §V 範圍為 `:216-257`，五條皆不在其中）**：
  ...
  3. `ASSERT 投影側、oracle 側、三段式純函式期望側 三者全等`（(G-4e) 第三份判準；任一不等即 FAIL）。
...
- 🔴 **`Task 9.2b` 前置（v10 補，R9 codex）**：`ASSERT WHEN train_rows 或 test_rows 為空、或兩者 row set 重疊 THEN raise`；`ASSERT derive 路徑確實呼叫 validate_split_pair_integrity`（碼證：該 validator 已存在但 derive 從未呼叫）。
...
**mutation 目錄**（🔴 逐處對應，不得以單一 generic mutant 冒充；每列皆須有**完整 ID** 與**應紅之測試**；共 **31** 條，🔴 本數字須與表列實數相符——前一版寫 23 而實列 22，由 R5 兩家以計數抓出；🔴 **一 mutation 一 defect**：R6 指出 `M-SU-D2-23` 曾以 OR 合併兩個獨立缺陷，已拆為 `23`／`26` 各配自己的應紅測試）
...
- `SU-RESID-9A-UI` — **本延伸不交付**「丟棄列數之終端可見性」（API 回應欄位與前端顯示），`blocked-by`（投影路徑**無生產接線**：`api/` 對 `EventSamplePipeline.run` 之呼叫點為 0，唯一帶 canonical 邊界之 `.run(` 只在 `tests/momentum/event_samples/test_splitunify_wiring.py`；此為前批「接線」遺留之架構現況，非本延伸範圍）。**觸發條件**：`api/` 出現走投影分支之生產 `run()` 呼叫點。**本延伸交付至 producer 層**（`build_event_keys` 回傳 ＋ `EventSplitPlan.summary` 鍵 ＋ `metadata.split_unify` 欄位）。🔴 **誠實邊界**：在本殘留解除前，「靜默丟棄」對**終端使用者仍然看不見**——`Task 9.1` 之驗收不得宣稱該缺陷已消除，只能宣稱 producer 層已誠實記錄。**`為何現在不做: blocked-by:投影路徑無 EventSamplePipeline.run 生產接線（api/ 呼叫點=0）`**——觸發判準之可執行形式為 `api/` 內 `EventSamplePipeline().run` 之命中數 > 0（現為 0）。（🔴 v10 更正依據：§N 殘留之權威為 `templates/SPEC_TEMPLATE.md:107-112`——值**只允許三種** `blocked-by:<具體依賴（檔/層/前置票）>`／`user-ruling:`／`needs-research:`，且**每條必須帶 `為何現在不做:` 欄**。v9 誤引 `templates/BRIEF_REVIEW_TEMPLATE.md:71` 之四值閉集——那是 brief 內「沒查的那句話」表之 `reason_code`，**不治理 SPEC §N**；三家 R9 一致指出此引用錯誤，惟亦一致認為**類別 `blocked-by` 本身仍成立**，因該範本明允「檔/層/前置票」而本案阻塞對象為**層**。另：本案**不選** `out-of-scope`，因投影接線屬本 epic 範圍、只是不在本延伸批次。）
