...
| `C5-20` | `split_projection` 之 `assignments` 組裝 | 乙 | 加 `feature_timeframe` 欄，`(event_id, feature_timeframe)` 唯一 | `Task 9.2a` | `M-SU-D2-20` |
| `C5-21` | `split_projection` 之 `purged` 組裝（`split_projection.py:556`） | 乙 | 加 `feature_timeframe` 欄；並受跨表互斥斷言約束 | `Task 9.2a` | `M-SU-D2-24` |
...
- 🔴 **(G-4e) v10：(G-4c) 擋不住「兩邊同錯」，須加第三份判準（R9 三家獨立撞題）**：G-3b 僅驗「投影 vs 第二份實作」是否相等，若投影與 oracle 被**同一次改動寫入同一錯誤語意**，兩者仍相等而放行。三家各給反例（`decision=950/cutoff=900`（`train_last=900`）正解 `purged` 而兩邊同判 train；`decision=cutoff=train_last=200`；`decision=250/cutoff=200`），皆為 G-3b 綠而成員集錯。⇒ **保留 (G-4c) 作為抓「不對稱錯誤」之 regression**，**另加第三份判準**——🔴 **v11 更正（三家 R10 撞題＋主委自跑探針）**：第三份**不得**再是「同一三段式的另一次編碼」（那擋不住「同一次對 SPEC 的錯誤解讀寫進三份」；主委探針 `handoffs/20260912-splitunify-b9-probe-g4e-triple.py` 實測：獨立實作時 `g4e_pass=False`、三份同錯時 `g4e_pass=True`）。**改為 fixture 內之字面 `expected_side` 欄**：於 `freeze_splitunify_golden.py` 之 `_event_keys()` 逐 row dict 增一欄，由**人手依 `Task 9.2b` 之三段式（含步驟 0）逐筆填入**每事件之正確側（含隔離帶、界外、以及 `decision_at_ms` 與 `feature_cutoff_ms` 不相等之邊界事件），再 `ASSERT 投影側 == oracle 側 == fixture.expected_side`；比對點內嵌於同檔 `main()`（現有 G-3b 比對骨架旁），**不新增 allowlist 檔**。🔴 **明禁**：第三份**不得** import／呼叫投影、`_oracle_membership` 或兩者之**共用 helper**——抽一個共用 `_side()` 餵三處即結構性保證三份同錯。🔴 **殘餘誠實邊界**：本條仍靠「人手填值時想清楚」，若填值者照錯誤理解填，三份仍會一致——**此為散文紀律、非機械保證**，與 `(G-4c)` 之「逐行重寫」同性質，須據實登記不得宣稱已機械閉合。
- 🔴 **(G-4d) 三項硬性附帶條件**：①**保留 v8 baseline 不覆寫**（重凍寫新鍵，舊鍵留為換錨前之回歸錨；`--write` 不得直接覆蓋，R8 codex）——🔴 **v11 具體化（R10 codex 實跑：`freeze_splitunify_golden.py:373-386` 現行為 `golden_path.write_text(...)` **直接覆寫**，且 `tests/golden/splitunify/splitunify_golden.json` 頂層實為 **11 鍵**——`_doc`／`g1_membership`／`g3b_oracle`／`g4_per_symbol_n`／`g5_answer_window`／`g5_row_fingerprint_first_ms`／`g5_row_fingerprint_last_ms`／`g5_row_fingerprint_n`／`g5_row_fingerprint_positions`／`g5_row_fingerprint_sha256`／`purge_reasons`——**不存在任何 v8 鍵**，`tests/golden/splitunify/` 下亦無 `.v8.json`（🔴 主委實跑 `jq keys` 與 `ls` 親驗；R10 codex 原述「只有 g1／g3b／g4／g5」為不精確措辭，本版依實測更正） ⇒ `M-SU-D2-29` 目前無標的可驗、本條為空心）**：落地順序寫死為——(i) 換錨**之前**先把現行 golden 內容另存為**獨立不可變檔** `tests/golden/splitunify/splitunify_golden.v8.json`，並將其 `sha256` 寫入 `tests/golden/splitunify/splitunify_golden.v8.sha256`；(ii) 重凍後之新成員集寫入 `splitunify_golden.json` 之**版本化新鍵** `g1_membership_v9`／`g3b_oracle_v9`（**只增鍵，不改既有鍵**）；(iii) `main()` 每次執行先 `ASSERT sha256(splitunify_golden.v8.json) 等於 .v8.sha256 所記之值`，不等即 FAIL；(iv) `--write` 對 `.v8.json` 一律拒寫（`raise`，非靜默略過）。🔴 **v12 補強（R11 三家撞題：codex P1-04／composer P1-02／grok P1-02 各給一條繞過構造，主委複驗皆成立）**：(i)–(iv) **擋不住兩條路徑**——①`sha256(檔)==旁檔` 是**自參照**，同時改寫 `splitunify_golden.v8.json` 與 `splitunify_golden.v8.sha256` 即恆為真；②`--write` 仍整檔 `write_text`，只拒 `.v8.json` **擋不住主檔既有 11 鍵被換錨後之值覆蓋**。⇒ 再加三條：**(v) write-once**——`.v8.json` 須以 `O_EXCL` 獨佔建立，檔案與旁檔一經建立即**一律拒絕更新**（非「比對後更新」）；**(vi) 外部錨**——期望 digest 須錨在**不由 `freeze_splitunify_golden.py` 改寫**的來源（首次凍結之 `sha256` 字面寫進本 SPEC §V，與 golden byte 閘同型），單靠旁檔不算；**(vii) 主檔護欄**——`--write` 寫入前須先比對主檔既有 11 個頂層鍵，任一值被改動即 `raise`，新成員集只允許落在 `g1_membership_v9`／`g3b_oracle_v9`。🔴 **允許差異集合（(G-4) 通過條件所指者）之可執行形式**＝`g1_membership_v9` 與 `splitunify_golden.v8.json` 之逐 `event_id` diff，且該 diff 內每一筆都須同時被 `_oracle_membership` 與 fixture 之 `expected_side` 認可（見 (G-4c)／(G-4e)）；②`decision_at_ms == feature_cutoff_ms` 之事件**必須零位移**（硬斷言）；③`decision != cutoff`（即 `decision_at_ms != feature_cutoff_ms`）之單 TF 邊界 fixture **須進 §V 與 mutation**，不得只寫在 §G 散文。
...
- **揭露落點（本延伸只交付 producer 層；API 與前端隨 §N 殘留延後，**不再**列入 9A 完成條件）**：
  ...
  - 🔴 **`metadata.split_unify` 是 exact-key 契約，加鍵須同批改（R8 codex；前版誤當「順手擴欄」）**：`build_split_unify_disclosure`（`split_projection.py:123-180`）現**恰回五鍵**；契約檔 `momentum/Analysis/contracts/split_unify.json`；唯一 caller `ic_filter_orchestrator.py:1530-1534`（現不傳 `discarded`）；`tests/api/test_splitunify_disclosure.py:149-151,270-277` 對該欄做 **exact-key 斷言**（不同步改即紅）；前端型別面。新鍵型別定為 `Dict[str, int]`（鍵＝被丟棄之 `feature_timeframe`、值＝列數）。🔴 **v10 定案：鎖定三層，本句之「二擇一」已裁決，不得再由實作者自選**（三家 R9 一致指出前版留了逃逸句）。裁決依據＝主委實讀契約：`split_unify.json` 之 `test_segment_count_keys` 雖為 deny-by-default 封閉登記，但其判準是「鍵名含 `test` **且值為整數**」，而 `discarded_rows_by_feature_tf` 之值為 `Dict[str, int]` ⇒ **不落入該登記**，實際須改者為**三處**（builder 五鍵回傳、`tests/api/test_splitunify_disclosure.py` 之 exact-key 斷言、前端型別），低於原估之五處；🔴 **v13 兩點更正（O7；R12 codex）**：①此「三處」**漏數**——`momentum/Analysis/contracts/split_unify.json` 之 `split_unify_keys` 五鍵陣列是 CONTRACT 唯一來源，而 `tests/api/test_splitunify_disclosure.py:272` 以 `set(su) == set(CONTRACT["split_unify_keys"])` 對證，不改即紅 ⇒ **實為四處**（主委實讀，非委員提出）；②更根本地，**整段隨 O1 移入 `SU-RESID-9A-UI` 殘留驗收**——`metadata.split_unify` 層既已判為不可達而延後，本段之施工面**不在本延伸完成條件內**，字面保留供殘留解除時使用，**不得**列入當輪交付；且兩層方案會讓「唯一真相源」缺少本延伸產出的揭露欄，反而誘發第二份真相源。
...
- 🔴 **側別判準（三段式，順序不得調換；R6 三家獨立撞題後定案）**：`decision_at_ms` 與 `feature_index` 未必落在同一網格——當 `feature_index` 之網格**粗於**觸發 TF 時（例：觸發 1h、特徵 4h），`decision_at_ms` 大量不在 `index_ms` 集合中（R5 實證 1h open 非 4h open 者 15,264／20,352）；但**單純改用不等式會把隔離帶收成 train**（R6 實證：gap 上集合成員給 `purged`、不等式給 `train`）。定案如下：
  ...
  0. 🔴 **前置條件（R8 codex：四條規則原本彼此重疊；v10 依 R9 codex 補 split pair 完整性）**：先驗 ①`train_rows` 與 `test_rows` **皆非空**、②兩者 row set **不重疊**、③呼叫既有之 `validate_split_pair_integrity`（🔴 碼證：該 validator 已存在，但 **derive 路徑從未呼叫它**，且 `split_projection.py:498-527` 對 train 空只 `continue`）——🔴 **v11 必附座標 adapter（R10 codex；否則此條會誤殺合法批）**：D-001 之 `row_index_local` 為**局部**座標，而該 validator 吃**全域** `row_index`；codex 實跑交錯 fixture 得 `IndexError: plan.row_index contains positions outside base universe` ⇒ 照原文實作會**錯誤拒絕合法之多標的交錯批**。🔴 **v12 再更正（R11 codex P1-03＋grok P1-03 撞題；主委自寫探針 `handoffs/20260912-splitunify-b9-probe-m5-coords.py` 四案複驗，與 grok 逐格一致）**：v11 寫的二擇一**既不窮盡、兩案又都違反 D-001**。四案真值表——全域索引＋全域宇宙 `PASS`；全域索引＋局部宇宙 `IndexError`；**局部索引＋局部宇宙 `PASS`**（v11 二擇一未涵蓋之第三條路徑）；局部索引＋全域宇宙 `CrossSymbolLeakageError`。🔴 **決定性碼證**：`D-001-C2` (4.10) 逐字「投影端只消費 `row_index_local`：`derive_event_split_from_plans` 內部**一律不得**索引 `row_index`；新增任何消費點亦同，**禁**再引入對 `row_index` 的轉換或索引」⇒ v11 之 (a)「傳 full-universe `row_index`」與 (b)「local→global rebase」**兩者都直接牴觸 D-001**，而本 SPEC 開頭又寫著「本延伸不動座標語意、`D-001-C2` 第 4 點全文繼續有效」，屬同檔自相矛盾。**改法（寫死，不得由實作者自選）**：①`validate_split_pair_integrity` **不在 derive 內呼叫**，改由仍持有 full `ts`／`symbols` 的 producer／adapter 層呼叫，驗過的 plans 才交給投影——🔴 **v13 具名（O4；R12 composer＋grok 撞題，本輪「可實作性」之直接反例）**：v12 只寫「producer／adapter 層」而**沒具名是哪一個**，三家實查後證實**事件投影路徑上該層不存在**（`ic_split_adapter.py:305` 與 `ic_filter_orchestrator` 已呼叫，但那是 **IC 路徑**；事件路徑之 `holdout_boundary` 對 `validate_split_pair_integrity` 零命中，`EventSamplePipeline.run` 只轉傳 plans、**不建構** `symbols` 陣列）⇒ 照原文無可編譯落點。**具名寫死**：呼叫點為 `EventSamplePipeline.run` 在呼叫 `derive_event_split_from_plans` **之前**，以 `feature_index` 之時刻序列作 `ts`、以 `train_plan.symbol` 廣播成等長陣列作 `symbols`（單標的）或由呼叫端提供 full `symbols`（多標的）；並**禁止** pipeline 接受未經該呼叫驗證之 plans（缺驗即 fail-closed）。②投影端維持**只讀** `row_index_local`，不收全框輸入、不做座標轉換；③契約明寫「`row_index` 與 `ts`／`symbols` 必須同一座標系，**禁混用**」，四案真值表逐格寫進 §V。④`index_ms[0] <= decision_at_ms <= index_ms[-1]`，**任一不滿足即 fail-closed raise**（訊息含 `event_id`）。⑤**既有不變量（具名引用，供讀者追溯前提來源）**：`train_last_ms < test_start_ms` **恆成立**——`train` 末位＝`split_point-1`、`test` 首位＝`split_point+purge_gap+embargo`，位置差 `≥ 1`，且 `split_projection.py:213` 明定 `feature_index` **必須嚴格遞增**（`:472` 註明該檢查不得關閉）⇒ 位置序即時刻序。故通過前置後，下列三條**互斥且窮盡**。沒有這一步，`index=[100,200,300,400]`、`train_last=200`、`test_start=300`（即 `train_last_ms=200`、`test_start_ms=300`）時 `decision=50`（`decision_at_ms=50`）會**同時命中**下列第 1 條與第 4 條，實作者可合法選先分類而讓**越界事件進 train**；`decision_at_ms=450` 同理進 test。通過本前置後，下列三條才**互斥且窮盡**。
...
- `Task 9.1`：`ASSERT build_event_keys WHEN per_tf 含 1h 與 4h 而 selected=1h THEN discarded == {"4h": 2}`；`ASSERT WHEN 單一 feature TF THEN discarded == {}`；`ASSERT EventSplitPlan.summary 帶 discarded_rows_by_feature_tf 且值與 producer 回傳相同`；🔴 **v13 移出當輪驗收（O1；R12 codex＋composer 撞題）**：下列兩條 metadata 斷言屬 `SU-RESID-9A-UI` 殘留之驗收，**不在本延伸完成條件內**——碼證：`EventPipelineResult`（`pipeline.py:52-63`）只有 `summary`／`split_plan`、**無** `metadata` 欄，故「值相等斷言必須跑在 `EventSamplePipeline.run`」與「metadata 併入殘留」在同一段**自相矛盾**，實作者照 §V 會去斷言一個該入口產不出的物件。⇒ 本延伸之 §V `Task 9.1` 只保留 producer → `EventSplitPlan.summary` 兩層並掛該入口；下列兩條字面保留供追溯，**於殘留解除後才生效**：`ASSERT metadata.split_unify["discarded_rows_by_feature_tf"] == EventSplitPlan.summary["discarded_rows_by_feature_tf"] == producer 回傳之 discarded`（🔴 v11：原寫 `ASSERT … 帶該鍵`，與同句 summary 斷言**不對稱**，孤立單測手塞空 dict 即可通過——三家 R10 撞題）；`ASSERT metadata.split_unify 之 reason 封閉值集未被改動`；🔴 **v12 補整鏈之可執行入口（R11 codex P1-01＋composer P2-01 撞題；兩家的修法前半段主委複驗為走不到）**：上述值相等斷言**必須**跑在 `EventSamplePipeline.run` 這條入口上——主委實跑全 repo `train_plan=` 呼叫點，唯一同時帶 `feature_index` 與 `selected_timeframe` 的事件投影入口只在 `tests/momentum/event_samples/test_splitunify_wiring.py`；`ic_filter_orchestrator` 走的是 `holdout_boundary`、**不呼叫** `build_event_keys`，其 `discarded` 命中數為 **0**，且 `build_split_unify_disclosure` 簽名無該參數 ⇒ **不得**把斷言寫成「經 `ic_filter_orchestrator` 驗值」（那會是本票第四次指名走不到的落點）。⇒ 本延伸之 `metadata.split_unify` 層**明確併入** `SU-RESID-9A-UI` 殘留、不宣稱已交付；可執行交付為 producer 回傳 → `EventSplitPlan.summary` 兩層，且該兩層之整鏈斷言掛在上述入口。；`ASSERT 移除上述三層後行為與 9A 前逐值相同`（獨立回退之可證偽斷言）。終端可見性之斷言待接線後補（見 §N 殘留）。
...
- 🔴 **`Task 9.2b`／§G (G-4d)(G-4e) 之五條（v10 補；前版只寫在 §G 散文，三家 R9 指出未進 §V——當時 §V 範圍為 `:216-257`，五條皆不在其中）**：
  ...
  3. `ASSERT 投影側、oracle 側、fixture 之字面 expected_side 三者全等`（(G-4e) 第三份判準；任一不等即 FAIL）。🔴 **v11 更正**：原寫「三段式**純函式**期望側」已作廢——那是同一三段式的第三次編碼，主委探針 `handoffs/20260912-splitunify-b9-probe-g4e-triple.py` 實測三份同錯時 `g4e_pass=True`，擋不住「同一次錯誤解讀寫進三份」。第三份改為 `_event_keys()` 內**人手逐筆填入**之字面 `expected_side` 欄，且**不得** import／呼叫投影、`_oracle_membership` 或兩者之共用 helper。
...
**mutation 目錄**（🔴 逐處對應，不得以單一 generic mutant 冒充；每列皆須有**完整 ID** 與**應紅之測試**；共 **34** 條，🔴 本數字須與表列實數相符——前一版寫 23 而實列 22，由 R5 兩家以計數抓出；🔴 **一 mutation 一 defect**：R6 指出 `M-SU-D2-23` 曾以 OR 合併兩個獨立缺陷，已拆為 `23`／`26` 各配自己的應紅測試）
...
| `M-SU-D2-03` | 🔴 **v12 改寫（R11 grok：M2 已把 §V 升級為值相等，本列仍只寫「鍵斷言」⇒ 鍵在而值被手塞空 dict 時不紅）**——下列任一即缺陷：summary 帶了但**不傳入** `metadata.split_unify`；鍵在而**值不等於** summary／producer 回傳；或改動了 `reason` 封閉值集 | `Task 9.1` 之 `metadata.split_unify` **值相等**斷言 ＋ `tests/api/test_splitunify_disclosure.py` 之 exact-key 斷言 |
...
| `M-SU-D2-29` | 🔴 **v12 改寫（R11 grok：v11 把 (G-4d)① 改成檔案制後，本列仍寫舊的「v8 鍵」字面，mutation 對新落點不可驗）**——`freeze_splitunify_golden.py` 之下列任一行為即缺陷：刪除或覆寫 `splitunify_golden.v8.json`；改其內容而不同步 `splitunify_golden.v8.sha256`；`--write` 寫入 `.v8.json` 而未 `raise`；或主檔既有 11 個頂層鍵被換錨後之值覆蓋 | §V 第 6 條之三條 ASSERT（`sha256` 相等、`--write` 對 `.v8.json` raise、既有 11 鍵逐值不變） |
| `M-SU-D2-30` | 🔴 **v13 改壞面同步（O3：原寫「derive 路徑略過 `validate_split_pair_integrity`」與 `Task 9.2b` 改法①互斥，字面保留供追溯）**——改為：**具名 producer 層**（`Task 9.2b` 改法①所指者）略過 `validate_split_pair_integrity`，或 train 段為空時 `continue` 而不 raise | `Task 9.2b` 前置之 pair 完整性斷言（v13 改寫後之「進入 derive 之前已呼叫」那條） |
| `M-SU-D2-31` | `baseline` 只輸出單一 `n_test` 而宣稱樣本數＝事件數 | `Task 9.4` 之物化失敗 fixture（test 含 `e1` 且物化失敗含 `e1` ⇒ 兩量不等） |
| `M-SU-D2-32` | `baseline` **保留舊鍵** `n_test`（與 `n_test_events`／`n_test_samples` 並存，或以舊鍵當 alias） | `Task 9.4` 之 `ASSERT baseline 回傳 dict 不含舊鍵 n_test`（§V v11 補之 exact-key 斷言） |
| `M-SU-D2-33` | 🔴 v12 新增：`splitunify_golden.v8.json` 以「比對後更新」而非 `O_EXCL` write-once 建立，或期望 digest 只錨在旁檔 ⇒ 同步改寫兩檔即可換掉回歸錨 | §V 第 6 條之 write-once 與**外部錨**兩條 ASSERT（只比旁檔者會綠，故必須靠外部錨那條） |
