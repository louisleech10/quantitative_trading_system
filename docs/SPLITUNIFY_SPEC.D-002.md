...
- **API 與前端揭露落點（逐處指名，缺任一層即視為 9A 未完成）**：
  ...
  - 🔴 **二擇一已定案＝採 (b)**：終端揭露自事件掃描端**移出**，改掛在**確實走投影**的消費者——即 IC 主線（`ic_filter_orchestrator` 為 `holdout_boundary` 之生產呼叫點）。選 (a)（改該 route 去取得 universe）會**推翻本票自己的既有裁定**，故不採。實作須指名：IC 投影路徑之回應欄位、其前端顯示位置，並附 **route 級契約測試**（明列鍵名，不得依賴型別）。**指名落點不等於該落點走得到**——此為第四次修訂之缺口，R6 三家獨立指出。
...
- 🔴 **側別判準（三段式，順序不得調換；R6 三家獨立撞題後定案）**：`decision_at_ms` 與 `feature_index` 未必落在同一網格——當 `feature_index` 之網格**粗於**觸發 TF 時（例：觸發 1h、特徵 4h），`decision_at_ms` 大量不在 `index_ms` 集合中（R5 實證 1h open 非 4h open 者 15,264／20,352）；但**單純改用不等式會把隔離帶收成 train**（R6 實證：gap 上集合成員給 `purged`、不等式給 `train`）。定案如下：
  1. **先驗位置落區**：`decision_at_ms` 映射之位置若落在**隔離帶**半開區間 `[split_point, split_point + purge_gap + embargo)` ⇒ **fail-closed raise**（訊息含 `event_id`）。該區間即 `[split_point, split_point+purge_gap+embargo)`，其定義逐字來自 `split_preview.holdout_test_row_index:41-43`——`split_point = floor((1 - oos_test_size) * n)`、`start = split_point + purge_gap + embargo`、`test = arange(start, n)`，而 train = `arange(0, split_point)`。🔴 **不得**把隔離帶收成 train，亦**不得**靜默 purge——隔離帶事件的正確處置是**明確報錯**。
  2. 位置落在 train 或 test 覆蓋區後，**才**以不等式定側（`decision_at_ms < test_start_ms` ⇒ train，否則 test）。
  3. 位置落在 `train_plan`／`test_plan` 兩段覆蓋區**之外**（非隔離帶，例如早於 `index_ms[0]` 或晚於末列）⇒ 同樣 **fail-closed**。
...
- 🔴 **同批必須補該旗標之終端揭露，否則修了也沒人看得到**：`insufficient_events_in_test` **不擋任何分析**，只被 `tables.py:162` 原樣放進報告，且**前端完全未顯示**（主委實跑 grep 確認）。⇒ 只改計數而不補揭露，等於把「樣本不足」這個唯一告知管道繼續留在無人看得見的地方。本條與 `Task 9.1` 之終端揭露同源，須一併處置。
- 🔴 **`baseline` 為明列例外**：`baseline` 之 `n_test` 語意為**實際模型輸入樣本數**（`baseline.py:105-120` 以 test 事件 ID 與 features 交集後寫入），複合鍵落地後即 `(event_id, feature_timeframe)` 列數，**不得**被本 Task 一併改判為事件數。驗收須配「一事件兩列」之 fixture，斷言 `baseline` 之 `n_test` 為 2 而報告分母之 `n_test` 為 1。
