...
**(5.3) 第二層｜表格鏈（D-001 原列六處）**：🔴 **本項為現況描述；每處之處置一律以 `(5.1)` 三分類為準**（v7 以前此處寫的「靜默／須改」是**改判前**之判定，已作廢）。`feature_materialization` 之 `merge validate="many_to_one"`（複合鍵後仍成立、**不需改**）與 `groupby("event_id")+row_vals.update`（**設計上的橫向合併**，(5.1) 甲類維持）與輸出 `set_index("event_id")`（同屬甲類，只需斷言多列輸入不靜默覆蓋）；`baseline`（繼承上游事件級索引，甲類）；`pattern_bridge` 之 `set_index("event_id")["split_label"]`（複合鍵後取到 Series ⇒ **(5.1) 丙類**，去重取唯一側）；`tables` 之 `set_index`（🔴 v12 更正：主委實查 `tables.py` 共 **4 處**——`:214` `receipts.event_level`、`:229` 與 `:373` `clusters`、`:372` `assignments`；前三處為甲類，`:372` 屬**丙類真缺陷**，已另立 `C5-29`；v11 以前寫「兩處」為漏數）；`ic_feed` 兩處 `set_index` ＋ `.loc[keep["event_id"]]`（先過濾單一 TF，索引本就唯一，甲類）；`dedupe` 之 `merge validate="one_to_one"`（對事件級 `events`，仍成立）與 `cluster_first` 保留集（甲類，保留之 `event_id` 廣播到 per-TF 列）。
...
**(5.6) 消費面 register（🔴 v11 新增；唯一計數依據與唯一施工清單）**：R10 codex 以機械計數指出前版標題宣稱「16 處」而四層 named entries 可計為 20，兩個數字都在文件裡、實作者依任一邊都會漏改漏測。⇒ **自本版起，(5.1)–(5.5) 為敘述，下表為唯一 register**：條數以本表列數為準（v11 為 25 條；🔴 **v12 補記帳鏈四條後為 29 條**），(5.1) 之三分類逐條落在「分類」欄，每條**必須**有 Task 欄與 mutation 欄；mutation 欄為 `—` 者表示該條之誤改**不會**被任何 mutation 抓到，屬**具名缺口**、不得當作已覆蓋。表列於本義務區塊**之後**（標題「D-002-C5 register 表」），因義務區塊內只允許編號義務項行型。
...
| `C5-13` | `tables` 之 `set_index` ＋ `.loc[eid]` **三處**：`tables.py:214`（`receipts.event_level`）／`:229`／`:373`（`clusters`）〔🔴 v13 更正：原寫「兩處」，實為四處中的甲類三處；`:372` 之 `assignments` 屬丙類、見 `C5-29`〕 | 甲 | 維持事件級＋加防誤改回歸 | `Task 9.3` | `M-SU-D2-06` |
| `C5-14` | `baseline` 之事件級索引（`baseline.py:105-121`） | 甲 | 隨上游同步；另依 `(6.2)` v11 拆 `n_test_events`／`n_test_samples` 並刪舊鍵 | `Task 9.4` | `M-SU-D2-31`／`M-SU-D2-32` |
...
- **大小**：大（命中 (a)(b)(d)）。
- **命中高風險原則**：(a) 多 TF 記帳影響 IC 樣本數與統計量；(b) 觸及 `D-002-C5` (5.6) register 之 29 條共用消費面，含前端與 golden（🔴 v11：原寫「16 處」，依 register 修正；v12：25 → 29，補記帳鏈四條）；(d) 錯誤折疊或異側放行會直接改變 ML／回測輸入之樣本構成。
- **RISK-HIT: a,b,d**
...
- `D-002-C5` (5.6) register 所列之消費面中（🔴 **v13：此處原重複寫死條數（v11 寫「16 處」、v12 漏改為「25 條」），依「同一個數字不得寫在兩個地方」改為指向 register，不再重複；條數只存在於 register 表標題**），標記**靜默**者驗收不得只看「有沒有報錯」，須斷言取到的**值**正確。
...
- **通過條件**（🔴 v10 更正；原「任一單 TF 舊值位移即 FAIL」與 (G-4a)／(G-4c) 之允許重凍互斥）：單 TF 路徑逐值不變（exact），**惟 (G-4d) 所允許之差異集合內的位移除外**；多 TF 路徑以新增之平行組比對。**該集合以外**之任一單 TF 舊值位移即 FAIL。
...
- 目標（🔴 v10 更正，舊敘述見沿革）：在複合鍵落地**之前**，先消除「靜默丟棄」之誠實性缺陷，**本延伸交付至 producer 層**（producer 回傳 → `EventSplitPlan.summary` → `metadata.split_unify` 三層完整記帳）；**終端可見性**（API 回應欄位與前端顯示）見 §N 之 `SU-RESID-9A-UI`，**不在本延伸完成條件內**。
...
- 🔴 `Task 9.4`（v11 補；原缺母斷言而 `M-SU-D2-31` 已指向它）：`ASSERT WHEN test 含 e1 且物化 failures 含 e1 THEN n_test_events=1 AND n_test_samples=0`（物化失敗 fixture；證明兩量**不**恆等；現況碼證落點 `baseline.py:120` 之 `len(idx)`）；`ASSERT baseline 回傳 dict 不含舊鍵 n_test`（舊鍵不得殘留）；`ASSERT per_symbol_n 與 tier_min 路徑之計數皆以 event_id 去重`。
...
單 TF 舊值保留為回歸錨；Phase 9A 可獨立回退（移除三層揭露即可，由 §V 之可證偽斷言保證）。Phase 9B 回退需連同 `D-002-C5` (5.6) register 之 **29 條**消費面一併還原（🔴 v11：原寫「16 處」與 register 不符，依 R10 codex 修正；v12：25 → 29，依 R11 三家補記帳鏈四條），故 9B 須在單一批次內完成，**不得部分上線**。
