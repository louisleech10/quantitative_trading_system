# ROADMAP — P1-6／B5 敘事歷史（2026-08-05 自 ROADMAP 移出）

> **為何移出**：使用者 2026-08-05 指出「ROADMAP 這類文件要更新到最新狀態，過期／舊的／被推翻的要移除」。
> 本節原佔 ROADMAP 393 行中的 322 行（82%），已成流水帳而非狀態文件，
> 並**實際造成誤讀**——使用者一度以為「線 C 已完成」而據以往後推進。
> **內容逐字保留於此供追溯；ROADMAP 原處改為當前狀態精簡塊。**
> 原始行號範圍：18-339。
...
- **★ 狀態（2026-07-28 更新）＝P1-6 委員未結案債狀態機：SPEC v2.8 定版、待實作**。目標＝派委員即自動開債、債未清擋所有新派工、跑機械合併驗 0 掉項才銷帳（根因＝主委手動合併委員意見**必掉項**，歷史事故漏 grok T1-01 害整份 SPEC 作廢）。
  ...
  **🔴 上述效能立論已於 2026-08-02 被實測推翻**：`debt_ledger --has-open` 對 30,960 行實跑 **46–64ms**
  ...
  ⇒ **線 C 不得以效能立論**。
  🔴 **2026-08-05 使用者更正本行原本的措辭**：原寫「只買到衛生與成長率，不買效能也不買正確性，屬 cleanup 非 fix」
  ——**這句把線 C 講小了，且與草案 §2 不符**。草案 `handoffs/20260802-LINEC-AUDIT-SPLIT-SPEC-DRAFT.md:4,24`
  明寫真問題是**資料壽命混裝**：債務事件因序號連續性 fail-closed 而**永遠不可刪**（289 筆），
  其餘 30,671 行本可自由輪替卻被綁在同一檔。**目的是「能讀到乾淨的帳本」，避免讀錯又重來。**
  **實證（2026-08-05）**：`audit.log` 34,000 行；`debt_ledger --list` 吐 **182 個 round**（ABANDONED 146＝80%、CLOSED 36）。
  主委當晚**實際讀錯一次**——以 session 名 grep 撈 round id 撈到已 ABANDONED 的舊筆，
  得 `ERROR: round 已 ABANDONED（不可逆）` 後改 `grep OPEN` 重來；且每次銷帳都得從 182 行裡 `sed` 撈唯一 OPEN。
  ⇒ **線 C 是可讀性/正確性問題，不是 cleanup**。仍待使用者裁定何時做。
