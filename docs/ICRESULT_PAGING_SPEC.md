# ICRESULT_PAGING — IC 結果分頁與按需載入 — SPEC（R6 修訂）

> 來源 PLAN/診斷：UAT 2026-09-09（使用者：「網頁顯示 30K 多的特徵，整個網頁很慢，幾乎不會動」）　|　日期：2026-09-09（R1 修訂）　|　對應 TODO：`docs/ICRESULT_PAGING_TODO.md`
> R1 修訂來源：`handoffs/reconcile/20260909-icresultpaging-x-review-r1/synth.md`（Z1–Z7）。R2 修訂來源：`handoffs/reconcile/20260909-icresultpaging-x-review-r2/synth.md`（Y1–Y6）。R3 修訂來源：`handoffs/reconcile/20260909-icresultpaging-x-review-r3/synth.md`（X1–X6）。R4 修訂來源：`handoffs/reconcile/20260909-icresultpaging-x-review-r4/synth.md`（W1–W6）。R5 修訂來源：`handoffs/reconcile/20260909-icresultpaging-x-review-r5/synth.md`（單一 normalized 樹、守衛 raise 之寫點語意、LATENCY gate 消費、snapshot 不可變、debounce owner、快取 accounting、Task 1.1／1.3 殘留「lock 內取 result」「deny 先跑」、`ICReportLight` Omit 對齊）。
...
- **不變式（本票紅線）**：
  ...
  6. **light 投影規則為封閉集合，全部住 contract JSON**（R1 P0）：(i) `drop_sections`（七段：`summary_table`／`ic_decay`／`quantile_returns`／`turnover_analysis`／`coverage_analysis`／`rolling_ic_series`／`grouped_ic`——**六個 per-feature 段（含 `rolling_ic_series`）全部由 Task 1.2 單特徵投影供圖**，Phase 2 六圖（decay／quantile／turnover／rolling／grouped／regime）皆改吃 `featureDetail`；R1 `GROK-R1-P2-02`、R4 `CODEX-R4-P1-02`）；(ii) `metadata` 只保留 `metadata_keep_keys`（**顯式列舉**＝前端＋後端消費者聯集，見 §A receipt；**不得**由 fixture 推導；R1 `CODEX-R1-P1-02`／`GROK-R1-P1-01`）；(iii) `collection_to_count_paths`＝顯式路徑規格清單，每條 `{path: ["filter_log", "*"]}` 或 `{path: ["metadata", "selection_scope"]}`：`*` **只匹配該層的直接子鍵**（一層，不遞迴），目標節點須為 dict，對其**直接子鍵**之 list／dict 值改為 `<key>_count`（int），標量原樣，更深層不動；目標缺席或非 dict ⇒ no-op（不拋、不補）；未知 stage 名同規則（R2 `CODEX-R2-P2-06`）；(iv) `funnel_stage_adapter`（R2 `CODEX-R2-P1-04`／`GROK-R2-P2-01`）：對 `filter_log` 每個 stage 依 contract 之「input 候選鍵序」`[input_features, feature_count_original]` 與「output 候選鍵序」`[output_features, feature_count_filtered]` 取第一個存在者：int 原樣；**dict 含 `count` 鍵（int）⇒ 取 `count`**（實機 `stage5_thresholds.output_features={"count":0,"pass_class":"oos"}` ⇒ 0，非 `len`=2；R3 `CODEX-R3-P1-03`）；其他 dict／list ⇒ `len`；兩者皆缺 ⇒ `null`（`stage3_event_filter` ⇒ `{null,null}`，不由前一 stage 推導；R3 `GROK-R3-P2-01`）。**funnel 必須對計數前的原始 `filter_log` 計算**（先算 `filter_log_funnel`，再對 light 的 `filter_log` 做 (iii) 計數；R3 `GROK-R3-P1-01`）；light 另寫 `filter_log_funnel: {stage: {input: int|null, output: int|null}}`，`FilterFunnelChart` 改讀此鍵（`null` ⇒ 顯示不適用，不補 0）；(v) 加 `view`、`total_features`、`result_revision`、`summary_page`。
  7. **結果世代戳**（R1 `CODEX-R1-P1-03`／`COMPOSER-R1-P1-02`／`GROK-R1-P2-01`）：service 對 `task_info["result"]` 每次寫入（完成／refilter／其他寫點）經單一 helper 遞增 `task_info["result_revision"]`（int，從 1 起）；light／summary／feature 回應皆帶 `result_revision`；summary／feature 請求可帶 `revision`，不符 ⇒ 409（回 `{current_revision}`）。**投影一律對 lock 內取出的不可變 snapshot `(normalized_report, revision)` 進行**，回應之 `result_revision` 為該 snapshot 的值（投影中途 refilter 不影響已取 snapshot；R2 `CODEX-R2-P1-02`）。**寫入時一次正規化＋守衛**（R4 `CODEX-R4-P1-01`：實測 39k 報告每請求 `_to_json_compatible` 2586 ms＋`deny_factor_in_ok_oos` 1461 ms，逐請求做則 §C-9 不可能達）：`_set_result` 於寫入當下對新報告做 `_to_json_compatible` 與 `deny_factor_in_ok_oos`（守衛不可省；違反 ⇒ raise，**不寫入**），並**只保留一棵樹**：`task_info["result"]` 即該 normalized 物件（不另存第二棵；R5 `CODEX-R5-P1-01`，39k 報告一棵樹已 >119 MB）；export（`:1896`）、apply-transforms（`:2532`）、status 等既有讀者照讀 `task_info["result"]`（同一物件）。預設 `/result` 讀取路徑**程式碼不變**（仍呼叫 `_to_json_compatible`＋守衛；對已 normalized 樹為冪等，測試 `_to_json_compatible(n) == n` 且 G-1 raw bytes 相等）；light／summary／feature 只讀 snapshot，**不得**再呼叫該兩函式。**寫點失敗語意**（R5 `CODEX-R5-P1-02`）：(a) 初次完成時 raise ⇒ task `failed`、無 result、`result_revision=None`（與現行背景 try `:1676-1685` 一致）；(b) refilter 時 raise ⇒ **先驗後寫**：舊 result／revision 不變、task 仍 `completed`、`POST /refilter` 回 422（現行「先寫壞 report 再讀時 500」為缺陷，本票修正）；兩者各一條 TestClient 斷言。**snapshot 不可變**（R5 `CODEX-R5-P1-04`）：投影函式禁 `pop`／就地改寫，一律建新 dict；測試：同一 task light 兩次、再 summary、再 feature，snapshot 整棵 deep-equal 於投影前 `copy.deepcopy`（含 `filter_log`／`selection_scope` 原始集合鍵；R6 `GROK-R6-P2-01`）。**refilter handshake**：`POST /refilter?task_id=&view=light` 回 light 視圖（含新 `result_revision`）；無 `view` ⇒ 既有全量回應**不變**。前端：refilter 成功 ⇒ 以回應 `result_revision` 更新 store、abort 所有進行中 summary／feature 請求；任何 summary／feature 回應之 `result_revision != store.resultRevision` ⇒ **丟棄**不套用。
...
- **效能與體驗預算（使用者 2026-09-09 裁定：「不只要可用，還要效率，包括載入或讀取時間，還有使用者體驗」）**：
  9. **後端延遲預算**（對 39,346 特徵實機報告、本機、暖機後單請求 p50／p95）：`view=light` ≤ 150／300 ms；`summary?limit=50`（任意 `sort_by`／`search`）≤ 100／200 ms；`feature/{name}` ≤ 50／100 ms。`paginate_summary` 對 39k 列每次請求 O(n log n) 排序須快取：同一 `(result_revision, sort_by, sort_order, pass_class, search)` 之排序索引在 service 內以 LRU 快取，第二次請求 ≤ 20 ms；revision 變更即整 task 失效。**容量契約**（R4 `CODEX-R4-P1-04`、R5 `CODEX-R5-P2-02`）：索引存 `numpy.int32` 陣列；`cache_bytes()`＝Σ `arr.nbytes`（不計 dict／key 開銷，明文）；每 task ≤ 8 組，第 9 組淘汰該 task 內 LRU 一組；**module-level 單例**（process-wide）LRU 上限 32 task，第 33 個 task 淘汰最舊 task 整組；測試：33×8、單 task 9 組、兩個 service instance 共用同一快取。`_tasks` 既有無界登記 `IP-RESID-5`，**本票新增記憶體（單一 normalized 樹、快取上限）不以此豁免**。
  10. **前端體驗預算**（B34 實機）：首屏可互動 ≤ 3 s（light ≤ 262 KB＋首頁 50 列）；翻頁／排序 ≤ 1 s 且顯示 skeleton 列（不閃白、不清空舊列直到新列到達）；點特徵 ⇒ 圖表 ≤ 2 s，切換期間保留舊圖並加 loading 遮罩（**禁止**切換時先清空 `featureDetail`；vitest 鎖「切換中舊 detail 仍在 DOM＋overlay」；R4 `GROK-R4-P2-01`／`COMPOSER-R4-P2-02`）；搜尋 300 ms 去抖（**owner＝表格搜尋框**，hook 不再去抖，fake timer 測試只允許一次延遲一次 API；R5 `CODEX-R5-P2-01`）；翻頁保留勾選（Set 跨頁）；頁碼／每頁筆數／排序狀態寫進 URL query（重整不丟）；請求失敗顯示可重試按鈕而非空白。
...
- **通過條件（可證偽）**：
  ...
  - G-9 延遲（受控 artifact，`--latency` 模式；R4 `CODEX-R4-P1-03`）：**量測規則**——TestClient、同一 process；每項先 warm-up 3 次不計，再取 20 次；p50＝第 10 小、p95＝nearest-rank `ceil(0.95×20)`＝第 19 小；請求集固定：`light`；`summary` 五組（`sort_by=icir&sort_order=desc`、`sort_by=feature_name&sort_order=asc`、`search=close`、`pass_class=oos`、快取命中＝第二次 `icir desc`）；`feature` 兩組（summary 首列特徵、`turnover_analysis` 序列化最大之特徵）。**stdout 唯一文法**：恰一行 `LATENCY_GATE=PASS|FAIL|BLOCKED`＋逐項 `LATENCY_<name>_p50_ms=`／`_p95_ms=`；rc 0／1／2；phase gate 解析規則與 `SIZE_GATE` 相同（FAIL ⇒ rc=1、BLOCKED 轉印不改 rc、缺行／多行／未知 ⇒ rc=1）；**消費點**（R5 `CODEX-R5-P1-03`）：`scripts/icresult_paging_phase_gate.sh 1` 跑探針 `--size`，`scripts/icresult_paging_phase_gate.sh 3` 跑 `--size --latency` 並要求兩者 `PASS`；parser 負向測試 `tests/api/test_icresult_paging.py::test_phase_gate_parser`（subprocess 餵缺行／多行／未知值 ⇒ rc=1）。receipt `handoffs/run_receipts/icresult_latency.log`。
...
- 目標：`task_info["result"]` 每個寫點經單一 helper 遞增 `result_revision`，並於寫入時一次 `_to_json_compatible`＋`deny_factor_in_ok_oos` 後**只保留**該 normalized 樹於 `task_info["result"]`（§C-7，單一樹）；`/task/{id}` 回 `result_revision`。　檔案：`api/services/ic_analysis_service.py::_set_result(task_info, report) -> int`（三寫點 `:1623`／`:2339`／`:2631` 改經 helper）；`api/models/ic_models.py::ICTaskStatusResponse.result_revision: Optional[int]`。　既有 caller：三寫點。
...
- **邊界**：①未完成 ⇒ `result_revision=None`；②refilter 失敗不遞增；③兩次 refilter ⇒ 3；④初次完成守衛 raise ⇒ 不寫入、task `failed`、無 result（§C-7(a)）；⑤refilter 守衛 raise ⇒ 先驗後寫、舊 result／revision 不變、task 仍 `completed`、`POST /refilter` 回 422（§C-7(b)；R6 `GROK-R6-P1-01`）。
- **存活至**：永久。　**覆蓋風險**：無。
...
- 改法：守衛已於 `_set_result` 完成，投影**不**再呼叫（R5 `GROK-R5-P1-02`）；`{"raw":…}` ⇒ 400；`view=light`＋`schema_version=2` ⇒ 400；投影純函式只讀 contract；snapshot `(report, revision)` 於 lock 內取；`filter_log_funnel` 依 (iv)；`POST /refilter` 加 `view` 參數（`light` ⇒ 回 light；無 ⇒ 不變）。
...
- 目標：per-feature 六圖（`ICDecayChart`／`QuantileReturnChart`／`TurnoverTimeSeriesChart`／`RollingICChart`←`featureDetail.rolling_ic_series`／`GroupedICBarChart`／`RegimeRadarChart`）由 `page.tsx` 餵 `featureDetail`（`sectionSplit` 與 `ICReportLight` 型別明列六段含 `rolling_ic_series`）；其餘讀 `report` 的元件 props 型別放寬為 `ICReportLight | ICReport`；`ExportButtons` **維持** `/export/{id}/{format}`，只把 `summaryTable` prop（PNG disable）改 `summaryPage.total > 0`。　檔案：`page.tsx`、各元件。　既有 caller：`page.tsx`。
- **驗證**：`ASSERT (cd frontend && npx vitest run src/components/ic-analysis src/app) THEN rc=0`（既有斷言不改；新增 page 整合測試（fixture＝**不含 `summary_table`** 之 light 報告＋`summary_page{total:39346, rows:50}`）：①表格 `<tr>` == 51 且不讀 `report.summary_table`（fixture 無此鍵仍渲染 50 列）；②點排序 ⇒ `fetchSummaryPage` 收 `{sort_by, sort_order, offset:0}`；③點列 ⇒ `fetchFeatureDetail(name)` 被呼叫；④初始 `featureDetail=null` ⇒ 六圖區「載入中」不 throw；⑤`featureDetail`＝golden 單特徵（含非空 `rolling_ic_series`）⇒ 六圖渲染且 `RollingICChart` 收到該特徵序列；⑥`filter_log_funnel` 含 `null` ⇒ 漏斗該 stage 不適用；⑦切換特徵中（新 detail 未回）⇒ 舊 detail 仍在 DOM＋overlay 出現、**未**被清空；⑧mock `fetchFeatureDetail` reject ⇒ 出現重試按鈕，點擊再呼叫一次；R3 `CODEX-R3-P1-05`、R4 `CODEX-R4-P1-02`／`GROK-R4-P2-01`／`COMPOSER-R4-P2-02`）；`npx tsc --noEmit -p tsconfig.json` `error TS` == 8；後端 `ASSERT venv/bin/python -m pytest tests/api -k export THEN rc=0`（匯出既有測試不改）。
...
- 殘留：`IP-RESID-5` `ICAnalysisService._tasks` 無界、無完成 task 淘汰／TTL（既有）— `為何現在不做: blocked-by:task 生命週期屬 service 既有設計，涉 WS／輪詢／deep-analysis 消費者，須另票`；觸發：多 run 後 RSS 持續增長之實機觀測。**射程只及既有 lifecycle**：本票新增記憶體（normalized 單一樹、排序快取）各有自己的上限與測試（§C-7／§C-9），不得引本條豁免。
