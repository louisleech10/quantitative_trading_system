# ICRESULT_PAGING — IC 結果分頁與按需載入 — SPEC（R5 修訂）
...
- **邊界**：①未完成 ⇒ `result_revision=None`；②refilter 失敗不遞增；③兩次 refilter ⇒ 3；④守衛 raise ⇒ 不寫入、revision 不變、task 標 failed（與現行讀取時 raise 語意等價）。
- **存活至**：永久。　**覆蓋風險**：無。
