...
| 320 | RM-FFSTAT | FF-STAT：平穩化處理之判定不得依欄名 | 進行中 | docs/FFSTAT_SPEC.md | 起因＝使用者 2026-09-24 第 4 點「看名稱決定是否要平穩化，在量化數據好像是很嚴重的問題，需要盡早研究處理」；研究輪（handoffs/reconcile/20260924-ffnamestat-x-consult-r1/synth.md）裁定先於 FF-NAME。主委 ADF 探針：名字白名單免檢之比值欄 60 抽 1 不平穩、多組件比值最大值 12h 2021／1h 210288。SPEC v1→v5 經 r1–r5 收斂（兩家 proceed，commit `ae8b3cba`）：逐欄結構化身分（全部層含 raw，產出端給出、缺即 fail-closed）、d* 讀／搜尋／寫三出口（搜尋失敗保原值並降級，不再靜默用 d=1）、刪 `layer1_only` 與自訂子字串設定、平穩化步驟 `apply_to` 只收兩值。使用者白話審閱時追問抽樣問題與前 500 根校準 ⇒ 研究 r1（handoffs/reconcile/20260924-ffstatres-x-consult-r1/synth.md）：免檢表三方一致刪除；🔴 查出校準值未綁任何無洩漏界線（warmup 裁切預設關閉，前 500 根即輸出範圍內）＝未來洩漏必修；1h 實測 n=2000 一致率 ≥97.8%（收據 handoffs/run_receipts/20260924-ffstat-calib-window.json）。研究 r2 定乙案（只用起始日前資料、獨立校準域；handoffs/reconcile/20260924-ffstatres-x-consult-r2/synth.md）；使用者 2026-09-24 裁定窗長「先實測再定」（可調、預設 500，Task 4.1 實測 500／1000／2000 後請使用者定）、未填起始日「自動預留校準段」；SPEC v6→v17 經 r6–r17 收斂後使用者「同意施工」，v18 為寫 TODO 時兩處事實更正（append 模式基礎欄不被改寫；真實 kline 無缺口）。TODO＝docs/manifests/FFSTAT.json（121 支測試；§G 基準凍結收據 handoffs/run_receipts/20260924-ffstat-golden-freeze.json）；TODO 審查 r18–r24 收斂、兩家戳記（handoffs/reconcile/20260924-ffstat-x-review-r24/synth.md）；r19 另查出既有退化（見 RM-ICFIRSTALIGN），本票 IC-first 驗收改於 L6.5 產物觀測。實作：b1（目標層只讀結構化來源、刪名字免檢、apply_to 封閉、逐欄決策紀錄；`02350721`＋審碼修補 `3bd29a22`，兩家收批）、b2（d* 讀／搜尋／寫三出口 fail-closed、事件併入品質降級；`6ad69bcf`＋`c7f78926`，收批）、b3a（三路同一 N、校準純函式；`413836ff`＋審碼修補 `f3c375b4`、`cbb057f6`，收批）、b3b（校準資料域與前置關卡四路接線；SPEC v19 前史不足改逐欄〔使用者 2026-09-26 裁定〕；`c76a9ecc`＋審碼 r3 修補 `f29355c2`〔IC-first 殘留 registry、校準域 L5 無重疊保留欄、L0／warmup／校準共用 UTC 換算、SPEC v20〕＋r4 `438faa8f`〔L5 參考快取鍵含載入時間窗〕＋r5 `695bbe13`〔多標的 IPC 暫存清理〕，r6 codex proceed 收批）。開 fracdiff 時實測 12h／600 根改前 336 秒、改後 431 秒；平穩化關閉（生產預設）無差異。config schema 變更已兩度以有證據遷移更新 FF-TFMETA golden。b4 動工前重設計「未填起始日」：原 manifest 之窗（end_date＝2024-04-30）不可行（收據 handoffs/run_receipts/20260926-ffstat-autostart-depth.json）；諮詢 r1 與審查 r2–r8（handoffs/reconcile/20260926-ffstatauto-x-review-r8/synth.md 等）期間使用者 2026-09-26 裁定「主訓練週期優先、次週期能做多少就做」、主週期＝介面之主框架（config.timeframes.primary）⇒ SPEC v21→v30：保守深度＋保留最少輸出之上限、主週期驗證於 generate_features 入口；r9 codex 指出同一 factory 先後生成兩窗時 run_ic_first 只能讀後者 ⇒ v29 改為平穩化開啟時 run_ic_first 自帶起訖、自行解析輸出窗與 hash（刪除 v24–v28 之身分比對）；r10 codex 指出失敗後殘留本次窗與 hash ⇒ v30 入口保存、結束一律還原；r11 codex 指出 layer_results 亦殘留（FF-STAT 動工前既有）⇒ 開 run_ic_first 去留查證（handoffs/reconcile/20260926-icfirstneed-x-consult-r1/synth.md），使用者 2026-09-26 裁定不刪、另開 IC-First 管線合一票（ICFIRSTALIGN）排 FF-STAT 之後立刻做 ⇒ v31 明文排除 run_ic_first 其餘既有狀態依賴；r12 兩家 proceed（handoffs/reconcile/20260926-ffstatauto-x-review-r12/synth.md），SPEC 審查收斂。下一步：使用者白話逐條審閱（白話說明/FF-STAT未填起始日審閱.md）→ 白話逐條審閱 → b4 實作（併 r6 之 IPC 清理 try 範圍修補）→ b5（§G 對照、成本收據請使用者定 N 與 N 之定義）。 |
| 330 | RM-ICFIRSTALIGN | ICFIRSTALIGN：IC-First 管線合一（頁面第三步改用正式實作並修其未來洩漏、run_ic_first 去除 factory 可變狀態依賴、修真實資料 IC 對齊） | 未開工 | docs/manifests/FFSTAT.json | 2026-09-24 FF-STAT TODO 審查 r19 追查時發現（主委實跑）：真實 kline 下 `run_ic_first` 於 IC 階段必拋 `AlignmentViolationError: label/group index mismatch with equal length`——label 為時間戳 index、自 L7 raw 讀回之群組為 RangeIndex；`tests/feature_engineering/test_b6_warmup_trim.py::test_warmup_trim_ic_first` 於 main 同紅；拒絕按位置對齊之防呆由 commit `78c85bb2`（IC 1-align B3）引入，防呆本身正確，缺的是讀回時接回時間軸。影響：IC-first 路徑對真實資料目前無法產出選特徵結果。另（FF-STAT 審查 r9 codex，handoffs/reconcile/20260926-ffstatauto-x-review-r9/synth.md）：平穩化關閉時 `run_ic_first` 無起訖參數、沿用同一 factory 最近一次生成之輸出窗，先後生成 A、B 窗只能讀 B（FF-STAT 動工前即如此；FF-STAT v29 只於平穩化開啟時改為自帶起訖）⇒ 本票一併定其選窗介面。去留查證（handoffs/reconcile/20260926-icfirstneed-x-consult-r1/synth.md，主委＋codex＋composer）：`run_ic_first` 源自 `db19af76` L6.5 V2「IC-First 兩段式管線」，無任何生產呼叫者，但為特徵工廠正式 transform_selected 與 write_processed 之唯一呼叫者；頁面第三步（IC 頁 apply-transforms，`api/services/ic_analysis_service.py:2877-2903`）為 API 內手寫簡化版，未勾 rank 時 Gaussian 以全樣本排名＝**現行可觸發之未來洩漏**；IC 對齊錯誤於 raw 讀回路徑同樣觸發、不因刪除而消失。🔴 使用者 2026-09-26 裁定：**不刪**；本票改為「IC-First 管線合一」——頁面第三步改用正式實作並淘汰手寫版與其洩漏、以不可變 run context 取代 factory 可變狀態（含 FF-STAT r11 所指 layer_results 等）、修對齊；排程＝**FF-STAT 之後立刻做（先於 FF-NAME、FFDSTAR）**。規模＝大（ML 正確性 d、跨模組 b）。 |
