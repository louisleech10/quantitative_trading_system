...
| 300 | RM-TODOFMT | TODOFMT：散文 TODO 廢止，改五類可執行落點＋對應範本與閘門 | 進行中 | docs/TODOFMT_SPEC.md | 起因＝RM-PROCOPT 之診斷（實作缺陷 40% 為「規格寫對但沒照做」、散文 TODO 為測試檔之低保真草稿）。consult r1 三家定方向（五類落點：生產 stub／tests+scripts／契約 JSON／機讀批次卡／實跑收據）。SPEC 審 11 輪後設計定案（L＝`f2146e3d`），下一步實作 Phase 0–3。🔴 硬約束：不得引入分鐘級以上之檢查、不得動 pre-push、mutation 靜態擴覆蓋維持 opt-in。其餘票依使用者裁定暫停至本票完工。 |
