...
- **狀態(2026-06-26)**:
  ...
  - **Phase 1 正確性 kernel + contract 🔵 進行中**:
    ...
    - **🔴 IC-ANALYSIS-LOOKAHEAD-REMEDIATION epic 開立(2026-07-15,四方全面盤點+使用者裁定深修)**:1c-FR-FULL 稽核揪出上游洩漏→使用者要求盤點全 IC Analysis→四方(Claude+codex+composer+grok)實跑證 look-ahead **結構性瀰漫**。**核心洞察=之前只建「OOS切分+stage1 train-fit」防線,從未審分析模組『內部計算』,切分給假安心感**。master=`handoffs/ICLOOKAHEAD-MASTER.md`。**P0(預設路徑必踩)**:①`ic_engine.py:290` rolling IC spearman **全序列 pre-rank**(污染 rolling IC/ICIR/門檻/trend/centrality;現成 `_rolling_spearman` 窗內版未用)②stage5 `monotonicity_tester`/`turnover_analyzer` 全窗 qcut→因子淘汰門檻(0.6)③`data_preprocessor` 特徵 winsorize/zscore full-sample fallback。**P1**:regime rule 全期 vol 分位/long_short/fallback silent/FR。**使用者裁定=先 P0 統一整治,FR 併入統一 PIT helper**。每 Phase 完整管線+三家 DATA-CORRECT(a,d)。**1c-FR-FULL SPEC v0.4 已凍結就緒**(`docs/IC1CFR_FULL_SPEC.md`,R1-R4 四輪 adversarial;canonical=P1 單標的擇時多空;PIT 分位+winsorize;codex+composer FREEZE-OK)→併入本 epic。相關 memory `project_ic_analysis_lookahead_remediation`+`project_1cfr_full_p1_canonical`。**1d attribution park**(偵察完成 handoffs/1d-RECON-*;含幽靈接線,待 1c-FR-FULL 後恢復)。
      - **✅ LA-0(P0)/LA-1(P1) 已合併 main**;**▶ LA-2(P2)治理凍結完成(2026-07-18)**:SPEC `docs/IC_LA2_SPEC.md` v0.5(R1-R5 三家 adversarial)+TODO `docs/IC_LA2_TODO.md` v0.4(R1-R4)**三家 RECONCILE-STAMP APPROVED+`reconcile_stamps_check.sh` 兩份 PASS**。scope=①winsorized label 禁用(DEC-1 使用者定)②model OOT-only 契約(綁 canonical SplitPlan+嚴格`<` off-by-one 閘+欄位級 eval_scope 28 path 表)③條件模組(pattern train-mask 晉升 server 權威/factor typed loud+proxy 因果化/regime `_fit_global` 硬移除=收官 LA-1 §N)。taxonomy 三分(C-1 causal-PIT/C-2 promotion-train-mask/C-3 diagnostic-loud)+軌2 model in-sample 樂觀。**下一步=實作 B0→{B1,B2,B3}→B4+雙家 review+三方 DATA-CORRECT**(branch `feat/ic-la2-p2-impl`)。
