...
- **🔵 治理 backlog — 排序 v3.1（2026-08-04 全量重排＋當日新增 3 張，唯一有效）＝當前施工序**：
  **37 張**（導出命令：`grep -c '^## B-' handoffs/20260801-GOV-AMEND-BACKLOG.md`）中
  ✅2（`B-7`／`B-10`）｜🗑3（`B-1`～`B-3`）｜🔗3（`B-18`→`B-13`、`B-25`→`GOV-XREF-SYNC`、`B-36`→`B-13`）｜**待辦 29**。
  最新：`B-37`（票的優先順序無數據依據；硬前置＝第 0 批 Phase 0，排 0.9 批）。
  ```
