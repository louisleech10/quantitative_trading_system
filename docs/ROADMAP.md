...
| 225 | RM-EVENTSCAN | EVENTSCAN 條件掃描→持有 N 根報酬 vs 隨機對照報酬（大票，單標的） | 待審 | 白話說明/EVENTSCAN方向與做法.md | 🔴 下一步＝**寫 SPEC**，不是實作。方向與做法已定案（偵察 r1 十一條＋量化方法 consult r2 十四條皆收斂銷帳；使用者九項裁定；方向書 v10 經逐章稽核），但那是 SPEC 之**前置**不是替代。大票完整管線：①寫 SPEC →②兩家對抗審→收斂→凍結 →③寫 TODO →④兩家對抗審→收斂→凍結 →⑤分批實作 →⑥每批審碼→收斂。權威＝白話說明/EVENTSCAN方向與做法.md（使用者要的數字、九項裁定、十步做法、八個缺口）。做法十步含：條件引擎吃 FF 欄且欄名打錯要報錯／欄位選擇器（該 run 欄名逾 18 萬）／掃描函式接 API／統一持有時鐘／連續成立只進場一次／報酬表補標準差與最好最差／隨機批參數由觸發批帶入＋抽樣四修／比較端點改比報酬＋Δ 之 bootstrap 區間／前端含硬性橫幅／事件版成本歸零點。單標的先做，多標的併 GAP-4 |
...
| 250 | RM-AGENTOPS | AgentOps 病因定義與外部解法調查（治理成效歸因） | 進行中 | docs/AGENTOPS_PROBLEM_DEFINITION.md | 使用者 2026-09-21 裁定：先把問題定義清楚，再查外部方案能否套用；治理票成效一律以「病有沒有少」量，不得以閘門綠燈充數。已落地＝病因目錄（掃 scripts/ 全部得 862 個真拒絕點、114 支腳本；掉項／遺失 214 為最大類，文檔類合計 308＝已分類 491 之 62.7%）＋audit 12,372 筆事件結構（gate_deny 2,630 筆全為 token_expired／open_debt，無一筆是內容被退回；表 B 那 862 個真拒絕點 audit 一筆未記）。歸因實查：掉項 406／708 輪 → 0／69 輪之唯一有效閘為 synth_attribution_hook，2026-09-11 由 VERDICTGATE B4 上線，**不歸屬 DOCROT（09-13 收票）或 DOCROT2（09-17 收票）**。下一步＝收齊 DOCFIX consult r1 三家外部方案調查後 reconcile，再由使用者裁定要不要動手（Claude 版結論＝外部無更好的矛盾偵測器，應改走結構性做法：fact_keys 擴到量化主線、裁定不可變＋失效窗） |
