...
| GAP-3 事件型分析（使用者重定義：外部標好正反例匯入→PIT 對齊→條件 IC／ML；非 event study） | **SPEC＋TODO 皆 FROZEN（2026-08-20 白話閘核准）**：SPEC 六輪對抗審 15→0＋三家戳記；TODO 兩輪 14→0（R7 12 群集寫回、R8 原提出方全 CLOSED）＋三家 RECONCILE-STAMP 蓋 r8 synth rc=0；殘留 G3-R1..R8 入 registry | **B1＋B2 CLOSED（2026-08-21）**：B1 七 Task（review 四輪 8→3→1→0）、B2 五 Task（review 三輪 11→4→0）皆三家 RECONCILE-STAMP rc=0；event_samples＋survivor 184 tests；§G-1 golden 凍結（`gap3_golden_pre.json`）接線前後 sha 不變；survivor contract v2（七鍵）；D-001 延伸檔生效。**下一步＝B3（新 session；HANDOFF 開工指令）** | registry #3＋SPEC／TODO（FROZEN）＋`docs/GAP3_EVENT_TODO.D-001.md`＋`handoffs/reconcile/20260821-gap3-b{1-review-r4,2-review-r3}/synth.md` |
