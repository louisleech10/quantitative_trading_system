...
> 維護:完成項移到「已完成」、新需求加到對應優先級、範圍/決策變更標日期。**每次 commit 一併更新本檔**(2026-06-26 使用者定)。最後更新 2026-07-11。
...
- **P2 — DECOUPLE-TRIAGE-2(follow-up,2026-07-14 立票)**:AST 掃描揭露舊 grep 盲區 5 筆 `momentum/Optimization` 縮排跨域 import(→`Strategy.performance_metrics`/`Analysis.strategy_registry`/`Analysis.pareto_analyzer`);現以 manifest **pending 表暫豁免**(owner=pending/DECOUPLE-TRIAGE-2,精準 symbol),須委員會 triage 真違規 vs 豁免並正式退場(豁免轉正或改 protocol/factory);另 `api/models/` 不在 R3 掃描根=已知缺口,擴根前須 triage 新紅字。未啟動;不擋 IC。
- **狀態**:主票完成(見上落地);殘餘=DECOUPLE-TRIAGE-2+P3 整理(route→service 下沉/hardware policy 收斂/`_registry` 穿透公開 API 化)。
