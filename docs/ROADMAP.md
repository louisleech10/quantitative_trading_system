...
- **狀態(2026-06-26)**:
  ...
  - **Phase 1 正確性 kernel + contract 🔵 進行中**:
    ...
    - **🔴 第二刀首項 bug（重凍時挖出，使用者裁排入第二刀）**：完整 IC 分析在 **12h 資料必崩**——物化 12h 特徵進切分驗證（`_validate_expected_frequency`/`contracts.py:555`）時 `features_df.index` 為位置整數非時間軸→連續性誤判 raise `rows purge requires continuous timestamps`。原始 12h kline 實測連續、1h golden 正常→12h 物化路徑掉時間戳（疑 `_materialize_features_for_ic`）。已 `test_analyze_real_run_with_config_hash_completes` xfail(strict) 追蹤。命中 (a)(d)。
