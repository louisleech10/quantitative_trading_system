...
- **狀態(2026-06-26)**:
  ...
  - **Phase 1 正確性 kernel + contract 🔵 進行中**:
    ...
    - **🔴 第二刀首項 bug（重凍時挖出，使用者裁排入第二刀）——全 tf 非 12h 專屬**：走 config_hash 叫完整分析時 `feature_library.load` 回的 index 為位置整數非時間軸→切分驗證誤判 raise。根因=時間軸有存（manifest `row_index`/`timestamps.parquet`）且 reader 有 `load_row_index_v2`，但 `feature_library.load` 從不呼叫、不 set_index→載入路徑不分 tf 一律掉時間軸；1h golden 走現成 fixture 故沒撞到。修法=load 貼回 row_index 一次全 tf 好。已 xfail(strict) 追蹤。命中 (a)(d)。
