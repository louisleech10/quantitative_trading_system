...
| GAP-3 事件型分析（使用者重定義：外部標好正反例匯入→PIT 對齊→條件 IC／ML；非 event study） | **SPEC 起草中**（討論階段 2026-08-20 收案：consult R1＋R2＋討論文檔 12 版；使用者裁定開 SPEC） | 完整管線：SPEC 初稿→三家 adversarial→reconcile＋戳記→白話閘→TODO→B1–B5；取材＝`白話說明/GAP-3事件型討論.md` §7.5；設計須含 R5 A′ 語意 | registry #3＋SPEC `docs/GAP3_EVENT_SPEC.md` |
