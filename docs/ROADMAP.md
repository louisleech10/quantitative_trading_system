...
- **🔵 治理 backlog — 排序 v3.1（2026-08-04 全量重排＋當日新增 3 張，唯一有效）＝當前施工序**：
  **36 張**（導出命令：`grep -c '^## B-' handoffs/20260801-GOV-AMEND-BACKLOG.md`）中
  ...
  ```
  第0批 摩擦止血    B-24 → B-15 → B-14      第3批 殺手寫漂移  B-17 → B-13(吸收 B-18/B-36) → B-26
                    → B-30 → B-32           第4批 散文與標記  B-16 → B-23
  第1批 機制        B-19 → B-29 → B-31      第5批 fail-open   B-11 → B-6 → B-5 → B-4 → B-8
  第1.5批 守衛漂移  B-33 → B-34             第6批 完整性監看  B-20 → B-21 → B-12 → B-22
  第2批 地基        B-27                    另排              B-9 → B-28 → B-35 → B-24(機械強制面)
  ```
