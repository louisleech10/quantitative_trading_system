...
1. 六時間欄不變式（validator 機械檢查；R4 W1 拆**三段鏈**——單一鏈 `…≤ entry_at ≤ label_start…` 廢除，`next_open`×`close_to_close` 組合下必炸，而該組合為使用者 U4b「兩數並排」之合法語意）：
   - **PIT 鏈**：`observed_through ≤ feature_cutoff[tf] ≤ decision_at ≤ entry_at`；
   - **label 鏈**：`decision_at ≤ label_start < label_end`；`label_start` 依 `label_return_mode` 機械定——`close_to_close` ⇒ t₀ bar close_time；`open_to_close`／`open_to_horizon_close` ⇒ entry bar 之 entry 時點；
   - **持有鏈**：`entry_at < label_end`（持有窗＝entry → label_end close；D1-4／D4 實際持有報酬）。
   `entry_at` 與 `label_start` **無強制順序**（label 錨依 D1-5 與 decision/entry 脫鉤）；`entry_after_label_start: bool` 入 receipt（`next_open`×`close_to_close` ⇒ true），報告兩數並排處必標。`feature_cutoff` **per-TF 展開**，規則＝`max{bar.close_ms ≤ decision_at}`（12h t₀ 對 1h/4h 在 UTC 整點為常見特例，一律走 as-of）。
...
- **AR-1 決策時點 t₀−k 契約形式** → **已裁（X1）**：`t0`＋`decision_offset_bars`（int ≥0，預設 0）、`decision_at_ms` 由對齊推導、不設 ms 覆寫欄；`entry_price_semantic` 值集擴 `decision_bar_open/decision_bar_close`；時間不變式＝D2-1 三段鏈（R4 W1 拆分）＋`decision_at ≤ t0_open_ms`；規格見 D1-1/D2-1/D2-2。
...
- **baseline 內容（四類）**：
  ...
  2. **對齊 golden（真實 kline 手算對照）**：自真實 kline 取 ≥3 個 t₀（12h UTC 整點、非整點邊界、資料末端），對 1h/4h/12h 各 TF **手算** `feature_cutoff[tf]`（`max{close_ms ≤ decision_at}`）與六時間欄，斷言 receipt 逐欄 **整數 ms exact（==，容差 0）**；含 `decision_offset_bars=0`、`k>0`、`entry_price_semantic=next_open` 三形 exact receipt oracle（R1 X1／R2 Y1，含 `entry_at_ms`＋`entry_price_source` 欄）；各 `label_return_mode` 之 `label_start_ms/label_end_ms` exact 手算入案；`next_open`×`close_to_close` 案例斷言 `entry_after_label_start=true` 且 D2-1 三段鏈全過（R4 W1）；資料末端案例預期 `label_window_incomplete` 拒絕。
