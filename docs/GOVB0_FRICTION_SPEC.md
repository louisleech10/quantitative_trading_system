...
- 🔴 **本批明確不受理範圍（R3 新增；依使用者定死「沒 100% 解就做 95% 那版現在收，殘留具名記錄不當阻塞」）**
  ...
  **殘留一（允許清單不完整）— 已部分修，殘留具名接受**：
  R7 補入 `~{}[]!*?` 八字元後，兩家點名的向量已解。仍可能有未列的合法字元。
  🔴 **但「列不完」對允許清單不構成問題，理由是失敗方向（使用者 2026-08-05 指正主委的框架）**：
  **允許清單不必列完，因為漏項會自己現形。** 未列字元 ⇒ ⑦ fail-closed ⇒ **誤擋**（可見、會被抱怨、可補），
  而非**漏放**（不可見、要出事才知道）。⇒ 允許清單**可收斂**：每發現一次誤擋補一項，補一次少一次；
  禁止清單**不可收斂**：永遠不知道還有幾項沒列，而未列的那些正在 fail-open。
  🔴 **此收斂循環的前提＝誤擋必須看得見** ⇒ **Phase 0（可觀測性）是硬前置**，
  否則使用者只會覺得工具難用而繞路，訊號永遠收不到。此亦為 `票 B-37` 的同一機制。
