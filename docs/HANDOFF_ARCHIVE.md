...
> 本檔為 `HANDOFF.md` 之日誌類移出目的地（`scripts/live_doc_registry.json` 登記為 LOG 類）。
> `HANDOFF.md` 進行中紀錄區中，所屬識別碼已完成之條目整段移入本檔（DOCROT2 Task 2.4）。
> 本檔為歷史紀錄，不承載現行狀態；現行狀態以 `scripts/fact_keys.json` 生成之投影為準。

## 2026-09-15 交接檔遷移前全文（DOCROT2 Task 2.4；標題降階、其餘逐字）
...
㊿**`Task 9.3`（B9D）✅ 完成、已 commit `4dcae642`**（TODO §B 狀態欄＝唯一權威；receipt `COMMIT` 與 `impl_token_issued.round_start_head` 逐字相等 `75be4280…` 已實跑複核）：`split_projection.py` 新增聚合 seam `_aggregate_event_level_split_rows`（值級 `event_id` 非空 str 閘＋同事件 `symbol` 衝突 raise）、`_assert_concat_event_level_unique`、`_unique_events_per_symbol`；兩表退回事件級 schema（單／多標的／空批共用常數）；`per_symbol_n`／門檻輸入／`n_purged` 去重、`n_event_tf_rows_purged` 改稽核列數公式；`pipeline.py` 加 `event_id` 去重斷言（不動 `C5-26` 錨點行）。SPEC 進 **v40**（`C5-21`／`C5-27` 錨點重出、`C5-20` TARGETS，body `1d82ed10…`）。
  - **驗證**：11 檔 240 passed（改前 233）；錨點 22 全過；golden 比對 `GOLDEN OK`；receipt `handoffs/run_receipts/20260914T183145Z-splitunify-task-9.3-register-rescan.txt` 驗收 1–4 實跑全過；mutation 腳本 `handoffs/20260911-splitunify-b9d-mutate.py` 11 條全判定通過＋C0 綠。
  - 🔴 **自查揭露**：多標的「串接後再驗唯一性」呼叫點經公開入口**不可達**（跨 symbol 碰撞先被錨點唯一性閘／manifest 重複閘擋下，探針實測呼叫 0 次；拔掉呼叫點 131 passed）——規格明訂之縱深防禦，helper 本身有直接反例測試；已列 r48 brief assumed 交審。
  - **review-r48 完成**（`handoffs/reconcile/20260911-splitunify-b9-review-r48/synth.md`；`debt_clear` rc=0）：**兩家零 finding、皆 APPROVED v40、皆判 9.3 可收案**；兩家 SPEC 戳記已 `register-output`，`reconcile_stamps_check` PASS。兩家否證主委 assumed「串接後唯一性呼叫點經公開入口一律不可達」（畸形多標的輸入：共用特徵索引＋同名事件＋manifest 去重成一列 ⇒ 可達且正確擋下）⇒ 主委補 `test_multi_symbol_concat_rejects_cross_symbol_event_id_collision`＋mutation `M-B9D-MULTI-CONCAT`（只動測試，交 9.4 審碼輪覆核）。codex 實際 `gpt-5.6-luna`／`max`。
  - 🔴 **授權依據第二道卡**：以 SPEC 領 impl token 仍被拒——`--adversarial` 另要求**已填實之 Verdict 行**（`verdict_filled_check.sh`），SPEC 無此行；r48 收斂檔有 Verdict 但無戳記。**不在委員所有之 SPEC 戳記區自加 Verdict 行**；比照 stamp-r5（B9C）於 r48 收斂檔尾補 `## 戳記` 區（並改裁定段兩句過時之下一步），派 **`stamp-r7`**（兩家）戳記＋覆核補測（body `b174cc9b…`）。補測後實跑：11 檔 241 passed；mutation 12 條＋C0 全判定通過。
  - **stamp-r7 完成**（兩家 proceed、零 finding、APPROVED r48 收斂檔 body `b174cc9b…`；判授權依據適切、主委對裁定段兩句之改寫忠實、補測有鑑別力且只測串接後檢查）；兩家戳記已 `register-output`，`reconcile_stamps_check` 與 `verdict_filled_check` 皆 PASS ⇒ 以 r48 收斂檔領 impl token 落地。codex 實際 `gpt-5.6-luna`／`max`。
  - **Task 9.4（B9E）**：`baseline.py` 刪 `n_test` 改 `n_test_events`／`n_test_samples`（exact 鍵集）；`pipeline.py:825-827` 三計數改 `event_id` 去重；新增 `test_baseline_splits_n_test_into_events_and_samples`、`test_baseline_dict_has_no_legacy_n_test_key`、`test_event_count_conservation`、`test_multi_feature_tf_split_labels_match_decision_anchor_per_event`；SPEC v41（`C5-14`／`C5-26` 錨點）。15 檔 304 passed；mutation `b9e-mutate.py` 12／13／31／32＋C0 全判定通過（13 在 9.3 唯一性斷言下字面等價，改取 `per_tf` 列數）。**review-r49 兩家零 finding、APPROVED v41**；SPEC 戳記已登記；r49 收斂檔補戳記區（body `2ffd6e9e…`）派 **stamp-r8**：兩家 proceed、戳記已 `register-output`、`reconcile_stamps_check` PASS ⇒ 以 r49 收斂檔領 token 落地（commit 訊息檔 scratchpad `commit_b9e.txt`）。自證抓到新測試一行與 `C5-23` 錨點逐 token 相同（全檔唯一性轉紅）已改寫。
  - **Task 9.4 已落地 `bd820ee6`**（stamp-r8 戳記授權依據）。
  - **Task 9.5（B9F）施工完成、待審碼**：`freeze_splitunify_golden.py` 新增兩標的交錯 × 兩 feature TF 平行組（`_interleaved_plans`／`_interleaved_event_keys`／`_build_interleaved_actual`，只增 `g2_interleaved_*` 6 鍵；單標的 `_event_keys` 維持單 TF 以保舊值）；golden `--write` 只新增 6 鍵；`test_splitunify_golden.py` 補 TODO 指名 8 條行為測試（tmp 導向 `GOLDEN_DIR`／`RECEIPT_DIR`／`REPO`）。後端 15 檔 312 passed；前端匯出 7 檔 43 passed；mutation `b9f-mutate.py` 16／17／27／28／29／33／34／11＋C0 全判定通過（34 以同內容重寫 SPEC 只動 mtime，不改 body）。SPEC 仍 v41。**review-r50**：兩家 proceed、APPROVED v41、判 9.5 可收案；codex `CODEX-R50-P2-01`（平行組 g5 明文 positions 凍全框列號、SHA 用標的內序號，明文不可重算）⇒ **本輪修掉**：positions 改凍 local＋新增逐列 `feature_ts_ms` 與 `global_row_index_positions` 兩鍵、平行組測試由明文 canonical JSON 獨立重算、mutation 加 `M-B9F-P2`；golden 以 HEAD 為底重凍只增 8 鍵。golden＋contract 42 passed、mutation 10 條＋C0 全過。r50 收斂檔補戳記區並改正第 3 條誤寫（重算方式），body `15f887a1…` → **stamp-r9**：兩家 proceed、戳記已 `register-output`；codex 以原提出方覆核 `CLOSED: CODEX-R50-P2-01`（修補後兩標的 `local_match=True`）⇒ 以 r50 收斂檔領 token 落地（訊息檔 scratchpad `commit_b9f.txt`）。
  - ✅ **Task 9.5 已落地 `c6972349`**（push 同步）。**Phase 9（9.1–9.5）程式面全數完成**。
  - **收尾進行中**：SPEC v42（body `a332d392…`；§N `SU-RESID-2` 全關＋沿革）與 TODO §E 同名列同步；白話 README／治理進度日誌／流程摩擦記錄／GAP-3驗收清單（WATCHED 命中 `scripts/`／`frontend/src/app/search/`）已補記；派 **stamp-r10**（`brief-kind: closure`；session kind 只收 `impl|review|stamp|consult|fix`，`closure-r1` 被命名規約 fail-closed 未派出）兩家戳記 v42 → reconcile／debt_clear → docs commit（無需 impl token）→ push。**stamp-r10 完成**：兩家 APPROVED v42、判 Phase 9 可宣告完工，並各自列出兩處範圍外過期字面（TODO §B B9C「現待 review 覆核 v32＋重簽」、SPEC §N 誠實邊界「`Task 9.3` 動工前須再掃」）⇒ 主委同步修掉、SPEC **v43**（body `977fe1d6…`）→ **stamp-r11**：兩家零 finding、APPROVED v43 ⇒ 收尾 docs commit＋push。**SPLITUNIFY 第 9 批（9.1–9.5）完工。** 下一步由使用者決定；委員仍為兩家暫調。
  - 🔴 **2026-09-15 殘留帳面更正**：上句原列之 `R-1`／`SU-RESID-3` 早於 B8（D-001）關閉，TODO §E 漏同步；另漏收 `SU-RESID-4`／`SU-RESID-5`／`SU-RESID-C5-TARGETS`。**現行未關閉 11 條**：`R-3`／`R-4`／`R-5`／`SU-RESID-1`（剩語意之半）／`SU-RESID-4`／`SU-RESID-5`／`SU-RESID-9A-UI`／`SU-RESID-V8-ATTEST`／`SU-RESID-C5-TARGETS`／`SU-RESID-PAUSED-NO-RESULT`／`SU-RESID-COMMITTEE-MODEL-EVIDENCE`（白話版見 `白話說明/SPLITUNIFY施工進度.md`「還剩什麼」）。TODO §E＋SPEC v44 經 `stamp-r12` 兩家 APPROVED、零 finding（codex 裁決行寫 `APPROVED` 被拒收，主委改 `proceed` 後重登解鎖）；composer 本輪自報底層為 Grok 4.6（Cursor runtime）。前 20 字碰撞實測 receipt `handoffs/run_receipts/20260915-splitunify-resid1-quote20-collision.txt`：處置 token 不同之碰撞配對 0 ⇒ 不改整句。DOCROT 成效：文字不同步類意見未減少（r1–r12 約 51/162、r13–r50 約 72/172，主委關鍵字粗估），待使用者裁定（`docs/SCAR_LEDGER.md`）。
  - 🔴 **Phase 9 收尾待辦（原文）**：`SU-RESID-2` 之 SPEC §N（`:341`）與 TODO §E 皆寫「退回完成即可全關」⇒ 9.3 已落地，兩處須同步改為關閉（SPEC body 變 v42 ⇒ 需 closure 輪兩家戳記後 commit）。
  - **9.5 前端已先起草**（未追蹤、未審）：`frontend/src/app/search/eventExportByEventId.test.tsx`（CSV 匯出 `meta.*` 附帶欄逐列值對回來源列；帶 `feature_timeframe` 仍以 `canonicalEventId` 建鍵）2 passed；`M-SU-D2-11` 手動破壞（`page.tsx` 建鍵改複合鍵）⇒ 2 failed、已還原 diff 0。9.5 後端：v8 write-once／外部錨／`_v9` 鍵已於 9.2b 落地；缺交錯平行組與 TODO 指名之 8 條測試名（部分為既有測試之同義）。
  - 🔴 **9.4 同型提醒**：`baseline.py:123` 與 `pipeline.py:825-827` 是 register 錨點 ⇒ 9.4 落地同樣須「commit 前審碼＋SPEC 戳記 → 收斂檔補戳記區＋stamp 輪 → 以收斂檔領 token」。
...
- **現行主線＝DOCROT2（修正 DOCROT 未做好之處；使用者 2026-09-15 裁定，排在 SPLITUNIFY R-5 之前）**。使用者逐字：「持續不接受用紀律或記憶當解法，但允許擴建治理工具，就是要修正DOCROT沒做好之處，真正減少文檔問題和降低不必要的輪數，而且要全專案涵蓋，不是只有SPEC」「交接文件能交接清楚明確是最重要，所以不一定要限制行數，怕的只是把舊案或已完成或不必要的殘留在裡面」「針對修正優化DOCROT的方法，你跟委員討論共識決定，先做好DOCROT這部分」。
  ...
  - **D2A 進度**：`Task 1.1`／`1.2`／`1.3` 皆已實作；四檔回歸（`tests/governance/test_docrot2_registry.py`、`test_govb1_factkey_gen.py`、`test_factkey_write_guard.py`、`test_govb1_factkey_hook.py`）298 passed、0 failed；mutation receipt `handoffs/run_receipts/20260915-docrot2-d2a-task11-mutation.txt`／`-task12-`／`-task13-`。D2A 已 commit `3077792d`；review-r1：codex blocked（P1×2、P2×2）、composer proceed，四條全採納修補（識別碼跨 `status_keys` 與 `docrot2_status_keys` 唯一、SPLITUNIFY 撞號三列改登 `SU-B1`／`SU-B3`／`SU-B4`；`--path`／`--staged` 擋 symlink 與非一般檔），回歸 304 passed、0 failed，收斂檔 `handoffs/reconcile/20260915-docrot2-b1-review-r1/synth.md`、`debt_clear` rc=0。修補 commit `688e8c5b`；review-r2 兩家 proceed、codex 以原提出方閉合 r1 四條（收斂檔 `handoffs/reconcile/20260915-docrot2-b1-review-r2/synth.md`）⇒ **D2A 收批**。下一步＝D2B（`Task 2.1`–`2.5`；batch 識別碼 `b2`；🔴 `Task 2.4` 交接檔遷移須與 `Task 2.5` 掛載同一 commit）。批次狀態見 `docrot2-batch-status` 生成區塊。
...
- 2026-09-23：RM-TODOFMT → `handoffs/reconcile/20260922-todofmt-b4-review-r1/synth.md`
- 2026-09-23：RM-TODOFMT → `handoffs/reconcile/20260922-todofmt-b4-review-r2/synth.md`
- 2026-09-23：RM-TODOFMT → commit `14ccb24b`
