# P2 債票 1 — governance 9 紅 fixture 遷移 — TODO(正式版)

> 定稿來源:handoffs/P2DEBT-T1-TODO-DRAFT-R4.md(R1 雙審→R2→R3→斷路器換手 codex R4)
> RECONCILE-STAMP:grok(R4)=handoffs/P2DEBT-T1-TODO-REVERIFY-R4-grok.md;composer(R4)=handoffs/P2DEBT-T1-TODO-REVERIFY-R4-composer.md(作者 codex 迴避)
> task-id:p2debt-t1 | 正式化:2026-07-11 | 內容=R4 凍結稿;正式化僅將 Task 1.3 閉合驗證 bullet 由「見上表」改為內嵌同一命令(機檢可證偽 token),語意零改動
...
- **防假綠（§C）**：
  ...
  3. scope 驗收 = 派工前擷取 pre-dirty（`git status --porcelain | awk '{print $NF}' | sort -u > /tmp/p2debt-t1-pre-dirty.txt`）；完工後取 post-dirty 同法；以 `comm -13 /tmp/p2debt-t1-pre-dirty.txt /tmp/p2debt-t1-post-dirty.txt | sort -u` 得 **delta**（post 相對 pre 新增/變更路徑）；delta 與四檔 whitelist 比對時**兩邊皆須 `sort -u`**，且須精確相等（見 Final Acceptance §2）；**不得**用未扣 pre-dirty baseline 的全域 dirty 清單或 `git diff --name-only` 當驗收。
...
- **風險緩解**：⊘
- **驗證（Task 閉合）**：`venv/bin/python -m pytest tests/governance/test_verify_gate_redteam.py::test_r7_gate_task_id_appends_committee_dispatch -v` → `PASSED`（見上表 1.3.1）
...
| scope | pre-dirty delta vs 白名單 | 派工前 `git status --porcelain \| awk '{print $NF}' \| sort -u > /tmp/p2debt-t1-pre-dirty.txt`；完工 `git status --porcelain \| awk '{print $NF}' \| sort -u > /tmp/p2debt-t1-post-dirty.txt` + `comm -13 /tmp/p2debt-t1-pre-dirty.txt /tmp/p2debt-t1-post-dirty.txt \| sort -u > /tmp/p2debt-t1-delta-dirty.txt` + whitelist 與 delta 各自 `sort -u` 後 `diff -u` → exit 0（whitelist 四檔見 Final Acceptance §2；壞基線無實作時預期 delta 空、**rc=1**；`pre + 四檔` 的 `/tmp` simulation 實跑 **rc=0**，receipt 見 Final Acceptance §2） |
...
comm -13 /tmp/p2debt-t1-pre-dirty.txt /tmp/p2debt-t1-post-dirty.txt | sort -u > /tmp/p2debt-t1-delta-dirty.txt
...
sort -u /tmp/p2debt-t1-whitelist.txt > /tmp/p2debt-t1-whitelist-sorted.txt
sort -u /tmp/p2debt-t1-delta-dirty.txt > /tmp/p2debt-t1-delta-dirty-sorted.txt
diff -u /tmp/p2debt-t1-whitelist-sorted.txt /tmp/p2debt-t1-delta-dirty-sorted.txt
# 壞基線（無實作變更）的誠實預期：post=pre，delta-dirty 0 行；diff 顯示 whitelist 四檔全缺；exit 1。
# 尚未觀察真實完工工作樹；不得宣稱真實完工後 delta=四檔或 diff rc=0。
#
# R4 synthetic simulation receipt（Codex 實跑 2026-07-11；全在 /tmp；fake pre={a,b}）：
#   printf '%s\n' a b > /tmp/r4-pre4
#   printf '%s\n' docs/VERIFY_GATE_SPEC.md tests/governance/test_verify_gate_b4.py tests/governance/test_verify_gate_b5.py tests/governance/test_verify_gate_redteam.py > /tmp/r4-white4
#   cp /tmp/r4-pre4 /tmp/r4-badpost4
#   comm -13 /tmp/r4-pre4 /tmp/r4-badpost4 > /tmp/r4-baddelta4
#   diff -u /tmp/r4-white4 /tmp/r4-baddelta4 → rc=1；wc -l /tmp/r4-baddelta4 → 0
#   printf '%s\n' a b docs/VERIFY_GATE_SPEC.md tests/governance/test_verify_gate_b4.py tests/governance/test_verify_gate_b5.py tests/governance/test_verify_gate_redteam.py | sort -u > /tmp/r4-simpost4
#   comm -13 /tmp/r4-pre4 /tmp/r4-simpost4 | sort -u > /tmp/r4-simdelta4
#   sort -u /tmp/r4-white4 > /tmp/r4-white4-sorted
#   diff -u /tmp/r4-white4-sorted /tmp/r4-simdelta4 → rc=0；wc -l /tmp/r4-simdelta4 → 4
#   delta 實際輸出：docs/VERIFY_GATE_SPEC.md、tests/governance/test_verify_gate_b4.py、tests/governance/test_verify_gate_b5.py、tests/governance/test_verify_gate_redteam.py。
# 真實完工驗收預期：delta-dirty 精確 4 行（= whitelist）；排序後 diff 無輸出（exit 0）；不得含 scripts/
...
R3-CLOSURE: B3→§0 L46、Phase Gate scope、Final §2 改 pre-dirty snapshot + `comm -23` delta vs 四檔 whitelist（壞基線無實作：delta 0 行、diff rc=1；原稱完工後 delta=四檔、diff rc=0 未實際觀察，已由 R4 修正）
R3-CLOSURE: NEW-B4→派工 prompt L72 + footer L262 `TODO-DRAFT-R1` → `TODO-DRAFT-R3`（self）
R4-CLOSURE: B3 → comm direction fixed by codex (breaker handover), receipts real
