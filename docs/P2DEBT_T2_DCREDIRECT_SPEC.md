# P2 債票 2 — legacy 測試 data_cache 污染 → tmp redirect — SPEC(正式版)

> 定稿來源:handoffs/P2DEBT-T2-SPEC-DRAFT-R4.md(R1-R3 Composer→斷路器換手 Codex R4;雙家四輪 adversarial+可執行原型)
> RECONCILE-STAMP:grok=handoffs/P2DEBT-T2-SPEC-REVERIFY-R4-grok.md;composer=handoffs/P2DEBT-T2-SPEC-REVERIFY-R4-composer.md(作者 codex 迴避)
> task-id:p2debt-t2 | 正式化:2026-07-11 | 內容=R4 凍結稿;正式化僅補機檢錨點 3 處(RISK-HIT canonical 行/§A 待確認宣告/§N 節標),語意零改動
...
- 待確認：無（修法方向與升級「大」均已裁定;技術事實全數以下列 FACT-RECEIPT 實跑確認,無僅使用者知悉之待決事實）
...
> **AMENDED C-5（2026-07-12，出處 `handoffs/P2DEBT-T2-IMPL-CHAIR-FINDING-C5.md`）**：S1 同時納入
> `ICFilterOrchestrator._persist_outputs`。此 caller 內的 `output_dir="data_cache/reports"` 是獨立寫入接縫；
> active 時其 reporter 三個落盤呼叫均先映到 `root/{reports,features}`。S10 的 bad-payload 測試直接寫檔
> 改由 `get_active_redirect_root()/models` 建立，確保寫端與 analyzer resolver 讀端使用同一犧牲根。
...
1. 正向：每個 target/subtarget 可 resolve；probe 只驗 manifest 宣告輸出的 redirect-root path shape，owning spy 無 violations。真正的正向落盤保證由各 body 測試與 hermetic digest 提供。
...
5. S9/S11 helper wiring 完整性由 inventory 的真實 caller 計數驗證，不以手寫 source 字串充當 mutation。

> **AMENDED C-CE8（2026-07-12，出處 `handoffs/P2DEBT-T2-IMPL-REVIEW-grok.md`、
> `handoffs/P2DEBT-T2-IMPL-REVIEW-composer.md`）**：manifest completeness 除 ID 集合外，強制
> S1≥2、S2≥3、S10≥2 installers；缺任一 subtarget 時 `activate()` 必須在安裝前拋
> `RedirectCompletenessError`，gate 保持 inactive 且不得留下部分 patch。manifest `probe` 僅證明
> path shape，不宣稱觸發 production write；正向落盤隔離以 body 測試與 digest 為準。
...
`normalize(result)` 保留 R3 定義：`hash_* = sha256(json.dumps(normalize(result), sort_keys=True, default=str))`（byte-exact 相等,無 atol/rtol 容差——A/B/C 三跑 sha256 必全等）；只豁免 `filtered_features_path`、`report_paths`、`artifact_mtime` 等純路徑/mtime 欄位。不得豁免數值、NaN pattern、feature count、selection、schema 欄位。stdout 必含 `ab_hash=`。
...
腳本 `set -euo pipefail`；除下述 V6 修訂外，pytest failure、digest command failure、digest mismatch 任一皆 exit 非 0。V6 以 `FAILED`/`ERROR` nodeid 集合相對 pinned main@492c4cc 基線判定；僅當本輪集合為基線子集且 digest=1 時 PASS。`all` 依序跑 V1、V2、V5、V6、V7；stdout 必有五個 `DIGEST_DIFF_EMPTY[Vn]=1`，且 V6 必有 `V6_NO_NEW_RED=1`；V6 依此準則 PASS 時，其 pytest rc=1 不使 `all` 整體失敗。禁止直接跑裸 V2/V5/V6/V7 當 hermetic acceptance；正式驗收以此 wrapper 為準。
...
| V6 | harness `--set V6`：三個 API polluter files | `V6_FAIL_SET ⊆ tests/fixtures/v6_baseline_bad_nodeids_492c4cc.txt`；`V6_NO_NEW_RED=1`；digest=1；exit 0。**AMENDED per C-4 dual-stamp(grok+composer 2026-07-11),原全綠列 superseded 僅限票2驗收** |
...
## §N N/A 登記與 NOT in scope

- **§G 已 filled**（Run A/B/C sha256 oracle,見 §G;無豁免）;baseline **重凍**不在本票=票 5 scope。
- **mutation probe(演算法級)**：N/A — 本票 mutation=redirect 撤除 canary,非數值演算法探針;數值守恆已由 Golden Run A/B/C sha256 oracle 承擔(已 filled,見上節)。
