...
### 🔴 兩路涵蓋宣告（使用者 2026-09-19 裁定，適用今後每一張 IC 相關票）

**逐字**：「IC Analysis 中全應該是都要兩種都支援和區分全域序列型 IC 和事件型 IC 吧，除非這功能是特定用在其中之一。」

⇒ 今後每張 IC 相關票之 SPEC §A 須有一行 **涵蓋宣告**，三值擇一且**不得省略**：
`兩路皆交付` ／ `只適用某一路＋逐字理由` ／ `不適用（不在 IC 兩路語彙內）`。
且「支援」含**區分**：輸出須讓讀者看得出這個數字是哪一路算的，不得兩路共用一個沒有標記的欄位。

🔴 **2026-09-19 使用者補充裁定（逐字）**：「我要確保已完成的和未完成的項目，都要支援和區分序列型和事件型，
已完成的看確保能支援，且計算和結果都要保證每項正確，前後端都是，也要驗收確認。未完成的也要寫明開發時候兩者都要支援。」
⇒ 兩條義務：
1. **未完成票**：涵蓋宣告之**預設值＝`兩路皆交付`**；要寫成單路**必須**附逐字理由，且該理由須說明「為何這功能本質上只屬於一路」，
   不得以「本批不做」當理由（那是排程，不是涵蓋面）。
2. **已完成項目**：須逐項**回頭查證**——後端算得出且算得對、前端顯示且看得出是哪一路、有測試釘住（含 mutation）、
   並經使用者驗收確認。查證結果逐項落在本節現況表，三值＝`已支援`／`未支援`／`不適用`；
   「沒查過」**不得**填成「已支援」。

**為何定此規**：本規之前，票只在少數情況寫明涵蓋面（`2b` 逐字「序列型／事件型同一座橋」、`2a` 逐字「不碰事件型」），
其餘（`4`／`5`／`6`／票 A／票 B／FU-1／FU-2）**全部沒寫** ⇒ 做完才發現只涵蓋一路，
而預設路徑下輸出看起來完全正常。此即 SPLITUNIFY B10E 連續踩到兩次之「兩端都有、只接一端」同型。

**現況登記（2026-09-19 逐票複查）**：

| 票 | 涵蓋宣告 | 依據 |
|---|---|---|
| `2a` 邊際 IC | 規格層**只做全域**（逐字「不碰事件型」，使用者 2026-08-18 裁定）；🔴 **但碼層無事件型排除** | `_stage6b_marginal_ic` 退出條件只有 `enabled`／`label_series is None`／無切分；對橫截面有具名 `not_applicable:cross_sectional_mode`，**對事件型沒有** ⇒ 事件 run 會照算（吃 stage3 後之樣本），且**無事件型驗收**。待裁：補事件型驗收，或事件 run 具名 `not_applicable` |
| `2b` IC→ML 橋 | 兩路皆交付 | 逐字「序列型／事件型同一座橋」＋`sample_scope` |
| `3` 事件型分析 | 只適用事件型 | 票之定義即事件路徑 |
| `4` Pooled/Panel IC | **未宣告**（動「主統計量」＝不共用層 ⇒ 開票前必須先宣告） | 本檔 #4 只一句「多標的合併估 IC」 |
| `5` 容量 ADV | **未宣告** | 本檔 #5 |
...
| R-GOVTEST-1 | 4 條 G-7 測試長期紅：`test_t01_f2_frozen_hashes_self_consistent`／`test_t01_f3_g7_when_committed`／`test_r6_u1u2u4_g7_worktree_space_quote_paths`／`test_g7_ambient_m_gate_check_not_red`。🔴 **2026-09-23**：四條已標 `xfail(strict=True)` 並引本 ID（`test_r6_u1u2u4…` 另加 `run=False`：建 worktree 逾 20 分鐘）——轉綠即 XPASS 轉紅，強制回頭移除標記 | user-ruling: 2026-09-05 —— 它們斷言 `govb1_final_gate.sh --only g7` rc==0，而 G-7 已被裁定停用且結構性恆紅；在 G-7 復工前修它們＝為已停用機制付工 | 同 R-G7-OFF-1（GOVB1 復工） | 四條轉綠，或隨 G-7 一併退役並刪除 |
| R-G7-OFF-2 | 2 條 `tests/governance/test_gov_check_cheap_first.py` 長期紅：`test_mutation_removing_early_exit_makes_pytest_run`／`test_mutation_removing_g7_script_turns_red`。🔴 **2026-09-06 首次發現並具名**——它們是 `R-G7-OFF-1`（G-7 停用）之直接 fallout：兩條都以「G-7 早退／G-7 腳本存在」為前提做 mutation，而 G-7 已改成只印警告不擋 ⇒ 移除早退不再改變行為、移除腳本不再轉紅。**已對等驗證非 `G3-D2` 造成**：在 B-D5 之父 commit `0ba599f3` 上單獨重跑同樣紅。🔴 **2026-09-23 更正**：`test_mutation_removing_early_exit_makes_pytest_run` 實為錨點字串過期（早退訊息改寫），非 G-7 fallout，已修綠；`test_mutation_removing_g7_script_turns_red` 標 `xfail(strict=True)` 並引本 ID | user-ruling: 2026-09-05 使用者裁定 G-7 停用；本兩條是該裁定之機械後果，與 `R-G7-OFF-1` 同一根因，**不另修**（修它等於把 G-7 開回來） | GOVB1 epic 復工（與 `R-G7-OFF-1` 同一觸發） | 復工票把 `gov_check.sh` 第 4 段改回擋之後，本兩條須隨之轉綠；若屆時仍紅，代表 mutation 之前提另有問題 |
| R-GOVTEST-2 | 4 條 factkey 測試長期紅：`test_t21_assert_clean_fixture_rc_zero`／`test_t22_clean_host_rc_zero`／`test_pre_push_delegation_reaches_factkey_segment`／`test_pre_push_rejects_on_factkey_drift`。🔴 **2026-09-23 現況**：`test_t21_…` 之紅因實為靜態夾具 `factkey_clean` 過期（註冊表每改一次狀態列即過期），與下列 pre-push 假設無關，已改以 repo 本身為乾淨宿主、自 repo 外 cwd 呼叫而修綠；其餘三條已於 2026-09-11 標 skip（`test_t22_…` 夾具過期；兩條 pre-push 依 2026-08-14 裁定），本列之觸發條件只剩該三條 | blocked-by: 測試內建假設「`pre-push` 之委派鏈會走到 `gov_check` 第 3 段」（見 `tests/governance/test_govb1_factkey_hook.py:9` 之檔頭原文），該假設於 2026-08-14 `pre-push` 改 `--fast` 時失效；修法須先定「fact-key 漂移該在哪一關擋」 | 決定 fact-key 漂移之強制點（維持產出端 hook 即可／或恢復某條 push 檢查） | 四條轉綠，且該強制點有一條 mutation 證明改壞會紅 |
| R-GOVTEST-3 | 4 條 GOVB1 期間凍結不變式測試長期紅：①第 5 批保護窗 `test_govb1_contract_matrix.py::test_waiver_b5_range_does_not_touch_forbidden`／`::test_waiver_b5_active_when_b5_start_anchored`、`test_govb49_path_grant.py::test_task32_granted_paths_are_the_only_behaviour_change`——保護窗為 `b5_start..HEAD` **開放區間**，B-49 授權又以 git blob 雜湊綁定 4 個 harness 檔 ⇒ GOVB1 暫停後任何票合法修改 B-45 harness 即觸發（2026-09-23 實測已在歷史 commit 中觸及 5 檔，含 DOCROT2／VERDICTGATE 期間之修改）；②`test_gate_only_g5_g6_green[g6]`——G-6 凍結 `cx_run.sh::_maybe_register_stamp_output` 與 base 逐字相同，而 VERDICTGATE Task 1.2（stamp 改走 `--kind stamp --family`）與 CXSTAMP review-r1（格式不合不登記）經審查合法修改之（`[g5]` 拆出仍受檢）。四條皆標 `xfail(strict=True)` 並引本 ID | blocked-by: GOVB1 epic 暫停——關窗（收窄上界或重錨 `b5_start`）與改 G-6 基準皆須改 `scripts/govb1_frozen_hashes.txt`，屬 out-of-epic 亦禁改之硬保護集；復原 G-6 函式即倒退上述兩修正。與 `R-G7-OFF-1` 同一觸發為主委判斷，非使用者逐字裁定 | GOVB1 epic 復工（同 `R-G7-OFF-1`） | 復工票關窗、重錨 G-6 基準後四條轉綠（strict xfail 轉 XPASS 即紅，強制移除標記） |
| R-GOVTEST-4 | `tests/governance/test_debt_gate.py::test_gate_check_latency_under_100ms` 長期紅：SPEC 字面「單次 gate_check cold <100ms」，以真實 audit 量測；2026-09-23 實測 audit 7.5MB／70,898 行 ⇒ 債務帳本核心 122ms、gate_check 148ms（空 audit 時 25ms／62ms）。帳本核心依 SPEC Task 2.1 改法①（已簽核）**禁前置過濾**（會吞半截寫入之偽 JSON），冷路徑必隨 audit 線性成長。已標 `xfail(strict=True)` 並引本 ID。影響面：只在派工類呼叫持有效 token 時付此成本，一般 Bash 呼叫不走帳本 | blocked-by: P1-6 線 C（債務事件分檔）未做；ROADMAP 該列「不排程：線 C 未做，治理不再擴建」 | 線 C 排入 | 線 C 落地後本條轉綠（strict xfail 轉 XPASS 即紅，強制移除標記） |
| R-GOVTEST-5 | `tests/governance/test_govb1_factkey_gen.py::test_generator_runs_under_two_seconds` 長期紅：`gen_fact_key_blocks.sh` 逐 key 多次呼叫 jq，耗時隨 key 數線性成長；2026-09-19→23 fact-key 數 17→35（EVENTSCAN／FFDSTAR 規格值入 fact-key、白話索引），實測輸出模式 2.45–2.53s（預算 2s）、`--check` 10.6s——後者即改 fact_keys.json／宿主檔時產出端 `factkey_write_guard` 之實付等待。🔴 2026-09-23 前曾誤判為「環境因素、乾淨 worktree 會過」：該 worktree 內生成器提早失敗（rc=1）才快，測試原本不驗 rc；已補 rc 斷言。已標 `xfail(strict=True)` 並引本 ID | blocked-by: 優化共用生成器屬中任務（完整管線含委員審），本批依使用者 2026-09-23 裁定「不用調用委員」處理舊紅燈，不在本批內改 | 使用者決定開票 | 生成器輸出模式回到 2s 內（strict xfail 轉 XPASS 即紅，強制移除標記） |
