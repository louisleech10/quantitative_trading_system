...
| **G1-R11** | `compute_sharpe` 對「浮點上非精確常數」序列（如 80 個 `0.01`；`std≈1e-18` 非精確 0）不視為退化，回巨大有限 SR（實測 ~5.7e15）；PBO 之 path 級退化判定沿用同語意（`pbo._sharpe_pp_1d` 逐位等價） | needs-research: 「常數」之相對容差判準無公認定義（相對 |mean|？|max|？機器 eps 倍數？），且改動會變更 Task 1.2 已蓋章之退化語意，須另輪三家審〔出處＝`CODEX-R18-P2-05`／`COMPOSER-R18-P2-02`；主委 B4 brief 段 B1 自揭；處置見延伸檔 A1-24 N5〕 | 任何真實回測序列命中「近常數且 |SR|>1e6」、或使用者裁定容差 | `compute_sharpe` 加相對容差退化判定後，`_sharpe_pp_1d` 同步且 `test_vectorized_sharpe_matches_compute_sharpe` 之近常數欄應轉為 NaN |
