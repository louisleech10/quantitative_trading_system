...
- **誠實邊界（本票不防之蓄意操作，依據＝`scripts/gate_check.sh:9`「本 gate 不驗證 token 內聲稱為真」之既有設計邊界）**：①放行後至委員 CLI 寫入產出檔前，蓄意把產出路徑換成 symlink——產出檔由委員 CLI 自行寫入（`scripts/cx_run.sh:1025`），派工器無從以 no-follow 開檔；②繞過 `gate.sh` 直接以 `scripts/audit_append.sh` 寫入發放、消費或認領事件並自造相符許可檔；③直接改寫 `.claude/gate/audit.log`；④使用者於終端機手動重跑 `cx_run.sh`（無待認領之消費時照常執行並持有租約；有逾寬限之待認領消費時被拒一次並清除該標記）；⑤預先設定 `CX_RUN_LEASE_HELD` 環境變數使派工器略過租約與認領。上述任一發生時，銷帳前之 `completeness_check`、`_synth_attr` 與委員審碼仍照常檢查產出內容；棄置寫入仍受 Task 1.3 之「輪未變動」條件約束。另⑥重派上限耗盡後之棄置（Task 1.3）不要求處置保存檔之 finding——棄置不產生可合併之結論，被棄置之輪須以新 session 重審；主委無從使委員產出失敗，除以⑤同類之蓄意環境設定（例：調低 `CX_MAX_SEC` 使 CLI 被終止）外，無路徑可反覆製造失敗以達上限。另⑦棄置前之產出檔綁定只保證「**最後一次重驗當下**之 bytes 與查核當下相等、為非 symlink 之一般檔、且 `dev:ino` 與查核當下為同一物件」——**不保證「查核後未被觸碰」**：內容被改動後又改回相同 bytes（且 mtime 亦被回填）者無法辨識，欲關閉須與委員 CLI 之產出寫入端共享同一把鎖（產出檔由委員 CLI 自行寫入，主控端無法強制），故列為具名邊界。另⑧鎖內重驗與 `debt_abandon` 之 append 之間存在一段**無法歸零**之窗口：重驗順序為「開檔→`fstat`→比 `dev:ino`→讀到 EOF→比雜湊→**最終確認（重開路徑，於新 fd 上比 `dev:ino` 後重讀內容比雜湊）**」，落在**最後一次觀測取得資料之後**的追加、覆寫或路徑身分替換仍不被偵測。🔴 措辭刻意寫成「最後一次觀測之後」而非「最終確認之後」：`fstat`／讀取取得資料到比較之間本身也有間隙，寫成後者即為 overclaim（b1 review-r8 由此被打穿一次）。🔴 此處刻意只留**一個**確認點：多個確認點之間會各自產生新的窗口（b1 review-r7 由此被打穿一次）。**再增加觀測只會把邊界往後移、不會歸零**，故不再追加；能利用此窗口者僅蓄意之主控端操作（§N 蓄意等價）。不列為可修缺陷之理由：關閉它須由產出寫入端（委員 CLI，第三方二進位）與本端共用同一把鎖，主控端無法強制；且 Task 1.3 之前置條件已要求該輪無待用／啟動中／執行中嘗試且租約未被持有，該窗口內**不存在執行中的產出寫入端**，能利用者只有蓄意之主控端操作（§N 蓄意等價）。
...
- 改法：
  ...
  4. 產出檔綁定（`audit_append.sh` 之 `--require-file-path <path>`、`--require-file-sha256 <sha256|none>`、`--require-file-dev-ino <dev>:<ino>|none`）：三者須同時給定且須併 `--require-round-unchanged`，缺一 ⇒ rc=2；路徑套用與 `path_token_ok` 同一文法，違反 ⇒ rc=2。鎖內重驗於 `_append_with_round_unchanged_guard` 內、`append` 之前執行：以 `O_RDONLY|O_NOFOLLOW|O_NONBLOCK` 開檔（`ELOOP` ⇒ rc=1），其後只用同一個 fd——非一般檔 ⇒ rc=1；`dev:ino` 不等於快照值 ⇒ rc=1；讀取到 EOF（不以 `st_size` 為界）後雜湊不等於快照值 ⇒ rc=1。`_emit_abandon` 於有產出路徑與雜湊卻無 `snapshot_output_dev_ino` 時 ⇒ 不降級為只綁雜湊，直接失敗。讀到 EOF 並比對雜湊後，最終確認**只做一次且為本函式最後一個動作**：以 `O_RDONLY|O_NOFOLLOW|O_NONBLOCK` 重開同一路徑取得新 fd（重開失敗 ⇒ rc=1），於**同一個新 fd** 上先判定 `dev:ino` 不等於原 fd ⇒ rc=1，再**重讀該 fd 至 EOF 並重算雜湊**，不等於快照值 ⇒ rc=1（原因碼 `final_hash_mismatch`，與讀取階段之 `hash_mismatch` 分開）。🔴 **不得拆成前後兩道**（先以舊 fd 驗、再重開驗）：兩道之間存在可用窗口，同長度覆寫落於其間時前一道已過、後一道只看身分，兩道皆不拒。🔴 **最後一次觀測須比內容本身，不得以長度或 `mtime` 當代理**：`mtime` 之偵測力取決於檔案系統解析度，粗解析度上同一時間刻度內之同長度覆寫無法辨識。殘餘窗口見 §C 誠實邊界⑧。**每個 rc=1 出口另於 stderr 印可區分之原因碼** `RD_GUARD_REASON=<code>`，值域＝`missing`／`symlink`／`not_regular`／`object_changed`／`hash_mismatch`（讀取階段）／`final_hash_mismatch`（最終確認之重讀）／`path_reopen_failed`／`path_object_changed`；🔴 兩個雜湊碼**不得合併**——對同一情境兩者給出相同 rc，共用一碼會使讀取階段之守衛被改壞後測試仍綠；**驗收測試須斷言原因碼而非只斷言 rc**——後加之守衛會對同一情境給出相同 rc 而遮蔽先前守衛，只比 rc 時被遮蔽者之 mutation 不再轉紅（＝假防護）。
- **驗證**（`venv/bin/python -m pytest tests/governance/test_redispatch.py -q` 0 failed，其中下列斷言各為一條具名測試；佔位值依 §V 對照表替換）：
  ...
  - `ASSERT bash scripts/debt_clear.sh --abandon --round-id R --kind collection-failed --reason r --approver a WHEN fixture=exhausted_output_unchanged THEN rc=0`（產出檔存在且自查核後未變動）
  - `ASSERT bash scripts/debt_clear.sh --abandon --round-id R --kind collection-failed --reason r --approver a WHEN fixture=exhausted_output_hardlink_swapped THEN rc!=0`（查核後換成同 bytes 之另一物件）
  - `ASSERT bash scripts/debt_clear.sh --abandon --round-id R --kind collection-failed --reason r --approver a WHEN fixture=exhausted_output_appended_after_fstat THEN rc!=0`（以僅限測試之掛鉤於 fstat 後、讀取前追加）
  - `ASSERT bash scripts/debt_clear.sh --abandon --round-id R --kind collection-failed --reason r --approver a WHEN fixture=exhausted_output_fifo THEN rc!=0`（須於 60 秒內返回；阻塞即視為失敗）
  - `ASSERT bash scripts/debt_clear.sh --abandon --round-id R --kind collection-failed --reason r --approver a WHEN fixture=exhausted_output_appended_after_read THEN rc!=0`（以僅限測試之掛鉤於讀到 EOF 後追加）
  - `ASSERT bash scripts/debt_clear.sh --abandon --round-id R --kind collection-failed --reason r --approver a WHEN fixture=exhausted_output_parent_swapped_after_read THEN rc!=0`（讀完後父目錄換成指向同 bytes 他物件之 symlink）
  - `ASSERT bash scripts/audit_append.sh --require-file-path P --require-file-sha256 H --require-file-dev-ino D --require-round-unchanged R@S --event debt_abandon WHEN fixture=read_hook_without_harness THEN rc=2`
  - `ASSERT bash scripts/audit_append.sh --require-file-path P --require-file-sha256 H --require-round-unchanged R@S --event debt_abandon WHEN fixture=no_dev_ino_flag THEN rc=2`
  - `ASSERT bash scripts/audit_append.sh --require-file-path P --require-file-sha256 H --require-file-dev-ino D --require-round-unchanged R@S --event debt_abandon WHEN fixture=fstat_hook_without_harness THEN rc=2`
...
- **邊界**：①多家皆耗盡 ⇒ 任一家符合即可棄置；②棄置後同 session 名不得重用（既有 `session_name_check.sh`），新輪須新 session；③快照後有其他輪之事件 ⇒ 不影響寫入；④查核當下無產出檔（`snapshot_output_dev_ino=none`）而寫入前被建出 ⇒ 拒寫；⑤查核後產出檔被換成 FIFO 或目錄 ⇒ 立即 rc=1，不得阻塞於開檔。
- **存活至**：全票完工後常設。
- **覆蓋風險**：無。
