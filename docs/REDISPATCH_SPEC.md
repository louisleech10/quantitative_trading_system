...
- 改法：
  ...
  4. 產出檔綁定（`audit_append.sh` 之 `--require-file-path <path>`、`--require-file-sha256 <sha256|none>`、`--require-file-dev-ino <dev>:<ino>|none`）：三者須同時給定且須併 `--require-round-unchanged`，缺一 ⇒ rc=2；路徑套用與 `path_token_ok` 同一文法，違反 ⇒ rc=2。鎖內重驗於 `_append_with_round_unchanged_guard` 內、`append` 之前執行：以 `O_RDONLY|O_NOFOLLOW|O_NONBLOCK` 開檔（`ELOOP` ⇒ rc=1），其後只用同一個 fd——非一般檔 ⇒ rc=1；`dev:ino` 不等於快照值 ⇒ rc=1；讀取到 EOF（不以 `st_size` 為界）後雜湊不等於快照值 ⇒ rc=1。`_emit_abandon` 於有產出路徑與雜湊卻無 `snapshot_output_dev_ino` 時 ⇒ 不降級為只綁雜湊，直接失敗。讀到 EOF 並比對雜湊後，最終確認**只做一次且為本函式最後一個動作**：以 `O_RDONLY|O_NOFOLLOW|O_NONBLOCK` 重開同一路徑取得新 fd（重開失敗 ⇒ rc=1），於**同一個新 fd** 上依序判定 `dev:ino` 不等於原 fd ⇒ rc=1、長度不等於實讀位元組數或 `mtime_ns` 改變 ⇒ rc=1。🔴 **不得拆成前後兩道**（先以舊 fd 驗長度、再重開驗身分）：兩道之間存在可用窗口，同長度覆寫落於其間時前一道已過、後一道只看身分，兩道皆不拒。殘餘窗口見 §C 誠實邊界⑧。**每個 rc=1 出口另於 stderr 印可區分之原因碼** `RD_GUARD_REASON=<code>`，值域＝`missing`／`symlink`／`not_regular`／`object_changed`／`hash_mismatch`／`size_or_mtime_changed`／`path_reopen_failed`／`path_object_changed`；**驗收測試須斷言原因碼而非只斷言 rc**——後加之守衛會對同一情境給出相同 rc 而遮蔽先前守衛，只比 rc 時被遮蔽者之 mutation 不再轉紅（＝假防護）。
- **驗證**（`venv/bin/python -m pytest tests/governance/test_redispatch.py -q` 0 failed，其中下列斷言各為一條具名測試；佔位值依 §V 對照表替換）：
