...
版本：**DRAFT R2**（2026-08-18；R7 TODO adversarial 三家 15 findings 全部寫回：gate 分跑、`summary_by_feature`＋root 注入 OOS、兩插入點＋`_in_fallback_rerun`、persist 顯式 kwargs＋`_features_path`、B5 toggle 具名 preset＋FeatureTierPanel、文案子字串、bench 觀測降級、mutation 唯一對映；SPEC 義務側擴張走延伸檔 A1-1／A1-2；待 R8 複核）｜基於 SPEC：`docs/GAP2_MARGINAL_IC_SPEC.md`（**R7 FROZEN**，2026-08-18 使用者白話閘核准；六輪三家 adversarial 收斂檔 `handoffs/reconcile/20260818-gap2-x-review-r{1..6}/synth.md` 皆三家 RECONCILE-STAMP）
...
- **批次間 Gate**：B1→B2：`pytest tests/momentum/Analysis/test_survivor_contract.py -k load -q` rc=0 **且** `pytest tests/momentum/Analysis/test_marginal_ic.py -q` rc=0（兩條分開跑；R7 COMPOSER-R7-P1-01／CODEX-R7-P1-04：共用 `-k` 會把後者整檔 deselect） ＋ `bash scripts/gap2_mutation_probe.sh --batch B1` rc=0 ＋ 三家 review CLOSED＋戳記；
...
### Phase B1 測試 ＋ Gate
- `pytest tests/momentum/Analysis/test_survivor_contract.py -k load -q` rc=0 **且** `pytest tests/momentum/Analysis/test_marginal_ic.py -q` rc=0（分開跑）；`bash scripts/mutation_probe_check.sh` 對新檔綠；`bash scripts/gap2_mutation_probe.sh --batch B1` rc=0；三家 code review CLOSED＋戳記。
...
- 實作要點：
  ...
  3. bench：合成 k=200 survivors＋200 removed、n=20000（seed 20260818）跑 `compute_marginal_ic`＋`combine_factors`；記 wall time、`resource.getrusage(RUSAGE_SELF).ru_maxrss` 至 receipt；斷言 `n_regressions==600`。**明示（R7 CODEX-R7-P1-03）**：receipt 為**觀測資料**、無 wall time／RSS 通過閾值；本票對 OOM 的保護宣稱**僅**＝迴歸次數計數上界（`≤600`、每次 lstsq `n×≤201`），**不**宣稱資源絕對上限（上限須有核准來源，列為觀測供日後定閾）。
