...
版本：**DRAFT R3**（2026-08-18；R8 TODO adversarial 三家 15 findings 十群集：14 接受寫回、1 駁回（CODEX-R8-P1-06 警語子字串，碼證於 `handoffs/reconcile/20260818-gap2-x-review-r8/synth.md` U6）：B5 白名單三檔（A1-4）、刪 4.1 `pass_class` 推導殘句、golden pre `case_id`＋`report_ref` 斷言、4.2 persist 顯式 kwargs＋三 caller 來源、`persist_suppressed` 五鍵 object、bench `fit_projection` spy、xsec N/A 節＋reporter 透傳、B1 gate 探針命令明列路徑＋每檔 `test_mutation_*` 名、「四處」→兩插入點；R7：gate 分跑、`summary_by_feature`＋root 注入 OOS、兩插入點＋`_in_fallback_rerun`、persist 顯式 kwargs＋`_features_path`、B5 toggle 具名 preset＋FeatureTierPanel、文案子字串、bench 觀測降級、mutation 唯一對映；SPEC 義務側擴張走延伸檔 A1-1..A1-4；待 R9 複核）｜基於 SPEC：`docs/GAP2_MARGINAL_IC_SPEC.md`（**R7 FROZEN**，2026-08-18 使用者白話閘核准；六輪三家 adversarial 收斂檔 `handoffs/reconcile/20260818-gap2-x-review-r{1..6}/synth.md` 皆三家 RECONCILE-STAMP）
...
- 修改檔案：既有三檔＝**A1-4 白名單 §C#6**（`types.ts`／`icAnalysisStore.ts`／`FeatureTierPanel.tsx`；R8 CODEX-R8-P1-01／GROK-R8-P0-01：SPEC 原只列 `types.ts`，已走延伸檔擴為三檔）＋新增 `MarginalICTable.tsx`／`MarginalICTable.test.tsx`；後端 `_apply_tier_config` 於 Task 4.1（白名單①）　既有 caller：`scripts/ic_wiring_check.py` R1a／R1b（新 toggle 須映射至後端 `STAGE_OVERRIDE_PATHS`，Task 4.1 已加）；`npm run build`。
...
| §C 白名單 7 處（#6 依 A1-4 擴三檔） | 唯此 | §0；Task 5.1 |
...
## 階段 4 handoff
`SPEC=docs/GAP2_MARGINAL_IC_SPEC.md AMEND=docs/GAP2_MARGINAL_IC_AMENDMENTS.md TODO=docs/GAP2_MARGINAL_IC_TODO.md FOCUS=TODO R9 複核（R8 十群集寫回逐條核對；U6 駁回由 codex 重跑 grep 確認；可 Frozen？）`
