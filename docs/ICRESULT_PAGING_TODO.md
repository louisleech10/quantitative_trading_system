# ICRESULT_PAGING TODO　（DRAFT／基於 `docs/ICRESULT_PAGING_SPEC.md`（R2 修訂）／2026-09-09）
...
- 批次 Gate：B0 ⇒ `tests/golden/icresult_paging/full_report_projection.json` 存在且 `handoffs/<date>-probe-icresult-golden.py --check` rc=0；B1 ⇒ `pytest tests/api/test_icresult_paging.py` rc=0＋G-1～G-4／G-6～G-8（repo fixture）＋G-5 探針三態轉印（`SIZE=PASS|FAIL|BLOCKED`；BLOCKED 不假綠不紅）＋`pytest tests/momentum/Analysis/test_gap2_golden.py tests/api/test_survivor_contract.py` rc=0＋mutation P1–P11 紅／C0 綠（`scripts/icresult_paging_phase_gate.sh 1`）；B2 ⇒ vitest（含 page 整合）rc=0＋tsc `error TS` == 8；B3 ⇒ 使用者簽 B34＋`SIZE=PASS`。
...
- 實作要點：①helper 在既有 lock 內呼叫（不另加鎖）；⓪`_snapshot_result(task_id) -> tuple[dict, int] | None` 於 lock 內同時取 `(result, result_revision)`，所有投影只吃 snapshot（R2 `CODEX-R2-P1-02`）；②源碼守衛測試：`re.findall(r'task_info\["result"\] = ', src)` 計數 == 0（只准 helper 內 `task_info["result"] = report` 一處，以 `# revision-helper` 註記排除）；③refilter 失敗路徑不呼叫 helper。
...
- 風險緩解：SPEC §C-7。
- 驗證：`venv/bin/python -m pytest tests/api/test_icresult_paging.py -k revision -q` rc=0；G-7a（TestClient：page1 → refilter → page2 帶舊 `revision` ⇒ status == 409 且 body `current_revision` == 2；不帶 revision ⇒ 200 且 `result_revision` == 2）；G-7b（monkeypatch `paginate_summary` 於投影中觸發 `_set_result` ⇒ 回應 `result_revision` == 1，不混代）；源碼守衛計數 == 0。
- **存活至**：永久。
- **覆蓋風險**：無。
...
- 檔案：`api/services/ic_result_projection.py::project_light_view(report: dict, contract: dict) -> dict`（`drop_sections` 各段 pop；`metadata={k: m[k] for k in keep_keys if k in m}`；`_collections_to_counts(node)`＝對 `collection_to_count_paths` 每個節點，list／dict 值改 `f"{k}_count": len(v)`、標量原樣；`summary_page=paginate_summary(rows, sort_by="icir", order="desc", offset=0, limit=limit_default)`；`filter_log_funnel=funnel_from_filter_log(report["filter_log"], contract["funnel_stage_adapter"])`＝每 stage `{input, output}`，候選鍵序取第一存在者（int 原樣、list／dict 取 len、皆缺 ⇒ null））；`api/routes/ic_analysis.py::refilter` 加 `view: Optional[Literal["light"]]`（`light` ⇒ 回 `project_light_view(snapshot)`；無 ⇒ 既有全量不變）；`ic_analysis_service.get_result(task_id, schema_version, view)`（`view=="light"` 且 `schema_version==2` ⇒ `ValueError` → route 400）；route `get_result(view: Optional[Literal["light"]] = Query(None))`。
...
- 風險緩解：SPEC §G G-1。
- 驗證：`venv/bin/python -m pytest tests/api/test_icresult_paging.py -k light_view -q` rc=0；G-1（預設回應 **raw body sha256** == golden `raw_body_sha256`，canonical 亦 ==）；G-4a–e（七段缺席；`metadata` ⊆ keep_keys 且逐鍵 ==；`filter_log` == golden `filter_log_light`；其餘頂層段逐鍵 ==；`summary_page` == `paginate_summary(icir,desc,0,50)`）；G-7c（`POST /refilter?task_id=&view=light` ⇒ 200、`view=="light"`、`result_revision` 遞增；無 `view` ⇒ 回應 raw body 與改前既有 refilter 測試斷言一致）；G-8（shape fixture 六 stage ⇒ `filter_log_funnel` == golden：`stage0_ingestion {input:int,output:null}`、`feature_filter {int,int}`、`stage1_preprocessing {null,null}`、`stage5_thresholds {int,int(len dict)}`、`stage6_redundancy {int,int}`）；G-5 **不在 pytest**：`venv/bin/python handoffs/<date>-probe-icresult-size.py` ⇒ `SIZE_GATE=PASS`（light ≤ 262144、summary50 ≤ 102400、feature ≤ 2097152）或 artifact 缺席 ⇒ `BLOCKED` rc=2；receipt `handoffs/run_receipts/icresult_size_budget.log`；`venv/bin/python -m pytest tests/momentum/Analysis/test_gap2_golden.py tests/api/test_survivor_contract.py -q` rc=0。
- **存活至**：永久。
- **覆蓋風險**：無。
...
- 實作要點：①`featureDetail` 未到 ⇒ 圖表「載入中」不填空陣列；②`FilterFunnelChart` 改讀 `reportLight.filter_log_funnel`（`null` ⇒ 該 stage 顯示「不適用」，不補 0；`types.ts::FilterLogStage` 改 `{input: number|null, output: number|null}`）；③匯出邏輯零改動；④page 整合測試（`frontend/src/app/ic-analysis/page.test.tsx`）：light 報告＋`featureDetail=null` ⇒ 六圖區「載入中」不 throw；golden 單特徵 detail ⇒ 六圖渲染；funnel 含 null ⇒ 不適用文案。
...
- 風險緩解：⊘。
- 驗證：`(cd frontend && npx vitest run src/components/ic-analysis src/app)` rc=0（既有斷言不改）；新測：page 整合三案例（載入中／六圖渲染／漏斗不適用）；`npx tsc --noEmit -p tsconfig.json` `error TS` == 8；後端 `venv/bin/python -m pytest tests/api -k "export" -q` rc=0（匯出既有測試不改）。
- **存活至**：永久。
- **覆蓋風險**：無。
...
- 風險緩解：⊘。
- 驗證：使用者實機簽 B34（`blocked-by:使用者`）；Claude 端 `venv/bin/python handoffs/<date>-probe-icresult-size.py` rc=0 印 `SIZE_GATE=PASS` 且 receipt `handoffs/run_receipts/icresult_size_budget.log` 含 `light_bytes=` ≤ 262144。
- **存活至**：永久。
- **覆蓋風險**：無。

## 追溯（SPEC ID → TODO）
Task 1.0→1.0；1.1→1.1；1.2→1.2；1.3→1.3；2.1→2.1；2.2→2.2；2.3→2.3；3.1→3.1；§G G-1～G-7→0.1＋1.0～1.3；§C-6～8（投影規則／世代戳／排序契約）→0.1 contract＋1.0／1.1／1.3；§C-6(iv) 漏斗 adapter→0.1 contract＋1.3＋2.3（G-8）；§V P1–P11／C0→Phase 1 Gate；§N IP-RESID-1～4→不入 Task（登記處 `docs/IC_QUANT_GAP_REGISTRY.md`，由 B3 收案時同步；原 IP-RESID-4 漏斗已收回為 Task）。合計：Task 8／Golden 8／mutation 12／殘留 4。
