# ICRESULT_PAGING TODO　（DRAFT／基於 `docs/ICRESULT_PAGING_SPEC.md`（R4 修訂）／2026-09-09）
...
- 批次 Gate：B0 ⇒ `tests/golden/icresult_paging/full_report_projection.json` 存在且 `handoffs/<date>-probe-icresult-golden.py --check` rc=0；B1 ⇒ `pytest tests/api/test_icresult_paging.py` rc=0＋G-1～G-4／G-6～G-8（repo fixture）＋G-5 探針唯一文法 `SIZE_GATE=PASS|FAIL|BLOCKED`（gate：FAIL ⇒ rc=1；BLOCKED ⇒ 轉印不改 rc；缺行／未知 ⇒ rc=1）＋`pytest tests/momentum/Analysis/test_gap2_golden.py tests/api/test_survivor_contract.py` rc=0＋mutation P1–P16 紅／C0 綠（`scripts/icresult_paging_phase_gate.sh 1`）；B2 ⇒ vitest（含 page 整合）rc=0＋tsc `error TS` == 8；B3 ⇒ 使用者簽 B34＋`SIZE_GATE=PASS`。
...
- 目標：`task_info["result"]` 每個寫點經單一 helper 遞增 `result_revision`，並於寫入時一次 `_to_json_compatible`＋`deny_factor_in_ok_oos` 存 `result_normalized`（SPEC §C-7；light／summary／feature 只讀它）；`/task/{id}` 回 `result_revision`。
- 檔案：`api/services/ic_analysis_service.py::_set_result(task_info: dict, report: Any) -> int`（先 `normalized = _to_json_compatible(report)`、`deny_factor_in_ok_oos(normalized)`（raise ⇒ 不寫、不遞增、上層標 failed），再寫 `result`／`result_normalized`／`result_revision = (task_info.get("result_revision") or 0) + 1`，回新值；此步在背景任務執行緒內做，不在請求路徑）；三既有寫點 `:1623`／`:2339`／`:2631` 改呼叫 helper；`get_task_status` 回 `result_revision`；`api/models/ic_models.py::ICTaskStatusResponse.result_revision: Optional[int] = None`。
- 實作要點：①helper 在既有 lock 內呼叫（不另加鎖）；⓪`_snapshot_result(task_id) -> tuple[dict, int] | None` 於 lock 內同時取 `(result_normalized, result_revision)`，所有投影只吃 snapshot，**不得**再呼叫 `_to_json_compatible`／`deny_factor_in_ok_oos`（單元測試 spy 兩者於 light／summary／feature 請求呼叫 == 0；R4 `CODEX-R4-P1-01`）；預設 `/result` 路徑不變；②源碼守衛測試以 `ast` 走訪 `ic_analysis_service.py`：對 `task_info["result"]` 之 Subscript 賦值，位於 `_set_result` 函式體內者 == 1、其餘 == 0（不用註記排除；R3 `CODEX-R3-P1-01`）；③refilter 失敗路徑不呼叫 helper。
...
- 邊界：①未完成 ⇒ `result_revision=None`；②refilter 失敗不遞增；③兩次 refilter ⇒ 3；④守衛 raise ⇒ 不寫入、revision 不變。
- 風險緩解：SPEC §C-7。
...
- 實作要點：⓪排序索引 LRU 快取：key `(task_id, result_revision, sort_by, sort_order, pass_class, search)` → `numpy.int32` index 陣列（39k ≈ 157 KB），上限 8 組／task、process-wide 32 task（最舊 task 整組淘汰）、`_set_result` 時整 task 失效；提供 `cache_bytes()` 供測試（SPEC §C-9；R4 `CODEX-R4-P1-04`）；①`sort_order` 只准 contract `sort_order_values`；②`pass_class` 精確匹配、`search` 為 `feature_name.lower()` 子字串；③`total` 為篩選後列數；④`rows` 為原 dict 不裁欄（前端表格已消費全部欄）；⑤不用 `_finite_or_neg_inf`（它是單向 `-inf`，asc 會置頂）；comparator 自 contract 產生；⑥回應含 `result_revision`。
...
## 追溯（SPEC ID → TODO）
Task 1.0→1.0；1.1→1.1；1.2→1.2；1.3→1.3；2.1→2.1；2.2→2.2；2.3→2.3；3.1→3.1；§G G-1～G-7→0.1＋1.0～1.3；§C-6～8（投影規則／世代戳／排序契約）→0.1 contract＋1.0／1.1／1.3；§C-6(iv) 漏斗 adapter→0.1 contract＋1.3＋2.3（G-8）；§V P1–P16／C0→Phase 1 Gate；§C-9／10（效能與體驗預算）→1.1 快取＋2.2 UX＋3.1 G-9；§N IP-RESID-1～4→不入 Task（登記處 `docs/IC_QUANT_GAP_REGISTRY.md`，由 B3 收案時同步；原 IP-RESID-4 漏斗已收回為 Task）。合計：Task 8／Golden 9／mutation 17／殘留 5。
