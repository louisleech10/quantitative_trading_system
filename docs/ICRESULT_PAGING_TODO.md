# ICRESULT_PAGING TODO　（DRAFT／基於 `docs/ICRESULT_PAGING_SPEC.md`（R6 修訂）／2026-09-09）
...
- 不可違反原則：不擅改輸出大小（落檔）；不弱化任何 gate；`deny_factor_in_ok_oos` 出口守衛於 `_set_result` **寫入時一次**（＝所有視圖之前）；投影請求路徑不得再呼叫。
...
- 批次 Gate：B0 ⇒ `tests/golden/icresult_paging/full_report_projection.json` 存在且 `handoffs/<date>-probe-icresult-golden.py --check` rc=0；B1 ⇒ `bash scripts/icresult_paging_phase_gate.sh 1`（含 `pytest tests/api/test_icresult_paging.py` rc=0＋G-1～G-4／G-6～G-8＋探針 `--size` 之 `SIZE_GATE` 解析：FAIL ⇒ rc=1；BLOCKED ⇒ 轉印不改 rc；缺行／未知 ⇒ rc=1）＋`pytest tests/momentum/Analysis/test_gap2_golden.py tests/api/test_survivor_contract.py` rc=0＋mutation P1–P16 紅／C0 綠（`scripts/icresult_paging_phase_gate.sh 1`）；B2 ⇒ vitest（含 page 整合）rc=0＋tsc `error TS` == 8；B3 ⇒ 使用者簽 B34＋`bash scripts/icresult_paging_phase_gate.sh 3`（探針 `--size --latency`，`SIZE_GATE=PASS` 且 `LATENCY_GATE=PASS`，同一 parser；R5 `CODEX-R5-P1-03`）。
...
- 目標：`task_info["result"]` 每個寫點經單一 helper 遞增 `result_revision`，並於寫入時一次 `_to_json_compatible`＋`deny_factor_in_ok_oos` 後**只保留**該 normalized 樹於 `task_info["result"]`（SPEC §C-7 單一樹；light／summary／feature 經 `_snapshot_result` 讀它）；`/task/{id}` 回 `result_revision`。
- 檔案：`api/services/ic_analysis_service.py::_set_result(task_info: dict, report: Any) -> int`（先 `normalized = _to_json_compatible(report)`、`deny_factor_in_ok_oos(normalized)`（raise ⇒ 不寫、不遞增、上層標 failed），再**只**寫 `task_info["result"] = normalized`（單一樹，不存第二份；R5 `CODEX-R5-P1-01`）與 `result_revision = (task_info.get("result_revision") or 0) + 1`，回新值；此步在背景任務執行緒內做，不在請求路徑；失敗語意：初次完成 raise ⇒ task `failed`、無 result；refilter raise ⇒ 先驗後寫、舊 result／revision 不變、task 仍 completed、route 回 422（R5 `CODEX-R5-P1-02`））；三既有寫點 `:1623`／`:2339`／`:2631` 改呼叫 helper；`get_task_status` 回 `result_revision`；`api/models/ic_models.py::ICTaskStatusResponse.result_revision: Optional[int] = None`。
- 實作要點：①helper 在既有 lock 內呼叫（不另加鎖）；⓪`_snapshot_result(task_id) -> tuple[Mapping, int] | None` 於 lock 內同時取 `(task_info["result"]（已 normalized）, result_revision)`，所有投影只吃 snapshot，**不得**再呼叫 `_to_json_compatible`／`deny_factor_in_ok_oos`（單元測試 spy 兩者於 light／summary／feature 請求呼叫 == 0；R4 `CODEX-R4-P1-01`）；預設 `/result` 路徑不變；②源碼守衛測試以 `ast` 走訪 `ic_analysis_service.py`：對 `task_info["result"]` 之 Subscript 賦值，位於 `_set_result` 函式體內者 == 1、其餘 == 0（不用註記排除；R3 `CODEX-R3-P1-01`）；③refilter 失敗路徑不呼叫 helper。
...
- 邊界：①未完成 ⇒ `result_revision=None`；②refilter 失敗不遞增；③兩次 refilter ⇒ 3；④初次完成守衛 raise ⇒ task failed、無 result；⑤refilter 守衛 raise ⇒ 舊 result／revision 不變、completed、422；⑥`_to_json_compatible(normalized) == normalized`（冪等，預設 `/result` 讀取碼不改仍 G-1）。
- 風險緩解：SPEC §C-7。
- 驗證：`venv/bin/python -m pytest tests/api/test_icresult_paging.py -k revision -q` rc=0；G-7a（TestClient：page1 → refilter → page2 帶舊 `revision` ⇒ status == 409 且 body `current_revision` == 2；不帶 revision ⇒ 200 且 `result_revision` == 2）；G-7b（monkeypatch `paginate_summary` 於投影中觸發 `_set_result` 寫入 sentinel 新報告（`feature_name` 前綴 `NEW__`、不同 total）⇒ 回應 `result_revision` == 1 且 `total` == 舊值且所有列 `feature_name` 不含 `NEW__`；`feature/{name}` 同法各段 sha == 舊世代）；AST 守衛 helper 內 == 1、helper 外 == 0；TestClient：初次完成守衛注入 ⇒ `/task` status == failed 且 `/result` 404；refilter 守衛注入 ⇒ 422、`/result` 仍舊 revision；多 task memory receipt（5 個 fake 39k task 後 RSS 增量 ≤ 5×單樹＋快取上限，寫 `handoffs/run_receipts/icresult_memory.log`）。
- **存活至**：永久。
- **覆蓋風險**：無。

### Task 1.1 — summary 分頁端點（`SPEC Task 1.1`）
- 目標：`GET /result/{task_id}/summary` 回分頁；純函式 `paginate_summary`。
- 檔案：`api/services/ic_result_projection.py`（**新模組**，純函式：`paginate_summary(rows: list[dict], *, sort_by: str, sort_order: str, offset: int, limit: int, pass_class: str|None, search: str|None, contract: dict) -> dict`（命名對齊 FF `/browse/{task}/features`）；`sort_key(row, field, sort_order)` 依 contract `sort_policy`：缺值（None／NaN／±inf）**兩向沉底**（回 `(1, 0, name)`），主鍵 `(0, ±value, name)`，`feature_name` 純字串序；`revision` 參數不符 ⇒ `RevisionMismatch` → route 409）；`api/services/ic_analysis_service.py::ICAnalysisService.get_result_summary_page(task_id, **params)`（經 `_snapshot_result` 取 `(normalized, revision)`，lock 外投影；不呼叫 `_to_json_compatible`／`deny_factor_in_ok_oos`；R5 `GROK-R5-P1-01`）；`api/routes/ic_analysis.py::get_result_summary_page`（Query 驗證：`limit` clamp、`sort_by` 非白名單 ⇒ `HTTPException(400)`）；`api/models/ic_models.py::ICSummaryPageResponse`。
- 實作要點：⓪排序索引 LRU 快取（module-level 單例）：key `(task_id, result_revision, sort_by, sort_order, pass_class, search)` → `numpy.int32` index 陣列；每 task ≤ 8 組（第 9 組淘汰該 task LRU 一組）、process-wide 32 task（第 33 個淘汰最舊 task 整組）、`_set_result` 時整 task 失效；`cache_bytes()`＝Σ `arr.nbytes`（不計 dict 開銷）供測試（SPEC §C-9；R4 `CODEX-R4-P1-04`、R5 `CODEX-R5-P2-02`）；①`sort_order` 只准 contract `sort_order_values`；②`pass_class` 精確匹配、`search` 為 `feature_name.lower()` 子字串；③`total` 為篩選後列數；④`rows` 為原 dict 不裁欄（前端表格已消費全部欄）；⑤不用 `_finite_or_neg_inf`（它是單向 `-inf`，asc 會置頂）；comparator 自 contract 產生；⑥回應含 `result_revision`。
...
- 風險緩解：SPEC §C-4 白名單。
- 驗證：`venv/bin/python -m pytest tests/api/test_icresult_paging.py -k summary_page -q` rc=0；G-2a（無篩選逐頁 `limit=limit_max` 串接後依 `feature_name` 排序 == golden 全量列，列數 == `summary_rows`）；G-2b（`search=close` 逐頁 == 單次 `paginate_summary(limit=10**9)`）；G-6（desc／asc 首 5 列 == `sort_golden`；4 列並列 fixture desc == `[A,B,C,D]`、asc == `[A,B,C,D]`（缺值兩向沉底、並列以名升冪）；3 列 fixture desc == `[B,A,C]`、asc == `[A,B,C]`）；TestClient `sort_by=evil` ⇒ status == 400；`limit=99999` ⇒ 回應 `limit == limit_max`；快取：同參數第二次 `sorted` spy 呼叫 == 1、refilter 後 == 2；容量：33 個 fake task 各 8 組後 `cache_bytes()` ≤ 32×8×`n×4` 且最舊 task 索引缺席；單 task 第 9 組 ⇒ 該 task 仍 8 組且最舊一組缺席；兩個 `ICAnalysisService` instance 共用同一快取（module 單例）；G-9（探針 `--latency` 對 39k，量測規則見 SPEC G-9：warm-up 3＋20 次、p95＝第 19 小、固定請求集；light ≤ 150／300 ms、summary ≤ 100／200 ms、feature ≤ 50／100 ms p50／p95，快取命中 ≤ 20 ms；stdout 恰一行 `LATENCY_GATE=PASS|FAIL|BLOCKED`＋逐項 `LATENCY_<name>_p50_ms=`／`_p95_ms=`；artifact 缺席 ⇒ BLOCKED rc=2）。
- **存活至**：永久。
- **覆蓋風險**：無。
...
- 檔案：`api/services/ic_result_projection.py::project_light_view(report: dict, contract: dict) -> dict`（`drop_sections` 各段以鍵過濾建新 dict（**禁 pop**）；`metadata={k: m[k] for k in keep_keys if k in m}`；`_collections_to_counts(node)`＝對 `collection_to_count_paths` 每個目標節點**先 `dict(node)` 建 light 私有副本**（不 deepcopy 整樹，只複製目標 stage／selection_scope 這一層），再於副本把 list／dict 值改 `f"{k}_count": len(v)`、標量原樣；**source 節點不得被改**（R6 `GROK-R6-P2-01`）；**先**算 `filter_log_funnel=funnel_from_filter_log(report["filter_log"], contract["funnel_stage_adapter"])`（對**原始** filter_log；每 stage `{input, output}`：候選鍵序取第一存在者，int 原樣、dict 含 `count` 取之、其他 dict／list 取 len、皆缺 ⇒ null；R3 `GROK-R3-P1-01`），**再**對 light 的 `filter_log` 做計數；`summary_page=paginate_summary(rows, sort_by="icir", sort_order="desc", offset=0, limit=limit_default)`）；`api/routes/ic_analysis.py::refilter` 加 `view: Optional[Literal["light"]]`（`light` ⇒ 回 `project_light_view(snapshot)`；無 ⇒ 既有全量不變）；`ic_analysis_service.get_result(task_id, schema_version, view)`（`view=="light"` 且 `schema_version==2` ⇒ `ValueError` → route 400）；route `get_result(view: Optional[Literal["light"]] = Query(None))`。
- 實作要點：①守衛已於 `_set_result`，投影**不**再呼叫（spy == 0；R5 `GROK-R5-P1-02`）；投影禁 `pop`／就地改寫，一律建新 dict（snapshot 不可變測試：light×2 → summary → feature，七段仍在、`filter_log`／`metadata.selection_scope` 原始集合鍵仍在且整棵 snapshot deep-equal 於投影前快照（`copy.deepcopy` 對照）；R5 `CODEX-R5-P1-04`）；②`{"raw": …}` 非 dict ⇒ 400；③`marginal_ic`／`correlation_matrix`／`cross_sectional_*`／`module_statuses`／`analysis_status`／`oos_guarantees`／`version`／`diversification_metrics` 原樣；④`view=light`＋`schema_version=2` ⇒ 400；v2 flag on／off × view 有／無 四格矩陣測試（IP-RESID-3 並存契約）；⑤`_collections_to_counts` 依 contract 路徑規格：`*` 一層、目標須 dict、非 dict／缺席 no-op、更深層不動。
...
- 檔案：`frontend/src/hooks/useICAnalysis.ts`（`fetchResult`：`requestJson<ICReportLight>(`/result/${taskId}?view=light`)`；回應 `view !== 'light'` ⇒ `setError('後端版本過舊：未回 light 視圖')` 不 setReport；`fetchSummaryPage(params: SummaryPageParams)`／`fetchFeatureDetail(name)` 皆帶 `revision=resultRevision`＋`AbortController`（hook **不**去抖；搜尋 300 ms 去抖 owner＝表格搜尋框，R5 `CODEX-R5-P2-01`）；409 ⇒ 以 `current_revision` 更新 store 後重拉一次，再 409 ⇒ setError 不迴圈）；`frontend/src/store/icAnalysisStore.ts`（`report: ICReportLight|null`、`summaryPage`、`summaryParams`、`featureDetail`、`selectedFeatureSet: Set<string>`；`setSelectedFeature` 觸發 detail 拉取由 hook 做）；`frontend/src/lib/types.ts`（`ICReportLight = Omit<ICReport, 'summary_table'|'ic_decay'|'quantile_returns'|'turnover_analysis'|'coverage_analysis'|'rolling_ic_series'|'grouped_ic'> & {view:'light'; total_features:number; result_revision:number; summary_page: ICSummaryPage; filter_log_funnel: Record<string,{input:number|null; output:number|null}>}`（Omit 與 contract `drop_sections` 七段對齊；R5 `GROK-R5-P2-01`）、`ICSummaryPage`、`ICFeatureDetail`、`SummaryPageParams`）。
...
- 檔案：`frontend/src/components/ic-analysis/ICSummaryTable.tsx`（props 改 `page: ICSummaryPage`、`params`、`onParamsChange`；移除 `sortedData` useMemo；`SortButton` ⇒ `onParamsChange({sort_by, sort_order, offset:0})`（typed `SummaryPageParams`，無 `order` 鍵；R4 三家）；分頁列：上一頁／下一頁／每頁 50|100|200／「第 x–y 列，共 total」；勾選 `selectedFeatureSet.has`）；`frontend/src/app/ic-analysis/page.tsx`（`summaryTable` 消費者改 `summaryPage.rows`；全選＝當頁）。
- 實作要點：①`limit` 選項 ≤ contract `limit_max`；②搜尋框 300 ms 去抖 → `search`；③`activeFeature` 預設＝首頁第一列；④翻頁時 skeleton 列覆蓋、舊列保留到新列到達（不閃白）；搜尋框為唯一 300 ms 去抖點（fake timer 測試：連打 5 字 ⇒ 一次延遲、一次 `onParamsChange`）；⑤勾選 Set 跨頁保留；⑥`page`／`limit`／`sort_by`／`sort_order`／`search` 同步到 URL query（`useSearchParams`），重整還原；⑦請求失敗 ⇒ 列內錯誤＋重試按鈕（SPEC §C-10）。
...
- 檔案：`frontend/src/app/ic-analysis/page.tsx::sectionSplit`（改讀 `featureDetail.{ic_decay,quantile_returns,grouped_ic,turnover_analysis,rolling_ic_series,coverage_analysis}`；`RollingICChart` 之 `series` 改 `featureDetail.rolling_ic_series`（現行 `:880-882` 讀 `report?.rolling_ic_series?.[activeFeature]`，light 已刪此段；R4 `CODEX-R4-P1-02`）；SectionStatus 透傳語意不變）；`lib/types.ts::ICFeatureDetail` 明列六段；`ICDecayChart`／`QuantileReturnChart`／`TurnoverTimeSeriesChart`／`RollingICChart`／`GroupedICBarChart`／`RegimeRadarChart` props 改單特徵資料；`FilterFunnelChart`／`DegradedBanner`／`IsolationNote`／`PeriodAlignmentBanner`／`MarginalICTable`／`CorrelationHeatmap`／`CrossSectionalICHeatmap`／`CrossSymbolValidationPanel`／`EventTablesPanel`／`EventBatchDisclosurePanel` props 型別放寬；`ExportButtons.tsx` 之 `summaryTable` prop（僅 PNG disable）改 `hasRows: boolean`＝`summaryPage.total > 0`。
- 實作要點：①`featureDetail` 未到 ⇒ 圖表「載入中」不填空陣列；②`FilterFunnelChart` 改讀 `reportLight.filter_log_funnel`（`null` ⇒ 該 stage 顯示「不適用」，不補 0；`types.ts::FilterLogStage` 改 `{input: number|null, output: number|null}`）；③匯出邏輯零改動；④page 整合測試（`frontend/src/app/ic-analysis/page.test.tsx`；fixture＝**不含 `summary_table`** 之 light＋`summary_page{total:39346, rows:50}`）：①`<tr>` == 51；②點排序 ⇒ `fetchSummaryPage` 收 `{sort_by, sort_order, offset:0}`；③點列 ⇒ `fetchFeatureDetail(name)`；④初始 `featureDetail=null` ⇒ 六圖「載入中」不 throw；⑤golden detail（含非空 `rolling_ic_series`）⇒ 六圖渲染且 `RollingICChart` 收到序列；⑥funnel 含 null ⇒ 不適用；⑦切換特徵中 ⇒ 舊 detail 仍在 DOM＋overlay、未清空；⑧mock reject ⇒ 重試按鈕、點擊再呼叫（R3 `CODEX-R3-P1-05`、R4 `CODEX-R4-P1-02`／`GROK-R4-P2-01`／`COMPOSER-R4-P2-02`）；表格與勾選照 FF explorer（`FeatureTimeSeriesChart.tsx` `browseFeatures`）模式。
...
- 風險緩解：⊘。
- 驗證：`(cd frontend && npx vitest run src/components/ic-analysis src/app)` rc=0（既有斷言不改）；新測：page 整合八案例（表格 51 列／排序 callback／detail 呼叫／載入中／六圖含 rolling 渲染／漏斗不適用／切換保留舊圖＋overlay／失敗重試）；`npx tsc --noEmit -p tsconfig.json` `error TS` == 8；後端 `venv/bin/python -m pytest tests/api -k "export" -q` rc=0（匯出既有測試不改）。
- **存活至**：永久。
- **覆蓋風險**：無。
...
## 追溯（SPEC ID → TODO）
Task 1.0→1.0；1.1→1.1；1.2→1.2；1.3→1.3；2.1→2.1；2.2→2.2；2.3→2.3；3.1→3.1；§G G-1～G-7→0.1＋1.0～1.3；§C-6～8（投影規則／世代戳／排序契約）→0.1 contract＋1.0／1.1／1.3；§C-6(iv) 漏斗 adapter→0.1 contract＋1.3＋2.3（G-8）；§V P1–P16／C0→Phase 1 Gate；§C-9／10（效能與體驗預算）→1.1 快取＋2.2 UX＋3.1 G-9；§N IP-RESID-1～4→不入 Task（登記處 `docs/IC_QUANT_GAP_REGISTRY.md`，由 B3 收案時同步；原 IP-RESID-4 漏斗已收回為 Task）。合計：Task 8／Golden 9／mutation 17／殘留 5（R5 修訂）。
