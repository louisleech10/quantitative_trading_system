...
> **狀態（2026-09-10）**：v4；R1/R2/R3 三家 RECONCILE-STAMP APPROVED（`0dbe40ff`）。**Phase 1／Phase 2 FROZEN 且已實作完成**（使用者 9/10 白話閘：「purge 的部分同意」；B1 `0c9cd69d`＋`b59ebcba`、B2 `f5ed2c48`＋`860724f0`＋`5e472611`，各批三家 code review 收斂、gate PASS）；**Phase 3 FROZEN，使用者 9/10 已放行**（逐字：「第 3 段 ok」）⇒ B3 起可動工。全域主線 horizon 靜默取第一個 ⇒ 另開小票 GLOBALH（ROADMAP），不入本票。
...
- **R-9 IC 頁 7 個「純前端」開關為具名 no-op（B1 review R1 `CODEX-R1-P2-04` 查證後登記）** — `為何現在不做: needs-research:ic_method_selection／winsorization_method／monotonicity_test／return_type_selection／ic_autocorrelation／redundancy_method_selection／vif_filter 七者在後端**有底層方法或 config**，但 UI toggle 不改任何 backend 旗標；各自「該不該接線、接哪一個旗標」須逐個定義（有些是方法選擇而非開關，接成 bool 反而錯）`；現況已於 `momentum/Analysis/contracts/ui_stage_toggles.json::ui_only_keys` 具名並由 vitest 擋住「新增開關未歸類」；觸發：使用者發現某開關按了沒反應；登記處：registry。
