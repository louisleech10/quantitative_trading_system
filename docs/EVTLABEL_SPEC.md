...
- 改法：只改 purge 之取值與 metadata 三鍵；`split_context["effective_horizon"]` **不改**（HAC lag 議題見 §N R-1）。
...
- 改法：值域閘（R1 C14c）：`label` 須 finite、整數、精確 ∈ {0,1}、批內無缺值；違反 ⇒ `requested=imported_binary` raise（422）、`requested=auto` ⇒ 不送 binary map 並記 `label_mode_hint=label_invalid_domain`（進 `metadata.label_mode.reason`）。`label` 欄缺（legacy）⇒ `hint=no_label_column`。其餘情況一律送 binary map，由 orchestrator 依 selection scope 決定。
...
- 目標：倖存者逐一以**依賴感知 block permutation**（R1 C10）對 AUC 重驗；置換帶內（不顯著）⇒ 移出倖存者 `removed["permutation_oracle_disagree"]`。整批一次 block 置亂（固定 seed）重跑 Task 3.5＋BH＋效應量閘 ⇒ `n_survivors_shuffled` 寫 `metadata.event_label_rule.negative_control`；`> 0` ⇒ **fail-closed**（R1 C3）：binary 倖存者**不寫** consumable survivor（`survivor_output.status="suppressed"`、`reason="negative_control_failed"`），報告 `degraded`、診斷表保留。
...
- 改法：block 長度 `L = max(1, ceil(label_window_feature_bars / median_event_gap_rows))`，事件依時間順序每 L 個為一 block；`n_blocks < 10` ⇒ 置換 `unavailable:insufficient_blocks`，該特徵不得成為 consumable 倖存者（記 `removed["permutation_unavailable"]`）。預算（R1 C14a）：候選 K 依 `|rb|` desc／`feature_name` 排序；`n_perm = clamp(perm_budget_total // K, 200, 1000)`；receipt（seed、`n_perm`、`L`、`n_blocks`、`first_permutation_digest`）寫 `metadata.event_label_rule.permutation_receipt`。受理 run（h=1、事件間距 ≥1 根 12h）⇒ `L=1`，與普通置換等價。
- **驗證**：`pytest tests/momentum/Analysis/test_evtlabel_oracle.py -q` rc=0：植入特徵 `in_band==False` 留下；置亂 label 之固定 seed fixture（200 合成特徵×165 真實 kline 列）⇒ `n_survivors_shuffled==0` 且 survivor 可寫；強制 `n_survivors_shuffled=1` ⇒ `survivor_output.status=="suppressed"`、`reason=="negative_control_failed"`、無 consumable 檔（M-P3-6）；mutation：monkeypatch `_permute` 為恆等 ⇒ 硬檢 (ii) raise；`L=3` fixture ⇒ 同一 block 內標籤在置換後仍相鄰（block 完整性）；benchmark：K=2000、`perm_budget_total=200000` 於 39,373×165 fixture 耗時 `< 120s` **否則 FAIL**（非只印 receipt）。
...
- **mutation 條件**：RISK-HIT 含 a/d ⇒ 必附。設計（`docs/TEST_DESIGN_CHARTER.md`）：
  ...
  - M-P3-5：stage3 驗後替換 `_ic_cache["event_binary_label"]` 為另一向量 ⇒ stage5 digest 比對 raise、不產 report（R1 C4；驗了＝用的）。
  - M-P3-6：Task 3.7 負對照非零改回 warning-only ⇒ `suppressed_not_consumable` 斷言紅（R1 C3）。
...
- **R-1 事件路徑 HAC lag 仍用主線 `effective_horizon`（5）而非 label 視窗列數（12）；binary MW p 之 iid 假設** — `為何現在不做: needs-research:事件列非等距，rolling IC 序列在受理 run 為空（pooled fallback），HAC 對事件序列之適用性與正確 lag 未定義；MW p 同樣假設事件可交換（R1 C10），本票以 block permutation 作依賴感知放行、MW p 標 iid_events 揭露，dependence-aware 之解析式 p 未定義`；觸發：委員會定出事件序列顯著性估計式（含 binary）；登記處：registry。
