...
- **行為表（＝驗收契約；每列一個 pytest 參數化用例）**：
  ...
  | `## U-01` | **rc==1** | **rc==1** | 🔴 同上 |
  | `## Z-999` | **rc==1** | **rc==1** | 🔴 同上 |
  | `## OUT-OF-SCOPE` | rc==1 | **rc==0** | 🔴 B-39 擴表（2026-08-06 三家裁定 E2b）——結構段名，由 `STRUCT_TOKEN_ALLOWLIST` 放行；實證 2 輪作廢 |
  | `## NON-BLOCKING` | rc==1 | **rc==0** | 🔴 同上；語料 4 次 |
  | `## FACT-RECEIPT` | rc==1 | **rc==0** | 🔴 同上；語料 10 次（最高頻結構段名） |
  | `### G-1 extra` | rc==1 | **rc==0** | 🔴 B-39 擴表——群集代號**帶尾綴**，由 arity 規則（n>1）放行。**與 `## A-1` 的差別只在 token 數**，故上面四列裸標題維持 rc==1 |
  | `### E-1 換行繞道` | rc==1 | **rc==0** | 🔴 同上；與第 153 列裸 `## E-1` 成對，證明 arity 是唯一區分點 |

  🔴 **B-39 擴表紀律（2026-08-06）**：本次**只增放行列，既有 18 列逐字未動**
  （三家一致裁定「零推翻」；`GROK-R2-P0-04`＋`COMPOSER-R2-P1-04`）。
  `## RECONCILE-STAMP`**刻意不加**——實測 47 個 synth 全用 `## 戳記`、**0** 個用該形式，
  戳記本體為行內格式 ⇒ 語料中 8 處為委員誤用，第 142 列成立。

  🔴 **同型 unicode 變體隨 arity 放行，不逐一列表**〔`COMPOSER-R1-P2-04` 建議採納〕：
  例 `### E-1~E-7 逐條 Verdict`（半形 `~`）與第 141 列的全形 `～` 版同屬**帶尾綴結構標題**，
  由 arity 規則（n>1）放行，**不新增對應列**。逐一列舉 unicode 變體即 `票 B-23` 否決的打地鼠。
  **本表列的是判準代表案例，不是窮舉集合。**

- **驗證（可證偽；`pytest tests/governance/test_completeness_idlike_fp.py` rc==0）**：
  上表每列一個用例（**機械計數，禁手寫**），逐列比對 rc；**另加 mutation 用例**（下）。
