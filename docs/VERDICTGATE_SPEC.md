...
> **版本：v9（R8 後修訂；SPEC 審查停輪，進白話審閱閘）**——R8：composer／grok `proceed`，三家皆同意 §N small 視窗凍結；codex 2 P1＋2 P2 全採納（T1 幽靈過濾可達性目標＝stdin local sha 聯集；T2 `--delete` 行跳過；T3 報表加 `stamp` 狀態；T4 §N「只准兩種」字面改指理由類別）。codex T1–T4 之 `CLOSED:` 由 TODO adversarial 輪補寫。收斂檔 `handoffs/reconcile/20260911-verdictgate-x-review-r8/synth.md`。
> v8（R7 後修訂）——R7：三家皆 `blocked`（5 條，全為契約文字互斥，無新設計缺口）全採納（U1–U4）：**U1** helper 回傳＝命中 round 之真 task_id 前綴（去 `-r<M>`）、大小寫不敏感 `-b<K>-`；**U2** Task 3.1 之 `:791-798` 回溯**由 helper 取代**；**U3** push 驗證範圍改讀 pre-push stdin 之 `<remote sha>..<local sha>`；**U4** `register-output --kind stamp` 分流、`closed` 只認 review／closure round。🔴 **使用者 2026-09-11 裁定（逐字）：「若是有地方是你跟委員判定無法收斂或無限窮舉或實作或落地後對整個流程的運作成本和時間成本太高，這就不要鑽下去，該適時停止」** ⇒ small 視窗規則**凍結於 v7 語意**，再進一步之繞法歸 §N「蓄意等價」不再修；R8 後若只剩文字同步類由主委修並直接進白話審閱閘。收斂檔 `handoffs/reconcile/20260911-verdictgate-x-review-r7/synth.md`。
> v7（R6 後修訂）——R6：composer `proceed`（全文掃描無 blocking）；codex 4 條、grok 1 條全採納（V1–V5）：**V1** 十條 `verdictgate_check` ASSERT 補第三參數；**V2** `prev_review_resolve.sh` 寫死 review 分類契約（`brief_kind=review` 或缺欄且不命中含尾碼型之排除 marker）；**V3** push 驗證範圍改 `HEAD --not --remotes`（首次 push 不再只驗 HEAD）；**V4** `register-output` path 須等於 `expected_outputs[family]`、fixture 全帶尾碼；**V5** 刪與消費視窗語意互斥之 ASSERT（R4 synth Z4「錨前 small 也在窗內」為主委寫錯，錨前不計；「小任務×3→真票 token＋生產 commit→小任務×3」放行屬授權路徑）。收斂檔 `handoffs/reconcile/20260911-verdictgate-x-review-r6/synth.md`。
> v6（R5 後修訂）——R5：composer `proceed`；grok 1 條、codex 3 條全採納（W1–W4）：**W1** Task 3.1 caller 補第三參數；**W2** 消費 token 須 `prod_files` 含生產路徑（docs-only 不消費）；**W3** 上線前無 `brief_kind` 之 round **預設視為 review**（只排除 CONSULT／STAMP／CLOSURE／IMPL／RECON 標記）；**W4** Task 3.3 對照 ASSERT 改為含消費事件。composer 之「`--no-verify`→領 token→`--amend`」判 by-design 入 §V。收斂檔 `handoffs/reconcile/20260911-verdictgate-x-review-r5/synth.md`。
> v5（R4 後修訂）——R4：composer／grok `proceed`（R3 全 CLOSED、五題皆「無構造」）；codex R3 六條全 CLOSED、新 5 條 P1 全採納（Z1–Z5）：**Z1** `register-output` 之 family 由檔名尾碼解析並對 roster 對證、`committee_output` 移出 legacy、`allowed_origin_scripts` 補齊；**Z2** `verdictgate_check` 第三參數＝caller 用共用 helper 算出的前批（descoped 一致）；**Z3** `post-commit` 寫 `ticket_commit.token_fresh`，`1c` 要求 batch commit 當下 token 有效（擋 `--no-verify` 後追認）；**Z4**（採較嚴版）small 視窗重置錨＝**被消費的** token（其後有同批 `ticket_commit.token_fresh=true`），未消費 b1 token 不是錨；**Z5** `round_open` 寫 `brief_kind`，C-4／C-9 只納 review round。收斂檔 `handoffs/reconcile/20260911-verdictgate-x-review-r4/synth.md`。
...
- **C-4 舊產出不推導；缺機械裁決即缺輸入 ⇒ fail-closed**（v4 改寫，`CODEX-R3-P1-05`）：閘只讀 Phase 1 上線後經 `register-output` 寫入 audit 之機械裁決，**不對任何 markdown 做字面推導**（v1 字面表被三家實跑證明雙向誤判而取消）。前批若在 audit 有 `brief_kind=review` 之 `committee_round_open`（v5：consult／stamp round 不算前批 review——`CODEX-R4-P1-05`；上線前 round 無此欄者**預設視為 review**，只有 task_id 命中 `-(CONSULT|STAMP|CLOSURE|IMPL|RECON)(-|[0-9]*$)`（含 `P16-B3-STAMP`／`-STAMP2` 尾碼型；composer R6 實跑 83 事件／71 task_id 為尾碼型，v7 補）才排除（v6：v5 之 `-REVIEW-` 白名單在 782 筆 `round_open` 中漏 196 筆 review-like round ⇒ under-block，`CODEX-R5-P1-03`；反轉為 fail-closed，只影響活票））但 `quorum_eligible` 任一家無同家含 `verdict` 之 `committee_output` ⇒ **blocked**，訊息指名「派補裁決輪（`brief-kind: closure`）」；前批**完全不存在** round ⇒ 不擋。舊產出仍一律 `unknown` 列透明度報表（Task 2.1）。v3 之「unknown 不進判定 ⇒ 放行」被 codex 證明使 SPLITUNIFY B5 可先於補裁決開輪 ⇒ 改為與 C-9 同一原則。
...
- 改法：`register-output` 之 **family 由產出檔名尾碼 `-<family>.md` 解析**（v5：現行 `gate.sh:202` 固定寫 `family=unknown`，C-9 同家匹配會落空——`CODEX-R4-P1-01`），並與該 task 最近一筆 `committee_round_open` 對證：尾碼不在 `quorum_eligible` ⇒ 拒收；**`out_rel` ≠ `expected_outputs[family]` ⇒ 拒收**（v7：否則同家任意 handoff 可註冊成該輪輸出——`CODEX-R6-P1-04`；parser 只接受 round 之 expected output，不接受 brief／handoff）；呼叫 `verdict_parse.sh <path> <family>` → 回 JSON `{verdict, blocked_by[], closed[]}`；經 `audit_append.sh` 寫入 `committee_output`（C-8 必填欄）。**拒收條件**：無 `VERDICT:` 行／值不在集合／`blocked` 卻無 `BLOCKED-BY`／`CLOSED` 之任一 ID 前綴家族 ≠ 本產出家族／ID 不在該檔 `## <ID>` 集合。**接線（v3）**：`cx_run.sh` 於 `brief-kind ∈ {review, closure}` 且 `STATUS: DONE` 且產出非空時**自動**呼叫 `register-output`；拒收 ⇒ 印 `ERROR: verdict 拒收 <原因>` 並在 `committee_family_result` 寫 `verdict_rejected`（**不**靜默 no-op、不改 cx_run rc）；`stamp` kind 走 **`register-output --kind stamp --family <fam>`**（v8，`CODEX-R7-P1-02`：stamp 目標是 reconcile synth.md，不是 expected output、無 `VERDICT:`，不能套上述拒收條件）——family 由 `cx_run` 之 `${fam}` 傳入、**不**做尾碼／`expected_outputs` 對證、**不**呼叫 `verdict_parse`、寫 `committee_output` 時 `verdict=null`；此類事件**不**進 C-9 roster 判定、其 `CLOSED` **不**解除任何 ID（Task 2.2）。`cx_run.sh:586` 改為帶 `--kind stamp --family "${fam}"`。
- **驗證**：`pytest tests/governance/test_verdictgate_*.py` rc=0，一條 ASSERT 對應一個 test；固定文法斷言如下：
  `ASSERT bash scripts/gate.sh register-output T handoffs/x-codex.md WHEN verdict_line=absent THEN rc!=0`
  `ASSERT bash scripts/gate.sh register-output T handoffs/x-codex.md WHEN verdict=blocked blocked_by=absent THEN rc!=0`
  `ASSERT bash scripts/gate.sh register-output T handoffs/x-codex.md WHEN closed_id_family=other THEN rc!=0`
  `ASSERT bash scripts/gate.sh register-output T handoffs/x-codex.md WHEN verdict=proceed expected_outputs[codex]=handoffs/x-codex.md THEN rc=0`（v7 fixture 帶尾碼——`CODEX-R6-P1-04`）
  `ASSERT bash scripts/gate.sh register-output T handoffs/y-codex.md WHEN expected_outputs[codex]=handoffs/x-codex.md THEN rc!=0`（path 未綁 expected output ⇒ 拒）
  `ASSERT bash scripts/gate.sh register-output ROOT-B2-REVIEW-R1 handoffs/x-codex.md WHEN closed_id_found_only_in=OTHERROOT THEN rc!=0`
...
- 改法（**v2 改寫**——v1 之 legacy 字面表被三家實跑打穿：`COMPOSER-R1-P1-01` 全庫 ≥63 份「需修補後派工／條件式可進」被判 proceed（under-block）、`COMPOSER-R1-P1-02` 10 份閉合輪因同區塊含歷史「不可進」被判 blocked（over-block）、`GROK-R1-P1-01` 金標跨批檔反被判 proceed；`COMPOSER-R1-P2-01` 掃描範圍未定義）：**不對舊產出做任何字面推導**。舊產出（無 `VERDICT:` 行）一律 `verdict=unknown`——**不推導其內容**（使用者 2026-08-05「面向未來不溯及既往」）；判定面依 C-4：`unknown`＝缺輸入 ⇒ 活票開下一批時 fail-closed、要求補裁決輪（v4）。本 Task 只產出**透明度清單**：`verdictgate_baseline.sh --report` 列出每個 `<root>-b<N>-review-r<M>` 各家是否有機械裁決（`has_verdict|unknown|stamp`；`stamp`＝`--kind stamp` 之 `verdict=null` 事件，**不計入** `unknown` 總數——v9 `CODEX-R8-P2-03`）與 `unknown` 總數，供人讀，**不作為判定輸入**。
- **驗證**：`bash scripts/verdictgate_baseline.sh --report` rc=0；輸出含 `unknown=<n>` 行且 n ≥ 621（FACT：本票前 review 產出 621 份皆無 `VERDICT:`）；對本票 R1／R2 已登記之產出印 `has_verdict`；**v3 新增兩行**（`GROK-R2-P2-01`／`CODEX-R2-P1-01`）：`live_roots_unwatched=<root,…>`（audit 中有 `committee_round_open` 但無對應 `debt_clear` 收票之 root，且其最新批 review 各家皆 `unknown`）與 `legacy_open_by_root=<root:b<N>,…>`。
  `ASSERT bash scripts/verdictgate_check.sh ROOT 2 ROOT-b1-review WHEN prev_round_open=present prev_round_outputs=legacy_unknown THEN rc!=0 且 stderr 含 補裁決輪`（v4 反轉：缺機械裁決＝缺輸入，C-4）；
  `ASSERT bash scripts/verdictgate_check.sh ROOT 2 "" WHEN prev_round_open=absent THEN rc=0`（對照組：helper 回空字串＝前批不存在不擋；v7 十條 ASSERT 全補第三參數——`CODEX-R6-P1-01`）。
...
- 改法：`verdictgate_check.sh <root> <N> <prev-review-prefix>`——第三參數**必填**，由 caller 以共用 helper `scripts/prev_review_resolve.sh <root> <N>` 算出——helper **契約**（v7，`CODEX-R6-P1-02`）：候選＝audit `committee_round_open` 中 task_id **大小寫不敏感**命中 `^<root>-b<K>-`（K<N，由大到小；`x` 層不命中故自然排除）者；review 判定＝`brief_kind=review`，或缺欄且 task_id 不命中 C-4 之排除 regex；回傳＝**命中之 round 的 task_id 去掉輪次尾碼 `-r<M>`**（大小寫保留，例 `SPLITUNIFY-B1-REVIEW`、`P16-B5-TASK31-REV`——即與該輪 `committee_output.task_id` 可對上的真前綴；v8：v7 寫「回傳 `<root>-b<K>-review`」對不上 audit 實際命名——`CODEX-R7-P1-01`／`GROK-R7-P1-01`），無則回空字串；**不**讀 handoff 檔名、**不**依賴既有 `gate.sh:793-798` 之 `grep "-review"` 字面（該字面會漏 `P16-B5-TASK31-REV` 型 round 使 `_rq_prev` 為空而跳過閘）；`gate.sh`／`committee_run.sh`／本 checker 三處只呼叫 helper；checker **不自行**找字面 `<root>-b<N-1>`（v5：v4 兩處寫法不一致，descoped 時 caller 找 B1、checker 找 B2 ⇒ absent ⇒ 越過補裁決閘——`CODEX-R4-P1-02`），取該批**全部** review 輪（不只最新輪）各家 `committee_output` 之 `blocked_by`，**聯集**成待閉合集合 `{(family, ID)}`；每個 `(family, ID)` 必須在**同家**之任一後續產出（append 序在其後；**只認 `brief_kind ∈ {review, closure}` round 之 `committee_output`**——stamp／consult round 之 `CLOSED` 不解除，v8 `CODEX-R7-P1-02`）的 `closed` 中出現，否則視為未閉合 ⇒ rc=1 並逐條指名。🔴 **`verdict: proceed` 本身不解除任何 ID**——同家下一輪改寫 `proceed` 而不寫 `CLOSED:` 仍擋（`GROK-R1-P0-01`：v1 只讀最新輪，等於讓「下一輪閉嘴」等同閉合）。「後續」之序＝audit 檔內 append 序，**不以 `ts` 排序**（`committee_output` 只有秒級 `ts`、同秒碰撞實測存在，見 §A）。`committee_run.sh` 與 `gate.sh dispatch` 皆在開輪前呼叫。**本閘只對 Phase 1 上線後 `register-output` 寫入 audit 之裁決生效**；無 `verdict` 欄之舊產出一律 `unknown`——不推導、但依 C-4 視為缺輸入而擋（v4；見 Task 2.1）。
- **驗證**：`pytest tests/governance/test_verdictgate_*.py` rc=0，一條 ASSERT 對應一個 test；固定文法斷言如下：
  `ASSERT bash scripts/verdictgate_check.sh ROOT 2 ROOT-b1-review WHEN prev_verdict=blocked closed=absent THEN rc!=0`
  `ASSERT bash scripts/verdictgate_check.sh ROOT 2 ROOT-b1-review WHEN prev_quorum_eligible=codex,composer,grok prev_committee_output=codex,composer round_abandoned=false THEN rc!=0 且指名 grok`（C-9 no_output；roster 來自 `round_open`，grok 無 `family_result` 亦擋——`CODEX-R3-P1-01`）
  `ASSERT bash scripts/verdictgate_check.sh ROOT 2 ROOT-b1-review WHEN prev_quorum_eligible=codex,composer,grok prev_committee_output=codex,composer grok_family_result=absent round_abandoned=true THEN rc=0`（對照組：真缺席可 abandon）
  `ASSERT bash scripts/verdictgate_check.sh ROOT 2 ROOT-b1-review WHEN grok_family_result=verdict_rejected grok_committee_output_after=present THEN rc=0`（C-9 解鎖①，`CODEX-R3-P1-03`）
  ...
  `ASSERT bash scripts/prev_review_resolve.sh P16 6 WHEN audit 有 P16-B5-TASK31-REV(無 brief_kind) THEN stdout=P16-B5-TASK31-REV`（`-review` 字面外之 review；v8 argv 改為真 root／N——`CODEX-R7-P1-01`／`GROK-R7-P1-01`）；`ASSERT bash scripts/prev_review_resolve.sh P16 4 WHEN audit 只有 P16-B3-STAMP THEN stdout=""`（同 root 候選集內之尾碼型排除）；`ASSERT bash scripts/prev_review_resolve.sh SPLITUNIFY 2 WHEN audit 有 SPLITUNIFY-B1-REVIEW-R1..R3 THEN stdout=SPLITUNIFY-B1-REVIEW`
  `ASSERT bash scripts/gate.sh register-output T handoffs/reconcile/S/synth.md --kind stamp --family codex WHEN 無 VERDICT: THEN rc=0 且 committee_output.verdict=null`（`CODEX-R7-P1-02`）；`ASSERT bash scripts/verdictgate_check.sh ROOT 2 ROOT-B1-REVIEW WHEN b1 blocked ID 只在 x-stamp round 之 CLOSED THEN rc!=0`
  ...
  `ASSERT bash scripts/verdictgate_check.sh ROOT 2 ROOT-b1-review WHEN prev_verdict=blocked closed=present THEN rc=0`
  `ASSERT bash scripts/verdictgate_check.sh ROOT 2 ROOT-b1-review WHEN prev_r1_verdict=blocked prev_r2_verdict=proceed closed=absent THEN rc!=0`
  `ASSERT bash scripts/verdictgate_check.sh ROOT 2 ROOT-b1-review WHEN prev_verdict=blocked closed_by_family=other THEN rc!=0`
  `ASSERT bash scripts/verdictgate_check.sh ROOT 2 WHEN argv 只有兩個 THEN rc!=0 且印 usage`（第三參數必填不可降級）
...
- 改法（v3 依 `GROK-R2-P1-01`／`CODEX-R2-P1-04`／`COMPOSER-R2-P2-02` 改寫）：`--impl-self` 僅允許 task_id `<root>-impl-b<N>-claude`（family 尾碼非 `claude` ⇒ 拒）；**不新開分支**——同一條 dispatch 路徑照跑 debt gate（`:574`）、brief gate（`:625`）、reconcile-stamp（`:673`）、template_check（`:807`）；前批定位：`:791-798` 內之 `grep "-review"` 回溯**由 `prev_review_resolve.sh` 取代**（呼叫點不變、實作＝helper；v8 `GROK-R7-P1-02`——v7 寫「沿用」與 Task 2.2「不依賴該 grep」互斥）取 `_rq_prev`：`_rq_prev` 非空 ⇒ `review_quorum_check.sh <_rq_prev> claude` **與** `verdictgate_check.sh <root> <N> "${_rq_prev}"`（第三參數逐字＝`prev_review_resolve.sh` 輸出；v6 補齊——`CODEX-R5-P1-01`）；`_rq_prev` 空（b1 常態、或前批無 review）⇒ 兩者皆跳過並印 `[gate] 無前批 review，跳過 quorum／verdictgate`（與既有行為一致）。通過後寫 `.claude/gate/impl.<root>-b<N>.token` 並寫 audit `impl_token_issued`（C-8）。
- **驗證**：`pytest tests/governance/test_verdictgate_*.py` rc=0，一條 ASSERT 對應一個 test；固定文法斷言如下：
...
- 改法：`1c`：對 **pre-push hook stdin 給的 range**（每行 `<local ref> <local sha> <remote ref> <remote sha>`；range＝`<remote sha>..<local sha>`，remote sha 全零（新 ref）⇒ `<local sha>` 全部可達 commit；**local sha 全零（`--delete`）⇒ 該行跳過，全為 delete ⇒ rc=0，混合 push 非 delete 行仍逐行驗**（v9 `CODEX-R8-P1-02`：否則 `rev-list <x>..0000` rc=128 拒掉正常刪支）；pre-push 以 `--range` 傳給 `gov_check 1c`；手動呼叫無 `--range` ⇒ 回退 `@{u}..HEAD`、無上游 ⇒ fail-closed 印 usage。v8：v7 之 `HEAD --not --remotes` 在第二 remote／fork tracking ref 已含本地 commit 時待驗集合變空——composer R7 實跑 Case B／C，`COMPOSER-R7-P1-01`；v6 之 `origin/main..HEAD`＋「首次 push 只驗 HEAD」為 fail-open——`CODEX-R6-P1-03`）每個 commit 跑 `ticket_batch_check.sh --commit <sha>`（驗 trailer，且每個 `<root>/b<N>` commit 須有對應 `ticket_commit.token_fresh=true`——即 **commit 當下** token 有效；缺事件或 `false` ⇒ 拒 push。v4 只驗 token「曾存在」可被「先 `--no-verify` commit、再領 token 追認」繞過——`CODEX-R4-P1-03`；token 於 push 時已過期屬正常）；**small 聯集之視窗（v4）**：讀 audit 自**最近一筆「被消費的」`impl_token_issued`**（v5：錨須滿足「其後存在同 `<root>/b<N>` 之 `ticket_commit.token_fresh=true` **且 `prod_files` 與 `momentum/|api/|frontend/src/` 交集非空**」（v6：docs-only commit 帶 batch trailer 合法但**不消費** token——`CODEX-R5-P1-02`）——未消費之 token（例：只為重置視窗而領的 b1 token，`CODEX-R4-P1-04`）**不是**錨；錨之序＝audit append 序；v3 之「或 batch commit」子句為死錨且可被 token→small×3→batch commit→small×3→一次 push 繞過（`GROK-R3-P1-01`），在「被消費 token」語意下該序列之錨仍是 token ⇒ 6 檔全在窗內 ⇒ 擋）以來**全部 `trailer=small` 之 `ticket_commit` 事件，**先以 `git merge-base --is-ancestor <sha> <local sha>` 過濾不可達 sha（目標＝本次 push 全部 stdin 行之 local sha 聯集，任一可達即算；v9：v8 用 `HEAD` 在 push 非當前 checkout 分支時漏計——`CODEX-R8-P1-01`）**（amend／reset 幽靈，`CODEX-R3-P1-04`／`GROK-R3-P2-01`），再對 `prod_files` 取聯集——push 成功**不清零**；視窗為**全 repo 全域**（`small` 無 root，不做 root／branch 過濾——這正是「small 不得連鎖」之語意）；聯集 > 3 檔或含 `factories.py|protocols.py|config.py` ⇒ 拒 push 並指名累計檔案與起算事件（Task 3.2 邊界④，`GROK-R1-P1-02`→`GROK-R2-P0-01`）；`pre-push` 之 `GOVERNANCE_SKIP_PREPUSH=1` 分支加 `audit_append.sh --event governance_bypass`。
  ...
  `ASSERT bash scripts/gov_check.sh --fast WHEN push1(small 3 檔) rc=0 impl_token_issued(ROOT/b2) ticket_commit(ROOT/b2,token_fresh=true,prod_files=momentum/x.py) THEN push2(small 3 檔) rc=0`（對照組：**被消費**的權限才重置視窗；v6 依 `GROK-R5-P1-01` 改——v5 此行寫成未消費即重置，與 Z4 互斥）；
  `ASSERT bash scripts/gov_check.sh --fast WHEN 序列=small{a,b,c},impl_token_issued(ROOT/b2),ticket_commit(ROOT/b2,token_fresh=true,prod_files=docs/x.md),small{d,e,f},push THEN union=6 rc!=0`（docs-only 不消費，`CODEX-R5-P1-02`）；
  `ASSERT bash scripts/gate.sh dispatch --impl-self --task-id ROOT-impl-b3-claude WHEN audit=descoped(b1 review, b2 無) THEN verdictgate_check argv[3]=ROOT-b1-review`（`CODEX-R5-P1-01`）；
  `ASSERT bash scripts/verdictgate_check.sh ROOT 2 P16-B5-TASK31-REV WHEN 該 round 無 brief_kind 且無 verdict THEN rc!=0`（上線前預設 review，`CODEX-R5-P1-03`）；對照：`… ROOT-b1-CONSULT-r1 WHEN 無 brief_kind THEN rc=0`；
  ...
  `ASSERT bash scripts/gov_check.sh --fast WHEN 序列=small{a,b,c},impl_token_issued(ROOT/b2),ticket_commit(ROOT/b2,token_fresh=true,prod_files=momentum/x.py),small{d,e,f},push THEN union=3 rc=0`（v7：錨前 small 不在窗內；R4 synth Z4「其後 small 全計」之 union=6 寫錯——`GROK-R6-P1-01`；此序列屬授權路徑，token 須過完整 `--impl-self` 判定）；
  `ASSERT bash scripts/gov_check.sh --fast --range 0000000..<sha> WHEN 兩個初始生產 commit、較早者無 trailer THEN rc!=0`（首次 push 不只驗 HEAD，`CODEX-R6-P1-03`）；
  `ASSERT bash scripts/gov_check.sh --fast --range <origin/main sha>..<sha> WHEN 另一 remote fork 已含該 commit 但 origin 未含 THEN 仍驗該 commit`（`COMPOSER-R7-P1-01`）；`ASSERT bash scripts/gov_check.sh --fast WHEN 無 --range 且無上游 THEN rc!=0 印 usage`；
  `ASSERT bash scripts/git_hooks/pre-push WHEN stdin 一行 local sha=0000000 THEN rc=0 且不呼叫 1c`（`CODEX-R8-P1-02`）；`… WHEN stdin 兩行(一 delete、一 feature 含無 trailer 生產 commit) THEN rc!=0`；
  `ASSERT bash scripts/gov_check.sh --fast --range <a>..<feature sha> WHEN checkout=main、feature 有 small commit THEN 該 small 計入 union`（可達性目標＝local sha，`CODEX-R8-P1-01`）；
...
- **邊界**：①首次 push（remote sha 全零）⇒ range＝`<local sha>` 全部可達 commit，與一般 push 同一條規則（v7 刪「只驗 HEAD」，v8 改讀 stdin）；②commit 全為 docs ⇒ 跳過 trailer 驗；③audit 無任何被消費之 `impl_token_issued` ⇒ 視窗自 audit 首筆 `ticket_commit(trailer=small)` 起算；連 `ticket_commit` 也無 ⇒ 聯集為空 ⇒ 放行（上線前歷史不溯及）；④`post-commit` hook 寫 audit 失敗 ⇒ 印 ERROR 但不擋 commit（commit 已成立），由 Task 3.3 於 push 時以 range 中含 `Ticket-Batch:` 卻無對應 `ticket_commit` 事件之 commit ⇒ 拒 push（補洞：寫入失敗不得成為繞過）。
- **存活至**：保留。　**覆蓋風險**：無。
...
- **邊界目錄**：空 audit／audit 含 legacy 無 VERDICT 產出／同批多輪／descoped 批次／閉合輪／`--amend`／merge commit／首次 push／docs-only batch trailer／上線前無 `brief_kind` 之 round。
- **by-design（非殘留）**：`--no-verify` commit → 領 token（過完整 gate）→ `--amend`（重觸 post-commit ⇒ `token_fresh=true`）⇒ push 放行——amend 當下持有經完整判定之有效 token，等同在權限下重新 commit（composer R5 必答 3）；`touch` 延長 token mtime 屬蓄意繞過，與 §N 第一條同族；audit append 序（`impl_token_issued` 先於 `ticket_commit` 且 `ts` 差 ≤900s）列為 TODO 第二層 ASSERT。
...
- **殘留**（依使用者 2026-09-11 規則，`為何現在不做` 之**理由類別**只准 `user-ruling`／`needs-research` 兩種；下列四項各標理由——v9 依 `CODEX-R8-P2-04` 修掉「只准兩種」指項數之歧義）：
  - `git push --no-verify` 之客戶端繞過 — `為何現在不做: user-ruling:2026-08-13 使用者裁定刪除 CI（唯一能在伺服端擋的機制）`；觸發：使用者恢復任何遠端檢查時；登記處：本檔。
  - `small` 視窗規則之進一步繞法（凡「先取得經完整 `--impl-self` 判定之 token、並真的在該批 commit 生產檔」之後再拆 small 者）— `為何現在不做: user-ruling:2026-09-11「若…無法收斂或無限窮舉…就不要鑽下去，該適時停止」`；本規則凍結於 v7 語意（被消費之 token 為唯一重置錨）。任何重置條件都能被「製造重置條件」的人繞過，而 v7 之重置成本＝合規成本（整票 bootstrap＋完整 gate），繞過等價於合規，與 `--no-verify`／`touch` token 同族（蓄意、非意外）；三家 R5／R6 對此判準一致。觸發：出現**非蓄意**（意外拆分）之新構造時；登記處：本檔。
