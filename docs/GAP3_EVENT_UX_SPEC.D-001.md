...
- 目標：抽樣契約與 estimand 唯一住契約，且 **wire 唯一**（R1 CODEX-R1-P1-03）：`random_control_spec` 為**批次級 envelope 物件**（非逐列欄）——匯入 API body `{"records": [...], "random_control_spec": {...}}`；`validate_event_import(records, *, random_control_spec: Optional[Mapping] = None, ...)` 新 keyword；規則：`control_kind == platform_random_bars`（批內常數）⇒ spec **必填**，缺 ⇒ `random_control_spec_missing`；`control_kind != platform_random_bars` 而 spec 出現 ⇒ `random_control_mixed_batch`；通過後 spec 原樣寫入 `receipt.batch.random_control_spec` 並隨批落檔（detail 端點回傳）。　檔案：`event_import_contract.json`：`receipt_schema.batch.random_control_spec{universe{symbol,timeframe,start_ms,end_ms}, strata{symbol,timeframe,period,direction}, allocation:"proportional_to_candidates", exclusion{trigger_ids_digest, neighborhood_bars, embargo_bars}, **label_rule{threshold: float, horizon_bars: int}**（必填；TODO review r2 三家 P1-01）, seed, n_requested, n_drawn, per_stratum:[{key, n_candidates, n_drawn}], replacement(false), candidate_count, sample_ids_digest, data_snapshot_digest, generator_version}`；**觸發批之規則身分** `receipt_schema.batch.label_rule{threshold: float, horizon_bars: int}`（optional；由平台產生器批（`generator.py` 之 `LabelRule`）或匯入 envelope 帶入；人標／`/search` 批通常缺）；`control_kind.accepted` 增 `platform_random_bars`、移除其 `rejected_with_reason`；`capability_unavailable_reasons` 增 `random_control_prevalence_missing`、`random_control_rule_identity_unverifiable`、`random_control_rule_mismatch`；`import_failure_reasons` 增 `random_control_spec_missing`、`random_control_mixed_batch`、`random_control_label_rule_missing`；`doc` 寫 estimand：「同 universe 之非觸發 eligible bars 以 **`random_control_spec.label_rule` 宣告之規則**（`all_bars_eval._label_from_rule`）自動標 label、同一 producer 算 `label_value`；回答觸發樣本相對無條件基準之 prevalence／IC lift——**僅當觸發批之 `receipt.batch.label_rule` 與之 typed 相等（threshold／horizon_bars 逐葉 `==`、direction 同批）時該比較成立**；不回答反例品質、不補足缺失反例」。
...
- 目標：隨機批可匯入、可算條件 IC、報表與觸發批並排 prevalence。　檔案：`api/routes/case.py`（既有 `/case/import-events*` 家族；新端點 `POST /case/import-events/random-control`，body `{event_import_id: <觸發批>, random_control_spec: {...}}`，回 `EventImportResponse`；**不新建** `api/routes/event_import.py`——R1 COMPOSER-R1-P1-01，該檔不存在）；`api/services/case_import_service.py`（落檔走既有 storage，`get_import` 回傳 `receipt.batch.random_control_spec`）；`ic_analysis_service`（`sample_design='unconditional_random'` 揭露；比較觸發批 vs 隨機批之**規則身分閘**（TODO review r2 三家 P1-01）：①觸發批 `receipt.batch.label_rule` 缺 ⇒ `unavailable:random_control_rule_identity_unverifiable`（既有 9 批皆如此，具名殘留）；②存在但與隨機批 `label_rule` 任一葉不等、direction 不同、或觸發批 `label_definition.label_return_mode != close_to_close` ⇒ `unavailable:random_control_rule_mismatch`（CODEX-R3-P1-02：身分 tuple＝`(threshold, horizon_bars, direction, label_return_mode)`）；觸發批 `receipt.batch.label_rule` 之**寫入來源**（CODEX-R3-P1-03；主委 r4 自查更正：`generate_events` 於 `api/` 無 caller、產生器批現無持久化路徑）：**唯一 wire＝匯入 envelope**（`import_records(..., label_rule=)`，validator typed 驗後寫入）；`generate_events` provenance 增 `label_rule`（單一 `LabelRule` ⇒ `{threshold, horizon_bars}`；多規則 ⇒ `None`）供未來接線；產生器→匯入接線列 §N 殘留（`blocked-by:產生器無持久化 caller`）；③相等時**以同一 `label_rule` 重評觸發批 label**，一致率 `!= 1.0` ⇒ `unavailable:random_control_rule_mismatch`（可證偽：mutation 把觸發批任一 label 翻轉 ⇒ 紅）；④缺任一 prevalence ⇒ `unavailable:random_control_prevalence_missing`）；`eventDimensions.ts` 鏡像同步（`accepted` 增值）；`/search` 或 `/ic-analysis` 提供「產生隨機對照批」入口（依附既有觸發批）。
...
- **邊界**：①比較之**唯一**判準＝上列四段規則身分閘（typed `label_rule` 逐葉相等＋direction 同批＋觸發批 `label_return_mode == close_to_close`）；`label_definition.canonical_digest` **不作為**比較條件（隨機批 digest＝S-9(label_rule)、產生器批 digest 含 `label_id`／mode／direction，兩者本就不相等；digest 僅作 provenance）——TODO review r3 CODEX-R3-P1-01／GROK-R3-P1-01 更正；②觸發批 `label_return_mode ∈ {open_to_close, open_to_horizon_close}` ⇒ `random_control_rule_mismatch`（隨機批固定 `close_to_close`；CODEX-R3-P1-02）；③產生器**多規則批**（`label_config` 含 >1 條 `LabelRule`）⇒ `receipt.batch.label_rule` **不寫**（缺）⇒ 比較 `random_control_rule_identity_unverifiable`＋揭露「多規則批；請以單規則批比較」（GROK-R3-P2-01）；④隨機批單獨分析允許（其 IC＝無條件 IC 估計，揭露）。
...
- **既有 9 批無 `receipt.batch.label_rule`（`rule_id` 為自由字串、無門檻），與隨機對照批之比較恆 `unavailable:random_control_rule_identity_unverifiable`** — `為何現在不做: user-ruling:2026-08-05 面向未來不溯及既往（舊批不遷移）；/search 人標批之規則身分需另立票（search_rule_summary → label_rule 之機械轉換）`；觸發：使用者要對既有批做隨機對照比較；登記處：GAP-3 殘留（TODO review r2 三家 P1-01）。
- **平台產生器批之持久化與 `label_rule` 自動落檔（`generate_events` → `import_records` 接線）** — `為何現在不做: blocked-by:generate_events 於 api/ 無任何 caller（產生器批現只在測試與純函式層存在，無落檔路徑）`；觸發：產生器接上 `/case/import-events` 家族之票；登記處：GAP-3 殘留（TODO review r4 主委自查）。
...
RECONCILE-STAMP: composer APPROVED 2026-09-03 sha256:7dbcbd0c954fc4c815a7fe2a319607d970c4f6d70ec14efd39b2677b7ae562bf task:20260903-GAP3D2-X-STAMP-R2
（composer 於 R2／R3／R4／STAMP-R1／STAMP-R2 連六次 CLI 失敗（`DEGRADE-COMPOSER-01..04`），於 TODO review r1（`20260903-gap3d2todo-x-review-r1`）恢復並親筆給出上列 APPROVED（交件檔 `handoffs/20260903-gap3d2todo-x-review-r1-composer.md`「D-001 RECONCILE-STAMP 判定」段，主委逐字鏡像至 r4 synth 與本區）。`bash scripts/reconcile_stamps_check.sh handoffs/reconcile/20260903-gap3d2-x-review-r4/synth.md` → PASS（三家全數 APPROVED、body hash 相符）。**三家戳記齊全 2026-09-03**。
🔴 戳記後修訂（TODO review r1 COMPOSER/GROK-R1-P1-02 命中 SPEC 層字面）：D5.1／D5.2 之「以 `label_definition.canonical_digest` 經條件引擎純函式評 label」改為「`random_control_spec.label_rule{threshold, horizon_bars}`（契約必填）＋`all_bars_eval._label_from_rule`」（digest 不可逆、匯出批無 `filters`）。此修訂由 TODO 戳記輪三家一併復核；未復核前本段為具名未戳記差異。）
