...
6. **事件路徑之 ICIR ⇒ 診斷欄，且列全消費端**（R1 `CODEX-R1-P1-03`）：`_apply_thresholds`（跳過 `icir_min`）；`redundancy_filter._score_value`（事件路徑 `tiebreaker_effective="ic_mean"`，不再拿 `-inf`）；`ic_reporter` 三處 `key=item.get("icir", -inf)` 排序（排序 key 以 `_finite_or_neg_inf` 包裝：`None`／NaN／非數 ⇒ `-inf`，防 TypeError）；事件路徑 `icir` 缺值一律寫 **`None`（JSON null）**——`save_report` 為 `allow_nan=False`（`ic_reporter.py:864`），寫 NaN 會落檔 raise（主委 R2 前自查）；survivor／top-features／export 之 ICIR 讀取點由 TODO 逐一列出並各釘測試。
...
- 檔案：`ic_filter_orchestrator.py::_apply_thresholds(..., icir_gate: bool=True)`（事件路徑 False，`removed["icir_skipped_event_path"]`）；stage6 呼叫 `redundancy_filter` 時事件路徑傳 `tiebreaker="ic_mean"`（記 `metadata["tiebreaker_effective"]`；`redundancy_filter.py` 不改）；`ic_reporter.py` 三處排序 key 改 `_finite_or_neg_inf(item.get("icir"))`（None／NaN ⇒ -inf；全域結果順序不變——既有值皆有限）；事件路徑 summary `icir` 缺 ⇒ `None`（JSON null；`allow_nan=False`）；`metadata["ic_window_disclosure"]`（**只事件路徑**）；前端 `IsolationNote` 旁一行（`icIsolation.ts::windowDisclosureLine`）。
