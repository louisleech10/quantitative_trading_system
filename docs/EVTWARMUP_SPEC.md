...
- **驗證**：`ASSERT venv/bin/python -m pytest tests/api/test_evtwarmup.py -k icir THEN rc=0`；事件路徑 34 事件 ⇒ `len(passed) >= 1`、`removed["icir_skipped_event_path"]` 非空、stage6 `tiebreaker_effective == "ic_mean"`、reporter 排序不 raise、`get_top_features(sort_by="icir")` 不 raise、三個序列化入口（`save_report` `:812-837`、`ic_reporter.py:726` `safe_report`、`scan_cube._dumps` `:71-73`）落檔後皆 `json.loads(text, parse_constant=<raise>)` 成功（無 `NaN` 字面）；全域路徑 ⇒ `removed["icir"]` 與改前相同、`test_gap2_golden` 不變；`ic_window_disclosure` 事件路徑存在、全域路徑**不存在**（mutation：全域也寫 ⇒ golden 紅）；vitest `ICSummaryTable.test.tsx` 綠、`npx tsc --noEmit` 無新錯。
...
- **殘留**：
  ...
  - `EW-RESID-5` 既有 summary 欄位（`long_short_spread`／`long_short_tstat`／`turnover_rate`／`quantile_turnover`／`rank_change_rate`）在 `save_report` 落檔本就為 JSON `NaN` 字面（B1 實作時實測，改前即如此）— `為何現在不做: blocked-by:報告 JSON 契約（ic_report_contract）與前端 parse 路徑另票盤點；本票只保證 icir／ic_mean 為 null`；觸發：前端 `response.json()` 對 `/result` 解析失敗之回報；登記處：`docs/IC_QUANT_GAP_REGISTRY.md`。
  - `EW-RESID-6` 兩條**既有**紅測試（HEAD worktree 對照證實改前即紅）：`test_ichc_contract_sync::test_r6_wider_contract_nodes_consistent`（契約 `report_sections.event_return_table` 在 orchestrator 源碼無字面）、`test_survivor_contract::test_v2_event_keys_fail_closed[4]`（`control_kind=platform_random_bars` 在 accepted 集合內故不 raise）— `為何現在不做: blocked-by:屬 ICHC 契約同步與 survivor accepted 集合之裁定，非本票`；觸發：ICHC 契約票；登記處：`docs/IC_QUANT_GAP_REGISTRY.md`。
