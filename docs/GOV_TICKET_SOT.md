...
| E-026 | B-63 | PreToolUse:Edit,Write:scripts/live_doc_write_guard.sh | 產出端 | 實作位置：scripts/_live_doc_write_guard.py:572（寫入目標先解析成實際被寫之檔之正名——以 inode 判定在 repo 內、逐段取實際檔名；所給路徑不等於正名、經 repo 內 symlink 或目標為硬連結即為別名，別名寫活文件即擋；Edit／Write 以寫入後全文判定：新增行之識別碼＋狀態字面〔判定碼與 gen_fact_key_blocks.sh 共用〕、歷史專區外之刪除線／考古字面／finding ID、歷史專區指標文法、交接檔封閉文法之內容型部分）。誠實邊界：Bash 轉向與委員 CLI 之寫入不經本閘，由 E-027 之 pre-commit 兜底 | 內容型 |
