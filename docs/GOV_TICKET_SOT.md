...
| 0080 | B-16 | 部分完成 | 機器實際依賴的契約長在散文裡，導致「用 regex 解析 markdown」成為必經之路。 | r4 主委裁定：template_check.sh L408+ 與測試檔碼內明文票號，歸屬成立 ｜還缺：四類偵測（ID 樣式／可執行斷言／路徑與函式樣式／複合句）僅部分落地；每項修法須附誤擋率 receipt；是否納入 白話說明 掃描未決，納入前不得宣稱覆蓋使用者可見面 |
