...
| 0640 | B-64 | 未開工 | 委員 CLI 失敗且無產出（例：模型容量滿）而該家仍在 active_stampers 時，同輪重派須 dispatch token、token 被本輪 OPEN 債拒發、銷帳缺該家產出、abandon 因該輪已有他家結果被擋 ⇒ 只能請使用者在終端機手動重派；違反「解除阻塞之路徑不得被同一族閘擋住」，同族第四次 | 還缺：使用者白話審閱 docs/REDISPATCH_SPEC.md → 實作批 b1（Task 1.1–1.6）→ 兩家審碼閉合；排在 DOCROT2 D2C 之前（交接列 HP-REDISPATCH） |
