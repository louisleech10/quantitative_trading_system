...
| 0620 | B-62 | 收案 | 批次間閘只驗「有沒有派出去審」不讀裁決，且只在派工給委員時觸發——主委自任實作從未經過；收斂檔掉意見無機械擋 | SPEC v9 停輪＋TODO v3 FROZEN（三家戳記）；B1–B4 三家審碼＋原提出方閉合 CLOSED；收票審 R11／R12 三家 proceed（C-4 只擋進入新批、只認 review 輪）；SPLITUNIFY 補裁決輪三家 proceed；四掛載點 E-022～E-025 ｜無殘留（可做而未做者）；§E 具名殘留 E-1／E-2／E-4／E-7 user-ruling、E-3／E-6／E-8 needs-research、E-5 研究完成（mtime 保留）；SPEC C-4 字面修訂併入下張治理票（三家接受） |
| 0630 | B-63 | 收案 | DOCROT 未治好文件類多輪：只管規格檔、進度仍多處手寫、交接檔殘留舊案；使用者 2026-09-15 裁定修正並全專案涵蓋（DOCROT2） | D2A–D2D 四批落地，各批三家審碼＋原提出方閉合（D2D：r1 兩條採納一條駁回、r2 三家 proceed）；收票前治理全套改動前後（61aa55b0 worktree 對照）逐名比對新增紅 0（69 failed 同名、passed 2329→2377）；產出端覆蓋 E-026～E-040；docrot2_metric_contract.json 之 closure_sequence＝4831，成效由其後首張票之前兩輪 review 以 bash scripts/docrot2_metrics.sh 量測｜無殘留（可做而未做者）；全專案遷移之具名留存 28 列見 scripts/docrot2_migration_residuals.json；SPEC 殘留 DOCROT2-RESID-D002-STRUCTURAL-RED 待 SPLITUNIFY 規格 R 重開 |
| 0640 | B-64 | 收案 | 委員 CLI 失敗且無產出（例：模型容量滿）而該家仍在 active_stampers 時，同輪重派須 dispatch token、token 被本輪 OPEN 債拒發、銷帳缺該家產出、abandon 因該輪已有他家結果被擋 ⇒ 只能請使用者在終端機手動重派；違反「解除阻塞之路徑不得被同一族閘擋住」，同族第四次 | b1（Task 1.1–1.8）落地，審碼 r1–r9 三家收斂；19 條 P1 與 1 條 P2 全部由原提出方閉合或經主委複驗後具名處置；測試 103 條、mutation 35 條全數可證偽。還缺：無——唯一不可歸零之殘餘（最後一次觀測取得資料之後的窗口）已具名於 SPEC §C 誠實邊界⑧，含前置條件與為何現在不做，三家明示同意其為具名殘留而非未修缺陷 |
...
> ### 🔴 怎麼讀（**三種，不是兩種**）
>
> `強制側` 這一欄的語意是「**有沒有擋在產出端**」，**不是**「有沒有掛」。
> 所以 `豁免` 不等於沒掛——它只表示「不在產出端擋」。要判斷有沒有掛，
> **必須同時看 `掛載點` 欄**：
>
> | 強制側 | 掛載點 | 意思 |
> |---|---|---|
> | `產出端` | 有值 | **已掛，而且擋在最早的地方**（寫檔／送指令當下） |
> | `豁免` | **有值** | **有掛，但掛在比較晚的關卡**（`commit-msg`／`pre-push`）。為何進不了產出端，寫在 `豁免理由` |
> | `豁免` | `—` | **完全沒掛**。為什麼不掛，寫在 `豁免理由` |
>
> 現況：第二種 3 列（`E-002`／`E-005`／`E-006`），第三種 8 列。
> 🔴 `E-002` 是 `B-25` 的**第二層**（defense-in-depth）——該票的主掛載是 `E-001`，
> 在產出端。**一張票可以有多列**，判斷該票有沒有掛要看它**全部**的列，不能只看一列。
>
> `豁免理由` 欄的体例（機械強制）＝須同時寫
> 「`PreToolUse不可：`」「`PostToolUse不可：`」「`部分閘：`」三段。
>
> 🔴 **具名缺陷：`豁免` 這個字取得不好。** 它字面像「免除／沒有」，實際是「產出端豁免」，
> 而該欄的列舉值鎖在 `產出端`／`豁免` 兩個。使用者 2026-08-14 一看到
> 「強制側寫豁免、卻又有掛載點」就問了——**代表這個命名會持續誤導**。
> 改名要動 `_schema.enforcement_side_enum` 與其集合相等測試，未擅自更動。
>
...
|---|---|---|---|---|---|
| E-001 | B-25 | PostToolUse:Edit,Write:scripts/factkey_write_guard.sh | 產出端 | 實作位置：scripts/factkey_write_guard.sh:119（寫檔當下對受管檔重跑 gen_fact_key_blocks --check） | 一致性型 |
| E-002 | B-25 | pre-push:gov_check.sh 第 3 段 | 豁免 | PreToolUse不可：本列記錄的是 pre-push 之第二層掛載，該階段依定義在 commit 之後。PostToolUse不可：**不適用**——同一判定已由 E-001 掛在 PostToolUse；本列僅記錄 defense-in-depth 之第二個掛載點，非缺口。部分閘：有——scripts/factkey_write_guard.sh:119（同一判定之產出端掛載） | n/a |
| E-003 | B-38 | PostToolUse:Edit,Write:scripts/doc_format_precheck.sh | 產出端 | 實作位置：scripts/doc_format_precheck.sh:151（findings 分支呼叫 cx_run --selfcheck；零 findings 之 sentinel 形態與必填欄於該路徑檢查，反例已實跑） | 內容型 |
| E-004 | B-31 | PostToolUse:Edit,Write:scripts/doc_format_precheck.sh | 產出端 | 實作位置：scripts/doc_format_precheck.sh:151（與委員交件跑同一支檢查、同一組參數）。誠實邊界：票 SoT 之對外用語限「產出端已有檢查點」，不得稱強制 | 內容型 |
| E-005 | G-7 | commit-msg:g7_trailer_precheck.sh ＋ 手動 gov_check.sh --no-probe:govb1_final_gate.sh --only g7 | 豁免 | PreToolUse不可：判定式為 base..HEAD 之 endpoint 淨差，寫入前無 commit 可算。PostToolUse不可：判定對象是 commit 的屬性（訊息末段之 Governance-Scope trailer），寫檔當下該物件尚不存在；而寫檔當下算得出的「路徑在 scope 外」若單獨告警，對本 repo 多數新增路徑恆真 ⇒ 高頻無訊號。部分閘：有——scripts/g7_trailer_precheck.sh:83（commit-msg 階段：staged 含 scope 外路徑而訊息末段無 trailer 即擋；四向反例已驗，含 trailer 放中間段仍擋、只動 scope 內檔則放行）。🔴 R6 更正：原登記「本票不存在可前移的靜態子集」為事實錯誤——同日兩次 G-7 紅正是此子集，且因豁免須「該路徑只被帶 trailer 的 commit 觸及」，補後續 commit 解不掉，只能重寫歷史 | n/a |
| E-006 | 測試套件 | pre-push:gov_check.sh 第 5 段 | 豁免 | PreToolUse不可：全套 pytest 為十分鐘級，寫入前執行會使每次編輯停擺。PostToolUse不可：同上——此為**成本型**而非判定型限制，技術上跑得動、代價不可接受，兩者須誠實區分。部分閘：有——各票之承重判準已分散於本表其他列之產出端掛載，代表兩處為 scripts/factkey_write_guard.sh:119 與 scripts/doc_format_precheck.sh:151；測試子集化之提案見 HANDOFF 之「把測試選擇機械化」節（未開工） | n/a |
...
| E-008 | B-7 | — | 豁免 | PreToolUse不可：判定對象為委員產出的戳記行，派工前尚不存在。PostToolUse不可：🔴 R8 實測更正（2026-08-14T19:0x+08:00）——原登記「不經主控端 Edit/Write ⇒ 該 hook 不會被觸發」**只對了一半**：真的把探針掛上 PostToolUse:Bash 後，派工指令**確實觸發**它（實證：scripts/probe_dispatch_posttooluse.sh，記錄於 .claude/tmp/probe_b50.log）。真正的限制是**時點**：委員派工依合約以背景執行（前景必 timeout），hook 觸發於**啟動當下**，該時點 handoffs/ 下只有 brief、無任何委員產出檔（實測 ls 僅列出 BRIEF 一列）⇒ 戳記行尚不存在，無標的可判。部分閘：有——scripts/cx_run.sh:1060（派工 prompt 逐字注入 task-id 並明令 brief 內範例值不得採用，從源頭消滅手抄）＋ scripts/cx_run.sh:422（task_id 缺漏即拒派）。🔴 R6 更正：原登記「改法未完成、現樹無對應檢查可掛、部分閘無」為事實錯誤 | n/a |
| E-009 | B-10 | PostToolUse:Edit,Write:scripts/doc_format_precheck.sh | 產出端 | 實作位置：scripts/template_check.sh:324（dext 分支之必填錨點檢查；由 scripts/doc_format_precheck.sh:81 判型後呼叫）。🔴 R6 更正：原登記為豁免，理由寫「輸入為完整文件，非單次編輯內容」——那只證明不能 PreToolUse；實跑反例（寫入缺錨點之 D 延伸檔）當下即紅，證明 PostToolUse 本就掛得上、且早已掛上 | 內容型 |
| E-010 | B-15 | PreToolUse:Task,Bash,Write:scripts/gate_check.sh | 產出端 | 實作位置：scripts/gate_check.sh:225（_gate_cmd_is_dispatch 之派工判定，掛 PreToolUse ⇒ 指令送出前即判）。🔴 R6 更正：原登記為豁免，理由「須完整指令上下文方能判」——完整指令字串正是 PreToolUse 的輸入，該理由不成立。誠實邊界：本票之殘留是**誤擋率**而非缺掛載，對外不得宣稱誤擋已修復 | 內容型 |
...
| E-012 | B-19 | PostToolUse:Edit,Write:scripts/doc_format_precheck.sh | 產出端 | 實作位置：scripts/doc_format_precheck.sh:196（handoffs 下含 brief-kind 標記之檔即路由至 brief_conformance_check）。🔴 R6 更正：原登記「部分閘是否已掛：無」為事實錯誤——實跑反例（寫入不引用範本之 brief）當下即紅。殘留 R-12（full path 不驗 EXPECTED-DELTA）仍在，屬檢查深度不足，非未掛載 | 內容型 |
...
| E-019 | B-39 | PostToolUse:Edit,Write:scripts/doc_format_precheck.sh | 產出端 | 實作位置：scripts/completeness_check.sh:157（heading 路由與必填欄判定；由 scripts/doc_format_precheck.sh:151 之 findings 分支經 cx_run --selfcheck 呼叫）。🔴 R6 更正：原登記為豁免且原填 :135 落在註解行；實跑反例（P0 來源摘要寫行號而非雜湊）當下即紅。誠實邊界：跨檔完整性（來源 ID 是否全在綜合）仍須 lock 與全部來源，屬合理的消費端檢查 | 內容型 |
| E-020 | B-50 | — | 豁免 | PreToolUse不可：判定需「派工前」與「派工後」兩個工作區快照之差，寫入前只有單點。PostToolUse不可：🔴 R8 實測更正（2026-08-14T19:0x+08:00）——真的掛上 PostToolUse:Bash 探針實測，得到**兩個各自獨立就足以判死**的理由：①hook 只有**一個**時間點，「派工前」那個快照不在其視野內，差值算不出來 ②派工依合約背景執行 ⇒ 觸發當下「派工後」根本尚未發生（實測該時點 git status --porcelain 計數等於派工前之值）。原登記之成本理由（對每個 Bash 呼叫做 git status 全掃）仍成立，但那是次要理由，主因是上述時點問題。部分閘：有——scripts/committee_run.sh:365（_ws_snapshot，派工前後比對）。🔴 R6 更正：原填 :267 落在註解行。🔴 R7 更正：R6 改填之 :311 於同輪內因本檔新增 mkdir 守衛而位移成註解行，三家 r2 一致以 --check rc=1 攔下——**行號引用會隨上游編輯漂移，這是本機制的內建代價，換來的是不會靜默腐爛** | n/a |
| E-021 | B-48 | PreToolUse:Task,Bash,Write:scripts/gate_check.sh | 產出端 | 實作位置：scripts/gate_check.sh:259（_gate_precheck_content 之 no-findings-expected 標籤真偽判定，掛 PreToolUse ⇒ debt_clear 指令送出前即判）。工具內部第一層在 scripts/debt_clear.sh:922（_dc_zero_findings_guard，結案前擋）。誠實邊界：本層**不比工具內部更早**（PreToolUse 之後工具緊接著就跑），價值在 defense-in-depth 與「指令字串即可判」，**不得對外稱前移**。逃生口 --zero-findings-verified 落審計（scripts/debt_clear.sh:771） | 內容型 |
| E-022 | B-62 | cx_run.sh 交件當下（執行端） | 豁免 | PreToolUse不可：委員產出由執行端 CLI（codex／cursor-agent／grok）直接寫檔，主控端沒有 Edit／Write 事件可掛，寫入前該檔不存在。PostToolUse不可：同一原因——觸發源不是主控端工具；主委事後手動 register-output 才是 Edit 路徑，但那已在交件之後。部分閘：scripts/cx_run.sh:714（cx_run 收到 STATUS: DONE 或 VERDICT: 行即呼叫 gate.sh register-output ⇒ verdict_parse 拒收即寫 audit verdict_rejected；B3／B4 審碼實戰各擋一次）。第二層：scripts/gate.sh:1150（開輪／領 token 前呼叫 verdictgate_check 讀裁決） | n/a |
| E-023 | B-62 | PreToolUse:Task,Bash,Write:scripts/gate_check.sh | 產出端 | 實作位置：scripts/gate.sh:1150（dispatch／--impl-self 於發 token 前呼叫 verdictgate_check；前批任一家 blocked 未由原提出方 CLOSED 或無裁決 ⇒ 拒發）。路由：gate_check 於 PreToolUse 擋無 token 之派工指令 ⇒ 開輪必經 gate.sh ⇒ 必經 verdictgate_check。誠實邊界：讀的是 audit 狀態（多輪累積），故為一致性型；本票 B2–B4 開輪皆由此放行 | 一致性型 |
...
| E-026 | B-63 | PreToolUse:Edit,Write:scripts/live_doc_write_guard.sh | 產出端 | 實作位置：scripts/_live_doc_write_guard.py:690（寫入目標先解析成實際被寫之檔之正名——以 inode 判定在 repo 內、逐段取實際檔名；所給路徑不等於正名、解析經過任何 symlink 或目標為硬連結即為別名，別名寫活文件即擋；Edit／Write 以寫入後全文判定：新增行之識別碼＋狀態字面〔判定碼與 gen_fact_key_blocks.sh 共用〕、歷史專區外之刪除線／考古字面／finding ID、歷史專區指標文法、交接檔封閉文法之內容型部分）。誠實邊界：Bash 轉向與委員 CLI 之寫入不經本閘，由 E-027 之 pre-commit 兜底 | 內容型 |
...
| E-028 | B-63 | PostToolUse:Edit,Write:scripts/factkey_write_guard.sh | 產出端 | 實作位置：scripts/_gen_fact_key_blocks.py:2127（docrot2_status_keys 之封閉狀態值、下一步非空、識別碼跨全部狀態 key 唯一；同一 --check 內另有 rows_source／rows_filter 物化與交接投影對讀）。誠實邊界：改狀態後尚未 --write 之中間態必紅，故為一致性型 | 一致性型 |
...
| E-030 | B-63 | pre-push:gov_check.sh 第 1b 段 | 豁免 | PreToolUse不可：同一計數字面雙落點須比對同檔多行，改一處而另一處待改之中間態必紅，寫入前判定會擋住正常兩步編輯。PostToolUse不可：同一中間態理由；寫檔當下只由 spec_xref_hook.sh 以警告提示。部分閘：scripts/gov_check.sh:276（對本次 diff 之 docs/*.md 跑 spec_count_audit.py --dupes，命中即計入失敗）。宣稱範圍（DOCROT Task 1.1–1.3）：只擋 docs/*.md 同一「共 N 條」字面寫在活文多行，不涵蓋交接檔，亦不涵蓋其他語意互斥寫法 | n/a |
| E-031 | B-63 | cx_run.sh 交件當下（completeness_check.sh --single） | 豁免 | PreToolUse不可：委員交件由執行端 CLI 直接寫檔，主控端沒有 Edit／Write 事件可掛。PostToolUse不可：同一原因——觸發源不是主控端工具。部分閘：scripts/completeness_check.sh:1712（--single 交件路徑：P0／P1 之 CODE-ANCHOR 與 MUTATION 兩行須在碼證欄位內、不得在 HTML 註解內，錨點不得落在 HISTORY-BEGIN..END 或沿革段；DOCROT Task 1.4、1.6） | n/a |
| E-032 | B-63 | git commit-msg hook（commit 成立前） | 豁免 | PreToolUse不可：判定輸入是 commit 訊息全文，主控端 Bash 指令字串只有 git commit -F 檔名，訊息本體寫入前拿不到。PostToolUse不可：Bash 完成時 commit 已成立，擋不了；非 Claude 之 commit 亦不經主控端 hook。部分閘：scripts/verification_claim_check.py:2142（commit-msg 掃描：「三家共同結論」類背書語須有同 task-id 之委員 audit 佐證，否則 rc=1；DOCROT Task 1.5） | n/a |
| E-033 | B-63 | — | 豁免 | PreToolUse不可：DOCROT Task 1.7 為成效判準之單一出處（定案收斂檔），不是對寫入之判定，沒有可擋之內容。PostToolUse不可：同上，無判定式可跑。部分閘：scripts/_docrot2_metrics.py:381（判準之機械計算由 DOCROT2 Task 3.2 承接：scripts/docrot2_metrics.sh 只讀 audit 與 scripts/docrot2_metric_contract.json，計算四條及格線） | n/a |
| E-034 | B-63 | committee_run.sh／cx_run.sh 開債前（brief_conformance_check.sh） | 豁免 | PreToolUse不可：brief 位於 handoffs/，判定須整份 brief 之逐行佔位比對，骨架剛產出、尚在逐欄填寫之中間態必紅，寫入當下擋會擋住正常填寫。PostToolUse不可：同一中間態理由。部分閘：scripts/brief_conformance_check.sh:339（開債前逐行比對骨架佔位字面，fence 與引用行不算；DOCROT Task 1.8） | n/a |
| E-035 | B-64 | PreToolUse:Task,Bash,Write:scripts/gate_check.sh | 產出端 | 實作位置：scripts/gate_check.sh:282（_gate_check_redispatch_allow 呼叫：封閉文法之同輪重派指令須有綁定許可、秘密與審計對證、鎖內消費才放行）。發放端 scripts/gate.sh redispatch、棄置例外與銷帳端保存檔處置查核 scripts/debt_clear.sh、開輪路徑檢查 scripts/committee_run.sh、派工器租約與認領 scripts/cx_run.sh 為主委主動呼叫或派工器自身之工具，非寫檔事件 | 內容型 |
...
| E-039 | B-63 | PreToolUse:Edit,Write:scripts/live_doc_write_guard.sh | 產出端 | 實作位置：scripts/_live_doc_write_guard.py:510（擋下事件：寫入前守衛擋下時每個相異規則寫一筆 docrot2_gate_block，寫入失敗不改變擋下結果）＋ scripts/_live_doc_registry.py:301（emit_block_event 唯一寫入實作，pre-commit 之 --staged 違規與未登記 .md 亦同）。誠實邊界：payload 不可解析或寫入目標無法解析為 repo 路徑者無路徑可記而不寫；--tree 重放非擋下不寫；DOCROT2 Task 3.2 | 內容型 |
| E-040 | B-63 | git pre-commit hook（commit 成立前） | 豁免 | PreToolUse不可：命中判定之識別碼取自 scripts/fact_keys.json 全部狀態 key，新增識別碼即可讓未被編輯之既有行命中，寫入前之單檔 payload 看不到其他檔；殘留列之過期須全專案掃描，而命中消失（改文件）與刪列（改清單）為兩步編輯，寫入當下判定中間態必紅。PostToolUse不可：同一中間態理由；新增行之手寫狀態已由 E-026 於寫入前擋。部分閘：scripts/git_hooks/pre-commit:200（暫存含 .md 或判定輸入檔時跑 live_doc_registry_check.sh --migration --index：以暫存快照〔清冊、內容、登記、fact_keys、殘留清單與生成器皆取 index〕判定，命中檔須列 scripts/docrot2_migration_residuals.json、已無命中或暫存已刪之列須刪、清單檔不合規即擋）＋ scripts/_live_doc_registry.py:579（判定實作，與寫入前守衛共用豁免區與 --status-hits 判定入口）；DOCROT2 Task 4.1。誠實邊界：hook 本身可被工作樹改寫或 --no-verify 略過，屬登記表具名殘留 5（只擋意外不擋蓄意） | n/a |
