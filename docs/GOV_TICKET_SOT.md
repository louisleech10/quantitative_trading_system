...
| 0630 | B-63 | 部分完成 | DOCROT 未治好文件類多輪：只管規格檔、進度仍多處手寫、交接檔殘留舊案；使用者 2026-09-15 裁定修正並全專案涵蓋（DOCROT2） | 還缺：收票前治理全套（gov_check.sh --no-probe）跑一次並以改動前基準逐名比對無新增紅，收票時寫入 docrot2_metric_contract.json 之 closure_sequence（四批皆經三家審碼閉合，批次進度見 docrot2-batch-status；委員組成見 governance_families.json 之 active_stampers） |
