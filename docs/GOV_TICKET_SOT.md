...
| 0640 | B-64 | 進行中 | 委員 CLI 失敗且無產出（例：模型容量滿）而該家仍在 active_stampers 時，同輪重派須 dispatch token、token 被本輪 OPEN 債拒發、銷帳缺該家產出、abandon 因該輪已有他家結果被擋 ⇒ 只能請使用者在終端機手動重派；違反「解除阻塞之路徑不得被同一族閘擋住」，同族第四次 | 實作批 b1（Task 1.1–1.8）已落地並 commit；審碼 r1–r5 共 13 條 P1 全部由主委複驗後處置（12 條機械修補、1 條降級為具名誠實邊界⑦）。還缺：r6 複審由原提出方閉合 → 收批 → 回 DOCROT2 D2C |
