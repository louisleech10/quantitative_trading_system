...
| 0630 | B-63 | 部分完成 | DOCROT 未治好文件類多輪：只管規格檔、進度仍多處手寫、交接檔殘留舊案；使用者 2026-09-15 裁定修正並全專案涵蓋（DOCROT2） | 還缺：D2C–D2D 兩批落地與委員審碼閉合（委員組成見 governance_families.json 之 active_stampers；批次進度見 docrot2-batch-status） |
...
| E-031 | B-63 | cx_run.sh 交件當下（completeness_check.sh --single） | 豁免 | PreToolUse不可：委員交件由執行端 CLI 直接寫檔，主控端沒有 Edit／Write 事件可掛。PostToolUse不可：同一原因——觸發源不是主控端工具。部分閘：scripts/completeness_check.sh:1742（--single 交件路徑：P0／P1 碼證須含 CODE-ANCHOR 與 MUTATION 兩行，錨點不得落在 HISTORY-BEGIN..END 或沿革段；DOCROT Task 1.4、1.6） | n/a |
...
| E-033 | B-63 | — | 豁免 | PreToolUse不可：DOCROT Task 1.7 為成效判準之單一出處（定案收斂檔），不是對寫入之判定，沒有可擋之內容。PostToolUse不可：同上，無判定式可跑。部分閘：scripts/_docrot2_metrics.py:349（判準之機械計算由 DOCROT2 Task 3.2 承接：scripts/docrot2_metrics.sh 只讀 audit 與 scripts/docrot2_metric_contract.json，計算四條及格線） | n/a |
...
| E-037 | B-63 | cx_run.sh 交件當下（completeness_check.sh --single --round-id） | 豁免 | PreToolUse不可：委員交件由執行端 CLI 直接寫檔，主控端沒有 Edit／Write 事件可掛。PostToolUse不可：同一原因——觸發源不是主控端工具；主委自產之 findings 檔另由 doc_format_precheck.sh 於寫檔當下經 cx_run.sh --selfcheck 走同一判定（未給 round id 即須類別）。部分閘：scripts/completeness_check.sh:1738（--single 之類別判定：輪次之開債序號大於門檻、查無輪次或未給 round id ⇒ 每條 finding 含零 findings sentinel 須 fence 外恰一行合法類別；門檻與值集唯一來源 governance_verdicts.json；DOCROT2 Task 3.1） | n/a |
| E-038 | B-63 | debt_clear.sh 收案前（_docrot2_metrics.py emit-round） | 豁免 | PreToolUse不可：量測事件之輸入為收案當下之收斂檔、委員交件、audit 與 HEAD，收案前收斂檔仍可修改，寫入前沒有定稿可計。PostToolUse不可：同一原因——收斂檔寫入當下之群集與類別為中間態，收案才凍結。部分閘：scripts/debt_clear.sh:891（門檻後之輪收案前寫 docrot2_round_metric；缺委員類別、缺主委欄、開債事件缺欄或寫入失敗即拒銷）＋ scripts/_docrot2_metrics.py:317（經 audit_append.sh 寫入）；DOCROT2 Task 3.2 | n/a |
