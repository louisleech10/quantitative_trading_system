...
| 0640 | B-64 | 未開工 | 委員 CLI 失敗且無產出（例：模型容量滿）而該家仍在 active_stampers 時，同輪重派須 dispatch token、token 被本輪 OPEN 債拒發、銷帳缺該家產出、abandon 因該輪已有他家結果被擋 ⇒ 只能請使用者在終端機手動重派；違反「解除阻塞之路徑不得被同一族閘擋住」，同族第四次 | 還缺：使用者白話審閱 docs/REDISPATCH_SPEC.md v12（三家 proceed、r1–r11 findings 皆由原提出方閉合）→ 實作批 b1（Task 1.1–1.8）→ 三家審碼閉合；排在 DOCROT2 D2C 之前（交接列 HP-REDISPATCH） |
...
| E-021 | B-48 | PreToolUse:Task,Bash,Write:scripts/gate_check.sh | 產出端 | 實作位置：scripts/gate_check.sh:259（_gate_precheck_content 之 no-findings-expected 標籤真偽判定，掛 PreToolUse ⇒ debt_clear 指令送出前即判）。工具內部第一層在 scripts/debt_clear.sh:851（_dc_zero_findings_guard，結案前擋）。誠實邊界：本層**不比工具內部更早**（PreToolUse 之後工具緊接著就跑），價值在 defense-in-depth 與「指令字串即可判」，**不得對外稱前移**。逃生口 --zero-findings-verified 落審計（scripts/debt_clear.sh:573） | 內容型 |
