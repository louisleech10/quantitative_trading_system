...
| `quant_standard_check.sh` | 常態檢查 | gov_check narrow_check_router  |
...
| `todofmt_manifest_guard.sh` | 常態檢查 | PostToolUse  |
...
各票狀態之唯一來源＝`docs/GOV_TICKET_SOT.md` 生成區塊；本檔不抄張數。
...
| 票 | 機制在用？ | 為何不能收案 |
|---|---|---|
| `B-25` | 是：產出端守衛在跑 | 票內另有三段未閉：語意互斥機械偵測不到、既有散文判準不溯及既往、機制證據登記訊號近零 |
| `B-31` | 是：產出端有檢查點 | 🔴 票面明寫**不得說「強制」**——只擋意外不防蓄意 |
| `B-38` | 是：產出端在跑 | 委員若沒讀到指示，交件照樣過，等收斂才炸 |
| `B-16` | 是：產出端在跑（`SPEC`／`TODO` 檔） | 原條文「散文契約偵測」主幹未做，只做了擴充 A/B/C；寫檔階段刻意**不執行** ASSERT |
| `B-39` | 是：產出端在跑（findings 檔） | 跨檔完整性須 lock 與全部來源，屬合理的消費端檢查；群集盲點未閉 |
| `B-10` | 是：產出端在跑（D 延伸檔） | 施工面無已知殘留，但狀態與現樹之落差尚未複核 |
| `B-19` | 是：產出端在跑（brief 檔） | 檢查深度不足：`R-12`——full path 不驗 `EXPECTED-DELTA` |
| `B-15` | 是：派工前在跑（`PreToolUse`）| 殘留是**誤擋率**而非缺掛載；誤擋事件無紀錄 ⇒ 無法量測、改完無法驗證 |
| `B-49` | 是：產出端在跑（閉合證據檔） | 🔴 2026-08-14 更新：靜態子集**已前移**（`scripts/b49_closure_static_check.py`，經 `narrow_check_router` 掛 `PostToolUse`）。四條具名殘留未閉（見 `docs/GOV_B49_ASBUILT_DELTA.md` §3）；隔離重放依其本質仍留在 pre-push |
| 其餘 8 張 | 否：多未掛 | **改法本身未完成** ⇒ 無檢查可掛，非「不想掛」 |

🔴 多數票卡在「**改法本身沒做完**」，不是卡在「機制沒掛上」。
🔴 **「在產出端跑」不等於「該票做完了」**——上表九張之票狀態以 `docs/GOV_TICKET_SOT.md` 為準。
...
2. **腳本檔頭出現票號，不代表是該票的產物。**
   ...
   B-23 在票 SoT 之狀態維持無交付是正確的。⇒ **提及 ≠ 產出**。
