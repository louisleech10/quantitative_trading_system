...
6. 🔴 **本表是目錄／投影，不是實作行為的 oracle。** 改表不改碼、改碼不改表，本檔仍會綠。
   行為承重在 `對應測試`（判準 `C-013`–`C-023`），不在本表。
...
9. 🔴 **偶發紅的證據界線。** r2 三家皆觀察到「registry key 被報未登記」的偶發假紅；
   主委以注入式實驗（把讀 key 的函式改成直接失敗）重現出**逐字相同**的訊息，
   據此把根因定在「`jq` 的 rc 被 process substitution 吞掉、失敗被當成空註冊表」並修補。
   **但從未獨立重現「pytest 全跑時 jq 真的失敗」這件事**（`codex` r3 主動保留此界線）。
   ⇒ 修補的正當性**不依賴**該重現：吞掉 rc 本身就是 fail-open——同一個 bug 在
   criteria／mechanism 路徑造成假紅，在「無 fact-key ⇒ rc=0」的兩條路徑造成**假綠**。
   不得宣稱「偶發紅已根治」，只能說「該 fail-open 已封、且它會產生完全相同的症狀」。
10. 🔴 **只有讀 fact-key 清單那條路徑的 `jq` rc 被封。** 其餘 `jq … 2>/dev/null` 呼叫
   （如 schema 陣列展開）屬**不同錯誤類別**——失敗時多為「迴圈空跑」而非「空清單」語意
   〔`grok` r3 指出，兩家未升級為 BLOCKING〕。全檔 rc 稽核不在本項範圍，
   不得宣稱「`jq` rc 已全面 fail-closed」。
11. 🔴 **子樹旁路是「未發現」不是「不存在」。** 發現曲線尚未收斂：
   r1 三家各找到一個（巢狀子彈／空行／裝飾起點）、r2 codex 又找到第四個（巢狀起點覆寫
   boundary）、r3 三家獵捕後未發現第五個。**日後修改 parser 必須重跑該組候選**
   （反向外層縮排、連續三層巢狀、有序清單、blockquote、peer 星號、全形空白、tab）。
