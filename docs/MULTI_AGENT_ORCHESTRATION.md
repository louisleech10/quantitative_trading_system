...
**現行分工(2026-07-10 使用者指示,同日二調):中/大實作=Codex(`codex`,gpt-5.6-sol high)/Grok 4.5(`grok`)依使用者額度動態切換;review/adversarial 主打=Composer 2.5(`cursor-agent`),非實作的另一實作主力當第二審查腿;實作型 SPEC/TODO 初稿=Composer 起草→兩家非作者審(試點中;設計/研究型仍全員各產完整版);簽核 quorum=Claude+Codex+Composer 不變;小=Claude 自做。Grok 資格報告→docs/reviews/grok_4_5_evaluation.md,前 ~5 個真實任務加密驗收+記分。** 選層為**動態**:一律以使用者最新指示為準(依各 agent usage 切換,未來或加新執行端;新執行端須先過 §8 T-D 對等性測試才可寫入)。
