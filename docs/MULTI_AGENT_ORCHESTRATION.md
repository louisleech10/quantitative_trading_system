...
🔴 **暫調(2026-09-14 使用者):討論/code review/adversarial/戳記暫為 Codex+Composer(Cursor) 兩家**——使用者原話「在我通知你變回三家全員前，委員就剩兩家Codex+Cursor」(grok CLI 帳戶餘額耗盡 402;Cursor 內模型已由使用者切為 Grok)。機器版=`scripts/governance_families.json` 之 `active_stampers`(暫停者=與 `review_families` 之差集);**恢復只在使用者通知後**把 grok 加回該行。委員之模型一律依使用者在各 CLI 設定檔之選擇(`cx_run.sh` 自 2026-09-14 起不再寫死型號)。下行為暫調前之常態分工。
...
<!-- BEGIN GENERATED: committee-roster -->
| 序 | 家族 |
|---|---|
| 001 | codex |
| 002 | composer |
...
### ★ 文件分層：工作文件 ≠ 對人類的說明（2026-07-29 使用者定死）

| | 工作文件 | 白話說明 |
|---|---|---|
| **是什麼** | SPEC／TODO／brief／reconcile | 文件**定案後**另外寫給使用者的解釋 |
| **讀者** | Claude ↔ 委員 | 使用者 |
| **要求** | **嚴謹、精確、無歧義、可用技術專業描述** | 門外漢看得懂；專有名詞附「中文(English/縮寫)」 |
| **禁止** | **為了讓人類好讀而散文化** | 貼原始碼／未解釋的縮寫 |

**兩者是兩份產物，不是同一份的兩種語氣。** 把「白話」要求套進 SPEC，會逼出修飾性散文，而散文無法機械驗證。

**出生事故**：P1-6 把條件式驗收寫成散文 → 「條文與真實行為相反」機械檢不出來 → 起草者寫字串比對補救，**被委員連六輪用不同散文形式打穿**（全形/半形標點各 6 種、斷言跨行、前綴/後綴否定 8 種），耗掉整個 session，且期間 B2 實作被卡住。

**根治＝條件式驗收斷言用固定文法**（詳見 `templates/SPEC_TEMPLATE.md` 檔頭）：
```
ASSERT <命令> WHEN <key>=<value> ... THEN rc=<n>|rc!=<n>
```
機檢規則一句話：**驗收段裡每次提到受管命令，都必須在 `ASSERT` 行內**；散文中出現即 `exit 1`。
⇒ 不需切段／判否定／處理標點 ⇒ **散文繞法全部不適用，逃脫點由「無限」降為「零」**。

> **相關記憶更正**：`feedback_brief_layman`（白話+術語對照）**只適用於給使用者的輸出**，
> **不適用** SPEC/TODO/brief。該記憶原文已寫「碼證留 SPEC」，是起草者過度外推所致。
...
- **codex 沙箱 shell 管線 hang（票4 候選族 2026-07-12，Grok recon 修正歸因）**：macOS `-s workspace-write` 下多進程 shell 管線間歇卡死，與運算量無關。**候選=macOS 專屬 upstream 族 #18243/#19020（皆 OPEN，danger-full-access 才行）**；原疑的 #7852 收斂 repro 是 `--full-auto` 特有，**我方無 --full-auto → 對不上**。無官方已修版本/安全區間/乾淨退化分界。模型 gpt-5.6-sol 非死鎖主因（CLI 層）。**mitigation 不受歸因不確定影響**。故多進程 shell 管線（`comm`/`sort`/`diff`、`<()` process substitution、長 `|` 串接、bash 腳本）會間歇卡死，**與運算量無關**（32 行檔 `comm` 也卡），Python/pytest/rg/單命令讀寫檔正常。
