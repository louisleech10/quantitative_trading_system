...
4. 🔴 **完成綁定**：票 SoT 中狀態屬**完成語意集合**（`S2.1`；值集唯一來源＝`scripts/fact_keys.json` 之 `_schema.enforcement_completed_statuses`）的
...
8. 🔴 **理由体例與引用對證**（`S6.1`，見下節）：豁免列須寫出兩側各自不可行的理由；
   具名段之後的 `<檔>:<行>` 須存在、非註解、且**不得零引用**。
9. **schema 散文不得提及作廢 key**（`S6.1` 之 `CODEX-R3-P2-01`）：
   `_schema.enforcement_note` 內任何 `governance-<名>` token 都必須是現存的 fact-key。
   🔴 連「原文指向 X，已刪除」這種歷史括註也不行——`grep` 仍會找到它。
10. **票須寫出「還缺什麼」**（`S6.2`）：票 SoT 之 `狀態依據` 欄須含
    `還缺：` 或 `無殘留`，且標記後之內容非空。標記集合與生成器寫死之值**集合相等**才算數
    （`// []` 然後長度 0 就跳過＝可被清空停用，三家各自實構過）。
11. **票全集對帳**（`S6.2` 之 `CODEX-R1-P1-04`）：`ticket_universe --check` 已接進
    同一個 `--check` 入口。🔴 沒有這道，檢查 ⑩ 只遍歷**現存**列
    ⇒ 把整列票刪掉就沒有「還缺什麼」可填——**刪列不等於票不存在**。
...
1. **涵蓋範圍＝全部已交付票**（`S4.3` 起），逐張有列；各票狀態與張數以票 SoT 生成區塊為準。
   檢查 ④ 綁的是**完成語意集合**（`S2.1`；值集見 `_schema.enforcement_completed_statuses`），非單一字面。
   尚未涵蓋者＝無交付物之票——無檢查可掛，
...
8. 🔴 **檢查 ④ 之鑑別力由自造測試票的 pytest 承擔**；實際攔截對象＝票 SoT 中轉入完成語意之票。
   `S6.1` 曾更正本條與殘留 1 之矛盾自述——**同一份文件內兩條互相矛盾的自述，正是本表要治的病。**
9. 🔴 **「改判為產出端」不等於該票達完成語意。** `S6.1` 把五列改判為產出端，
   意思只是「該票**已交付的那個檢查**跑在產出端」，不是它沒有殘留。
   五票之狀態與各自殘留見票 SoT 之狀態依據欄。
