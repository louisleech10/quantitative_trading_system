...
## §5 白話閘（使用者要裁的題，consult 收斂後再問；先列）

- A／B 的「反例」用哪一類（§2-4 之 a／b／c）、是否需要「全部 K 線驗證」才算 (a) 完成？
- `decision_offset_bars > 0` 之上限與預設；`two_stage` 之 UI 表達。
- (b) 之隨機對照組：抽樣範圍（同 symbol／同 TF／排除事件鄰域？）與 estimand。
