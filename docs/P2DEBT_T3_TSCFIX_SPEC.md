# P2 債票 3 — frontend feature-factory 測試檔 TypeScript 錯誤清零 — SPEC(正式版)

> 定稿來源:handoffs/P2DEBT-T3-SPEC-DRAFT-R4.md(R1 Composer→grok APPROVE+codex 3 輪定向修 R2/R3/R4)
> RECONCILE-STAMP:grok=handoffs/P2DEBT-T3-SPEC-REVERIFY-R4-grok.md;codex=handoffs/P2DEBT-T3-SPEC-REVERIFY-R4-codex.md
> task-id:p2debt-t3 | 正式化:2026-07-11 | 內容=R4 凍結稿,除本頭注外零改動
...
- FACT-RECEIPT: 錯誤清單摘自 `cd frontend && npx tsc --noEmit 2>&1`（§A 基線 TypeScript 條目；Composer 實跑 2026-07-11）
...
- **待確認：無**
- **已確認結果**：FACT-RECEIPT: 2026-07-11 HANDOFF 票 3 — 修法=測試檔對齊既有生產型別；禁 `@ts-ignore` / `@ts-expect-error` / `as any` 靜默；禁改 `src/lib/types.ts` 與非測試 `src/`。
...
- **防假綠（可執行 gate；區分測試數 vs expect 站點）**：
  ...
  - **Gate A — 禁靜默型別繞過（負向掃描；用 rg exit code，禁止 `\| wc -l` fail-open）**：
    ```bash
    for f in \
      frontend/src/components/feature-factory/__tests__/run_lifecycle.test.tsx \
      frontend/src/components/feature-factory/__tests__/RunManagerPanel.batchDeleteWhole.test.tsx \
    ...
    rg_rc=$?
    # PASS: rg_rc=1（無命中）→ gate exit 0
    # FAIL: rg_rc=0（命中行已印出）或 rg_rc=2（rg 錯誤，如路徑不存在）→ gate exit 1
    test "$rg_rc" -eq 1
    ```
    - **R3 缺陷（Codex BLOCK）**：`rg … | wc -l` 在 rg 錯誤（rc=2，如路徑不存在）時 pipe 仍回 shell rc=0 且印 `0`，假 PASS。
    - **R4 polarity receipts（Composer 唯讀實跑 2026-07-11）**：
      - **Polarity 1 — 現樹五檔**：existence `test -f` 全過；`rg` 無輸出、`rg_rc=1` → `test "$rg_rc" -eq 1` exit 0（PASS）
      - **Polarity 2 — 合成命中**：`echo '// @ts-ignore' > /tmp/p2debt-t3-gate-a-synth.ts`；`rg -n '…' /tmp/p2debt-t3-gate-a-synth.ts` → `1:// @ts-ignore`、`rg_rc=0`（FAIL，命中已印）
      - **Polarity 3 — bogus path**：`rg -n '…' /nonexistent/p2debt-t3-bogus.ts` → `rg: … No such file or directory`、`rg_rc=2`（FAIL）；五檔 existence gate 亦會在缺檔時 `exit 2`（FAIL）
...
R4-CLOSURE: Gate A fail-open 修正——廢止 `rg … | wc -l`（rg rc=2 時 pipe 假印 0、shell rc=0）；改五檔 `test -f` 前置 + 直接讀 `rg_rc`（1=PASS、0=FAIL 印命中、2=FAIL）；三極性唯讀 receipt 已附 §V Gate A
