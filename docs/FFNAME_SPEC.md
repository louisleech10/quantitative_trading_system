...
- **凍結時機 / reference**：動工前以當下 HEAD 跑一次，存 `tests/_golden/ffname/baseline.json`：逐欄（欄名序）`dtype`、`shape`、NaN mask sha256、值 sha256，以**舊欄名**為鍵；並錄下該 run 中 `FeatureFactory._build_indicator_specs` 回傳之**全部 spec 之 `source` 與 `indicator` 值**（凍結腳本包裝該方法記錄其回傳；各週期聯集）與 run 之原始資料欄名（r3 codex P1-01）。§G 輕量設定須另啟用 entropy（`windows` 至少兩個）以涵蓋自製 atomic metadata。
- **舊名→新名映射算法（封閉，r1 composer P2-01；r2 codex P1-02 補全）**：作用於 L2 Cross／Ratio、L4 lag（含其 `_Lag_<k>` 後綴，涵蓋 frame 路徑 `lag_features.apply_to="all"` 把 L2 欄送入 L4 之產物）；①raw 來源段：清單＝baseline 錄得之 `source` 值與原始資料欄名中含 `_` 者（含三段式 `taker_buy_volume`、自製 metadata 之 `close_return`；r3 codex P1-01）；來源 `{p1}_{p2}_…_{pn}` 經 CGSA 標記器（`split("_",1)` 於首段後插週期）之舊形為 `{p1}_{tf}_{p2}_…_{pn}`，改寫為 `{p1}-{p2}-…-{pn}_{tf}`；②指標段：清單＝baseline 錄得之 `indicator` 值中含 `_` 者——即 L2 分組鍵實際使用之指標段（涵蓋 TA-Lib 多輸出之 `spec.name + "_" + output_name`，如 `BBANDS_Upper`、`MACDFIX_Hist`、`AROON_aroondown`，與自製 metadata 之 `fractal_dimension` 等），自錄得值列舉，禁手寫；把整段 `_{X}_{Y}_` 改寫為 `_{X}-{Y}_`；其餘字元一律不動。**禁**以全欄 `replace("_","-")` 作映射。映射 helper 須有單元測試：`close_12h_trend_BBANDS_Upper_13_55_Cross`→`close_12h_trend_BBANDS-Upper_13_55_Cross`、`taker_12h_buy_volume_Lag_1`→`taker-buy-volume_12h_Lag_1`、`close_12h_return_entropy_fractal_dimension_55_100_Cross`→`close-return_12h_entropy_fractal-dimension_55_100_Cross`。
