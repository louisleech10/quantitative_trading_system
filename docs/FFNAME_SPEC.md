...
> 版本：v1（主委起草；偵察與決策檔之差異見 §A「對決策檔之更正」）
...
- **對決策檔之更正**（以實測為準）：
  ...
  3. 決策檔「ADF 白名單」未提；改名之數值副作用見本節第 4 條 receipt 與 Task 1.4。
- **待使用者確認**：待確認：無
...
- 特徵**值**、列數不變；欄數不變（僅改名）。允許變動：3,596 欄之欄名、132 欄 Ratio 之 ADF／fracdiff 路徑（fracdiff 覆蓋 L2 時）、LINEARREG_SLOPE 等 Cross／Ratio 欄於 `column_group_registry._column_sort_key` 之排序位置、`feature_schema_hash`、config hash。**manifest 新增一鍵 `feature_naming_version`**（決策檔 A-5 之機械防護，唯一新增鍵）。
...
## §G Golden / Baseline
- **feature/kline 條件**：適用。真實 `data_cache/feature_klines/kline_cache.h5`；沿用 `tests/feature_engineering/fftfmeta_golden_helpers.py` 之 run 隔離與輕量真實設定，但設定須開啟 L2 Cross／Ratio、L4 lag（含 raw）、`taker_ratio` 資料源、至少一個多組件指標（`PLUS_DI` 或 `LINEARREG_SLOPE`）；多週期 `["1h","12h"]`；禁合成 fixture。
- **凍結時機 / reference**：動工前以當下 HEAD 跑一次，存 `tests/_golden/ffname/baseline.json`：逐欄（欄名序）`dtype`、`shape`、NaN mask sha256、值 sha256，以**舊欄名**為鍵。
- **通過條件（可證偽）**：改後同參數重跑，以 Task 1.1 之「舊名→新名」純函式映射（僅供測試，生產碼不得持有對照表）將 baseline 鍵轉為新名後——①新舊欄名集合經映射後**全等**（無多無少）；②每欄四個 hash **全等**（fracdiff 關閉之 reference，故 ADF 白名單變化不影響值）；③改名欄數等於映射中「新名≠舊名」之數，且 >0；④新 run 所有欄名不含子字串 `taker_ratio`、`taker_buy_volume`、`quote_volume`、`number_of_trades`；⑤新 run 之 manifest `feature_naming_version == 2`。任一不等即列出欄名與 diff＝FAIL。
...
- **驗證**：真實 kline 輕量 run 之 L4 lag 群組含 `taker-ratio_<tf>_Lag_1` 且無 `taker_<tf>_ratio_Lag_1`；原 `data` 物件欄名仍含 `taker_ratio`（identity 斷言）；§G ①–④。
...
- **驗證**：`is_safe_skip("close_12h_momentum_PLUS-DI_14_28_Ratio") is True`、`is_safe_skip("close_12h_momentum_PLUS_DI_14_28_Ratio") is False`（記錄改前行為）；§G fracdiff 對照之收據欄數與類別分佈。
...
- 改法：`config_payload["_feature_naming_version"] = CURRENT_FEATURE_NAMING_VERSION`；manifest 根寫 `feature_naming_version`（IC-first 二次寫入同值保留）。
...
- **邊界**：①`FFACT_CGSA_WORK_DIR` 指定固定工作目錄且內有舊世代 manifest（resume 閘因新 hash 下無 L7 manifest 而拒絕——斷言之）；②`force_regenerate`（寫入新 hash 目錄，不覆寫舊 hash 目錄）。
- **存活至**：永久。
- **覆蓋風險**：無。
- 不可做：不得修改或刪除舊 run 目錄、舊 cgsa_work、registry 舊條目。

**Task 2.2 — 跨世代合併 fail-closed**
- 目標：多 run 合併點遇到兩世代即拒絕。　檔案：`feature_library.py` `_load_internal`（讀 manifest 處設 `features_df.attrs["feature_naming_version"]`，manifest 無鍵或走 HDF5 fallback ⇒ `1`）、`load_multi`；`consumer_gate.py` 新增 `assert_single_feature_naming_version(versions: Dict[str, int], *, error_type)`；`api/services/xgboost_batch_service.py` 逐 symbol 載入迴圈（載入後立即讀 attrs，再做欄篩選）。　既有 caller：`ic_analysis_service.py:1712`、`cross_symbol_training_service.py:38`（經 `load_multi`，不改）。
...
- **邊界**：①單 symbol（不檢）；②全部舊世代（通過：同世代內自洽）；③HDF5 fallback 與 v2 混用（fallback 視為 1）。
- **存活至**：永久。
- **覆蓋風險**：無。
...
## §V 驗證策略與邊界測試目錄
- **mutation 條件**：適用（RISK-HIT 含 a、d）。至少五個 mutant 必使具名測試紅：①Task 1.2 只改欄名不改分組鍵（兩分支不合併）；②Task 1.2 移除重複參數 ValueError；③Task 1.3 只改 CGSA 分支之 raw 改名；④Task 2.1 移除 hash 鹽；⑤Task 2.2 `load_multi` 不呼叫 assert。
