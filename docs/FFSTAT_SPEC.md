...
- **前史深度**（v21 註：此深度只作 Task 2.1 前置關卡**載入**之保守上界——實測「warmup 估計」與「首個有效值延遲」有重疊，偏大只會多讀、實際前史短於深度即自資料起點載入；**Task 2.3 之有效起始日不使用此深度**，見「未填起始日」）：`calibration_ingest_start`＝輸出起始日往前 `estimate_max_warmup_bars`（依原生週期）＋ N ＋ 該週期之首個有效值最大延遲（以輕量真實 run 量得並登記於設定常數，Task 2.1 邊界④驗）根；實際前史短於此深度時自資料起點載入，稀疏欄（值常為 NaN 者）於此深度內有效值不足 N 時深度加倍至不再不足或達資料起點。某欄於輸出起始日之前之有限值少於 N（使用者 2026-09-26 裁定）⇒ **該欄不做平穩化**（不 fracdiff、不 ADF 差分、保留原值），其決策記事件 `calibration_insufficient_history` 並列缺少根數，事件經 `apply_quality_degradation` 之 `extra_failure_reasons` 併入 `failure_reasons`（`<事件>:<欄數>`）使品質降為 `partial`，其餘欄照常；該欄公開序列亦全無有效值者（如結構恆為 NaN 之欄）只記未檢定事件；不得縮窗、不得以不足 N 之值判定、不得退回用輸出範圍內資料。該週期輸出起始日之前全無 K 線 ⇒ 生成失敗（零寫入）。
- **未填起始日**（使用者 2026-09-24 裁定；v21 推算方式依使用者 2026-09-26 裁定）：開啟平穩化而請求未帶 `start_date` 時，自動推算**有效起始日**：
  - **上限（保住輸出）**：主週期 index 中至結束日（未填結束日＝資料末端）之列，倒數第 `最少輸出根數` 根之時間為上限；`最少輸出根數`＝主週期之 N（隨 N 變）。主週期至結束日之列數＜最少輸出根數 ⇒ fail-closed，訊息列出主週期可用根數與需要根數。
  - **各週期就緒點（因果掃描）**：對每個原生週期，以該週期實際 K 線列（依實際存在之列計數，缺口不補、不以固定秒數換算）自資料起點起算校準域（同 Task 2.1 之校準域：獨立實例、L1–L6＋縮尾，只取早於上限之列；可分段加長計算，計算範圍不得跨過上限）；對每個候選列索引 t（由小到大），只看 `[0, t)` 之校準域值：某欄於 t 之前已出現有效值、且其自首個有效值起至 t 之 NaN 比例 ≤ 0.9（同 `_calibration_short_columns` 之死欄判準）者為「待湊欄」；**第一個使所有待湊欄於 `[0, t)` 之有效值皆 ≥ N 之 t** 即該週期之就緒點（其時間＝第 t 列之時間）。NaN 比例 > 0.9 之稀疏欄與於就緒點之前尚未出現有效值之欄不等候，於前置關卡依 v19 處理（前史不足記事件、公開亦全 NaN 記未檢定）。掃至上限仍無就緒點 ⇒ 該週期就緒點取上限。
  - **有效起始日**＝各週期就緒點中最晚者，對齊為主週期 index 中第一個不早於它之時間戳，再與上限取較早者；
  - 因果性：就緒點之判定只依候選點之前之資料；上限只依列數與結束日（不依任何特徵值）。
  - 其餘：此推算於請求入口、**計算設定 hash 與任何快取查詢之前**完成，設定 hash 與快取鍵含有效起始日與 `output_start_source`，之後一律視同使用者給了此起始日（校準只用其之前、公開輸出自其開始）；有效起始日、其來源（`output_start_source`＝`user`｜`auto_reserved_calibration`）、各週期就緒點與是否受上限截停（`auto_start_capped`）記入 manifest 與生成結果 metadata，前端於生成結果顯示「特徵自 X 起（前段保留供校準）」，受上限截停時另註「部分欄歷史不足，未做平穩化」。主週期至結束日之列數不足最少輸出根數 ⇒ fail-closed 並指名可用與需要根數。自動推算之額外計算（校準域掃描）之耗時與峰值記憶體於 Task 2.3 實測並記收據（8GB 設定），不得未量先稱秒級。平穩化關閉時未填起始日之行為不變。
...
- 檔案：`warmup_window.py`（v21：不再承載有效起始日之深度推算，僅沿用其 `to_utc` 與 warmup 估計供 Task 2.1 載入上界）、`preprocessing/calibration.py`（`resolve_effective_output_start` 改為依 §C v21：上限、各週期因果就緒點、取最晚對齊主週期、與上限取較早；就緒點掃描為純函式，輸入為校準域值與 N，不讀檔）、`feature_factory.py`（`start_date` 為 None 且開啟平穩化時，於設定 hash 與快取查詢之前推算有效起始日並以之進入前置關卡與公開域；`output_start_source`、各週期就緒點、`auto_start_capped` 寫入 manifest 與生成結果 metadata；前置關卡可重用掃描之校準域前段，須以「截短與重算逐欄值與有限遮罩全等」之測試成立為前提，否則獨立重算）、`api/models/feature_factory_models.py` 與對應 route（生成結果回傳有效起始日、來源、是否截停）、`frontend/src/lib/types.ts`、`frontend/src/app/feature-factory/page.tsx`（生成結果顯示有效起始日；來源為 `auto_reserved_calibration` 時加註「前段保留供校準」，截停時另註「部分欄歷史不足，未做平穩化」）。
- **驗證**：`pytest tests/feature_engineering/test_ffstat_calibration.py -k auto_start` 綠——①真實輕量 1h run 不帶 `start_date`、開啟平穩化 ⇒ 未截停、公開輸出第一列時間＝有效起始日（已對齊主週期 index）、每個待湊欄於有效起始日前有效值 ≥ N 且無 `calibration_insufficient_history`、manifest 與 metadata 之 `output_start_source == "auto_reserved_calibration"`；②真實輕量多週期（1h＋12h）run 不帶 `start_date` ⇒ 12h 受上限截停、主週期輸出根數＝最少輸出根數、湊不滿之欄記 `calibration_insufficient_history`、品質 `partial`、metadata `auto_start_capped == True`；③因果性：以真實 kline 複本改動就緒點之後之 close（×1.5）⇒ 就緒點與有效起始日不變；就緒點掃描對 `[0, t)` 與 `[0, t′)`（t′＞t）兩段計算所得之就緒判定於 t 之前全同；④以真實 1h index 刪除一段列模擬缺口 ⇒ 就緒點依實際列計數；⑤先以同設定帶 `start_date=None` 且平穩化關閉跑一次留下快取後再開平穩化重跑 ⇒ 不命中舊快取、前置關卡確有執行（spy）、每欄 `max(校準時間) <` 有效起始日；⑥同一有效起始日以 `start_date` 明示重跑 ⇒ 公開輸出、決策與 `d` 全同；⑦平穩化關閉、不帶 `start_date` ⇒ 輸出與改前逐位元組相同；⑧主週期至結束日之列數 < 最少輸出根數 ⇒ 經自動推算路徑 fail-closed，訊息含可用與需要根數；⑨自動推算額外計算之耗時與峰值記憶體（輕量與完整設定各 1 標的 × 1h、12h，8GB 設定）記收據；`cd frontend && npm run build` 成功，vitest 斷言生成結果元件在兩種來源與截停下之顯示字樣。
- **邊界**：①多週期：有效起始日取各週期就緒點之最晚者，再與上限取較早者；②帶 `start_date` 者 `output_start_source == "user"`，行為同 Task 2.1；③稀疏欄（自首個有效值起 NaN 比例 > 0.9）與於就緒點前尚未出現有效值之欄不等候，依 v19。
- **存活至**：永久。**覆蓋風險**：無。
