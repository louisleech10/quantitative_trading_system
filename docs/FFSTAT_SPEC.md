...
- **前史深度**：`calibration_ingest_start`＝輸出起始日往前 `estimate_max_warmup_bars`（依原生週期）＋ N ＋ 該週期之首個有效值最大延遲（以輕量真實 run 量得並登記於設定常數，Task 2.1 邊界④驗）根；實際前史短於此深度時自資料起點載入，稀疏欄（值常為 NaN 者）於此深度內有效值不足 N 時深度加倍至不再不足或達資料起點。某欄於輸出起始日之前之有限值少於 N（使用者 2026-09-26 裁定）⇒ **該欄不做平穩化**（不 fracdiff、不 ADF 差分、保留原值），其決策記事件 `calibration_insufficient_history` 並列缺少根數，事件經 `apply_quality_degradation` 之 `extra_failure_reasons` 併入 `failure_reasons`（`<事件>:<欄數>`）使品質降為 `partial`，其餘欄照常；該欄公開序列亦全無有效值者（如結構恆為 NaN 之欄）只記未檢定事件；不得縮窗、不得以不足 N 之值判定、不得退回用輸出範圍內資料。該週期輸出起始日之前全無 K 線 ⇒ 生成失敗（零寫入）。
- **未填起始日**（使用者 2026-09-24 裁定）：開啟平穩化而請求未帶 `start_date` 時，自動推算**有效起始日**——各原生週期取「該 symbol 該週期實際 K 線列（依實際存在之列計數，缺口不補、不以固定秒數換算）以 0 起算之索引 `前史深度` 那一列之時間」（即其之前恰有 `前史深度` 根 K 線），取各週期中最晚者，再對齊為主週期 index 中第一個不早於它之時間戳；此推算於請求入口、**計算設定 hash 與任何快取查詢之前**完成，設定 hash 與快取鍵含有效起始日與 `output_start_source`，之後一律視同使用者給了此起始日（校準只用其之前、公開輸出自其開始）；有效起始日與其來源（`output_start_source`＝`user`｜`auto_reserved_calibration`）記入 manifest 與生成結果 metadata，前端於生成結果顯示「特徵自 X 起（前段保留供校準）」。可用歷史不足以推出有效起始日（有效起始日晚於結束日或晚於資料末端）⇒ fail-closed 並指名缺少根數。平穩化關閉時未填起始日之行為不變。
...
- 檔案：`warmup_window.py`（有效起始日推算，逐原生週期取最大）、`feature_factory.py`（`start_date` 為 None 且開啟平穩化時以有效起始日進入前置關卡與公開域；`output_start_source` 寫入 manifest 與生成結果 metadata）、`api/models/feature_factory_models.py` 與對應 route（生成結果回傳有效起始日與來源）、`frontend/src/lib/types.ts`、`frontend/src/app/feature-factory/page.tsx`（生成結果顯示有效起始日；來源為 `auto_reserved_calibration` 時加註「前段保留供校準」）。
- **驗證**：`pytest tests/feature_engineering/test_ffstat_calibration.py -k auto_start` 綠——真實輕量 run 不帶 `start_date`、開啟平穩化 ⇒ 公開輸出第一列時間＝有效起始日（已對齊主週期 index）、各週期校準域於有效起始日前之實際 K 線列數 ≥ 前史深度（真實 kline 十標的三週期皆無缺口〔主委實跑 2026-09-24〕⇒ 以真實 1h index 刪除一段列模擬交易所停機缺口，驗依列計數）、先以同設定帶 `start_date=None` 且平穩化關閉跑一次留下快取後再開平穩化重跑 ⇒ 不命中舊快取、前置關卡確有執行（spy）、每欄 `max(校準時間) <` 有效起始日、manifest 與 metadata 之 `output_start_source == "auto_reserved_calibration"`；同一有效起始日以 `start_date` 明示重跑 ⇒ 公開輸出、決策與 `d` 全同；平穩化關閉、不帶 `start_date` ⇒ 輸出與改前逐位元組相同；歷史短於前史深度之標的 ⇒ fail-closed 指名缺少根數；`cd frontend && npm run build` 成功，vitest 斷言生成結果元件在兩種來源下之顯示字樣。
- **邊界**：①多週期：有效起始日取各週期推算值之最大者，較短歷史之週期決定起點；②帶 `start_date` 者 `output_start_source == "user"`，行為同 Task 2.1。
- **存活至**：永久。**覆蓋風險**：無。
