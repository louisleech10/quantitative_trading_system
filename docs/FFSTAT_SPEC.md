...
- **未填起始日**（使用者 2026-09-24 裁定）：開啟平穩化而請求未帶 `start_date` 時，自動推算**有效起始日**＝各原生週期「該 symbol 可用 K 線之第一根時間＋前史深度根數」之最大者，之後一律視同使用者給了此起始日（校準只用其之前、公開輸出自其開始）；有效起始日與其來源（`output_start_source`＝`user`｜`auto_reserved_calibration`）記入 manifest 與生成結果 metadata，前端於生成結果顯示「特徵自 X 起（前段保留供校準）」。可用歷史不足以推出有效起始日（有效起始日晚於結束日或晚於資料末端）⇒ fail-closed 並指名缺少根數。平穩化關閉時未填起始日之行為不變。
...
- **驗證**：`pytest tests/feature_engineering/test_ffstat_calibration.py -k auto_start` 綠——真實輕量 run 不帶 `start_date`、開啟平穩化 ⇒ 公開輸出第一列時間＝有效起始日、每欄 `max(校準時間) <` 有效起始日、manifest 與 metadata 之 `output_start_source == "auto_reserved_calibration"`；同一有效起始日以 `start_date` 明示重跑 ⇒ 公開輸出、決策與 `d` 全同；平穩化關閉、不帶 `start_date` ⇒ 輸出與改前逐位元組相同；歷史短於前史深度之標的 ⇒ fail-closed 指名缺少根數；`cd frontend && npm run build` 成功，vitest 斷言生成結果元件在兩種來源下之顯示字樣。
...
## §V 驗證策略與邊界測試目錄
- **mutation 條件**：適用。下列二十三個 mutant 各自必使具名測試紅：①判定退回欄名子字串免檢；②目標層退回欄名解析；③層對照缺欄時當非目標而不 fail；④d\* 例外改回 `d=1.0`；⑤`layer1_only` 分支恢復；⑥校準改回取輸出範圍最早 N 根；⑦前史不足時縮窗；⑦′公開域計算起點改用校準域起點（公開值變動）；⑦″校準域例外被降級路徑吞下而照常寫出特徵；⑦‴多週期 worker 以主週期校準值充數；⑦⁗校準前置關卡移到主週期落盤之後（第二週期失敗時已有寫入）；⑦⁵前置關卡只校準 worker 待辦週期（resume 已完成週期缺封包）；⑦⁶封包身分鍵不核對；⑦⁷不核對最晚校準時間早於輸出起始日；⑦⁸平穩化開啟時 `run_ic_first` 接受自帶層；⑦⁹來源紀錄只雜湊最末時間戳（前史值改動不改指紋）；⑦¹⁰校準域與公開域共用同一 `FeatureFactory` 實例（L5 參考標的快取跨域命中）；⑦¹¹未填起始日時以輸出範圍最早 N 根校準而不預留；⑧d\* 內層 ADF 寫死 500；⑨d\* 失敗欄重回 ADF 差分候選；⑩parallel 主程序於判 `status` 前寫快取；⑪快取載入失敗改回靜默空快取；⑫`flush_atomic` 失敗改回只記 warning。
