...
- **前史深度**（v22 註：此深度為保守上界——實測「warmup 估計」與「首個有效值延遲」有重疊，偏大只會多讀、實際前史短於深度即自資料起點載入；Task 2.1 以之定載入起點，Task 2.3 以之定各週期起點並與「保住 N 根輸出」之上限取較早，見「未填起始日」）：`calibration_ingest_start`＝輸出起始日往前 `estimate_max_warmup_bars`（依原生週期）＋ N ＋ 該週期之首個有效值最大延遲（以輕量真實 run 量得並登記於設定常數，Task 2.1 邊界④驗）根；實際前史短於此深度時自資料起點載入，稀疏欄（值常為 NaN 者）於此深度內有效值不足 N 時深度加倍至不再不足或達資料起點。某欄於輸出起始日之前之有限值少於 N（使用者 2026-09-26 裁定）⇒ **該欄不做平穩化**（不 fracdiff、不 ADF 差分、保留原值），其決策記事件 `calibration_insufficient_history` 並列缺少根數，事件經 `apply_quality_degradation` 之 `extra_failure_reasons` 併入 `failure_reasons`（`<事件>:<欄數>`）使品質降為 `partial`，其餘欄照常；該欄公開序列亦全無有效值者（如結構恆為 NaN 之欄）只記未檢定事件；不得縮窗、不得以不足 N 之值判定、不得退回用輸出範圍內資料。該週期輸出起始日之前全無 K 線 ⇒ 生成失敗（零寫入）。
- **未填起始日**（使用者 2026-09-24 裁定；v21 推算方式依使用者 2026-09-26 裁定）：開啟平穩化而請求未帶 `start_date` 時，自動推算**有效起始日**：
  - **上限（保住輸出）**：每個原生週期於輸出範圍內至少保留**該週期**之 N 根（`最少輸出根數`＝各週期之 N，隨 N 變）——各週期取其至結束日（未填結束日＝資料末端）之實際列中倒數第 N 根之時間，上限＝其中最早者（使每個週期皆保住 N 根）。任一週期至結束日之列數＜其 N ⇒ fail-closed，訊息列出各週期可用根數與需要根數。（例：1h＋12h 時上限由 12h 決定，輸出為 500 根 12h＝約 6,000 根 1h。）
  - **各週期起點（v22）**：各原生週期取「該 symbol 該週期實際 K 線列（依實際存在之列計數，缺口不補、不以固定秒數換算）以 0 起算之索引 `前史深度` 那一列之時間」（`前史深度` 即下方 §C 前史深度式，只依設定與週期）；該週期實際列數 ≤ `前史深度` ⇒ 該週期起點視為晚於一切（由上限截停）。
  - **有效起始日**＝各週期起點中最晚者，對齊為主週期 index 中第一個不早於它之時間戳，再與上限取較早者；取上限者記 `auto_start_capped=True`，此時湊不滿 N 之欄於前置關卡依 v19 處理（前史不足記事件、公開亦全 NaN 記未檢定）。
  - 因果性：有效起始日只依設定（前史深度、N）、各週期 K 線列數與結束日決定，不讀任何特徵值、不計算校準域；故改動任何 K 線值不改變有效起始日。
  - 其餘：此推算於請求入口、**計算設定 hash 與任何快取查詢之前**完成，設定 hash 與快取鍵含有效起始日與 `output_start_source`，之後一律視同使用者給了此起始日（校準只用其之前、公開輸出自其開始）；有效起始日、其來源（`output_start_source`＝`user`｜`auto_reserved_calibration`）、各週期就緒點與是否受上限截停（`auto_start_capped`）記入 manifest 與生成結果 metadata，前端於生成結果顯示「特徵自 X 起（前段保留供校準）」，受上限截停時另註「部分欄歷史不足，未做平穩化」。任一週期至結束日之列數不足其 N ⇒ fail-closed 並指名該週期之可用與需要根數；某原生週期於有效起始日之前全無 K 線 ⇒ 依 §C 前史深度之規則零寫入失敗並指名週期。自動推算只做各週期 index 查列（不計算校準域），其耗時於 Task 2.3 實測並記收據。平穩化關閉時未填起始日之行為不變。
...
- 檔案：`warmup_window.py`（沿用其 `estimate_max_warmup_bars` 與 `to_utc`，供前史深度與時間比較）、`preprocessing/calibration.py`（`resolve_effective_output_start` 依 §C v22：各週期起點＝實際列索引「前史深度」那一列、列數不足者視為晚於一切；取最晚對齊主週期、與上限取較早；純函式，輸入為各週期 index、深度、主週期 index、最少輸出根數與結束日，不讀檔、不算特徵）、`feature_factory.py`（`start_date` 為 None 且開啟平穩化時，於設定 hash 與快取查詢之前推算有效起始日並以之進入前置關卡與公開域；`output_start_source`、各週期起點、`auto_start_capped` 寫入 manifest 與生成結果 metadata）、`api/models/feature_factory_models.py` 與對應 route（生成結果回傳有效起始日、來源、是否截停）、`frontend/src/lib/types.ts`、`frontend/src/app/feature-factory/page.tsx`（生成結果顯示有效起始日；來源為 `auto_reserved_calibration` 時加註「前段保留供校準」，截停時另註「部分欄歷史不足，未做平穩化」）。
- **驗證**：`pytest tests/feature_engineering/test_ffstat_calibration.py -k "auto_start or effective_output_start or boundary_10"` 綠——①真實輕量 1h run 不帶 `start_date`、開啟平穩化 ⇒ 未截停、有效起始日＝1h 實際列索引「前史深度」那一列（對齊主週期）、公開輸出第一列時間＝有效起始日、無 `calibration_insufficient_history`、manifest 與 metadata 之 `output_start_source == "auto_reserved_calibration"`；②真實輕量多週期（1h＋12h）run 不帶 `start_date` ⇒ 12h 實際列數 ≤ 前史深度而受上限截停、上限由 12h 決定且 12h 輸出恰為其 N 根（1h 輸出為同一時段之列）、湊不滿之欄記 `calibration_insufficient_history`、品質 `partial`、metadata `auto_start_capped == True`；③因果性：以真實 kline 複本改動前史與輸出範圍內之 close（×1.5）⇒ 有效起始日不變；spy 斷言自動推算不呼叫校準域計算與任何特徵計算；④以真實 1h index 刪除一段列模擬缺口 ⇒ 起點依實際列計數；⑤先以同設定帶 `start_date=None` 且平穩化關閉跑一次留下快取後再開平穩化重跑 ⇒ 不命中舊快取、前置關卡確有執行（spy）、每欄 `max(校準時間) <` 有效起始日；⑥同一有效起始日以 `start_date` 明示重跑 ⇒ 公開輸出、決策與 `d` 全同；⑦平穩化關閉、不帶 `start_date` ⇒ 輸出與改前逐位元組相同；⑧任一週期至結束日之列數 < 其 N ⇒ 經自動推算路徑 fail-closed，訊息含該週期可用與需要根數；⑨以真實 kline 複本刪除某原生週期於有效起始日前之全部列 ⇒ 零寫入 fail-closed、訊息指名週期；⑩自動推算耗時（輕量設定 1 標的 × 1h、1h＋12h）記收據；`cd frontend && npm run build` 成功，vitest 斷言生成結果元件在兩種來源與截停下之顯示字樣。
- **邊界**：①多週期：有效起始日取各週期起點之最晚者，再與上限取較早者；②帶 `start_date` 者 `output_start_source == "user"`，行為同 Task 2.1；③截停時湊不滿 N 之欄依 v19。
- **存活至**：永久。**覆蓋風險**：無。
