...
- **待使用者確認**：
  - 校準窗長度（研究 r2 兩家外推：n=500 全 L1＋L2 ADF 約 47–312 秒／標的／次；n=2000 約 1942–2320 秒）：①維持 500（1h 一致率 83–88%，誤判多為多做差分）；②1h 改 2000（1h 一致率約 98%，每次生成多約 30–40 分鐘／標的，且需起始日前多約 1500 根＝62 天歷史）；12h 無足夠歷史驗證，兩案皆維持 500。
- **已確認結果**：2026-09-24 使用者「看名稱決定是否要平穩化，在量化數據好像是很嚴重的問題，需要盡早研究處理」「fracdiff layer1 only……是寫死的也沒有要給使用者動」「舊名稱或數據都可以刪掉舊的重新生成」「但會不會有某個幣種的某特徵在某個週期要做，但你這樣抽到的是不做？也就是抽樣問題」「將量化業界和統計分析怎麼做考量進去做研究」。
...
- **校準資料無洩漏**：ADF 差分候選、fracdiff 目標篩選、d\* 搜尋三路之校準值，只取**該 symbol、該原生週期、輸出起始日之前**之資料（每欄最後 N 個有效值，N＝校準長度），且該段不進入公開輸出與任何切分。此前史由明示之前史載入取得，**不依賴** `FFACT_WARMUP_TRIM`（其預設 `0` 不改）。無輸出起始日、或任一欄之前史有效值不足 N ⇒ 該次生成 fail-closed，錯誤指名 symbol、週期、欄數與缺少根數；不得縮窗、不得退回用輸出範圍內資料。
...
**Task 2.1 — 前史校準來源**
- 檔案：`warmup_window.py`（前史載入範圍：輸出起始日之前 N 根有效值所需之 K 線，逐原生週期）、`feature_factory.py`（前史與公開輸出分界；前史只供校準、不入輸出）、`feature_preprocessor.py`（`_calibration_series`／`_calibration_values` 改取起始日前最後 N 個有效值）。
- **驗證**：`pytest tests/feature_engineering/test_ffstat_calibration.py` 綠——每欄 `max(校準時間) < 輸出起始日`；公開輸出列數與起始日不變；改動輸出範圍內值 ⇒ 決策與 `d` 不變；改動前史值 ⇒ 決策可變（至少一欄）；無起始日 ⇒ fail-closed。
- **邊界**：①多週期：各原生週期各自取前史（不得以主週期 ffill 之重複值充數）；②晚生欄（前史內有效值不足）⇒ fail-closed 指名；③`FFACT_WARMUP_TRIM=0` 與 `1` 兩設定皆成立。
- **存活至**：永久。**覆蓋風險**：無。
- 不可做：不得縮窗、不得退回輸出範圍內資料、不得改 `FFACT_WARMUP_TRIM` 預設。
...
**Task 4.1 — §G 對照、成本與收據**
- **驗證**：`pytest tests/feature_engineering/test_ffstat_golden.py` 綠；收據 `handoffs/run_receipts/<日期>-ffstat-golden.json` 列逐欄決策、校準範圍、N、p 值、`d` 與改前改後 hash；冷 run 成本收據列 ADF 與 d\* 各自耗時、峰值記憶體，於 3 標的 × 2 週期之真實 run 實測（非外推）。
- **邊界**：①fracdiff 與 ADF 差分皆關閉 ⇒ 輸出與改前逐位元組相同。
- **存活至**：收據永久。**覆蓋風險**：無。
- 不可做：不得放寬 §G 通過條件。

## §V 驗證策略與邊界測試目錄
- **mutation 條件**：適用。至少十二個 mutant 必使具名測試紅：①判定退回欄名子字串免檢；②目標層退回欄名解析；③層對照缺欄時當非目標而不 fail；④d\* 例外改回 `d=1.0`；⑤`layer1_only` 分支恢復；⑥校準改回取輸出範圍最早 N 根；⑦前史不足時縮窗；⑧d\* 內層 ADF 寫死 500；⑨d\* 失敗欄重回 ADF 差分候選；⑩parallel 主程序於判 `status` 前寫快取；⑪快取載入失敗改回靜默空快取；⑫`flush_atomic` 失敗改回只記 warning。
