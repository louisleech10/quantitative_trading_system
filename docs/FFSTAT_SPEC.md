...
- **校準前置關卡**：開啟平穩化時，校準為一次生成之**第一步**——對該次須處理之**全部原生週期**完成校準域計算、截取與有效值檢查，才進行 `_prepare_cgsa_registry`、主週期任何層之落盤與多週期 worker 之啟動；校準結果（每欄校準值與決策所需資料）交各週期之 L6.5 使用，worker 不自行重算校準。
