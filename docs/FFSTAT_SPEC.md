...
- **未填起始日**（使用者 2026-09-24 裁定；v21 推算方式依使用者 2026-09-26 裁定）：開啟平穩化而請求未帶 `start_date` 時，自動推算**有效起始日**：
  - **只依主週期**（v23，使用者 2026-09-26 再裁定「以主訓練週期優先、次週期能做多少就做」）：次週期（非主週期之原生週期）不參與起點推算。
  ...
  - **主週期起點**：主週期實際 K 線列（依實際存在之列計數，缺口不補、不以固定秒數換算）以 0 起算之索引 `前史深度_主` 那一列之時間；`前史深度_主` 只用下方 §C 前史深度之**基礎式**（warmup 估計＋N_主＋延遲常數），**不含**前置關卡載入時之稀疏欄加倍（加倍依特徵值，只影響 Task 2.1 之載入、不影響有效起始日）；主週期列數 ≤ `前史深度_主` ⇒ 起點取上限。
  ...
  - 其餘：此推算於請求入口、**計算設定 hash 與任何快取查詢之前**完成，設定 hash 與快取鍵含有效起始日與 `output_start_source`，之後一律視同使用者給了此起始日（校準只用其之前、公開輸出自其開始）；manifest 與生成結果 metadata 記具名鍵 `effective_output_start`、`output_start_source`（`user`｜`auto_reserved_calibration`）、`auto_start_primary_start`、`auto_start_capped`（次週期前史不足之欄數沿用 `failure_reasons` 之事件計數）；前端於生成結果顯示「特徵自 X 起（前段保留供校準）」，截停時另註「部分欄歷史不足，未做平穩化」。自動推算只做主週期 index 查列（不計算校準域），單次 ≤ 2 秒。平穩化關閉時未填起始日之行為不變。
...
- **驗證**：`pytest tests/feature_engineering/test_ffstat_calibration.py -k "auto_start or effective_output_start or boundary_10"` 綠——①真實輕量 1h run 不帶 `start_date`、開啟平穩化 ⇒ 未截停、有效起始日＝1h 實際列索引「前史深度基礎式」那一列、公開輸出第一列時間＝有效起始日、記 `calibration_insufficient_history` 之欄恰為同設定全史校準域湊不滿 N 之欄（其餘欄皆有 N 個有效值）、metadata 四具名鍵齊備且 `output_start_source == "auto_reserved_calibration"`、`auto_start_capped == False`；②真實輕量多週期（主週期 1h＋次週期 12h）run 不帶 `start_date` ⇒ 有效起始日與①同（12h 不延後起點）、12h 前史不足之欄記 `calibration_insufficient_history`、品質 `partial`；③主週期截停：以真實 kline 複本截短主週期至列數 ≤ 前史深度基礎式 ⇒ 有效起始日＝上限、主週期輸出恰 N_主 根、`auto_start_capped == True`；④各週期 N 分設（`calibration_bars_by_timeframe` 主、次週期不同值）⇒ 上限與主週期起點用 N_主、次週期前置關卡用其自身 N；⑤因果性：以真實 kline 複本改動前史與輸出範圍內之 close（×1.5）⇒ 有效起始日不變；spy 斷言自動推算不呼叫校準域計算與任何特徵計算；自動推算（真實 1h＋12h，含讀 index）單次 ≤ 2 秒，超過即紅；⑥非 UTC 時區之主週期 index（真實值、index 改 America/New_York）以 date-only `end_date` ⇒ 上限所數之列與公開 L0 以同一 `end_date` 篩出之列相同；⑦以真實 1h index 刪除一段列模擬缺口 ⇒ 起點依實際列計數；⑧先以同設定帶 `start_date=None` 且平穩化關閉跑一次留下快取後再開平穩化重跑 ⇒ 不命中舊快取、前置關卡確有執行（spy）、每欄 `max(校準時間) <` 有效起始日；⑨同一有效起始日以 `start_date` 明示重跑 ⇒ 公開輸出、決策與 `d` 全同；⑩平穩化關閉、不帶 `start_date` ⇒ 輸出與改前逐位元組相同；⑪至結束日之主週期列數 < N_主 ⇒ 經自動推算路徑 fail-closed，訊息含可用與需要根數；⑫以真實 kline 複本刪除某次週期於有效起始日前之全部列 ⇒ 零寫入 fail-closed、訊息指名週期；`cd frontend && npm run build` 成功，vitest 斷言生成結果元件在兩種來源與截停下之顯示字樣。
